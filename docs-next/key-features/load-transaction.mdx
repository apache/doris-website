...
> **TL;DR** Doris wraps every data load in a transaction. You can group several inserts under one `BEGIN ... COMMIT`, dedupe retries with a load `Label`, and drive a two-phase commit from headers in a [Stream Load](./stream-load) HTTP request. That last part is what lets Flink, Spark, and [CDC pipelines](./kafka-cdc-integration) write to Doris exactly once without keeping their own bookkeeping table.
