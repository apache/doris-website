...
description: Doris's two-level data sharding. Partitions split a table by column ranges or lists for pruning and lifecycle; buckets shard each partition into tablets for parallel reads and writes.
...
> **TL;DR** Every Doris table is split twice: a `PARTITION BY` clause carves the rows by a column value (usually a date), and a `DISTRIBUTED BY` clause shards each partition into N tablets. Partitions give the planner something coarse to drop and the operator something to archive; buckets give the cluster something parallel to scan and write. Pick the date column queries filter on for the partition key, pick a high-cardinality equality column for the bucket key, and let `BUCKETS AUTO` size the rest.

![Partitioning and Bucketing hero illustration](/images/next/key-features/partitioning-and-bucketing.jpg)
...
- Dropping last quarter's data turns into a multi-hour `DELETE` job instead of a metadata change.
...
- Wide-fanout joins between two large tables on the same key. Match the bucket key on both tables and enable [Colocate Join](../query-acceleration/colocation-join.md) to skip the shuffle.
...
- [Basic Concepts: Partition + Bucket walkthrough with diagrams](../table-design/data-partitioning/basic-concepts.mdx)
- [Manual Partitioning: Range, List, and the four Range forms](../table-design/data-partitioning/manual-partitioning.md)
- [Auto Partitioning: on-demand partition creation at write time](../table-design/data-partitioning/auto-partitioning.md)
- [Data Bucketing: choosing the method, the key, and the bucket count](../table-design/data-partitioning/data-bucketing.md)
- [Data Pruning: the layered pruning chain that runs after partition and tablet selection](./data-pruning.mdx)
- [`ALTER TABLE PARTITION` reference](../sql-manual/sql-statements/table-and-view/table/ALTER-TABLE-PARTITION.md)
