...
## Why use high-concurrency point query in Apache Doris? {#why}
...
## What is the Apache Doris high-concurrency point query? {#what}
...
## How does the Apache Doris high-concurrency point query work? {#how}
...
1. **Storage layout.** When you set `store_row_column=true` at table creation, the BE writes each row both into the columnar segments and into a row-encoded hidden column. From Apache Doris 3.0 onward you can scope this to a subset with `row_store_columns="k1,v1,v2"` to limit the storage overhead.
...
## When should you use the Apache Doris high-concurrency point query? {#when}
...
- [MPP Architecture](./mpp): the full distributed execution path the point-query short-circuit is built to skip.
- [Unique Key](./unique-key): the table model point query pairs with via `store_row_column` to act as a KV store.
