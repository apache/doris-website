...
- Lakehouse deployments that already use Ranger for Hive, HDFS, or Iceberg, and want the same policies on Doris-managed tables. See [Ranger Authorization](../admin-manual/auth/authorization/ranger.md).
...
- [Authentication and Authorization overview](../admin-manual/auth/authentication-and-authorization.md): the full conceptual model, including User Identity, roles, and the priority rules that decide which user a connection lands on.
- [Built-in Authentication](../admin-manual/auth/authentication/internal.md): password policies, expiration, lockout, and the `validate_password_policy` strength rules.
- [LDAP Authentication](../admin-manual/auth/authentication/ldap.md): `ldap.conf` reference, group-to-role mapping, LDAPS setup, and the cleartext plugin steps for MySQL and JDBC clients.
- [Ranger Authorization](../admin-manual/auth/authorization/ranger.md): installing the Doris Ranger plugin, policy examples for catalog/database/table/column, and the row-level filter and masking flows.
- [Security Overview](../admin-manual/auth/security-overview.md): the wider picture, including audit logs, SSL transport, and UDF guidance.
- [Multi-Catalog](./multi-catalog.mdx): how Ranger-style authorization plugs into per-catalog access controllers for Hive, Iceberg, and other external sources.
