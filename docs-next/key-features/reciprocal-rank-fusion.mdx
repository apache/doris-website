...
- Hot-path queries where the extra subqueries and join hurt latency. A pre-filter pattern (`MATCH_ANY` + `ORDER BY l2_distance_approximate`) returns a single ranked list in one pass and is what [Hybrid Search](./hybrid-search.mdx) uses by default.
...
- [Hybrid Search](./hybrid-search.mdx): Doris's default single-pass pattern for combining inverted-index filtering with ANN ranking, and the alternative this card compares against.
- [Relevance Scoring (BM25)](../table-design/index/inverted-index/scoring.md): how `score()` is computed and when it activates, which is the input to one half of the fusion.
- [Inverted Index Overview](../table-design/index/inverted-index/overview.md): index types, tokenizers, and the `MATCH_*` operators that drive the BM25 subquery.
- [Vector Index Overview](../table-design/index/vector-index/overview.md): HNSW and IVF backends, distance functions, and constraints on the ANN subquery.
- [ROW_NUMBER](../sql-manual/sql-functions/window-functions/row-number.md): the window function that turns each ranked subquery into integer ranks.
