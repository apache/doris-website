...
> **TL;DR** Doris does not ship a built-in `rrf()` function, but you can implement Reciprocal Rank Fusion in plain SQL using `ROW_NUMBER()` over a BM25 ranking and a vector ranking, then summing `1.0 / (60 + rank)` across both. The result is a single fused top-K list whose ordering does not depend on calibrating BM25 scores against cosine distance.

![Reciprocal Rank Fusion hero illustration](/images/next/key-features/reciprocal-rank-fusion.jpg)
