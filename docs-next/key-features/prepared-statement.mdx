...
> **TL;DR** Doris speaks the MySQL wire protocol, including server-side prepared statements. Set `useServerPrepStmts=true` on the JDBC URL and Doris parses and plans your statement once per session, then reuses that plan on every `EXECUTE`. For high-QPS point-query traffic on the short-circuit path, that cuts FE CPU enough to lift throughput by roughly 4x. For batch INSERTs and any other shape that repeats, you stop paying parse and plan cost on every row.

![Prepared Statement hero illustration](/images/next/key-features/prepared-statement.jpg)

## Why it matters

Most analytical engines pay parse and plan cost on every query. That cost is invisible in a daily report job. It is the whole story once a service starts asking the same query thousands of times per second. A user-facing dashboard reads a row by primary key on every click. A feature store reads a vector by entity ID on every prediction. A bulk loader fires the same `INSERT INTO t VALUES (?, ?, ?)` over and over.
...
- Batch INSERTs over [Group Commit](./group-commit.mdx), where a loader fires the same `INSERT INTO t VALUES (?, ?, ?)` thousands of times per second.
...
- [High-Concurrency Point Query feature card](./high-concurrency-point-query.mdx)
- [Group Commit for high-throughput INSERTs](./group-commit.mdx)
- [Data pruning for non-point queries](./data-pruning.mdx)
