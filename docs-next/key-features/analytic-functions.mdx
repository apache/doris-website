...
description: SQL window functions in Doris let you rank, accumulate, lag, and average across rows without self-joins, and the planner pushes Top-N filters down into the window operator.
...
![Analytic Functions hero illustration](/images/next/key-features/analytic-functions.jpg)
...
- `ORDER BY` on a non-unique column when you care about deterministic output. `SUM(x) OVER (ORDER BY day)` can return different cumulative totals across runs when several rows share the same day. Add a tie-breaker; see [Window Functions Overview](../sql-manual/sql-functions/window-functions/overview.md).
- Recomputing the same window result on every refresh. If the same `ROW_NUMBER()` query runs every minute against a slow-moving table, an [async materialized view](../query-acceleration/materialized-view/async-materialized-view/overview.md) with a partial refresh is cheaper than re-windowing each time.

## Further reading

- [Analytic Functions (Window Functions): full guide with worked examples](../query-data/window-function.md)
- [Window Functions Overview: syntax, frame clauses, and the unique-ordering caveat](../sql-manual/sql-functions/window-functions/overview.md)
- [`LAG`](../sql-manual/sql-functions/window-functions/lag.md), [`LEAD`](../sql-manual/sql-functions/window-functions/lead.md), [`ROW_NUMBER`](../sql-manual/sql-functions/window-functions/row-number.md), [`RANK`](../sql-manual/sql-functions/window-functions/rank.md), [`NTILE`](../sql-manual/sql-functions/window-functions/ntile.md): per-function reference pages
- [Pipeline Execution Engine: how partitioned windows get parallelized across BEs](./pipeline-execution-engine.mdx)
- [Vectorized Execution](./vectorized-execution.mdx): the engine that runs each window's batch through SIMD-accelerated operators.
- [Async Materialized View: precompute window results that re-run every minute](../query-acceleration/materialized-view/async-materialized-view/overview.md)
