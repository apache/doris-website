---
{
    ...
    "language": "zh-CN",
    "description": "Doris 2.1 起基于 Arrow Flight SQL 协议构建高速数据传输链路，支持 Python/Java 等多语言以列存格式高速读取大批量数据，相比 MySQL/JDBC 性能提升数十至百倍。",
    "keywords": [
        "Arrow Flight SQL",
        "ADBC Driver",
        "Doris 列存读取",
...
**一句话定义：** Arrow Flight SQL 是 Doris 自 2.1 起支持的列存格式数据传输协议；客户端若同样支持 Arrow，整个链路无需行列转换，相比 MySQL/JDBC 协议性能提升数十至百倍。
...
| 你的场景                                | 跳转章节                                                    |
| --------------------------------------- | ----------------------------------------------------------- |
| 想用 Python 从 Doris 读数据             | [Python 使用方式](#python-使用方式)                         |
| 想用 Java/JDBC 从 Doris 读数据          | [Java 使用方式](#java-使用方式)                             |
| 想用 DBeaver 等 BI 工具连接 Doris       | [BI 工具（DBeaver 等）](#bi-工具dbeaver-等)                 |
| 想在 Spark/Flink 中以 Arrow 格式读 Doris | [Spark 与 Flink](#spark-与-flink)                           |
| 想让多 BE 节点并行返回查询结果          | [多 BE 并行返回结果](#多-be-并行返回结果)                   |
| BE 节点在内网，外部 Client 无法访问     | [多 BE 共享同一个外部访问 IP](#多-be-共享同一个外部访问-ip) |
| 报错 `i/o timeout`                      | [FAQ](#faq) — 第 1 项                                       |
| 报错 `module java.base does not "opens java.nio"` | [FAQ](#faq) — 第 2 项                                 |
| 报错 `invalid bearer token`             | [FAQ](#faq) — 第 10 项                                      |
| ARM 环境报错 `arrow flight schema timeout` | [FAQ](#faq) — 第 3 项                                    |
