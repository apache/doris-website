...
| timezone | Specifies the time zone, which affects some functions affected by the time zone, such as `strftime`, `alignment_timestamp`, `from_unixtime`, etc. For details, please refer to the [Time Zone](../../../../admin-manual/cluster-management/time-zone.md) documentation. If not specified, "Asia/Shanghai" will be used. |
