...
- Renaming columns
- Modifying the length of VARCHAR columns (except for UNIQUE and DUP key columns).
...
Users can check the progress of schema change jobs through the [`SHOW ALTER TABLE COLUMN`](../sql-manual/sql-statements/Show-Statements/SHOW-ALTER.md) command. This command allows users to view currently executing or completed schema change jobs. When a schema change job involves multiple indexes, the command will display multiple rows, with each row corresponding to one index. An example is as follows:
...
For specific syntax, refer to [ALTER TABLE COLUMN](../sql-manual/sql-statements/Data-Definition-Statements/Alter/ALTER-TABLE-COLUMN.md).

- If adding a value column to an aggregate model, the `agg_type` must be specified.

- If adding a key column to a non-aggregate model (such as DUPLICATE KEY), the KEY keyword must be specified.
...
1. Add a key column `new_col` after `col1` in `example_db.my_table`
...
2. Add a value column `new_col` after `col1` in `example_db.my_table`
...
Add a key column `new_col` after `col1` in `example_db.my_table`
...
Add a value column `new_col` of SUM aggregation type after `col1` in `example_db.my_table`
...
- If adding a key column to an aggregate model, the KEY keyword must be specified.

*Adding multiple columns to an aggregate table*

Add multiple columns to `example_db.my_table` (aggregate model)
...
- If modifying a value column in an aggregate model, the `agg_type` must be specified.

- If modifying a key column in a non-aggregate type, the **KEY** keyword must be specified.
...
- The following type conversions are currently supported (users need to be aware of precision loss):
  ...
  - VARCHAR/CHAR can be converted to DATE (currently supports six formatting formats: "%Y-%m-%d", "%y-%m-%d", "%Y%m%d", "%y%m%d", "%Y/%m/%d", "%y/%m/%d").
  ...
  - Any type other than DATE and DATETIME can be converted to STRING, but STRING cannot be converted to any other type.
...
**1. Modify the type of key column col1 to BIGINT and move it after column col2.**
...
**2. Modify the maximum length of the val1 column in the base table. The original val1 was (val1 VARCHAR(32) REPLACE DEFAULT "abc").**
...
**3. Modify the length of a certain field of a key column in a duplicate key table.**
...
Reorder the columns in `example_db.my_table`
...
- When modifying column types, all fields except Type must be supplemented according to the original column information.
