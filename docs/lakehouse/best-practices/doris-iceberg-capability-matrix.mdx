---
{
    "title": "Iceberg Catalog Capability Matrix",
    "language": "en",
    "description": "Capability matrix for Iceberg Catalog backends supported by Apache Doris."
}
---

The following matrix summarizes the Iceberg Catalog capabilities currently available in Doris. The rows are grouped by capability area, and the backend columns show whether each operation is available when Doris uses the corresponding metadata service. ✅ indicates that Doris supports the operation, and ❌ indicates that the operation is not currently supported.
