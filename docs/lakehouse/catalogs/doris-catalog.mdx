---
{
    "title": "Doris Catalog",
    "language": "en",
    "description": "Doris Catalog enables cross-cluster federated analysis across multiple Doris clusters via Arrow Flight or virtual cluster modes, with configuration, type mapping and query optimization guides."
}
---
...
* `fe_thrift_hosts`
  ...
  > Note: In version 4.0.2, user Master FE's address. This issue will be fixed in next version.
...
> since 4.1 Virtual Cluster Mode also supports insert capability
...
After configuring the Catalog in Virtual Cluster Mode, you can insert data into tables in the Catalog through the following methods:
...
After configuring the Catalog in Virtual Cluster Mode, the following capabilities are not supported:
- [Group Commit](../../data-operate/import/group-commit-manual)
- [Transaction](../../data-operate/transaction)
