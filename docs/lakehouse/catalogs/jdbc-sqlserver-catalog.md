...
| image/binary/varbinary             | varbinary     |Controlled by the enable.mapping.varbinary property in properties (supported since 4.0.3). The default is false, which maps to string; when true, it maps to varbinary type.|
| other                                  | UNSUPPORTED   |                                                  |
