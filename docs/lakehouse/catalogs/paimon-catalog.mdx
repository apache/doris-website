...
Starting from Doris 4.1.x, Paimon Catalog metadata caches are configured with the unified `meta.cache.*` properties.
This section covers configuration and observability for Paimon-related cache modules.
...
|---|---|---|
| `schema` | `meta.cache.paimon.schema.` | Schema cache entry for table schema loading. |
...
- `schema` and `table` are separate entries. `schema` uses FE defaults unless `meta.cache.paimon.schema.*` is configured on the catalog.
