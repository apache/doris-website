...
1. `Could not find a file io implementation for scheme 's3a' in the classpath`
    ...
    Versions prior to 3.1 do not support the `s3a` protocol. If you must use it, you can temporarily add `s3a` protocol-related parameters as a workaround. This operation is not needed for versions 3.1 and later.
