...
* `{HiveProperties}`
  ...
  * `hive.staging_dir`: The root directory used for staging data during Hive write operations. Data is first written to this directory and then moved to the final Hive table location after the write completes. The path can be relative to the Hive table root or an absolute path. Defaults to `.doris_staging`. This parameter has been supported since version 4.0.3.
