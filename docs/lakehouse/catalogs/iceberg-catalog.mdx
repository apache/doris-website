...
To improve the performance of accessing external data sources, Apache Doris caches Iceberg metadata. Metadata includes loaded table metadata, partition data, manifests, derived partition projections, the latest snapshot id, and the inferred file format.
...
Starting from Doris 4.1.x, Iceberg Catalog's external metadata cache is configured using the unified `meta.cache.*` keys. The entries below describe the current version; Doris 4.1.x and 4.2.x use a different set of entries, described in the 4.x version of this page.
:::

### Cache Property Configuration {#meta-cache-unified-model}

Each cache entry uses a unified configuration key format: `meta.cache.<engine>.<entry>.{enable,ttl-second,capacity,max-weight}`. For Iceberg the `<engine>` is `iceberg`. `meta.cache.iceberg.table.ttl-second` is the shared TTL knob: it is the default TTL of the `table` and `partition` entries and the TTL of the caches without their own `<entry>` token. A value of `0` disables all of them, so every query reads the latest snapshot live.
...
The following entries have their own configuration keys and accept `max-weight`:

| Entry (`<entry>`) | Property Key Prefix | `ENTRY_NAME` | Cached Content and Impact | Default enable / TTL / capacity |
|---|---|---|---|---|
| `table` | `meta.cache.iceberg.table.` | `iceberg-table` | Loaded Iceberg table metadata. Impact: reduces Catalog round-trips and metadata file reads. | `true` / 86400 s / 1000 |
| `partition` | `meta.cache.iceberg.partition.` | `iceberg-partition` | Raw partition data of a table snapshot. | `true` / same as `table` / 1000 |
| `manifest` | `meta.cache.iceberg.manifest.` | `iceberg-manifest` | Parsed manifest contents. Scan planning uses it only when `enable` is `true`; the default is `false`. | `true` for the cache itself, `false` for its use / no expiration / 100000 |
| `partition_view` | `meta.cache.iceberg.partition_view.` | `iceberg.mvcc-partition-view`, `iceberg.list-partitions-view` | Two derived partition projections that share one entry limit. | `true` / 86400 s / 1000 |

The following caches have no `<entry>` token. They follow the Catalog and FE-wide memory limits and take their TTL from `meta.cache.iceberg.table.ttl-second`: `iceberg-latest-snapshot` (latest snapshot id), `iceberg-format` (inferred file format), `iceberg-comment` (table comments; created only for REST Catalogs with vended credentials), and `iceberg-equality-delete-field-ids`.

The column schema of Iceberg tables is cached by the shared `default` engine (`meta.cache.default.schema.*`).
...
* **Real-time access to the latest data**: If you want each query to see the latest snapshot of Iceberg tables, set `meta.cache.iceberg.table.ttl-second` to `0`.
  ```sql
  -- Disable the table, partition, latest-snapshot, and format caches to read the latest snapshot live
  ALTER CATALOG iceberg_ctl SET PROPERTIES ("meta.cache.iceberg.table.ttl-second" = "0");
  ```
* **Performance optimization**:
    * Enabling the manifest cache can significantly speed up query planning for large tables. Bound its memory with `max-weight` when manifests are large:
      ```sql
...
Since version 4.1.4, the file format is determined per data file (split), so **a single Iceberg table containing both Parquet and ORC data files can be queried normally**.

Since version 4.1.4, the Iceberg identity partition metadata is delivered with every split, and the path partition keys cover the identity columns of all partition specs of the table. As a result, partition column values are also returned correctly for tables that have gone through **Partition Evolution**.
...
Iceberg UUID continues to map to STRING or VARBINARY according to `enable.mapping.varbinary`, preserving the 16 raw bytes in both mappings; it is not automatically mapped to native Doris UUID. Use `CAST(HEX(uuid_column) AS UUID)` to obtain a native UUID, rather than parsing the raw bytes as canonical UUID text. Parquet writes to Iceberg UUID fields retain the UUID logical annotation, including nested fields. See the [UUID type](../../sql-manual/basic-element/sql-data-types/uuid.md).
...
Versions in the 4.0 line before 4.0.8 located columns by exact match when reading, so Iceberg and Paimon tables with mixed-case column names could read the wrong column or fail the query. This was fixed in 4.0.8.
...
- For an Iceberg target table, `DEFAULT()` is resolved against the Iceberg schema pinned for this write. Referencing a column that is not part of the write target reports `DEFAULT requires a target column reference` or `Cannot find column information for DEFAULT(<col>)`.
...
- Other errors: `DEFAULT requires a column reference, but got: <expr>` and `DEFAULT cannot be used on generated column '<name>'`.
...
Since version 4.1.4, Iceberg system tables are queried through the native execution path, and the `iceberg_meta()` table function has been **removed**. Use the `<table>$<system_table_name>` system table syntax instead. See [ICEBERG_META](../../sql-manual/sql-functions/table-valued-functions/iceberg-meta.md).
