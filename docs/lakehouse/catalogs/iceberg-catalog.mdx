...
### Rest Metastore
    ...
    <Tabs>
        <TabItem value='no-auth' label='No Authentication' default>
            <Tabs>
                <TabItem value='static-storage' label='Static Storage Credential' default>
                    
                    Supports S3, OSS, COS and other storage systems based on Iceberg native configuration.
                ...
                <TabItem value='vended-credentials' label='Vended Credentials'>
                    ...
                    Tested with Gravitino + S3. Since 3.1.1, `s3.endpoint/s3.region` are optional.
        ...
        <TabItem value='oauth2' label='OAuth2 Authentication'>
            <Tabs>
                <TabItem value='credential' label='Credential Authentication' default>
                    <Tabs>
                        <TabItem value='credential-static' label='Static Storage Credential' default>
                            
                            Tested with MinIO/S3 + Polaris.
                        ...
                        <TabItem value='credential-vended' label='Vended Credentials'>
                            ...
                            Tested with MinIO/S3 + Polaris. Since 3.1.1, `s3.endpoint/s3.region` are optional.
                ...
                <TabItem value='token' label='Token Authentication'>
                    <Tabs>
                        <TabItem value='token-static' label='Static Storage Credential' default>
...
- **No Authentication**: Direct access to REST Catalog service
- **OAuth2**: Authentication provided by REST service (e.g., Polaris built-in auth or Gravitino with Google OAuth)
  - **Credential**: Username/password authentication (format: `username:password`)
  - **Token**: Pre-obtained access token authentication

**Storage Credentials**
- **Vended Credentials**: Storage credentials dynamically provided by REST service, requires `'iceberg.rest.vended-credentials-enabled' = 'true'`
- **Since 3.1.1**: `s3.endpoint` and `s3.region` are optional when using Vended Credentials
