...
You can view this section to understand the file selection logic for rewriting. You can also use the following SQL queries to analyze which files will be rewritten based on system table information. This helps you debug and understand the rewrite behavior before executing the actual rewrite operation.

The rewrite operation uses a two-stage filtering and grouping strategy:

**Stage 1: File-level filtering**
Files are first filtered based on file-level conditions. Only files that meet at least one of the following conditions will be considered for rewriting:
...
After file-level filtering, files are grouped by partition, and then within each partition, files are grouped using the **BinPack algorithm** based on file size and `max-file-group-size-bytes`. The BinPack algorithm intelligently packs files into groups to optimize the rewrite operation.

After grouping, file groups are filtered based on group-level conditions. A file group will be rewritten if it meets at least one of the following conditions:

1. **Enough input files**: Group has more than 1 file and file count >= `min-input-files`
2. **Enough content**: Group has more than 1 file and total size > `target-file-size-bytes`
3. **Too much content**: Group total size > `max-file-group-size-bytes`
4. **Has files with delete issues**: Group contains at least one file with too many delete files or high delete ratio ⚠️ *Cannot be accurately calculated via system table SQL queries*

⚠️ **Note on BinPack grouping:** The BinPack algorithm used for grouping files within each partition is a complex optimization algorithm that cannot be accurately simulated using system table SQL queries. The algorithm considers file sizes and `max-file-group-size-bytes` to intelligently pack files into groups, which may result in different groupings than simple partition-based grouping. Therefore, the SQL queries provided below can only identify files that meet file-level conditions, but cannot accurately calculate the final file groups that will be rewritten.

#### Query 1: Identify files that meet file-level conditions
...
⚠️ **Note:** This query only identifies files that meet file-level conditions (file size). The actual file groups that will be rewritten depend on the BinPack grouping algorithm, which cannot be accurately simulated using SQL queries. Files identified by this query may be grouped differently by the BinPack algorithm, and some groups may be filtered out based on group-level conditions.
...
- Replace `iceberg_table` in the examples with your actual table name
- Adjust the parameter values (`@min_file_size_bytes`, `@max_file_size_bytes`) according to your actual rewrite parameters
- The default values shown above correspond to the default rewrite parameters (min-file-size-bytes = target-file-size-bytes * 0.75, max-file-size-bytes = target-file-size-bytes * 1.8)
