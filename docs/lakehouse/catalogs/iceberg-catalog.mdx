...
| fixed(N)                               | char(N)              |                                         |
| string                                 | string               |                                         |
| binary                                 | string/varbinary     | Controlled by the enable.mapping.varbinary property in properties (supported since 4.0.3). The default is false, which maps to string; when true, it maps to varbinary type.|
| uuid                                   | string/varbinary     | Before 4.0.3, it was mapped to STRING; after that, it is mapped to VARBINARY type.|
