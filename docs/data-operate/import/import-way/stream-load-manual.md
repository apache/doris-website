...
1. Specify it through an HTTP Header parameter.
    ...
    When both `compute_group` and `cloud_cluster` are set, `compute_group` takes precedence.
...
The `strict_mode` property is used to set whether the import task runs in strict mode. This property affects the results of column mapping, transformation, and filtering. For more about strict mode, see the [Strict mode](../handling-messy-data#strict-mode-strict_mode) documentation.
