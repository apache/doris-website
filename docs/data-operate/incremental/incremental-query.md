---
{
    ...
    "language": "en",
    "description": "Read the row-level changes of a Doris internal table by time window with @incr, without creating a Table Stream, in the APPEND_ONLY / MIN_DELTA / DETAIL incremental modes."
}
---
...
On a table with [Row Binlog](row-binlog.md) enabled, besides consuming changes through a [Table Stream](table-stream.md), you can read the changes of the table within a time window directly in a query with `@incr`, without creating any object or recording a consumption offset. It is a stateless, ordinary query and can be combined freely with WHERE, JOIN, aggregation, and so on.
