---
{
    ...
    "language": "en",
    "description": "Doris Table Stream guide: create, view and drop Streams, append_only / min_delta / detail consumption types, show_initial_rows, reading vs consuming.",
    "keywords": [
        "Table Stream",
        "Doris Stream",
        "CREATE STREAM",
...
- Doris 5.0.0 or later, with `enable_feature_binlog = true` and `enable_table_stream = true` on the FE.
- An internal base table with Row Binlog enabled; the `min_delta` / `detail` types additionally require a Unique Key MoW table with `binlog.need_historical_value` enabled, see [Row Binlog](row-binlog#supported-scope-and-limitations).
- `CREATE_PRIV` on the database of the Stream and `SELECT_PRIV` on the base table.

## Creating and managing Streams

<!-- Knowledge type: Operational steps + Syntax reference -->
...
Requirements for creating a Stream and the error when a requirement is not met:

| Requirement | Error when not met |
|---|---|
| Row Binlog enabled on the base table | `Base Olap table ... need to enable row binlog for table stream` |
| For `type = min_delta`, a MoW base table with `binlog.need_historical_value` enabled | `MIN_DELTA table stream requires base mow table to enable binlog.need_historical_value=true` |
| `CREATE_PRIV` on the database of the Stream and `SELECT_PRIV` on the base table | - |

The Stream and its base table can live in different databases. A Stream cannot share a name with a table or view. `CREATE OR REPLACE STREAM` is not supported yet. The full syntax is in [CREATE STREAM](../../sql-manual/sql-statements/table-and-view/stream/CREATE-STREAM).
...
The full column descriptions of the two system tables are in [table_streams](../../admin-manual/system-tables/information_schema/table_streams) and [table_stream_consumption](../../admin-manual/system-tables/information_schema/table_stream_consumption); the statements are described in [SHOW STREAMS](../../sql-manual/sql-statements/table-and-view/stream/SHOW-STREAMS) and [SHOW CREATE STREAM](../../sql-manual/sql-statements/table-and-view/stream/SHOW-CREATE-STREAM).
...
The consumption type and `show_initial_rows` cannot be changed after creation; drop and recreate the Stream instead. The statement is described in [ALTER STREAM](../../sql-manual/sql-statements/table-and-view/stream/ALTER-STREAM).
...
The statement is described in [DROP STREAM](../../sql-manual/sql-statements/table-and-view/stream/DROP-STREAM).

## Consumption types

<!-- Knowledge type: Behavior rules + Selection guide -->
<!-- Use cases: Choosing between append_only / min_delta / detail -->

| Consumption type | Output | Best for | Base table requirements |
|---|---|---|---|
| `append_only` | Only inserted rows; updates and deletes are not emitted | Append-only logs and event tables; downstream only cares about new data | Row Binlog enabled |
| `min_delta` (default) | All changes between two consumptions folded per key; only the net change is emitted | Keeping a downstream table in sync with the fewest rows | Unique Key MoW table with `binlog.need_historical_value` enabled |
| `detail` | Every change row by row, nothing folded | Auditing, replay, any scenario that needs the complete modification trail | Unique Key MoW table with `binlog.need_historical_value` enabled |
...
Duplicate Key tables only record inserts, so the three types produce the same output: every written row is one `APPEND`, multiple writes of the same key are not merged, and rows removed by `DELETE` statements are not emitted (see [Row Binlog](row-binlog#duplicate-key-tables)). A `min_delta` Stream created on a Duplicate Key table is automatically handled as `append_only`.

## Initial rows

<!-- Knowledge type: Configuration parameters + Behavior rules -->
<!-- Use cases: Syncing the existing data first, then the increments -->
...
<!-- Use cases: Understanding which operations advance the consumption offset -->
...
| One statement can consume multiple Streams | All their offsets advance. A Stream referenced several times in one statement (aliases, CTEs, subqueries) advances only once, and every reference sees the same data |
...
`<stream>@snapshot()` and `<stream>@reset()` read table images rather than changes and provide no virtual columns; see [Table Stream Advanced](table-stream-advanced).

## Limitations

<!-- Knowledge type: Usage limitations -->

- The base table must be an internal table with Row Binlog enabled; supported table models and column type restrictions are listed in [Row Binlog](row-binlog#supported-scope-and-limitations).
...
| Question | Answer |
|---|---|
| Why is a freshly created Stream empty? | With `show_initial_rows = false` (the default), the data that already exists at creation is not emitted as changes; create the Stream with `show_initial_rows = true` if the existing data is needed, see [Initial rows](#initial-rows) |
| Why does `SELECT *` show no change type? | Virtual columns are not included in `SELECT *`; list `__DORIS_STREAM_CHANGE_TYPE_COL__` and the other virtual columns explicitly |
| Can rows filtered out by WHERE during consumption be read again? | No. The offset advances by read range regardless of filtering; write every change type you need in the same statement |
| What happens when two jobs consume the same Stream at the same time? | The transaction that commits later fails and rolls back with `target offset already consumed`; make sure each partition has only one consumer at a time, see [Multiple Streams and concurrent consumption](table-stream-advanced#multiple-streams-and-concurrent-consumption) |
| Can a `min_delta` Stream be created on a Duplicate Key table? | Yes, but it is automatically handled as `append_only`, and rows removed by `DELETE` statements are not emitted |
| Can the consumption type of a Stream be changed? | No. `ALTER STREAM` only modifies the comment at the moment; drop and recreate the Stream |
| Where are the other errors explained? | See [Common errors](table-stream-advanced#common-errors) |
