...
Versions in the 4.0 line before 4.0.8 prefixed such errors with `failed to initialize storage reader`. Starting from 4.0.8 that prefix is removed and the error message keeps the real underlying cause, so do not use the prefix to identify this situation.
