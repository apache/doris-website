...
## Why use partitioning and bucketing in Apache Doris? {#why}
...
## What are Apache Doris partitioning and bucketing? {#what}
...
## How do Apache Doris partitioning and bucketing work? {#how}
...
## When should you use Apache Doris partitioning and bucketing? {#when}
...
- [MPP Architecture](./mpp): Bucket Shuffle and Colocate Join read the bucket layout you choose here to avoid network shuffles.
- [Unique Key](./unique-key): the table model where partition columns must be a subset of the unique key — otherwise dedup breaks.
