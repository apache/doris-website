...
>
> `LIMIT` and `OFFSET` are applied after sorting. Without `ORDER BY`, the scan stops as soon as enough rows have been gathered: with a `LIMIT`, the returned rows are a prefix in scan order; without a `LIMIT`, the whole result is ordered by `(TabletId, ReplicaId)`.
