...
| compression | Starting from version 4.0.3, the default compression method for Doris tables is ZSTD. Before version 4.0.3, the default compression method was LZ4F. |
...
| binlog.enable | Together with `binlog.format = "ROW"`, enables [Row Binlog](../../../../data-operate/incremental/row-binlog) on the table to record row-level changes for Table Stream consumption and `@incr` incremental queries. Can only be enabled at table creation and cannot be disabled afterwards. Available since 5.0.0. |
...
| binlog.need_historical_value | Whether Row Binlog records the values before a change (the before image), default `false`. Only Unique Key Merge-on-Write tables can set it to `true`; `min_delta` / `detail` Table Streams depend on it. Cannot be changed after creation. |
