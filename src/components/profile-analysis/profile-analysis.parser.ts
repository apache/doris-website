...
export type ProfileParserErrorCode = 'DAG_UNAVAILABLE' | 'DAG_TOO_LARGE' | 'DAG_PARSE_FAILED';
...
function validateGraph(nodes: ProfileDagNode[], edges: ProfileDagEdge[]): void {
    ...
    for (const edge of edges) {
        ...
        edgeKeys.add(key);
        (outgoing.get(edge.source) ?? outgoing.set(edge.source, []).get(edge.source) as string[]).push(edge.target);
...
export function parseProfileText(text: string): ProfileGraphIR {
    ...
    if (!inMerged || parsedNodes.length === 0) fail('DAG_UNAVAILABLE', 'The Profile has no usable MergedProfile operators.');
    ...
    for (const node of nodes.filter(candidate => candidate.operatorType === 'EXCHANGE_OPERATOR' && candidate.planNodeId !== null)) {
        const id = node.planNodeId as number;
        (receiversByPlanNode.get(id) ?? receiversByPlanNode.set(id, []).get(id) as ProfileDagNode[]).push(node);
    }
    ...
    for (const node of nodes) {
        if (node.nereidsId !== null && node.nereidsId !== undefined) {
            ...
            (semanticIndex.get(key) ?? semanticIndex.set(key, []).get(key) as ProfileDagNode[]).push(node);
        }
    ...
    for (const [family, sourceRoles, targetRoles, kind] of PAIRING_RULES) {
        for (const source of nodes.filter(node => node.operatorFamily === family && sourceRoles.includes(node.role as DagOperatorRole))) {
            ...
            candidates = candidates.filter(node => node.id !== source.id && targetRoles.includes(node.role as DagOperatorRole));
            if (candidates.length === 0 && source.planNodeId !== null && source.planNodeId !== undefined) {
                ...
                candidates = (semanticIndex.get(relationKey(source.fragmentId, family, 'plan', relationId)) ?? []).filter(
                    node => node.id !== source.id && targetRoles.includes(node.role as DagOperatorRole),
                );
            }
    ...
    for (const node of nodes.filter(candidate => candidate.operatorType === 'MULTI_CAST_DATA_STREAM_SOURCE_OPERATOR' && candidate.planNodeId !== null)) {
        const id = node.planNodeId as number;
        (multicastConsumers.get(id) ?? multicastConsumers.set(id, []).get(id) as ProfileDagNode[]).push(node);
    }
