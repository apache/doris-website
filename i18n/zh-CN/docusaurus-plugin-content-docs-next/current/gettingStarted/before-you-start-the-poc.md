---
{
    "title": "如何完成 Apache Doris POC 验证？",
    "language": "zh-CN",
    "description": "POC 过程中建表失败、查询慢、数据湖卡顿？本文汇总新用户常见问题与解决方案，覆盖建表设计、数据导入、查询调优、数据湖查询四大场景。",
    "sidebar_label": "POC 前必读",
    "tags": ["POC", "建表", "查询调优", "数据湖", "新用户"]
}
---
...
A: 一般 1-2 天完成基础功能验证（建表、导入、简单查询），详细性能调优需要 3-5 天。

**Q: 建表时如何选择分桶数？**

A: 优先设为 BE 节点数的整数倍，确保数据均匀分布。每个分桶压缩后数据建议 ≤ 20 GB（Unique Key 表 ≤ 10 GB）。
...
A: 如果涉及湖上查询（Hive/Iceberg/Paimon），建议开启。首次查询会自动缓存数据，重复查询可获得接近内表的性能。
