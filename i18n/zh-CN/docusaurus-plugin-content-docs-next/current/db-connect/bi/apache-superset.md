...
1. 访问 Superset Web 界面，点击右上角 **Settings** → **Database Connectors**。
    ...
    ![进入数据库连接器](/images/next/connect-dev/superset/02-superset-connect.png)

2. 点击 **+ Add Database**，在 **Connect a Database** 弹窗中选择 **Apache Doris**。

    ![选择 Apache Doris](/images/next/connect-dev/superset/03-superset-choose-db.png)

3. 在连接信息中填写 SQLAlchemy URI，完成连接验证后，点击 **Connect**。

    ![填写连接信息](/images/next/connect-dev/superset/04-superset-choose-test-connect.png)

4. 添加成功后，界面将显示已添加的数据源。

    ![添加成功](/images/next/connect-dev/superset/05-superset-after-connect.png)
...
1. 点击左侧导航栏 **Datasets** → **+ Add Dataset**。
    ...
    ![添加 Dataset](/images/next/connect-dev/superset/06-superset-add-dataset.png)
...
2. 依次选择配置后，点击右下角 **Create dataset and create chart**：
    ...
    ![选择数据表](/images/next/connect-dev/superset/07-superset-add-db.png)
...
1. 在 Dataset 编辑页面，点击 **Metrics** → **Add item**，添加计算指标：
    ...
    ![添加指标](/images/next/connect-dev/superset/09-superset-metrics.png)

### 3. 创建图表

1. 进入 **Chart** 页面，点击 **+ Add Chart**，选择 **lineitem** 作为数据集，**Line Chart** 作为图表类型。

    ![选择图表类型](/images/next/connect-dev/superset/10-superset-add-chart.png)
...
2. 配置图表参数：
    ...
    ![配置图表](/images/next/connect-dev/superset/11-superset-edit-chart.png)

3. 点击 **Update chart** 预览图表，确认无误后点击 **Save** 保存看板。

    ![保存看板](/images/next/connect-dev/superset/12-superset-edit-save-chart.png)
