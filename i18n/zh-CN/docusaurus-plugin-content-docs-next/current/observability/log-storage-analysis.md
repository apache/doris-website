---
{
    ...
    "language": "zh-CN",
    "description": "基于 Apache Doris 构建高性能、低成本的日志存储与分析平台，支持倒排索引、全文检索与冷热分层，替代 Elasticsearch。",
...
日志是系统运行的详细记录，包含事件主体、时间、位置、内容等关键信息。出于运维可观测、网络安全监控及业务分析等多重需求，企业通常需要将分散的日志采集起来，进行集中存储、查询和分析，以从日志数据中挖掘价值。

针对此场景，Apache Doris 在通用 OLAP 能力之外，增加了倒排索引和极速全文检索能力，并对写入性能与存储空间进行了极致优化，使用户可以基于 Apache Doris 构建开放、高性能、低成本、统一的日志存储与分析平台。
...
| 层级 | 组成 | 说明 |
| :--- | :--- | :--- |
| 日志采集与预处理 | Logstash、Filebeat、Fluentbit、Kafka 等 | 通过 HTTP APIs 将日志数据写入 Apache Doris |
| 日志存储与分析引擎 | Apache Doris | 提供高性能、低成本的统一存储，并通过 SQL 接口提供丰富检索分析能力 |
| 日志分析与告警界面 | Grafana、Superset、Doris WebUI 等 | 通过标准 MySQL 协议查询 Doris，提供易用的可视化界面 |
...
基于 Apache Doris 构建的日志平台具备以下核心特点：

| 能力维度 | 说明 |
| :--- | :--- |
| 高吞吐、低延迟写入 | 支持每天百 TB 级、GB/s 级日志数据持续稳定写入，延迟控制在 1s 以内 |
| 海量数据低成本存储 | 支持 PB 级存储，相对 Elasticsearch 节省 60%–80% 存储成本，冷数据存储到 S3/HDFS 后再降 50% |
| 高性能全文检索分析 | 支持倒排索引和全文检索，关键词检索明细、趋势分析等查询秒级响应 |
| 开放、易用的上下游生态 | 上游对接 Logstash/Filebeat/Fluentbit/Kafka 等；下游通过标准 MySQL 协议对接 Grafana、Superset、Doris WebUI |

### 2.1 高性能、低成本

经过 Benchmark 测试与生产验证，基于 Apache Doris 构建的日志平台性价比相对于 Elasticsearch 具有 5～10 倍的提升。优势主要源自高性能存储与查询引擎，以及针对日志场景的专门优化：

- **写入吞吐提升**：Elasticsearch 写入瓶颈在于解析数据和构建倒排索引的 CPU 消耗。Apache Doris 利用 SIMD 等 CPU 向量化指令提升 JSON 解析与索引构建性能；同时简化了倒排索引结构，去掉日志场景不需要的正排等数据结构。同样资源下，Doris 写入性能是 Elasticsearch 的 3～5 倍。
...
- **查询性能提升**：Doris 简化了全文检索流程，跳过相关性打分等日志场景不需要的算法；针对“查询包含某关键字的最新 100 条日志”等典型查询，在查询规划与执行上做 TopN 动态剪枝等专门优化。
...
Apache Doris 对 Flexible Schema 的支持包括：
...
3. **评估查询资源**：查询的资源消耗随查询量与复杂度而异。建议初始预留 50% 的 CPU 资源用于查询，再根据实际测试情况调整。
...
完成资源评估后，可以开始部署 Apache Doris 集群，推荐在物理机或虚拟机环境中进行部署。手动部署步骤可参考 [手动部署](./install/deploy-manually/integrated-storage-compute-deploy-manually)。
...
更多分区分桶信息可参考 [数据划分](./table-design/data-partitioning/data-distribution)。
...
- 使用 time_series 策略（`"compaction_policy" = "time_series"`），减轻写放大效应，对高吞吐日志写入的资源消耗很重要。

#### 3.4.4 建立和配置索引

- 对经常查询的字段建立索引（`USING INVERTED`）。
- 对需要全文检索的字段，将分词器（parser）参数赋值为 `unicode`，一般可满足大部分需求。如有支持短语查询的需求，将 `support_phrase` 参数赋值为 `true`；如不需要则设为 `false`，以降低存储空间。
...
完成建表后即可进行日志采集。Apache Doris 提供开放、通用的 Stream HTTP APIs，可与常用日志采集器（Logstash、Filebeat、Kafka 等）打通。下表概括了不同采集方式的适用场景：
...
更多配置说明可参考 [Logstash Doris Output Plugin](./ecosystem/observability/logstash)。
...
更多配置说明可参考 [Beats Doris Output Plugin](./ecosystem/observability/beats)。
