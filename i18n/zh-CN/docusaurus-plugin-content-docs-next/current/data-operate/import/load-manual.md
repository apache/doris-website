...
| 应用程序高并发或高频小批量写入              | JDBC / HTTP                    | [Group Commit](./group-commit-manual.md) + JDBC INSERT 或 Stream Load                                                       |
...
- **高并发或高频次（大于 20 并发，或 1 分钟内多次写入）**：建议开启 [Group Commit](./group-commit-manual.md)，配合 JDBC INSERT 或 Stream Load 使用。
...
4. **Streaming Job 持续导入（无需外部组件）**
    ...
    | :--------- | :---------------------------------------- | :------------- | :------------- | :--------------------------------------------- |
    | 表级同步   | Job + TVF（INSERT INTO SELECT）           | 需预建         | exactly-once   | 需要列裁剪、字段重命名、类型转换、条件过滤      |
    | 库级同步   | Job + 原生整库 DDL                        | 首次自动创建   | at-least-once  | 整库或一组表的镜像复制，下游表结构自动跟随上游  |
...
建议开启 [Group Commit](./group-commit-manual.md)，并配合 JDBC INSERT 或 Stream Load 使用。当并发大于 20，或 1 分钟内写入多次时，Group Commit 可显著降低导入压力。
