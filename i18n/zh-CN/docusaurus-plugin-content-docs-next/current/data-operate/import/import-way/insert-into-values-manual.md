...
:::tip 性能提示
并发的 INSERT INTO VALUES 性能会受到 commit 阶段瓶颈的限制。当导入数据量较大时，建议开启 [Group Commit](../load-best-practices/group-commit-manual.md) 以获得更高吞吐。
:::
...
INSERT INTO VALUES 通常用于测试和演示，**不建议**用于导入大量数据的场景。如有大批量写入需求，建议使用 [Group Commit](../load-best-practices/group-commit-manual.md) 或其他批量导入方式。
...
A：INSERT INTO VALUES 在高并发或大数据量场景下会受到 commit 瓶颈限制，建议开启 [Group Commit](../load-best-practices/group-commit-manual.md)，或选择 Stream Load、Broker Load 等专用导入方式。
...
| 高并发下吞吐不高 | 受 commit 阶段瓶颈限制 | 开启 [Group Commit](../load-best-practices/group-commit-manual.md) |
