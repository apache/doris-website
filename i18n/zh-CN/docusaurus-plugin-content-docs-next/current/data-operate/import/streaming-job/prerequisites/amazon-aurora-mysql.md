---
{
    ...
    "language": "zh-CN",
    "description": "在 Amazon Aurora MySQL 上配置 Binlog 以支持 Doris 持续导入的前置步骤。"
}
---

## 概述

Doris 持续导入支持 Amazon Aurora MySQL-Compatible Edition 5.6、5.7、8.0.x 版本。在同步数据之前，需要确保 Aurora 集群已开启 Binlog 并正确配置。本文档将指导您完成所有前置配置步骤。

## 步骤一：检查当前配置

首先检查 Binlog 是否已开启以及格式是否正确，连接到 Aurora 写入实例后执行以下 SQL：
...
否则，请继续以下步骤。Aurora MySQL 默认不开启 Binlog，需要通过集群参数组开启。

## 步骤二：配置集群参数组

1. 登录 [AWS RDS 控制台](https://console.aws.amazon.com/rds/)。
2. 在左侧导航栏选择 **Parameter groups**，点击 **Create parameter group**。
3. 类型选择 **DB Cluster Parameter Group**，选择对应的 Aurora MySQL 版本族。
4. 编辑集群参数组，搜索 `binlog_format`，将值设置为 `ROW`：
...
建议将 Binlog 保留时间设置为至少 72 小时，以确保在故障场景下用于复制的二进制日志文件仍然可用。
