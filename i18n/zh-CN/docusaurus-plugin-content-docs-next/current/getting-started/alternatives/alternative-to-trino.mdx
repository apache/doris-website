...
**Q: Doris 与 Trino 可以共存吗？**

A: 功能上，在 Doris 满足数据源类型要求的情况下，可以代替 Trino。但需根据迁移情况和现有业务的兼容情况而定。

**Q: 迁移到 Doris 需要多长时间？**

A: Doris 提供丰富的 ETL 工具和 Hive 兼容 API，迁移周期通常为 2-4 周。

**Q: Doris 的湖仓查询性能真的比 Trino 快 2-3 倍吗？**

A: 是的。根据 TPC-DS 1TB 性能测试，当对外部 Hive 表进行查询时，Doris 在数据湖场景下展现出更优的查询加速性能。

**Q: 如何选择合适的物化视图策略？**

A: Doris 支持多种物化视图刷新策略，建议根据数据更新频率和查询模式选择增量刷新或全量刷新。查询优化器会自动匹配最优物化视图。
