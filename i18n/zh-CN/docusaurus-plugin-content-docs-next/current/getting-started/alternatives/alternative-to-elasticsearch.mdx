...
Apache Doris 提供多种数据导入方案，支持从 Elasticsearch 全量迁移数据。具体步骤请参考[数据导入文档](../../data-operate/import/load-manual)。

### Q: Doris 的学习曲线如何？

Doris 支持标准 SQL，使用 MySQL 协议连接，对于熟悉 SQL 的工程师无需额外学习查询语言。

### Q: Doris 和 Elasticsearch 的日志检索性能差异？

两者在简单关键词检索场景性能相近，但 Apache Doris 在聚合分析、时序数据分析场景下性能更优，且支持更丰富的分析函数。
