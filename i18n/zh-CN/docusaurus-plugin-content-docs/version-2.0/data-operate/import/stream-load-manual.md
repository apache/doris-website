...
相比于直接使用 `curl` 的单并发导入，更推荐使用专用导入工具 Doris Streamloader 该工具是一款用于将数据导入 Doris 数据库的专用客户端工具，可以提供**多并发导入**的功能，降低大数据量导入的耗时。拥有以下功能：
...
点击 [Doris Streamloader 文档](https://doris.apache.org/zh-CN/docs/2.0/ecosystem/doris-streamloader) 了解使用方法与实践详情。
:::
...
关于导入时，如何表达部分列更新，可以参考 [数据操作/数据更新](../../data-operate/update/update-overview) 文档
