...
不同的权限可以授予不同的层级。通过 `GRANT` 语句指定的资源路径，决定了权限的作用范围。

- **全局权限**：通过 `GRANT` 语句授予 `*.*.*` 上的权限。被授予的权限适用于任意 Catalog 中的任意库表。
- **数据目录 (Catalog) 权限**：通过 `GRANT` 语句授予 `ctl.*.*` 上的权限。被授予的权限适用于指定 Catalog 中的任意库表。
- **库级权限**：通过 `GRANT` 语句授予 `ctl.db.*` 上的权限。被授予的权限适用于指定数据库中的任意表。
- **表级权限**：通过 `GRANT` 语句授予 `ctl.db.tbl` 上的权限。被授予的权限适用于指定表的任意列。
- **列级权限**：用于限制用户对数据表中某些列的访问权限。可以通过 `GRANT Select_priv(col1,col2) ON ctl.db.tbl TO user1` 授予指定表部分列的权限。目前列权限仅支持 `Select_priv`。
- **行级权限**：使得管理员能够基于数据的某些字段定义访问策略，从而控制哪些用户可以访问哪些数据行。具体配置参见[数据访问控制](../authorization/data)。
- **Resource 使用权限**：Resource 权限是为 Resource 单独设置的权限，和库表等权限没有关系，只能分配 `Usage_priv` 和 `Grant_priv`。例如：`GRANT USAGE_PRIV ON RESOURCE '%' TO user1`。
- **Workload Group 使用权限**：Workload Group 权限是为 Workload Group 单独设置的权限，和库表等权限没有关系，只能分配 `Usage_priv` 和 `Grant_priv`。例如：`GRANT USAGE_PRIV ON WORKLOAD GROUP '%' TO user1`。
