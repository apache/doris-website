...
是否使用 mmap 分配块。 如果启用此功能，最好增加 vm.max_map_count 的值，其默认值为 65530。您可以通过 "sysctl -w vm.max_map_count=262144" 或 "echo 262144 > /proc/sys/vm/max_map_count" 以 root 身份进行操作 max_map_count" ，当这个设置生效时，你必须将chunk_reserved_bytes_limit设置为一个相对较大的数字，否则性能非常非常糟糕。
