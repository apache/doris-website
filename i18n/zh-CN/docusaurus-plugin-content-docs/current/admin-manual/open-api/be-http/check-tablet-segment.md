...
当用户查询时，会报出打开数据文件失败的错误，并在错误信息末尾带上 `tablet=<tablet_id>` 与 `backend=<be_ip>`。该接口的功能是检测出当前 BE 节点上所有存在文件丢失的 tablet。

:::info 版本说明

4.0.8 之前的 4.0.x 版本会统一在这类错误前加上 `failed to initialize storage reader` 前缀。自 4.0.8 起该前缀被去掉，错误信息保留底层的真实原因，因此不要以该前缀作为判断依据。
