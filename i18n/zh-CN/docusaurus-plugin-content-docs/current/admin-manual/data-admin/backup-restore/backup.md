---
{
    "title": "备份",
    "language": "zh-CN",
    "description": "Doris 提供了备份功能，将数据库、表或分区的数据以快照形式备份到远程存储系统。本指南提供了创建 Repository 和备份数据的操作步骤。"
}
---

Doris 提供了备份功能，使用户能够将数据从库、表或者分区备份到远程存储系统，并在需要时进行[恢复](./restore)。

## 关键概念

- **快照 (Snapshot)**：数据库、表或分区中数据的时间点捕获。创建快照时需要指定一个快照 Label，快照完成时会生成一个时间戳，可以通过 Repository、快照 Label 和时间戳唯一标识一个快照。
