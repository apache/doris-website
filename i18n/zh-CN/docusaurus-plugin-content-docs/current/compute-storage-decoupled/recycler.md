...
| `g_bvar_recycler_instance_last_recycle_duration` | `recycler_instance_last_round_recycle_duration` | instance_id | 最近一轮回收的总用时 |
...
1. 开启 `enable_recycler_stats_metrics = true`，在 MS 面板查看 `recycler_instance_last_recycle_duration` 和 `recycler_instance_recycle_last_success_ts` 指标。
2. 若 `recycler_instance_last_success_ts` 长时间未更新，说明回收可能卡住，需排查日志。
