...
- **主动预热**：尽管远端存储的数据可实现多计算组共享，但缓存数据并不会共享。当新计算组创建时，缓存为空，此时可主动预热，使集群迅速从远端存储拉取所需数据至本地缓存。
...
在存算分离模式下，Doris 支持多计算组部署，各计算组间共享数据但不共享缓存。当新计算组创建时，其缓存处于空置状态，此时查询性能可能受到影响。为此，Doris 提供缓存预热功能，允许用户主动从远端存储拉取数据至本地缓存。目前，该功能支持以下三种模式：

- **计算组间预热**：将计算组 A 的缓存数据预热至计算组 B 中。Doris 会定期收集各计算组在一段时间内被访问的表/分区的热点信息，并作为内部表存储下来，预热时根据这些信息选择性地预热某些表/分区。
...
当执行以下 SQL 时，`cluster_name1` 计算组会获取 `cluster_name0` 计算组的访问信息，来尽可能还原出与 `cluster_name0` 集群一致的缓存。
