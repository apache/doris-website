...
Doris 主键模型支持UPDATE 语句更新和基于导入的批量更新，其中 UPDATE 适用于小规模数据更新，而批量更新基于 UPSERT 语义，适用于高效处理大规模数据变更。
...
Doris 通过表级锁和 MVCC 机制控制主键模型的更新并发，其中 UPDATE 语句默认串行执行，可通过配置放宽并发限制，而基于导入的批量更新依赖版本控制，支持原子性并发更新，并可通过 sequence 列确保更新顺序的确定性。
...
详细原理参考文档 [主键模型的更新并发控制](../update/unique-update-concurrent-control.md) 中对更新的并发控制机制进行详细介绍。
...
在使用主键模型和聚合模型更新时，建议遵循以下规则：

- 大部分有数据更新需求的场景，**首选主键模型**。例如从 TP 数据库 CDC 同步到 Doris，用户画像，人群圈选等。
- 下面两类场景，建议使用聚合模型：
