---
{
    ...
    "language": "zh-CN",
    "description": "Doris Table Stream 使用指南：创建、查看、删除 Stream，append_only / min_delta / detail 三种消费类型的语义与对比，show_initial_rows 的含义，查询与消费的事务语义，以及虚拟列和使用限制。"
}
---
...
Table Stream（下文简称 Stream）是建立在 [Row Binlog](row-binlog.md) 之上的变更消费对象。它记住"消费到哪了"，每次读取只返回上次消费之后基表发生的变化，并在把变更写入目标表的同一个事务里推进消费位点，从而做到不重不漏。
...
- 基表已开启 Row Binlog，否则报 `Base Olap table ... need to enable row binlog for table stream`。
- `type = min_delta` 时基表必须是 MoW 表且开启 `binlog.need_historical_value`，否则报 `MIN_DELTA table stream requires base mow table to enable binlog.need_historical_value=true`。
- 在 Stream 所在数据库有 `CREATE_PRIV`，对基表有 `SELECT_PRIV`。

Stream 与基表可以位于不同的数据库。同名的 Stream 不能与表、视图重名。`CREATE OR REPLACE STREAM` 目前不支持。
...
两张系统表的完整列说明见 [table_streams](../../admin-manual/system-tables/information_schema/table_streams) 与 [table_stream_consumption](../../admin-manual/system-tables/information_schema/table_stream_consumption)。
...
消费类型和 `show_initial_rows` 创建后不能修改，需要删除后重建。
...
Duplicate Key 表只记录新增，三种类型的输出相同：每一行写入都是一条 `APPEND`，同 key 的多次写入不会合并，`DELETE` 语句删除的数据也不会输出（见 [Row Binlog](row-binlog.md#duplicate-key-表)）。在 Duplicate Key 表上创建 `min_delta` 类型的 Stream 会自动按 `append_only` 处理。
...
- 基表必须是开启 Row Binlog 的内表，支持的表模型和列类型限制见 [Row Binlog](row-binlog.md#支持范围与限制)。
...
- 基表被删除、分区被删除或替换等操作对 Stream 的影响见 [Table Stream 进阶](table-stream-advanced.md#基表变更的影响)。
