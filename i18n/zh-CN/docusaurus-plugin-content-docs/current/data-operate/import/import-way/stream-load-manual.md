...
| timezone                     | 指定本次导入所使用的时区。默认为[集群当前时区](https://doris.apache.org/zh-CN/docs/3.0/admin-manual/cluster-management/time-zone?_highlight=time_zone#1-%E5%87%BD%E6%95%B0)。该参数会影响所有导入涉及的和时区有关的函数结果。例如，指定导入时区为 Africa/Abidjan，需要指定命令 `-H "timezone:Africa/Abidjan"`。 |
