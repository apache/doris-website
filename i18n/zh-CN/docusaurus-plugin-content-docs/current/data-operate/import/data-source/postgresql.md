...
- [PostgreSQL CDC 自动建表同步](../streaming-job/continuous-load-postgresql-database.md)：首次同步时由 Doris 自动建表（通过 `include_tables` 可控制同步一张、多张或全部表），提供 at-least-once 语义。
- [PostgreSQL CDC SQL 映射同步](../streaming-job/continuous-load-postgresql-table.md)：目标表需预先在 Doris 中创建，支持灵活的列映射和数据转换，提供 exactly-once 语义。
...
更多通用操作和完整参数说明，请参考 [PostgreSQL CDC 自动建表同步](../streaming-job/continuous-load-postgresql-database.md)。
...
更多通用操作（暂停、恢复、删除、查看 Task 等）以及完整参数说明，请参考 [PostgreSQL CDC SQL 映射同步](../streaming-job/continuous-load-postgresql-table.md)。
