...
从 Doris 4.1.x 开始，Hive Catalog 的外表元数据缓存使用统一键 `meta.cache.*` 进行配置。本节只介绍**如何使用**与**如何观测**。
...
| 模块 | 属性键前缀 | 典型缓存内容 |
|---|---|---|
| `partition-values` | `meta.cache.hive.partition-values.` | 分区值/分区名称列表（常用于分区剪枝与分区枚举）。 |
