...
* `{PaimonProperties}`
    ...
    **Paimon 读参数**
    ...
    Doris 支持一组经过限制的 Paimon 批读参数。可以通过 Catalog 属性
    `paimon.table-option.<option-key>` 为所有查询设置默认值，也可以通过
    `table@options('<option-key>'='<value>')` 仅覆盖单个表引用的参数。

    | Paimon 参数 | 默认值 | Doris 校验范围 | 作用 |
    | --- | --- | --- | --- |
    | `read.batch-size` | `1024` | `1` 到 `65536` 的整数 | Paimon JNI Reader 每批返回的行数。 |
    ...
    | `scan.manifest.parallelism` | 可用处理器数量 | `1` 到 FE 可用处理器数量的整数 | Paimon 规划扫描时读取 manifest 的并行度。 |
    ...
    每个表引用使用独立的参数集合。优先级从高到低为：

    ```text
    表引用 @options > Doris Catalog 属性 > 物理 Paimon 表参数 > Paimon 默认值
    ```

    例如，物理表设置了 `read.batch-size=1024` 时，Catalog 中的 `4096` 会在 Doris 查询中覆盖它；
    上述查询又会仅针对该表引用把值覆盖为 `8192`。split 和 manifest 规划参数在 FE 中生效，
    Reader 参数还会被序列化并发送给 BE 上的 Paimon JNI Scanner。

    Doris 会在 `CREATE CATALOG` 和 `ALTER CATALOG` 时拒绝未知或非法的
    `paimon.table-option.*` 属性。旧 Doris 版本持久化的 Catalog 仍可加载，但当前白名单之外的历史参数
    会被忽略。这些动态参数不会修改物理 Paimon 表元数据。

    快照和启动位置参数应使用下文查询章节介绍的专用 `@options` 语法，不应配置为 Catalog Reader
    默认值。Doris 也不支持 Paimon Flink Source Enumerator 专用的 `scan.max-splits-per-task`，并排除
    流式 Source、表布局、写入和 Compaction 参数。Bucket、Primary Key、Partition、Merge Engine
    及其他物理表行为应在 Paimon 中配置。
