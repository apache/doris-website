...
本节说明 Paimon 相关 cache 模块的配置与观测方式。
...
| 模块 | 属性键前缀 | 典型缓存内容 |
|---|---|---|
| `schema` | `meta.cache.paimon.schema.` | 表 schema 加载对应的 schema cache entry。 |
| `table` | `meta.cache.paimon.table.` | Paimon 表元数据（用于查询规划，实际涉及 schema/snapshot/partition 等元数据加载）。 |

说明：

- 属性键使用上表中的模块名。这些名字也会出现在 `information_schema.catalog_meta_cache_statistics` 的 `ENTRY_NAME` 中。
- `schema` 和 `table` 是两个独立 entry。若 catalog 未设置 `meta.cache.paimon.schema.*`，`schema` 使用 FE 默认值。
...
Paimon 缓存指标可通过 `information_schema.catalog_meta_cache_statistics` 查询。
