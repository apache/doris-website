...
显示表的审计日志信息。如果需要了解 Paimon 表中每条记录的具体操作，可以从相应的审计日志表中读取数据。与原始 Paimon 表相比，审计日志表在每条记录的开头插入一个名为 `rowkind` 的列来存储操作类型。`rowkind` 列中的有效值为：+I（插入）、-U（更新前）、+U（更新后）、-D（删除）：
...
显示表的二进制日志信息，用于捕获表的变更操作。与 `audit_log` 不同，`binlog` 记录的是底层的二进制变更日志，而 `audit_log` 在每条记录开头插入 `rowkind` 列来标识操作类型（+I、-U、+U、-D）：
...
显示表的只读优化信息。对于主键表，如果要提高批量读取或即席 OLAP 查询的效率，可以从相应的只读优化表中读取数据。只读优化表只读取不需要压缩的文件，消除了压缩过程，提高了查询效率：
