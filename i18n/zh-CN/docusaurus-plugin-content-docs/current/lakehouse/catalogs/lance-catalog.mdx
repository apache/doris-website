...
索引能否用于 `vector_search()`，取决于向量元素类型以及创建索引时使用的距离类型，因为内置的 Lance 版本并未实现全部组合。下表描述使用索引检索的支持情况。表中每个可用组合都经过端到端验证：物理索引创建成功，Doris 通过 `lance-c` 读取该索引，并且查询经由索引路径返回了结果。

| 向量元素类型 | `l2` | `cosine` | `dot` | `hamming` |
|---|---|---|---|---|
| `float16` | 仅支持 Flat Search [1] | 支持 | 支持，`IVF_PQ` 除外 [1] | 不支持 |
...
1. 使用 `l2` 创建 `float16` 索引时，Lance 在累加聚类中心的过程中可能发生溢出，索引构建无法完成；`IVF_PQ` 使用 `float16` 和 `dot` 训练子量化器时也可能遇到同样的问题。这是 Lance 索引构建器的限制，而不是 Doris 查询路径的问题：`float16` 向量的 Flat Search 可以正常使用，使用 `cosine` 或 `dot` 创建的 `float16` 索引也可以正常查询。
...
只有当查询请求的 `metric` 与创建索引时使用的距离类型一致时，Doris 才会选择该向量索引；否则 Doris 不使用索引，改为执行 Flat Search——结果仍然正确，但需要直接扫描向量。`EXPLAIN` 可以区分两者：使用索引的计划中 `lanceSearchIndexSegments` 不为 0，未选中索引时显示 `lanceSearchIndexSegments=0`。由于未设置的 `metric` 会被视为 `l2`，查询使用 `cosine` 或 `dot` 创建的索引时请记得显式设置 `metric`。
