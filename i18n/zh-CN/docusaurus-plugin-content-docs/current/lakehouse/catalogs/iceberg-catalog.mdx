...
1. **表数据操作**：对表中的数据文件进行优化和重写
2. **元数据管理操作**：对表的快照（Snapshot）和分支（Branch）进行管理

### 表数据操作

#### rewrite_data_files

`rewrite_data_files` 操作用于对 Iceberg 表进行数据文件重写，实现小文件合并（Compaction）功能，以优化查询性能和存储效率。
...
1. `rewrite_data_files` 操作会读取数据文件并重新写入，会产生额外的 I/O 和计算开销，建议在业务低峰期执行该操作
2. 执行前可以通过【查看数据文件分布】章节中的 SQL 来评估是否需要执行重写操作
3. WHERE 条件可用于限制重写的分区或数据范围，这个条件会过滤掉那些不包含 符合WHERE 条件的数据的文件，从而减少重写的文件数量和数据量
4. 执行前可以通过【查看会被重写的文件】章节中的 SQL 来预测哪些文件会被重写

### 元数据管理操作

元数据管理操作用于管理 Iceberg 表的快照和分支，包括快照的回滚、切换、合并等功能。
...
你可以使用以下 SQL 查询来基于系统表信息分析哪些文件会被重写。这有助于你在执行实际重写操作之前进行调试和理解重写行为。

重写操作会在文件同时满足文件级别和分区组级别条件时重写文件：

**文件级别条件：**
1. **文件大小超出范围**：文件小于 `min-file-size-bytes` 或大于 `max-file-size-bytes`
2. **删除文件过多**：关联的删除文件数量 >= `delete-file-threshold` ⚠️ *无法通过系统表 SQL 查询准确计算*
3. **删除比例过高**：删除记录比例 >= `delete-ratio-threshold` ⚠️ *无法通过系统表 SQL 查询准确计算*
...
**分区组级别条件**（文件按分区分组）：
1. **足够的输入文件**：分区文件数 > 1 且文件数 >= `min-input-files`
2. **足够的内容**：分区文件数 > 1 且总大小 > `target-file-size-bytes`
3. **内容过多**：分区总大小 > `max-file-group-size-bytes`
4. **包含有删除文件问题的文件**：分区内至少有一个文件有太多删除文件或删除比例过高 ⚠️ *无法通过系统表 SQL 查询准确计算*
5. **包含有大小问题的文件**：分区内至少有一个文件满足文件大小条件

只有当文件满足至少一个文件级别条件 AND 其所在分区组满足至少一个分区组级别条件时，该文件才会被重写。
...
该查询提供所有会被重写文件的综合分析，同时考虑文件级别和分区组级别的条件：

```sql
-- 设置参数（根据你的重写参数调整）
SET @min_file_size_bytes = 16 * 1024 * 1024;   -- 16 MB
SET @max_file_size_bytes = 768 * 1024 * 1024;  -- 768 MB
SET @min_input_files = 5;                      -- 默认: 5
SET @max_file_group_size_bytes = 107374182400; -- 默认: 100GB
SET @target_file_size_bytes = 536870912;       -- 默认: 512MB
...
SELECT 
    ...
    -- 文件会被重写当且仅当：文件有文件级别问题 AND 分区组满足组级别条件
    (fa.size_issue 
...
- 根据你的实际重写参数调整参数值（`@min_file_size_bytes`、`@max_file_size_bytes`、`@min_input_files`、`@max_file_group_size_bytes`、`@target_file_size_bytes`）
- 上面显示的默认值对应于默认重写参数（target-file-size-bytes = 512MB，min-input-files = 5，max-file-group-size-bytes = 100GB）
- 对于大表，这些查询可能需要一些时间。建议先添加分区过滤条件或使用 `LIMIT` 在子集上测试
- 文件按分区分组进行分区组级别分析。在重写操作中，一个分区对应一个文件组
- **删除文件条件：** 重写操作也会考虑 `delete-file-threshold` 和 `delete-ratio-threshold` 参数，这些参数会影响文件重写选择。但是，这些条件无法通过系统表 SQL 查询准确计算，因为删除文件与数据文件的关联需要更详细的元数据，这些信息在当前系统表中不可用。因此，分析查询仅关注可以准确计算的文件大小条件
