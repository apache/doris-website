...
为了提升访问外部数据源的性能，Apache Doris 会缓存 Iceberg 的元数据，包括已加载的表元数据、分区数据、Manifest、派生的分区投影、最新 Snapshot ID 以及推断出的文件格式。

:::tip
Doris 4.1.x 之前的版本，元数据缓存主要由 FE 配置项全局控制，详见[元数据缓存](../meta-cache)。
自 Doris 4.1.x 起，Iceberg Catalog 的外表元数据缓存使用统一的 `meta.cache.*` 键配置。下文的模块描述当前版本；Doris 4.1.x 和 4.2.x 的模块集合与此不同，见本页的 4.x 版本。
:::

### 缓存属性配置 {#meta-cache-unified-model}

每个缓存模块使用统一的配置键格式：`meta.cache.<engine>.<entry>.{enable,ttl-second,capacity,max-weight}`。Iceberg 的 `<engine>` 为 `iceberg`。`meta.cache.iceberg.table.ttl-second` 是共享的 TTL 开关：它既是 `table` 和 `partition` 模块的默认 TTL，也是没有独立 `<entry>` 名的各缓存的 TTL。设为 `0` 时这些缓存全部关闭，每次查询实时读取最新 Snapshot。
...
以下模块拥有独立配置键并接受 `max-weight`：

| 模块（`<entry>`） | 属性键前缀 | `ENTRY_NAME` | 缓存内容与影响 | 默认 enable / TTL / capacity |
|---|---|---|---|---|
| `table` | `meta.cache.iceberg.table.` | `iceberg-table` | 已加载的 Iceberg 表元数据。影响：减少访问 Catalog 和读取元数据文件的次数。 | `true` / 86400 秒 / 1000 |
| `partition` | `meta.cache.iceberg.partition.` | `iceberg-partition` | 表快照的原始分区数据。 | `true` / 与 `table` 相同 / 1000 |
| `manifest` | `meta.cache.iceberg.manifest.` | `iceberg-manifest` | 解析后的 Manifest 内容。只有 `enable` 为 `true` 时扫描规划才会使用，默认 `false`。 | 缓存本身 `true`，是否使用默认 `false` / 不过期 / 100000 |
| `partition_view` | `meta.cache.iceberg.partition_view.` | `iceberg.mvcc-partition-view`、`iceberg.list-partitions-view` | 两种派生分区投影，共用一个模块上限。 | `true` / 86400 秒 / 1000 |

以下缓存没有 `<entry>` 名，只受 Catalog 和 FE 总内存上限约束，TTL 取自 `meta.cache.iceberg.table.ttl-second`：`iceberg-latest-snapshot`（最新 Snapshot ID）、`iceberg-format`（推断出的文件格式）、`iceberg-comment`（表注释，仅为使用 vended credentials 的 REST Catalog 创建）和 `iceberg-equality-delete-field-ids`。

Iceberg 表的列结构由共享的 `default` 引擎缓存（`meta.cache.default.schema.*`）。
...
* **实时访问最新数据**：希望每次查询都看到 Iceberg 表的最新 Snapshot 时，将 `meta.cache.iceberg.table.ttl-second` 设为 `0`。
  ```sql
  -- 关闭 table、partition、latest-snapshot 和 format 缓存，实时读取最新 Snapshot
  ALTER CATALOG iceberg_ctl SET PROPERTIES ("meta.cache.iceberg.table.ttl-second" = "0");
  ```
* **性能优化**：
    * 开启 Manifest 缓存可以显著加速大表的查询规划。Manifest 很大时用 `max-weight` 限制其内存：
      ```sql
...
自 4.1.4 版本起，文件格式按每个数据文件（Split）单独判定，因此**同一张 Iceberg 表中同时存在 Parquet 和 ORC 数据文件时也可以正常查询**。

自 4.1.4 版本起，Iceberg 的 Identity 分区元数据会随每个 Split 下发，路径分区键覆盖表所有 Partition Spec 中的 Identity 列。因此经历过 **Partition Evolution** 的表，其分区列取值在查询中也能正确返回。
...
Iceberg UUID 继续按 `enable.mapping.varbinary` 映射为 STRING 或 VARBINARY，两种映射均保留 16 字节原始值；不会自动映射为 Doris 原生 UUID。使用 `CAST(HEX(uuid_column) AS UUID)` 转换为原生 UUID，不要直接将原始字节当作标准 UUID 文本解析。向 Iceberg UUID 字段写入 Parquet 时保留 UUID 逻辑标记，包括嵌套字段。详见 [UUID 类型](../../sql-manual/basic-element/sql-data-types/uuid.md)。
...
4.0.8 之前的 4.0.x 版本在读取时按精确匹配定位列，含大小写混合列名的 Iceberg / Paimon 表可能读到错误的列或查询失败。该问题已在 4.0.8 中修复。
...
2. 隐藏列由系统进行维护，不允许用户在使用 `INSERT` 等写入语句时显式指定写入 `_row_id` 或 `_last_updated_sequence_number` 列。自 4.1.4 版本起，对 `format-version >= 3` 的表，在 `INSERT` 的列清单中指定行血缘列会直接报错：`Cannot specify invisible column '<name>' in INSERT statement`。
...
- 对 Iceberg 目标表，`DEFAULT()` 会基于本次写入固定（Pin）的 Iceberg Schema 解析；引用不在写入目标中的列会报错 `DEFAULT requires a target column reference` 或 `Cannot find column information for DEFAULT(<col>)`。
...
- 其他报错：`DEFAULT requires a column reference, but got: <expr>`、`DEFAULT cannot be used on generated column '<name>'`。
...
自 4.1.4 版本起，Iceberg 系统表改为通过原生执行路径查询，同时**移除了 `iceberg_meta()` 表函数**。请统一使用 `<table>$<system_table_name>` 的系统表语法，详见 [ICEBERG_META](../../sql-manual/sql-functions/table-valued-functions/iceberg-meta.md)。
...
5. 这些操作同时适用于分区表和非分区表。
6. **自 4.1.4 版本起**，`MERGE INTO` 会校验源表与目标表的匹配关系：当同一行目标数据被多行源数据匹配时，语句会报错 `multiple source rows matched the same target row`，而不是产生不确定的结果。如果确实存在一对多匹配，请先在源侧去重。
