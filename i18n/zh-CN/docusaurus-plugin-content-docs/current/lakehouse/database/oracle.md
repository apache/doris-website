...
Doris JDBC Catalog 支持通过标准 JDBC 接口连接 Oracle 数据库。本文档介绍如何配置 Oracle 数据库连接。
...
- Doris 每个 FE 和 BE 节点和 Oracle 服务器之间的网络连接，默认端口为 1521。
...
如果在查询中带有 limit 关键字，Doris 会将 limit 转义为 Oracle 的 `rownum` 语法，以减少数据传输量。

### 转义字符

Doris 会在下发到 Oracle 的查询语句中，自动在字段名与表名上加上转义符：("")，以避免字段名与表名与 Oracle 内部关键字冲突。
