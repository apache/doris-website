...
**分桶数尽可能少，且设为 BE 数量的整数倍。**更少的分桶意味着更大的 tablet，可以提升扫描效率并减少元数据开销。设为 BE 数量的整数倍可确保数据均匀分布在各节点上。在生产环境中，大表通常有很多分区，查询往往涉及多个分区，因此整体并行度主要来自分区——性能对分桶数并不敏感。
...
**已有分区的分桶数不可更改。**只能调整**新分区**的分桶数。每个 tablet 的压缩后数据大小（不含索引）应保持在 **1 GB 到 20 GB** 之间，Unique Key 表建议不超过 **10 GB**——可通过 `SHOW TABLETS FROM your_table` 查看。如果单分区数据小于 1 GB，单个分桶即可。否则设置多个分桶使每个 tablet 落在合理范围内。分桶过少限制查询并行度；过多则产生过多小文件。
