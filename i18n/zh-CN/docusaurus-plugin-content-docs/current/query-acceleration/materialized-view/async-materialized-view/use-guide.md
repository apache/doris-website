---
{
    ...
    "keywords": ["异步物化视图", "使用建议", "最佳实践", "刷新策略", "Incremental View Maintenance", "IVM", "分区物化视图", "透明改写", "数据分层建模", "Doris"],
}
---
...
- **问题**：全量或分区刷新成本可能过高。如果每次变化的行数只占全表或分区的一小部分，可以评估 [物化视图增量维护（IVM）](incremental-materialized-view)；IVM 仍是异步刷新，不提供实时一致性。
...
异步物化视图需要分别选择计算粒度和触发方式。计算粒度包括 `INCREMENTAL`、`PARTITIONS`、`COMPLETE` 和 `AUTO`；触发方式包括手动刷新、定时刷新和触发式刷新。

| 计算粒度 | 推荐场景 |
|---|---|
| `INCREMENTAL` | 变化行数远小于全表或受影响分区，且定义满足 IVM 要求 |
| `PARTITIONS` | 变化集中在少量分区，重算这些分区的成本可接受 |
| `COMPLETE` | 数据量较小、首次建立基线或需要恢复基线 |
| `AUTO` | 希望 Doris 自动选择可用方式 |

IVM 从 Doris 5.0.0 开始支持，目前处于实验阶段。选型、前置条件和限制见 [物化视图增量维护（IVM）](incremental-materialized-view)。
...
不能。异步物化视图存在数据延迟，具体取决于触发方式、排队和刷新耗时。IVM 可以降低高频小批量变更的计算量，但仍不提供实时一致性。高时效性场景请考虑同步物化视图。
...
先选择计算粒度，再选择触发方式。变化行数远小于受影响分区时评估 [IVM](incremental-materialized-view)；变化集中在少量分区时优先评估[分区物化视图](#31-优先选择分区物化视图)。触发方式参考 [三种刷新策略对比](#32-三种刷新策略对比) 与 [刷新策略组合建议](#34-刷新策略组合建议)。
