...
sidebar_label: 字典表加速
language: zh-CN
description: 使用 Doris 字典表（Dictionary）将维度表 JOIN 转换为内存键值查找，加速查询性能。
keywords:
    - Doris 字典表
    - Dictionary
    - JOIN 加速
...
| 属性名 | 值类型 | 含义 | 必须项 |
| --- | --- | --- | --- |
| `data_lifetime` | 整数，单位为秒 | 数据有效期。当该字典上次更新距今时间超过该值且基表有数据变化时，将会自动发起重新导入，导入逻辑详见[自动导入](#自动导入) | 是 |
...
| `<query_key_values>` | 一个包含该字典**所有 key 列**的需查询数据的 STRUCT |
