...
# Hive Hll UDF
 ...
 Hive Hll UDF 提供了在 hive 表中生成 hll 运算等 UDF，Hive 中的 hll 与 Doris hll 完全一致 ，Hive 中的 hll 可以通过 spark hll load 导入 doris

 主要目的：
  1. 减少数据导入 doris 时间 , 除去了构建字典、hll 预聚合等流程；
  2. 节省 hive 存储 ，使用 hll 对数据压缩 ，减少了存储成本，相对于 bitmap 的统计更加节省存储；
...
创建 Hive 表指定为 TEXT 格式，此时，对于 Binary 类型，Hive 会以 base64 编码的字符串形式保存，此时可以通过 Hive Catalog 的形式，直接将 hll 数据通过 hll_from_bash64 函数插入到 Doris 内部。
...
```sql
CREATE TABLE IF NOT EXISTS `hive_test`.`hive_hll_table`(
`k1`   int       COMMENT '',
`k2`   String    COMMENT '',
`k3`   String    COMMENT '',
`uuid` binary    COMMENT 'hll'
