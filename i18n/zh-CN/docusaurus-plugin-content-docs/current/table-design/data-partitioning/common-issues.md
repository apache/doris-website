...
-   以下罗列一些常见的 tablet 创建失败错误，包括但不限于：
    ...
    -   BE 没有收到相关 task，此时无法在 be.INFO 中找到 tablet id 相关日志或者 BE 创建成功，但汇报失败。以上问题，请参阅 [安装与部署](../../../version-3.0/install/deploy-manually/integrated-storage-compute-deploy-manually) 检查 FE 和 BE 的连通性。
...
关于数据划分更多的详细说明，我们可以在 [CREATE TABLE](../../sql-manual/sql-statements/table-and-view/table/CREATE-TABLE) 命令手册中查阅，也可以在 MySQL 客户端下输入 `HELP CREATE TABLE;` 获取更多的帮助信息。
