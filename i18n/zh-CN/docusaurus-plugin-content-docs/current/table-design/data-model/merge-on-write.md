...
| 实现 | 合并时机 | 查询性能 | 谓词下推 | 适用场景 |
| --- | --- | --- | --- | --- |
| 写时合并（默认） | 写入时 | 高 | 支持 | 大多数场景，兼顾查询与写入性能 |
| 读时合并 | 查询或 Compaction 时 | 较低 | 不支持 | 写多读少场景 |

## 写时合并的工作原理

采用写时合并时，Doris 在数据写入时消解相同 Key 的记录。对于每一条写入的数据，Doris 会检查该 Key 是否已存在，并将其旧版本标记为删除。最终存储中每个 Key 只保留最新的一行。
