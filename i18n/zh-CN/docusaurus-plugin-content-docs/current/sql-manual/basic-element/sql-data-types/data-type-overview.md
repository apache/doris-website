...
FILE 类型是一种语义化数据类型，用于表示对象存储中的文件元数据。它将固定结构（URI、文件名、内容类型、大小、凭证）以 JSON 二进制格式存储。

- **[FILE](../../basic-element/sql-data-types/semi-structured/FILE.md)**：表示对象存储（S3、OSS、COS、OBS、HDFS）中的文件。FILE 类型只能在 [Fileset 表](../../sql-statements/table-and-view/table/CREATE-FILESET-TABLE.md)（ENGINE = fileset）中使用，设计用于与 `embed()` 等 AI 函数配合，实现多模态数据处理。
