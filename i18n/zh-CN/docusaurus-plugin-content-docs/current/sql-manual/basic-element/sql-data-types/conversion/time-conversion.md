...
结果为输入的日内时间部分。由于 `TIME(p)` 支持从 0 到 6 的精度 `p`，Doris 会将 9 位小数秒四舍五入到目标精度。进位可能传递到下一秒，因此 `23:59:59.999999999` 舍入后可能得到 `24:00:00`。

所有合法 `TIMESTAMP_NS` 的日内时间都在 `TIME` 范围内，因此严格和非严格模式下转换都会成功。输入为 `NULL` 时返回 `NULL`。

#### 示例

| 输入 TIMESTAMP_NS | 目标类型 | 转换为 TIME 的结果 | 说明 |
| --- | --- | --- | --- |
| `2024-02-29 12:34:56.123456789` | `TIME(0)` | `12:34:56` | 舍入到整秒 |
| `2024-02-29 12:34:56.123456789` | `TIME(3)` | `12:34:56.123` | 舍入到毫秒 |
| `2024-02-29 12:34:56.123456789` | `TIME(6)` | `12:34:56.123457` | 舍入到微秒 |
| `1969-12-31 23:59:59.999999999` | `TIME(6)` | `24:00:00.000000` | 进位到下一秒 |
