...
`TIMESTAMP_NS` 与 `DATETIME` 一样不带时区，因此转换会保留日期时间字段，不进行时区调整。`TIMESTAMP_NS` 固定包含 9 位小数，而 `DATETIME` 最多支持 6 位，因此小数部分会四舍五入到目标 `DATETIME` 精度，进位可能传递到下一秒。

所有合法的 `TIMESTAMP_NS` 值都在 `DATETIME` 的日期范围内，因此严格和非严格模式下转换都会成功。输入为 `NULL` 时返回 `NULL`。

| 输入 TIMESTAMP_NS | 目标类型 | 结果 DATETIME | 说明 |
| --- | --- | --- | --- |
| `2024-02-29 12:34:56.123456499` | Datetime(6) | `2024-02-29 12:34:56.123456` | 舍弃部分向下舍入 |
| `2024-02-29 12:34:56.123456500` | Datetime(6) | `2024-02-29 12:34:56.123457` | 舍弃部分向上舍入 |
| `1969-12-31 23:59:59.999999500` | Datetime(6) | `1970-01-01 00:00:00.000000` | 进位到下一秒和下一天 |
| `2024-02-29 12:34:56.123456789` | Datetime(3) | `2024-02-29 12:34:56.123` | 四舍五入到毫秒精度 |
