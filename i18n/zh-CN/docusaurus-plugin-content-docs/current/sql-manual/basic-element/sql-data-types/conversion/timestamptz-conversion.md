...
将 `TIMESTAMP_NS` 转换为 `TIMESTAMPTZ(p)` 时，Doris 会把不带时区的 `TIMESTAMP_NS` 值解释为当前会话时区中的日期时间，转换为对应的绝对时间，并将纳秒小数四舍五入到目标精度 `p`（`0` 到 `6`）。

所有合法的 `TIMESTAMP_NS` 值都在 `TIMESTAMPTZ` 的日期范围内，因此严格和非严格模式下转换都会成功。输入为 `NULL` 时返回 `NULL`。

假设 `time_zone = '+08:00'`：

| 输入 TIMESTAMP_NS | 目标类型 | 结果 TIMESTAMPTZ | 说明 |
| --- | --- | --- | --- |
| `2024-02-29 12:34:56.123456499` | Timestamptz(6) | `2024-02-29 12:34:56.123456+08:00` | 向下舍入到微秒 |
| `2024-02-29 12:34:56.123456500` | Timestamptz(6) | `2024-02-29 12:34:56.123457+08:00` | 向上舍入到微秒 |
| `1969-12-31 23:59:59.999999500` | Timestamptz(6) | `1970-01-01 00:00:00.000000+08:00` | 进位到下一秒 |

反向转换请参见[转换为 TIMESTAMP_NS](./timestamp-ns-conversion.md)。
