...
* 不支持转换为 tinyint、smallint 或 int，因为一定会溢出。

* 支持转换为 bigint 和 largeint。丢弃 TIMESTAMP_NS 的纳秒部分，然后将年、月、日、小时、分钟、秒按顺序拼接成整数；月、日、小时、分钟、秒均按两位数处理，不足 10 时在前面补 0。

### 示例

| 输入 TIMESTAMP_NS | 目标类型 | 结果 |
| --- | --- | --- |
