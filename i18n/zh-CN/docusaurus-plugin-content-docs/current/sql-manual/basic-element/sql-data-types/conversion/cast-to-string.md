...
`TIMESTAMP_NS` 按照 `yyyy-MM-dd HH:mm:ss.SSSSSSSSS` 格式输出。小数秒始终固定包含 9 位，包括末尾的零；输出不包含时区。

转换为 `CHAR`、`VARCHAR` 或 `STRING` 时都使用相同的表示形式。输入为 `NULL` 时返回 `NULL`。
...
原生 UUID 输出为 36 字符的小写标准文本（`8-4-4-4-12`），与输入时的大小写或是否带连字符无关。输入为 `NULL` 时返回 `NULL`。详见 [UUID 类型转换](./uuid-conversion.md)。
