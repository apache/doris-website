...
Doris 直接丢弃 9 位小数秒，不进行舍入，再按照 `YYYYMMDDHHMMSS` 的顺序将年、月、日、时、分、秒拼接成整数，然后通过 `static_cast` 将该整数转换为 `FLOAT`。由于 `FLOAT` 的有效数字少于这个 14 位整数，转换通常会丢失精度。
...
| TIMESTAMP_NS | float | 说明 |
| --- | --- | --- |
| `2024-02-29 12:34:56.123456789` | `2.02402284E13` | 丢弃小数部分，并且 14 位整数转换后丢失精度 |
...
Doris 直接丢弃 9 位小数秒，不进行舍入，再按照 `YYYYMMDDHHMMSS` 的顺序将年、月、日、时、分、秒拼接成整数，然后通过 `static_cast` 将该整数转换为 `DOUBLE`。`DOUBLE` 可以精确表示 `TIMESTAMP_NS` 范围内的所有 14 位结果。
...
| TIMESTAMP_NS | double | 说明 |
| --- | --- | --- |
| `2024-02-29 12:34:56.123456789` | `20240229123456` | 不丢失精度 |
| `2262-04-11 23:47:16.854775807` | `22620411234716` | 丢弃小数部分 |
