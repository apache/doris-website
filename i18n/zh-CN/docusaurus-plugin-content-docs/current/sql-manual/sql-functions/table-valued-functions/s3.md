...
- `resource`：（选填）指定 Resource 名，S3 TVF 可以利用已有的 S3 Resource 来直接访问 S3。创建 S3 Resource 的方法可以参照 [CREATE-RESOURCE](../../../sql-manual/sql-statements/cluster-management/compute-management/CREATE-RESOURCE)。该功能自 2.1.4 版本开始支持。
