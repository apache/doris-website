...
基本替换示例,在此示例中，字符串 'a b c' 中的所有空格都被替换为连字符。
...
+-----------------------------------+
```
使用捕获组,这里，字符 'b' 被模式中的组 (b) 捕获，然后使用替换字符串中的反向引用 \1 将其替换为<b>
...
表情包测试
```sql
select regexp_replace('👋 Hi 👋 There!', '👋', '👍') ;
```
+-------------------------------------------------------+
| regexp_replace('👋 Hi 👋 There!', '👋', '👍')       |
+-------------------------------------------------------+
| 👍 Hi 👍 There!                                      |
+-------------------------------------------------------+
```text
...
匹配中文字符,此示例将字符串中所有连续的中文字符替换为 '123'
