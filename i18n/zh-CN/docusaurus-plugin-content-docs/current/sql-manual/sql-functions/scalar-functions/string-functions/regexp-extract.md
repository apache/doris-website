...
若未找到匹配项，将返回空字符串。
执行字符集匹配时，应使用 Unicode 标准字符类。例如，要匹配中文字符，使用\p{Han}。

1. str参数为'string' 类型，表示要进行正则匹配的字符串。
2. pattern参数为'string' 类型，表示目标正则表达式模式。
3. pos参数为 'integer' 类型，用于指定字符串中开始搜索正则表达式匹配的位置。位置从 1 开始，此参数必须指定。
