...
- `default_llm_resource`: 全局默认使用的资源

- `defualt_llm_classify_resource`: 函数 LLM_CLASSIFY 默认使用的资源

- `default_llm_extract_resource`：函数 LLM_EXTRACT 默认使用的资源

- `default_llm_fixgrammar_resource`: 函数 LLM_FIXGRAMMAR 默认使用的资源

- `default_llm_generate_resource`: 函数 LLM_GENERATE 默认使用的资源

- `default_llm_mask_resource`: 函数 LLM_MASK 默认使用的资源

- `default_llm_sentiment_resource`：函数 LLM_SENTIMENT 默认使用的资源

- `default_llm_summarize_resource`：函数 LLM_SUMMARIZE 默认使用的资源

- `default_llm_translat_resource`：函数 LLM_TRANSLATE 默认使用的资源
...
2. 如果没有指定，查看该函数是否有设置特定的默认resource
3. 全局默认resource(default_llm_resource)
...
SELECT LLM_SENTIMENT('this is a test'); -- 调用名为'function_default_resource'的资源
