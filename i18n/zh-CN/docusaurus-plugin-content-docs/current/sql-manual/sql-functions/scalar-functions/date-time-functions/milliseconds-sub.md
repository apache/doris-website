---
{
    ...
    "language": "zh-CN",
    "description": "MILLISECONDS_SUB 从 DATETIME、TIMESTAMP_NS 或 TIMESTAMPTZ 值中减去指定的毫秒数，并返回相同类型的结果；结果超出该类型范围时抛出错误。"
}
---

## 描述

MILLISECONDS_SUB 函数从 `DATETIME`、`TIMESTAMP_NS` 或 `TIMESTAMPTZ` 值中减去指定的毫秒数，并返回相同类型的计算结果。
