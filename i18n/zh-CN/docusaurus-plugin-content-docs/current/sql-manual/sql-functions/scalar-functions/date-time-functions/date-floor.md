...
DATE_FLOOR 函数用于将指定的日期或时间值向下取整（floor）到最近的指定时间间隔周期的起点，支持 `DATE`、`DATETIME`、`TIMESTAMP_NS` 和 `TIMESTAMPTZ` 输入。即返回不大于输入日期时间的最大周期时刻，周期规则由 period（周期数量）和 type（周期单位）共同定义，所有周期均以固定起点 0001-01-01 00:00:00 为基准计算。
...
返回类型为 TIMESTAMPTZ、TIMESTAMP_NS、DATETIME 或 DATE。返回一个日期按照 period 周期向下取整的结果，类型和 `<date_or_time_expr>` 保持一致。
