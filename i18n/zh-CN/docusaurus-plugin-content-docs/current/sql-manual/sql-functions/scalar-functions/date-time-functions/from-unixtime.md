...
FROM_UNIXTIME 函数用于将 Unix 时间戳（以秒为单位）转换为指定格式的日期时间字符串或 VARCHAR 类型值。Unix 时间戳的基准时间为 1970-01-01 00:00:00 UTC。`%f` 输出微秒，并将值四舍五入到 6 位；`%n` 保留并输出最多 9 位小数作为纳秒。
...
- 格式字符串不能同时包含 `%f` 和 `%n`，因为两者采用不同的舍入精度。
