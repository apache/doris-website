---
{
    "title": "SECOND_FLOOR",
    "language": "zh-CN",
    "description": "SECOND_FLOOR 将 DATE、DATETIME、TIMESTAMP_NS 或 TIMESTAMPTZ 值向下取整到最近的指定秒周期，并保持输入对应的日期时间类型和精度。"
}
---

## 描述

SECOND_FLOOR 将 `DATE`、`DATETIME`、`TIMESTAMP_NS` 或 `TIMESTAMPTZ` 值向下取整到最近的指定秒周期。若指定起始时间（origin），则以该时间为基准划分周期并取整；若未指定，默认以 0001-01-01 00:00:00 为基准。
...
返回类型为 TIMESTAMPTZ、TIMESTAMP_NS、DATETIME 或 DATE。返回以输入日期时间为基准，向下取整到最近的指定秒周期后的时间值。返回值的精度与输入参数 datetime 的精度相同。
