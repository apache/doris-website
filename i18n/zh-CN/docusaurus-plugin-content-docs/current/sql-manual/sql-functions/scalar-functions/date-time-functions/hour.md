...
| 参数 | 说明 |
| -- | -- |
| `<date_or_time_expr>` | 合法的日期或时间表达式，支持 `DATE`、`DATETIME`、`TIMESTAMP_NS` 和 `TIME`。`DATE` 输入按 00:00:00 处理。具体格式请查看 [datetime 的转换](../../../../../current/sql-manual/basic-element/sql-data-types/conversion/datetime-conversion)、[date 的转换](../../../../../current/sql-manual/basic-element/sql-data-types/conversion/date-conversion)和 [time 的转换](../../../../../current/sql-manual/basic-element/sql-data-types/conversion/time-conversion)。 |

## 返回值

返回整数类型（INT），表示输入表达式中的小时部分。
- 对于 DATETIME ，返回 0-23 的整数。
- 对于 TIMESTAMP_NS，返回 0-23 的整数。
- 对于 DATE 类型 ，返回 0.
