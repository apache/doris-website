...
`explode_map_outer` 函数接受一个 map (映射类型)，将 map（映射类型）展开成多个行，每行包含一个键值对。通常与 LATERAL VIEW 配合使用，可以支持多个 Lateral view。仅支持新优化器。

`explode_map_outer` 和 `explode_map` 区别主要在于空值处理。
...
当 map 不为空或 NULL 时，`explode_map_outer` 和 `explode_map` 的返回值相同。
