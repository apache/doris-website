...
- `resource`：（选填）指定resource名，s3 tvf 可以利用已有的 s3 resource 来直接访问s3。创建 s3 resource 的方法可以参照 [CREATE-RESOURCE](../../sql-statements/Data-Definition-Statements/Create/CREATE-RESOURCE.md)
