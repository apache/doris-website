...
session variable或导入属性`partial_update_new_key_behavior`用于控制部分列更新中插入的新行的行为。

当`partial_update_new_key_behavior=ERROR`时，插入的每一行数据必须满足该行数据的 Key 在表中已经存在。而当`partial_update_new_key_behavior=APPEND`时，进行部分列更新时可以更新 Key 已经存在的行，也可以插入 Key 不存在的新行。
