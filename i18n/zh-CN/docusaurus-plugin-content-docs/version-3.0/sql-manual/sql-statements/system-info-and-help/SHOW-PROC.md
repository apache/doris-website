...
SHOW PROC '/binlog' 命令用于查询系统中启用了二进制日志 (Binlog) 功能的数据库和表的相关信息。该命令显示了 Binlog 的详细状态，包括日志数量、大小、时间范围以及清理策略等关键指标。
Binlog 是 Doris 中的一项重要功能，用于记录数据变更，支持数据同步、恢复等操作。通过此命令，管理员可以监控 Binlog 的状态，确保其正常运行并合理规划存储空间。

| 字段名 | 数据类型 | 描述 |
|--------|----------|------|
| Name | 字符串 | 数据库或表的名称。|
| Type | 字符串 | 类型，可以是 "db"（数据库）或 "table"（表） |
| Id | 数字 | 数据库ID或表ID |
