...
MILLISECONDS_ADD 函数用于向输入的日期时间值中添加指定的毫秒数，并返回计算后的新日期时间值。该函数支持处理含毫秒精度的 DATETIME 类型及符合格式的字符串。
...
| 参数 | 说明 |
| ---- | ---- |
| `<basetime>` | 输入的日期时间值，支持输入 datetime 类型和符合日期时间格式的字符串,具体 datetime 格式请查看 [datetime 的转换](../../../../../current/sql-manual/basic-element/sql-data-types/conversion/datetime-conversion) |
