...
- 若两个时间的分钟及以上部分完全相同（忽略秒和微秒），返回 0。
- 若输入为 DATE 类型（仅包含年月日），默认其时间部分为 00:00:00。
- 若输入的日期时间包含秒或微秒部分，计算时会被忽略（仅取 HH:MM 部分参与计算）。
- 若任一参数为 NULL，返回 NULL。
...
--- 忽略秒和微秒部分
SELECT MINUTES_DIFF('2020-12-25 21:00:00.183', '2020-12-25 22:00:00.923');
+--------------------------------------------------------------------+
| MINUTES_DIFF('2020-12-25 21:00:00.183', '2020-12-25 22:00:00.923') |
+--------------------------------------------------------------------+
|                                                                -60 |
+--------------------------------------------------------------------+
...
--- 两个时间的分钟部分相同（忽略秒）
 SELECT MINUTES_DIFF('2023-07-13 22:30:59', '2023-07-13 22:30:01') AS result;
+--------+
| result |
+--------+
|      0 |
