...
MICROSECONDS_ADD 函数用于向输入的日期时间值中添加指定的微秒数，并返回计算后的新日期时间值。该函数支持处理含微秒精度的 DATETIME 类型及符合格式的字符串。
...
返回 DATETIME 类型的值，表示基准时间添加指定微秒后的结果（格式为 YYYY-MM-DD HH:MM:SS.ffffff，其中小数部分精度与 `basetime` 一致）。
