...
MICROSECONDS_DIFF 函数用于计算两个日期时间值之间的微秒差值，结果为结束时间减去开始时间的微秒数。该函数支持处理含微秒精度的 DATETIME 类型及符合格式的字符串。
...
| 参数 | 说明 |
| ---- | ---- |
| `<end_date>` | 结束时间，类型为 DATETIME 或 符合格式的字符串，具体 datetime 格式请查看 [datetime 的转换](../../../../../current/sql-manual/basic-element/sql-data-types/conversion/datetime-conversion) |
| `<start_date>` | 开始时间，类型为 DATETIME 或 符合格式的字符串，具体 datetime 格式请查看 [datetime 的转换](../../../../../current/sql-manual/basic-element/sql-data-types/conversion/datetime-conversion) |
