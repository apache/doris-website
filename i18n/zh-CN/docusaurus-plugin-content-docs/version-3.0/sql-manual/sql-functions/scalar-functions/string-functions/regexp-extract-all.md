...
执行字符集匹配时，应使用 Unicode 标准字符类。例如，要匹配汉字，使用\p{Han}。
...
### 示例 1：围绕 'C' 的小写字母基本匹配
解释： 在这个示例中，模式([[:lower:]]+)C([[:lower:]]+)匹配字符串中一个或多个小写字母后跟 'C' 再跟一个或多个小写字母的部分。'C' 之前的第一个子模式([[:lower:]]+)匹配 'b'，因此结果为['b']。
...
### 示例 2：字符串中的多个匹配项
解释：在这里，模式在字符串中匹配两个部分。第一个匹配的第一个子模式匹配 'b'，第二个匹配的第一个子模式匹配 'f'。因此结果为['b', 'f']。
...
### 示例 3：从键值对中提取键
解释： 该模式匹配字符串中的键值对。第一个子模式捕获键，因此结果为键的数组['abc', 'def', 'ghi']。
...
```
### 示例 4：匹配汉字
解释： 模式(\p{Han}+)(.+)首先通过第一个子模式(\p{Han}+)匹配一个或多个汉字，因此结果为['这是一段中文']。
...
### 示例 5：插入数据并使用 REGEXP_EXTRACT_ALL
