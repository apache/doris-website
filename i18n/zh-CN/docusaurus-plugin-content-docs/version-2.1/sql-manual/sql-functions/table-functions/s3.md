...
- `resource`：（选填）指定 Resource 名，S3 TVF 可以利用已有的 S3 Resource 来直接访问 S3。创建 S3 Resource 的方法可以参照 [CREATE-RESOURCE](../../sql-statements/Data-Definition-Statements/Create/CREATE-RESOURCE.md)。该功能自 2.1.4 版本开始支持。(注意事项：直接查询 tvf 或基于该tvf 创建 view ，需要拥有该 resource 的 usage 权限，查询基于 tvf 创建的 view ，只需要该 view 的 select 权限)
