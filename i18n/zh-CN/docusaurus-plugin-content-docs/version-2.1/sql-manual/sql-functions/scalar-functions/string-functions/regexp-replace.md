...
### 测试基本替换示例
### 在此示例中，字符串 'a b c' 中的所有空格都被替换为连字符。
...
```
### 测试使用捕获组
### 这里，字符 'b' 被模式中的组 (b) 捕获，然后使用替换字符串中的反向引用 \1 将其替换为<b>
...
### 插入和测试用例
### 在这组测试用例中，我们创建一个表来存储原始字符串、模式和替换字符串。然后插入各种测试数据，并使用相应的模式和替换字符串对原始字符串执行 REGEXP_REPLACE 操作。最后，我们检索并显示替换后的字符串。
