...
2. 如果 `dynamic_partition.create_history_partition` 为 `false`，创建当前时间到 `END` 之间的所有分区；如果 `dynamic_partition.create_history_partition` 为 `true`，除当前时间到 `END` 之间的分区外，还会创建 `START` 到当前时间的分区。若定义了 `dynamic_partition.history_partition_num`，则从当前时间向前创建的分区数量不超过 `dynamic_partition.history_partition_num`。
...
1. `START` 时间之前，除 `dynamic_partition.reserved_history_periods` 所指定范围以外，**不包含**任何分区；
...
3. 除手动删除或意外丢失的分区外，表包含**特定范围**内的全部分区；如果其中某一段范围有手动创建的分区，则这部分分区被保留；其他范围均被dynamic partition 按照 `dynamic_partition.time_unit` 为单位创建的单位分区覆盖。
    - 如果 `dynamic_partition.create_history_partition` 为 `true`：
      - 若定义了 `dynamic_partition.history_partition_num`，则**特定范围**为 `[max(START, 当前时间 - dynamic_partition.history_partition_num), END]`；
      - 若未定义 `dynamic_partition.history_partition_num`，则**特定范围**为 `[START, END]`；
