...
建表时在表的 `PROPERTIES` 中指定是否开启行存，以及行存的存储压缩单元大小 `page_size`。

| 参数 | 默认值 | 说明 |
|---|---|---|
| `"store_row_column"` | `false` | 对所有列开启行存。 |
| `"row_store_page_size"` | `16384`（16 KB） | 行存 page 大小（字节）。 |
...
:::note
2.1 版本不支持通过 `row_store_columns` 属性指定部分列开启行存。
:::

## 使用示例

下面的例子创建一个 8 列的表，为了高并发点查性能配置 `row_store_page_size` 为 4KB。
...
3. 不支持通过 `ALTER TABLE` 修改 `store_row_column` 属性。
