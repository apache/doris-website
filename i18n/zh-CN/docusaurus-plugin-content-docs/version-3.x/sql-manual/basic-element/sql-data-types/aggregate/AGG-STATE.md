...
## 描述
  ...
  AGG_STATE 只能配合[state](../../../../sql-manual/sql-functions/combinators/state)
    /[merge](../../../sql-functions/combinators/merge.md)/[union](../../../sql-functions/combinators/union)函数组合器使用。
...
插入数据示例：
  ...
  ```
  对于 agg_state 列，插入语句必须用[state](../../../sql-functions/combinators/state)函数来生成对应的 agg_state 数据，这里的函数和入参类型都必须跟 agg_state 完全对应。
...
查询数据示例：
  ...
  如果需要获取实际结果，则要用对应的[merge](../../../sql-functions/combinators/merge)函数。
...
如果想要在过程中只聚合 agg_state 而不获取实际结果，可以使用[union](../../../sql-functions/combinators/union)函数。
