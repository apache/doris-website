---
{
    "title": "UNHEX",
    "language": "zh-CN",
    "description": "unhex 函数用于将十六进制字符串转换为原始字符串。将每两个十六进制字符转换为一个字节。当传入的参数为 NULL 时，将返回 `NULL`；其他无效值返回空字符串。 unhex_null 函数与unhex函数作用一致，但是当传入的参数为 NULL 时，也将返回 `NULL`。"
}
---

## 描述

`unhex` 函数用于将十六进制字符串转换为原始字符串。将每两个十六进制字符转换为一个字节。当传入的参数为 NULL 时，将返回 `NULL`；其他无效值返回空字符串。
