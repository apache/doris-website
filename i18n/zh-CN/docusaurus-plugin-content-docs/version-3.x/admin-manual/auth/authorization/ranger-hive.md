...
Apache Doris 3.x 可以为单个 Hive Catalog 指定 Ranger-Hive Access Controller，使用 Apache Ranger 中已有的 Hive 策略校验该 Catalog 下数据库、表和列的访问权限，并支持 Ranger Row Level Filter、Data Masking 和审计。该模式适合已经使用 Ranger 管理 Hive，且希望 Doris 查询同一批 Hive 数据时复用 Hive 策略的场景。
...
- 配置 Ranger Admin Kerberos 前，先确认 Doris 小版本：

    | Doris 版本 | 工厂类配置 | Ranger Admin Kerberos |
    | --- | --- | --- |
    | 3.0 | 只能使用完整类名 | 内置 Ranger 2.4，不支持本文使用的 UGI keytab 登录 |
    | 3.1 | 完整类名或 `ranger-hive` | 内置 Ranger 2.7，支持本文配置方式 |
...
`ranger-hive-security.xml` 和 `ranger-hive-audit.xml` 需要成对部署。即使不需要审计，也应保留 `ranger-hive-audit.xml` 并显式关闭审计，以避开内置 Ranger Plugin 的旧审计配置回退问题，详见常见问题。`ranger-policymgr-ssl.xml` 仅在 Ranger Admin 使用 HTTPS 且需要自定义信任库或客户端证书时使用。
...
Ranger-Hive 不管理 Doris Catalog 级权限。Doris 3.x 还需要通过默认 Access Controller 让用户能够看到和使用已经创建的 `hive_ctl`。如果默认使用 Doris 内置鉴权，执行：
...
| 配置位置 | 示例值 | 实际含义 |
| --- | --- | --- |
| Catalog 的 `access_controller.class` | `org.apache.doris.catalog.authorizer.ranger.hive.RangerHiveAccessControllerFactory` | Ranger-Hive Access Controller 工厂的完整类名。为兼容所有 Doris 3.x 版本，应使用完整类名；不要填写 Ranger WebUI Service Name |
...
下表是 3.x 源码中 `RangerHiveAccessController` 对 Doris 权限谓词的基础映射。Doris 会先移除用户名、角色名和数据库名中的 Cluster Namespace 前缀，再发送给 Ranger。表中存在映射不代表每个 SQL 语句都一定执行相应检查，具体支持范围还取决于该 Doris 版本中该语句的权限检查路径，应以对应版本的实际验证结果为准。
...
- Doris 认证成功后，会先移除当前 Doris 用户名中的 Cluster Namespace 前缀，再把处理后的名称作为 Ranger 请求用户。Ranger 用户名应与该名称一致。
...
- Doris 会先移除当前用户所持角色名称中的 Cluster Namespace 前缀，再把处理后的角色名作为请求上下文传给 Ranger。Ranger 策略中的 Roles 可以匹配这些名称；这只是 Ranger 策略的匹配条件，不表示 Doris 内部对象权限也参与本次判断。
...
| FE/BE → HDFS | NameNode/DataNode 的服务 principal | `doris/hive-client.example.com@EXAMPLE.COM` | 读取底层数据；配置在 Hive Catalog 的 `hadoop.security.authentication` 和 `hadoop.kerberos.*` 属性中 |

:::caution 版本要求
本节 Ranger Admin Kerberos 配置要求使用 **Doris 3.1 或更高版本**。Doris 3.0 内置 Ranger 2.4，不包含本文使用的 Ranger Plugin UGI keytab 登录逻辑；下面的 `ranger.plugin.hive.ugi.*` 配置不能在 Doris 3.0 中启用该能力。Doris 3.1 起将内置 Ranger Plugin 升级到 2.7。
:::
...
| 现象 | 优先检查 |
| --- | --- |
| 创建 Catalog 后找不到 Access Controller | `access_controller.class` 是否为 Doris 3.x 使用的完整工厂类名 |
...
使用适用于所有 Doris 3.x 版本的完整工厂类名：

```sql
'access_controller.class' = 'org.apache.doris.catalog.authorizer.ranger.hive.RangerHiveAccessControllerFactory'
```

Doris 3.1 也支持短标识 `ranger-hive`，但 Doris 3.0 不支持。如果不确定具体的 3.x 小版本，或需要从 3.0 升级到 3.1，应始终使用上面的完整类名。不要填写 `hive`、`hive_prod`，也不要使用旧类名 `org.apache.doris.catalog.authorizer.RangerHiveAccessControllerFactory`。检查所有 FE 是否运行同一 Doris 版本，并在 FE 日志中搜索 `ClassNotFoundException` 和配置的工厂类名。
...
- Catalog 中 `access_controller.class` 使用兼容所有 3.x 版本的 Ranger-Hive 工厂完整类名，配置前缀值为 `hive`。
