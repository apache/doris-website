---
{
    "title": "Vector",
    "language": "zh-CN",
    "description": "介绍如何使用 Vector Doris Sink 通过 Stream Load 将日志、指标、链路追踪等可观测性数据实时写入 Apache Doris，并提供 TEXT 与 JSON 日志采集示例。",
    "keywords": [
        "Vector",
        "Vector Doris Sink",
        "Apache Doris 日志采集",
...
<!-- 知识类型: 操作步骤 / 配置参数 -->
<!-- 适用场景: 使用 Vector 将日志、指标、Trace 等可观测性数据写入 Apache Doris -->

[Vector](https://vector.dev/) 是一个采用 Rust 开发的高性能可观测性数据管道，专门用于收集、转换和路由日志、指标以及链路追踪（Trace）数据。为了更好地支持 Doris 生态，Doris 社区为 Vector 开发了 **Doris Sink** 组件，可将各类数据源的数据通过 [Stream Load](../../data-operate/import/import-way/stream-load-manual.md) 高效写入 Apache Doris 进行分析处理。
...
当 `log_request = true` 时，Vector 会输出每次 Stream Load 请求的参数与响应结果，便于调试：
...
该示例演示如何采集 NDJSON 格式的结构化日志，以 [GitHub Events Archive](https://www.gharchive.org/) 数据为例。
...
| 参数名称                              | 类型             | 默认值        | 说明                                                                |
| ------------------------------------- | ---------------- | ------------- | ------------------------------------------------------------------- |
| `request.concurrency`                 | string / integer | `"adaptive"`  | 并发策略，支持 `"adaptive"`、`"none"`（串行）或正整数并发上限       |
...
| `encoding.timestamp_format`                       | string           | -          | 时间戳输出格式，支持 `rfc3339`、`unix` 等                                   |
| `encoding.only_fields` / `encoding.except_fields` | array\<string\>  | -          | 控制字段白名单或黑名单                                                      |
| `encoding.framing.method`                         | string           | 自动推断   | 自定义帧格式，如 `newline_delimited`、`character_delimited`                 |
...
- **动态分区 + 时间序列 Compaction**：对日志类表启用 `dynamic_partition` 与 `compaction_policy = "time_series"`，可显著降低存储与合并开销。
