---
{
    "title": "数据操作问题",
    "language": "zh-CN",
    "description": "本文档主要用于记录 Doris 使用过程中的数据操作常见问题。会不定期更新。"
}
---
...
自 4.0.8 版本起，扫描阶段的报错不再统一加上 `failed to initialize storage reader` 前缀，而是保留底层的真实错误信息，只在末尾追加 `. tablet=... backend=...`。

因为 Reader 初始化过程中还会做一次预读并计算下推的表达式，数据错误或表达式错误也会走到这里，旧的前缀会把这类问题误导成存储层故障。定位 -214 时，请以错误码和 `tablet=` / `backend=` 信息为准，不要依赖前缀文本匹配。
