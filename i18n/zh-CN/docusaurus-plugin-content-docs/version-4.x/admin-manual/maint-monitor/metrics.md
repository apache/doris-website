---
{
    "title": "监控指标",
    "language": "zh-CN",
    "description": "全面梳理 Doris FE 与 BE 的监控指标，涵盖进程、JVM、机器三类指标，标注 P0 重要等级，便于集群健康监控与告警配置。"
}
---
...
<!-- 知识类型: 配置参数 / 参数参考 -->
<!-- 适用场景: 集群健康监控 / 告警配置 / 性能排查 -->

Doris 的 FE 和 BE 进程都内置完备的监控指标，默认以 Prometheus 兼容格式暴露。本文系统列出所有可观测指标，并标注重要等级（Priority），方便接入 Prometheus + Grafana 后快速搭建监控告警体系。

## 适用场景

| 场景 | 用途 |
| --- | --- |
| 集群健康监控 | 通过 P0 指标快速发现 FE/BE 节点异常 |
| 告警配置 | 基于 QPS、错误率、Compaction Score 等指标设置阈值告警 |
| 性能排查 | 通过查询延迟、线程池排队、内存使用等指标定位瓶颈 |
| 容量规划 | 通过磁盘使用率、Tablet 数量、连接数评估集群负载 |
| 故障复盘 | 结合机器指标（CPU、IO、网络）与进程指标分析故障根因 |

## 指标分类

Doris 监控指标按观测对象分为两类：

1. **进程监控**：展示 Doris 进程自身的运行指标，如查询数、事务数、Compaction 状态等。
2. **节点监控**：展示 Doris 进程所在机器的资源指标，如 CPU、内存、IO、网络等。

## 获取监控指标

### Prometheus 兼容格式（默认）

直接访问 FE 或 BE 节点的 HTTP 端口即可获取：
...
- 每张指标表的最后一列为重要等级（Priority），**P0 表示最重要**，数值越大重要性越低。建议优先接入 P0 指标。
- 绝大多数监控指标类型为 Counter（累计值）。**通过间隔采集（如每 15 秒）并计算单位时间斜率，才能获得有效信息**。

    例如：通过计算 `doris_fe_query_err` 的斜率，可获取查询错误率（error per second）。

> 欢迎补充完善此表格，以提供更全面、有效的监控指标。

## FE 监控指标

<!-- 知识类型: 配置参数 -->
<!-- 适用场景: FE 节点健康监控 / 元数据同步监控 / 查询性能监控 -->
...
| `doris_fe_image_push` | {type="failed"} | Num | 将元数据镜像文件推送给其他 FE 节点失败的次数 |  |  |
|  | {type="success"} | Num | 将元数据镜像文件推送给其他 FE 节点成功的次数 |  |  |
...
| `doris_fe_routine_load_get_meta_count` |  | Num | 统计集群内所有 Routine Load Job 获取元数据的操作次数 |  |  |
...
| `doris_fe_routine_load_task_execute_count` |  | Num | 统计集群内所有 Routine Load Task 的执行次数 |  |  |
...
| `doris_fe_routine_load_abort_task_num` |  | Num | 统计集群内所有 Routine Load Job 失败的 Task 数量 |  |  |
| `doris_fe_rps` |  | Num | 当前 FE 每秒请求数量（包含查询以及其他各类语句） | 和 QPS 配合来查看集群处理请求的量 | P0 |
| `doris_fe_scheduled_tablet_num` |  | Num | Master FE 节点正在调度的 tablet 数量。包括正在修复的副本和正在均衡的副本 | 该数值可以反映当前集群正在迁移的 tablet 数量。如果长时间有值，说明集群不稳定 | P0 |
| `doris_fe_tablet_max_compaction_score` |  | Num | 各个 BE 节点汇报的 Compaction Score。如 {backend="172.21.0.1:9556"} 表示 "172.21.0.1:9556" 这个 BE 的汇报值 |  |  |
...
|  | `{name="tcp_in_segs"}` | Num | tcp 包接收的个数 |  |  |
...
<!-- 知识类型: 配置参数 -->
<!-- 适用场景: BE 节点健康监控 / 查询写入性能监控 / Compaction 监控 -->
...
| `doris_be_compaction_deltas_total` |  | Num | Compaction 处理的 rowset 个数的累计值 | 记录的是 Compaction 任务中，input rowset 的个数 |  |
...
| `doris_be_disks_total_capacity` |  | 字节 | 指定数据目录所在磁盘的总容量 | 配合 `doris_be_disks_avail_capacity` 计算磁盘使用率 | P0 |
...
| `doris_be_fragment_endpoint_count` |  | Num | 同 `doris_be_data_stream_receiver_count` | FIXME: 同 `doris_be_data_stream_receiver_count` 数目。并且向量化引擎缺失 |  |
| `doris_be_fragment_request_duration_us` |  | 微秒 | 所有 fragment instance 的执行时间累计 | 通过斜率观测 instance 的执行耗时 | P0 |
...
| `doris_be_stream_load_txn_request` | {type="rollback"} | Num | stream load 执行失败的事务数的累计值 | 同上 |  |
...
| `routine_load_get_msg_latency` |  | 毫秒 | Routine Load 获取 Kafka 消息的延迟 |  |  |
| `routine_load_get_msg_count` |  | Num | Routine Load 获取 Kafka 消息的次数 |  |  |
| `routine_load_consume_bytes` |  | 字节 | Routine Load 消费 Kafka 的数据量 |  |  |
| `routine_load_consume_rows` |  | Num | Routine Load 消费 Kafka 的行数 |  |  |
...
| `doris_be_disk_bytes_read` |  | 字节 | 磁盘读取量累计值。从 `/proc/diskstats` 采集。会分别采集每块磁盘的数值。如 `{device="vdd"}` 表示 vdd 盘的数值 |  |  |
| `doris_be_disk_bytes_written` |  | 字节 | 磁盘写入量累计值。采集方式同上 |  |  |
| `doris_be_disk_io_time_ms` |  | 毫秒 | 采集方式同上 | 可计算得出 IO Util | P0 |
| `doris_be_disk_io_time_weighted` |  | 毫秒 | 采集方式同上 |  |  |
| `doris_be_disk_reads_completed` |  | Num | 采集方式同上 |  |  |
| `doris_be_disk_read_time_ms` |  | 毫秒 | 采集方式同上 |  |  |
| `doris_be_disk_writes_completed` |  | Num | 采集方式同上 |  |  |
| `doris_be_disk_write_time_ms` |  | 毫秒 | 采集方式同上 |  |  |
...
| `doris_be_snmp_tcp_in_segs` |  | Num | tcp 包接收的个数。采集自 `/proc/net/snmp` |  |  |
...
确认访问的是 FE 的 `http_port` 或 BE 的 `webserver_port`，并核对端口在 `fe.conf` / `be.conf` 中的实际值。

### Q: Counter 类指标只增不减，是正常的吗？

这是正常现象。Counter 表示累计值，需通过间隔采集计算斜率（如 PromQL 的 `rate()`）才能得到瞬时速率。

### Q: `doris_fe_max_journal_id` 在多个 FE 之间差距过大，怎么处理？

表示元数据同步延迟。检查 Follower / Observer FE 与 Master FE 的网络连通性以及回放速度。

### Q: Compaction Score 持续过高，怎么排查？

关注 `doris_be_tablet_base_max_compaction_score` 和 `doris_be_tablet_cumulative_max_compaction_score`，结合磁盘 IO、Compaction 任务数判断瓶颈。

### Q: 查询/导入出现堆积，怎么排查？

检查 `fragment_thread_pool_queue_size`、`doris_be_scanner_thread_pool_queue_size`、`doris_be_send_batch_thread_pool_queue_size` 等线程池排队指标。
