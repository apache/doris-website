...
在 4.0.8 之前，这部分数据量会被记入 `LOCAL_DATA_SIZE`，导致 `information_schema.partitions`、`SHOW TABLETS` 与 [`information_schema.backend_tablets`](./backend_tablets) 三者的本地/远端口径互相矛盾。该变更只调整展示口径，不改变数据的实际存储位置与总量。
