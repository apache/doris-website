...
| 参数                  | 说明                                                       |
|---------------------|----------------------------------------------------------|
| `<date_or_time_expr>`       | 要向上舍入的日期时间值，支持输入 date/datetime/timestamptz 类型,具体格式请查看 timestamptz的转换, [datetime 的转换](../../../basic-element/sql-data-types/conversion/datetime-conversion) 和 [date 的转换](../../../basic-element/sql-data-types/conversion/date-conversion)                              |
...
返回与输入类型一致的结果（TIMESTAMPTZ, DATETIME 或 DATE），表示向上舍入后的年间隔起始时间：
