---
{
    "title": "MINUTES_SUB",
    "language": "zh-CN",
    "description": "MINUTESSUB 函数用于从输入的日期时间值中减去指定的分钟数，并返回计算后的新日期时间值。该函数支持处理 DATE、DATETIME、TIMESTAMPTZ类型."
}
---

## 描述

MINUTES_SUB 函数用于从输入的日期时间值中减去指定的分钟数，并返回计算后的新日期时间值。该函数支持处理 DATE、DATETIME、TIMESTAMPTZ类型.
...
| 参数 | 说明 |
| ---- | ---- |
| ``<date_or_time_expr>`` | 输入的日期时间值，类型可以是 DATE、DATETIME、TIMESTAMPTZ ，具体格式请查看 timestamptz的转换, [datetime 的转换](../../../basic-element/sql-data-types/conversion/datetime-conversion), [date 的转换](../../../basic-element/sql-data-types/conversion/date-conversion) |
| ``<minutes>`` | 要减去的分钟数，类型为 BIGINT，可以为正数或负数 |

## 返回值


返回基准时间`<datetime_like_type>`减去指定分钟`<minutes>`后的结果，返回类型与第一个参数类型相关:
