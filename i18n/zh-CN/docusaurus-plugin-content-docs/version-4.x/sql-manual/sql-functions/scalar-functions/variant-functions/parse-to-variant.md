...
- `PARSE_TO_VARIANT` 会显式解析 JSON。相比之下，`CAST(string AS VARIANT)` 会把输入保留为 VARIANT 字符串，不解析 JSON，详见 [VARIANT CAST 规则](../../../basic-element/sql-data-types/semi-structured/VARIANT#支持的运算与-cast-规则)。
