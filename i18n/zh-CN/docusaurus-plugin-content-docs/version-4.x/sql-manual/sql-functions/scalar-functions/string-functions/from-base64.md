...
- 支持填充字符 =
- 遵循 RFC 4648 标准
- 输入长度必须是 4 的整数倍（空字符串除外）
- 解码结果可能包含不可打印字符

特殊情况：
- 如果输入为 NULL，返回 NULL
- 如果输入包含非法的 Base64 字符（包括空格、换行等空白字符），返回 NULL
- 如果输入为空字符串，返回空字符串
- 如果输入长度不是 4 的整数倍，返回 NULL
- 填充错误或格式不正确，返回 NULL

:::info 版本说明（4.0.8）

4.0.8 修正了 `FROM_BASE64` 的解码缓冲区大小计算，并明确了非法输入的处理：输入长度不是 4 的整数倍时直接返回 NULL。此前的缓冲区按输入长度的一半计算，长度偏小，非法输入可能触发越界写入。`TO_BASE64` 的编码缓冲区大小计算也在同一版本中修正。
