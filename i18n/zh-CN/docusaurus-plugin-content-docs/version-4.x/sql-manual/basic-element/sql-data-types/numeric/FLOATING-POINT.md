...
自 **4.1.4** 版本起，浮点数转换为字符串时，Doris 输出**能够无损还原（Round-Trip）该浮点值的最短字符串**：即再把这个字符串解析回 FLOAT / DOUBLE，可以得到完全相同的二进制值，并且不会输出多余的位数。

输出格式规则：

- 十进制指数落在 `[-4, 16)` 区间时使用普通定点表示；
- 否则使用科学计数法，指数部分使用小写 `e`、带显式符号、且至少两位数字，例如 `1.23456e-05`；
- 正负零区分输出（`-0` 与 `0`）；
- `NaN`、`Infinity`、`-Infinity` 的写法保持不变。
...
4.1.4 之前的版本按固定有效位数输出（FLOAT 约 7 位、DOUBLE 约 16 位有效数字），可能出现"输出的字符串再解析回来与原值不同"的情况。4.1.4 改为最短往返表示后，**存储的数值没有任何变化，只是文本形式可能不同**，例如：
...
| DOUBLE 值 `2.0` 在部分文本输出中为 `2.0` | 输出为 `2` |
| `0.0000123456` | `1.23456e-05` |

影响范围较广，包括：MySQL 协议查询结果、`CAST(... AS STRING)`、ARRAY / MAP / STRUCT 元素展示、JSON / VARIANT 展示、`SELECT INTO OUTFILE` 的 CSV / Parquet / ORC 文本输出、EXPORT、Stream Load 返回值，以及外表（Hive / Iceberg / Paimon / JDBC / ES / TVF）的浮点列。

如果下游系统对浮点数的字符串形式做了精确匹配（例如比对导出文件），升级后需要相应调整。
