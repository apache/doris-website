...
> **注意：** 迁移完成到缓存下载就绪之间存在时间窗口，此期间可能出现文件缓存未命中。开启 [Peer 读](./file-cache-peer-read)后，新 BE 在这个窗口内的未命中读取会优先从源 BE 的缓存直接拉取数据块，而不是回源远程存储。
...
4. **缓存未命中：** 对于未命中范围，Scanner 从远程存储下载数据，写入文件缓存后返回，同时按淘汰策略腾出空间。开启 [Peer 读](./file-cache-peer-read)时，会先尝试从其他 BE 的文件缓存读取，其他 BE 也没有时再从远程存储下载。
...
- 横向扩容后新 BE 的数据下载需要一定时间，期间缓存未命中为预期行为。需要缩短这个窗口的影响时，可开启 [Peer 读](./file-cache-peer-read)。
