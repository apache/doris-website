---
{
    ...
    "language": "zh-CN",
    "description": "介绍存算分离下的 Peer 读：本地 File Cache 未命中时先从其他 BE（含其他计算组）读取缓存，再回源对象存储。涵盖开启方式、回源填充、参数、副作用与排障。",
    "keywords": ["Peer 读", "Peer Cache", "跨计算组缓存", "跨 Compute Group 读取", "File Cache 未命中", "冷读优化", "计算组扩缩容", "enable_cache_read_from_peer", "peer_cache_fill_compute_group_id", "Peer S3 race", "存算分离"]
}
---
...
Peer 读是存算分离架构下 File Cache 的一种未命中处理策略：本 BE 的 File Cache 未命中时，先尝试从其他 BE 的 File Cache 读取所需数据块，其他 BE 也没有时再回源对象存储。它只作用于冷读路径，命中本地缓存的读取不受影响。

如果不关心实现细节，可以把 Peer 读理解为："本机没有缓存时，先问其他 BE 有没有；其他 BE 也没有，再去对象存储拿。" 本文中的"计算组"即 Compute Group（CG）。

:::caution 版本支持

Peer 读自 4.2.0 版本起支持，默认关闭，需要在 BE 上显式开启。
...
| 场景 | 说明 | 收益 |
|---|---|---|
| 同计算组扩缩容 | 计算组扩容或 tablet 均衡后，新 BE 首次读取优先从原来负责该 tablet 的 BE 拉取缓存块，而不是直接请求对象存储 | 缩短均衡后的性能抖动窗口，减少回源流量 |
| 跨计算组冷读 | 本计算组没有缓存、但其他计算组已经读热过同一份数据时，直接从对方 BE 的 File Cache 读取 | 冷查询延迟下降，对象存储回源流量下降 |
| 跨计算组回源填充 | 指定一个计算组作为"填充计算组"：对方 BE 自己也没有缓存时，代为回源对象存储并写入它自己的 File Cache，再把数据返回给发起查询的 BE | 由指定计算组承担回源成本，并顺带把数据留在它的缓存里 |

## 适用范围与限制

- 仅在存算分离模式下生效；仅支持 Doris 内表，外表（Hive、Iceberg 等）的数据缓存不走 Peer 读。
- 仅在本地 File Cache 未命中的路径上触发。缓存预热任务的读取，以及回源填充时服务端自身的对象存储读取，都不会再走 Peer 读。
- `enable_cache_read_from_peer` 同时控制同计算组和跨计算组两种 Peer 读，**目前不支持只开启同计算组 Peer 读而关闭跨计算组 Peer 读**。候选节点由 FE 给出，天然包含其他计算组的 BE。
- 回源填充的服务端暂不支持小文件合并（Packed File）功能产生的文件，这类请求会按正常流程回退到对象存储读取。
...
2. 未命中时，查看该 tablet 在本 BE 是否已有 Peer 候选节点。**没有候选节点时，本次读取直接回源对象存储**，同时后台异步向 FE 拉取候选节点信息，当前请求不等待。
3. 有候选节点时，默认（`enable_peer_s3_race = true`）让 Peer 读与对象存储读并发竞速：先向候选节点发起 Peer 读，并给它 `peer_race_hedge_delay_ms`（默认 20 毫秒）的先手时间；Peer 在先手时间内返回，对象存储读就不会发起。
4. 先返回的一方胜出。无论数据来自 Peer 还是对象存储，都会写入本地 File Cache，后续读取直接命中本地。
5. Peer 读失败（候选节点上没有该数据块、RPC 失败、并发槽位已满或超时）时自动回退到对象存储，查询不会因此报错。

关闭 `enable_peer_s3_race`，或同时进行的竞速数超过 `max_concurrent_peer_races`（默认 64）时，退化为串行：依次尝试候选节点，全部失败再读对象存储。

一次冷查询通常包含大量并发读取，一般第一次读取 Segment footer 就会触发候选节点拉取，同一查询后续对该 tablet 的读取就能用上 Peer 读。所以"第一次冷读"仍然请求对象存储是预期行为。
...
- BE 会记住每个 tablet 上次成功读取的计算组，后续优先尝试该计算组的候选节点。
- 某候选节点上没有所需数据块时，被移到列表末尾，下次换一个节点；连续 RPC 失败达到 `peer_rpc_failure_eviction_threshold`（默认 3）次的节点会被剔除。
- 某 tablet 连续 `peer_all_miss_cooldown_threshold`（默认 5）次"所有候选节点都没有数据"后进入冷却期，冷却 `peer_all_miss_cooldown_duration_s`（默认 300 秒）期间直接走对象存储。
- 候选节点信息 `peer_candidate_expiry_s`（默认 3600 秒）内未被使用则从内存清理，下次未命中时重新向 FE 拉取。

### 跨计算组回源填充

在发起读取的 BE 上配置 `peer_cache_fill_compute_group_id` 后，如果本次选中的候选节点属于该计算组，请求会带上填充标记。收到请求的 BE 若自己也没有该数据块，且开启了 `enable_peer_server_cache_fill`（默认开启），就会代为从对象存储读取，写入自己的 File Cache 后再返回。
...
以下参数均为 BE 配置。除线程池参数外都支持动态修改，见[动态修改](#动态修改)。

### 第一步：开启 Peer 读

在所有 BE 的 `be.conf` 中设置：
...
开启后同计算组和跨计算组的 Peer 读同时生效，候选节点由 FE 自动给出，不需要额外配置。

### 第二步（可选）：指定回源填充计算组

如果希望跨计算组冷读时由指定计算组负责回源并填充缓存，再做以下配置。

1. 查询目标计算组的 ID。`SHOW BACKENDS` 结果 `Tag` 列中的 `compute_group_id` 即计算组 ID，注意不是 `compute_group_name`：
...
2. 在**发起查询的计算组**的所有 BE 上配置：

    ```properties
    peer_cache_fill_compute_group_id = cg_write_01
    ```

3. 确认**填充计算组**的 BE 上 `enable_peer_server_cache_fill` 为 `true`（默认值）。
...
```bash
curl -X POST "http://<be_host>:<be_webserver_port>/api/update_config?enable_cache_read_from_peer=true&persist=true"
...
| 参数 | 默认值 | 说明 |
|---|---|---|
| `enable_cache_read_from_peer` | `false` | Peer 读总开关。开启后本地 File Cache 未命中时先尝试从其他 BE 读取，同计算组和跨计算组同时生效 |
...
| `max_concurrent_peer_races` | `64` | 单个 BE 上同时进行的竞速数上限。超过后新的未命中请求退化为串行 |
...
| 参数 | 默认值 | 说明 |
|---|---|---|
| `peer_rpc_failure_eviction_threshold` | `3` | 某候选节点连续 RPC 失败达到该次数后被剔除 |
| `peer_all_miss_cooldown_threshold` | `5` | 某 tablet 连续多少次"所有候选节点都未命中"后进入冷却期 |
...
- **打破计算组隔离。** 跨计算组 Peer 读会让一个计算组的查询消耗另一个计算组的网络、磁盘和 File Cache 读取能力；回源填充还会让对方计算组替你消耗对象存储带宽。查询的资源消耗不再完全局限在当前计算组。
- **填充方的缓存污染。** 回源填充会把数据写入服务端 BE 的本地 File Cache，可能挤掉它自己的热点数据。
- **竞速带来的双路流量。** 开启竞速后，部分未命中请求会在短时间内同时产生 Peer RPC 和对象存储读取。20 毫秒的先手时间可以减少但不能完全消除这种重复。
- **Peer 命中不保证成功。** 服务方并发槽位满、填充槽位满、数据块不存在或超时，都会自动回退到对象存储。开启竞速时冷查询通常不会比直接读对象存储更慢，但也不保证一定更快。
- **跨 AZ 费用。** 跨可用区部署时开启跨计算组 Peer 读会产生跨 AZ 流量费。
...
- `peer_race_s3_win` 明显高于 `peer_race_peer_win`：当前环境下 Peer 不占优，可以考虑增大 `peer_race_hedge_delay_ms` 或关闭 Peer 读。
...
在 TPC-H 1 TB 数据集的冷读测试中，开启 Peer 读后的冷读性能约为直接读取对象存储的 2 倍。实际收益取决于其他 BE 是否已经缓存了所需数据、Peer 网络延迟与对象存储延迟的差距。

## 最佳实践

<!-- 知识类型: 架构选型决策 -->

- **与缓存预热配合使用。** [缓存预热](./file-cache#缓存预热)是主动把数据从对象存储拉到本地，Peer 读是查询时被动从其他 BE 读取，两者互补：预热覆盖已知热点，Peer 读兜底预热没覆盖到的冷读。
- **扩缩容场景优先开启。** 计算组扩缩容或 tablet 均衡后，Peer 读能让新 BE 直接复用源 BE 的缓存；配合 `balance_type = peer_read_async_warmup` 可以在均衡时立即切换映射并依赖 Peer 读平滑过渡，详见[计算组扩缩容](../managing-compute-cluster#计算组扩缩容)。
- **读写分离场景指定写入组为填充组。** 只读计算组冷读时，把写入计算组配置为 `peer_cache_fill_compute_group_id`，可以让写入组承担回源并保留一份缓存，详见[读写分离场景 File Cache 缓存优化最佳实践](../rw/file-cache-rw-compute-group-best-practice)。
- **需要严格隔离时不要开启。** 对计算组之间的资源隔离有硬性要求，或跨 AZ 部署且对流量费敏感时，保持 `enable_cache_read_from_peer = false`。
...
**Q：开启了 Peer 读，为什么第一次冷读还是访问了对象存储？**

本地还没有该 tablet 的候选节点时，当前请求不会等待 FE 返回，而是直接回源，同时后台异步拉取候选节点。同一查询后续对该 tablet 的读取，以及之后的查询，就会走 Peer 读。Profile 中 `PeerLazyFetch` 大于 0 即对应这种情况。
...
说明当前环境下从 Peer 读取并不比对象存储快，常见于 Peer 网络延迟高或 Peer BE 负载高。可以增大 `peer_race_hedge_delay_ms` 观察变化，或关闭 Peer 读。
