...
两种方案都可以叠加[跨计算组 Peer 读](../file-cache/file-cache-peer-read)作为兜底，减少预热未覆盖到的冷读回源。通过合理运用上述缓存预热策略和相关配置，可以有效管理 Apache Doris 在读写分离架构下的缓存行为，最大限度减少缓存未命中带来的性能损失，确保只读查询业务稳定高效运行。
