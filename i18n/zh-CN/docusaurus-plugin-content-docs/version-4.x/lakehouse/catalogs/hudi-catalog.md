...
Hudi cache entry 使用配置键格式 `meta.cache.hudi.<entry>.{enable,ttl-second,capacity}`。Hudi 模块当前仍按条目数管理，不接受模块级 `max-weight`，详见[外表元数据缓存内存管理](../external-meta-cache-memory-management)。
