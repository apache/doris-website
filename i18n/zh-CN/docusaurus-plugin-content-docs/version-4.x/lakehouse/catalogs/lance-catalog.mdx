...
Doris 支持 Lance Namespace、表和顶层列的元数据 DDL。`INSERT`、`UPDATE`、`DELETE`、`TRUNCATE TABLE` 等数据写入操作暂不支持。
...
| Lance DDL | 支持 Namespace 和表的创建、删除，以及顶层列 Schema 演进 |
| 向 Lance 写入数据 | 暂不支持 |
| Time Travel | 暂不支持 |
| Hybrid Search | 暂不支持 |
...
Doris BE 数据读取器使用 `lance-c v0.1.9` 加 Doris 补丁构建，补丁包含多向量检索支持，并将其内置的 Lance Rust crates 升级到 `11.0.0`（commit `ab6b5bbe`）。Doris FE 使用 `org.lance:lance-core:11.0.0` 读取 Namespace 和数据集元数据，FE 与 BE 的 Lance 版本统一为 `11.0.0`。Dataset 必须同时满足 FE 元数据加载和 BE 数据读取的兼容性要求。这些实现版本与数据集中记录的 Lance `data_storage_version` 不是同一个概念。
...
Lance Catalog 支持以下元数据 DDL 操作：

| 操作 | 支持情况 |
|---|---|
| `CREATE DATABASE` / `DROP DATABASE` | 支持非根 Namespace |
| `CREATE TABLE` / `DROP TABLE` | 支持 |
| `RENAME TABLE` | 暂不支持 |
| `ALTER TABLE ADD COLUMN` | 支持顶层列 |
| `ALTER TABLE MODIFY COLUMN` | 支持顶层列 |
| `ALTER TABLE RENAME COLUMN` | 支持顶层列 |
| `ALTER TABLE DROP COLUMN` | 支持顶层列 |
| `INSERT`、`UPDATE`、`DELETE`、`TRUNCATE TABLE` | 暂不支持 |

### 创建和删除 Namespace

使用 `CREATE DATABASE` 和 `DROP DATABASE` 创建或删除 Lance Namespace：
...
通过 `lance.namespace.root_database` 映射的根 Namespace 不能通过 SQL 创建或删除。

### 创建和删除表

使用 `CREATE TABLE` 创建带指定 Schema 的空 Lance 表：
...
`DROP TABLE` 会删除 Lance 表及其底层 Dataset 数据：
...
Lance Catalog 支持每条 `ALTER TABLE` 语句变更一个顶层列：
...
- 列默认值。
- `FIRST` 或 `AFTER` 等列位置变更。
- Schema 变更中的列注释。
- 嵌套字段 Schema 变更。
- 一条 `ALTER TABLE` 语句中包含多个 Schema 变更子句。
- 涉及不支持的 Lance/Arrow 类型的类型变更。
...
- Lance Catalog 支持元数据 DDL，但 Lance TVF 仍为只读。`INSERT`、`UPDATE`、`DELETE`、`TRUNCATE TABLE` 或写回 Lance 暂不支持。
