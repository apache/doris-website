...
|---|---|---|---|
| AWS S3 | 建议设置 `fs.s3.support=true` | `false`，可省略 | 省略 `s3.endpoint`，由客户端生成。 |
...
<Tabs groupId="lance-filesystem-storage">
    <TabItem value="s3" label="AWS S3" default>
        ...
        本例省略 `s3.endpoint`，客户端根据 Bucket 和 region 生成 AWS S3 地址。`use_path_style=false` 是默认值，示例中显式写出便于与其他访问方式对照。
    </TabItem>
