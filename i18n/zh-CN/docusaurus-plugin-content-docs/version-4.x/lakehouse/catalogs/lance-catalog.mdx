...
- `FOR VERSION AS OF` 接受正整数版本号；与 Iceberg、Paimon 表一样，也可以写带引号的 tag 名（`FOR VERSION AS OF 'v2'` 等同于 `@tag(v2)`）。
- `FOR TIME AS OF` 接受会话时区下的时间戳，支持秒或毫秒精度（`yyyy-MM-dd HH:mm:ss` 或 `yyyy-MM-dd HH:mm:ss.SSS`）。Doris 选择提交时间不晚于该时间戳的最新版本，提交时间按毫秒精度比较；早于第一个版本的时间戳会报错。
...
- 每个 branch 有自己的版本序列，从创建它时所基于的版本开始。`@branch` 后的 `FOR VERSION AS OF` / `FOR TIME AS OF` 只在该 branch 的版本中选择，因此早于 branch 创建时间的时间戳在 branch 上选不到版本。`@branch`（`main` 除外）后的 `FOR VERSION AS OF` 不能写 tag 名，因为 tag 本身已经确定了 branch。
...
- 如果 Namespace 记录的 manifest 还在 staged 路径，Lance 会在首次读取时把它复制到规范路径，因此读取所用的存储凭证需要允许这次写入；权限不足时报错会提示这一点。
