...
Doris 支持通过多种元数据服务访问 Iceberg 表数据。除支持数据读取外，Doris 也支持对 Iceberg 表进行写入操作，包括 INSERT、INSERT OVERWRITE、UPDATE、DELETE 和 MERGE INTO。
...
* `<warehouse>`
    ...
    Iceberg 的仓库路径。当 `<iceberg_catalog_type>` 为 `hadoop` 时，需指定该参数。
...
从 Doris 4.1.x 开始，Iceberg Catalog 的外表元数据缓存使用统一键 `meta.cache.*` 进行配置。
:::
...
在 4.1.x 之前，Iceberg 表级元数据主要受 FE 全局缓存策略控制，`schema.cache.ttl-second` 是常见的 Catalog 兼容属性；详见[元数据缓存](../meta-cache.md)。升级到 4.1.x 后，建议改写为 `meta.cache.iceberg.*`，并按需要分别配置表、View 和 Manifest 缓存。

| 4.1 前属性键/旧模型 | 适用范围 | 4.1.x+ 统一键 | 升级建议与影响 |
|---|---|---|---|
| `schema.cache.ttl-second` | 4.1 前 Iceberg Catalog 兼容属性 | `meta.cache.iceberg.schema.ttl-second` | 控制 Schema 新鲜度。若希望列变更每次查询立即可见，设置为 `0`。 |
| Iceberg 表信息旧模型 | 4.1 前 FE 全局缓存策略（见旧版元数据缓存文档） | `meta.cache.iceberg.table.ttl-second` | 控制表级元数据新鲜度。若希望每次查询都读取最新 Snapshot/表属性，设置为 `0`。 |
| 无一一对应的旧 Catalog 键 | 4.1 前未单独暴露 View / Manifest TTL | `meta.cache.iceberg.view.*`、`meta.cache.iceberg.manifest.*` | 这是 4.1.x 中拆分出的新模块。升级后如需保证最新 View 定义，单独调低 `view.ttl-second`；`manifest` 主要用于性能优化。 |

4.1.x 的统一模型把缓存拆分为 `enable`、`ttl-second`、`capacity` 三个维度；旧模型主要描述 TTL/全局缓存行为，不覆盖这些新模块的独立开关和容量上限。升级时建议一并评估是否需要补充 `enable/capacity`。

### 最佳实践 {#meta-cache-best-practices}

* **实时查看最新数据**：如果您希望每次查询都能看到 Iceberg 表的最新快照或 Schema 变动，可以将 `schema` 或 `table` 的 `ttl-second` 设置为 `0`。
  ```sql
  -- 关闭表对象缓存，以便感知快照变动
  ALTER CATALOG iceberg_ctl SET PROPERTIES ("meta.cache.iceberg.table.ttl-second" = "0");
  ```
* **性能优化**：
    * 启用 Manifest 缓存可以显著提升大表的查询规划速度：
      ```sql
...
自 4.1.4 版本起，文件格式按每个数据文件（Split）单独判定，因此**同一张 Iceberg 表中同时存在 Parquet 和 ORC 数据文件时也可以正常查询**。表级别的 `write.format.default` 未设置时默认按 `parquet` 处理，不再通过嗅探数据文件来推断。

自 4.1.4 版本起，Iceberg 的 Identity 分区元数据会随每个 Split 下发，并且路径分区键只取所有 Partition Spec 共有的 Identity 列。因此经历过 **Partition Evolution** 的表，其分区列取值在查询中也能正确返回。
...
| timestamptz (Timestamp with timezone)  | datetime(6)/timestamptz(6)          | 固定映射到精度为 6 的 datetime。由 properties 中 `enable.mapping.timestamp_tz`（4.0.3 后开始支持）属性控制，默认为 `false`，则映射到 `datetime`；为 `true` 时，则映射到 `timestamptz` 类型 |
| fixed(N)                               | char(N)              |                       |
| string                                 | string               |                       |
...
| list                                   | array                |                       |
| variant                                | variant              | 自 4.1.4 版本起支持读写。需要开启 FE 配置 `enable_variant_v2`，且只支持 Parquet 格式，详见下方说明 |
| other                                  | UNSUPPORTED          |                       |
...
Doris 保留 Iceberg 表中列名的原始大小写，并在读取时按不区分大小写的方式匹配列。含大写字母或大小写混合的列名（如 `UserId`、`ORDER_ID`）可以正常查询。
...
### VARIANT 类型 {#iceberg-variant}

> 自 4.1.4 版本开始支持。

Doris 支持读写 Iceberg 表中的 `VARIANT` 列。

**读取前提：**

- FE 配置 `enable_variant_v2 = true`（默认关闭）；
- 会话变量 `enable_file_scanner_v2 = true`（默认开启）；
- 数据文件格式必须是 **Parquet**。Avro、ORC 格式以及旧的扫描器路径都不支持读取 VARIANT，会直接报错。

**写入前提（在读取前提之上）：**

- 表的 `format-version` 必须为 `3`；
- 数据文件格式为 Parquet，且表属性 `write.parquet.shred-variants` 必须为 `false`；
- DDL 中只支持定义**顶层** VARIANT 列，不支持在 `ARRAY` / `MAP` / `STRUCT` 内部嵌套定义 VARIANT 列。

不满足上述条件时，写入会报错，例如 `Iceberg VARIANT columns are read-only and cannot be written`。

## Namespace 映射

Iceberg 的元数据层级关系是 Catalog -> Namespace -> Table。其中 Namespace 可以有多级（Nested Namespace）。
...
- REST Catalog：使用 `iceberg.rest.role_arn` 指定要扮演的 Role ARN，并可选地通过 `iceberg.rest.external-id` 指定 External ID。
- AWS S3 Tables：使用 `s3.role_arn` 与 `s3.external_id`（参见下文 AWS S3 Tables 章节）。
:::

:::caution 版本行为变更（4.1.4）

自 4.1.4 版本起，当 `iceberg.rest.signing-name` 取值为 `glue`、`s3tables` 或 `osstables` 这类托管签名服务时，**必须**同时设置 `'iceberg.rest.sigv4-enabled' = 'true'`，否则建 Catalog 会报错：

```text
Rest Catalog requires sigv4-enabled set to true when signing-name is <name>
```

升级前如果已有 Catalog 只设置了 `signing-name` 而没有开启 `sigv4-enabled`，升级后会校验失败，需要补齐该属性。
...
`iceberg.rest.signing-name` 新增 `osstables` 取值，用于对接阿里云 OSS Tables。此时 REST 请求签名复用 OSS 的凭证（`oss.access_key`、`oss.secret_key`、`oss.session_token`），而不是 `s3.*` 凭证。
...
使用 `osstables` 时，`iceberg.rest.signing-region` 与 `iceberg.rest.sigv4-enabled = 'true'` 都是必填项，否则会报错：
...
可以通过 `$snapshots` 系统表查询指定 Iceberg 表的 Snapshot：

:::caution 版本行为变更（4.1.4）

自 4.1.4 版本起，`iceberg_meta()` 表函数已被移除，请统一改用 Iceberg 系统表（如 `iceberg_tbl$snapshots`）查询元数据，详见下文【系统表】章节。
...
> 该功能自 3.1.0 版本支持。
>
> 关于 Branch、Tag 的创建、删除和维护操作，请参阅【管理 Branch & Tag】章节。
...
支持查询 Iceberg 视图。视图查询方式与普通表方式一样。有以下几点注意事项：
...
Doris 支持查询 Iceberg V3 表的行血缘（Row Lineage）隐藏列。Iceberg V3 引入了包含行血缘数据的系统隐藏列，用于追踪数据的变更记录，非常适合用于增量同步或数据审查：

- `_row_id`：唯一标识数据行的数值（基于数据写入时所在文件和位置生成）。
- `_last_updated_sequence_number`：最后一次修改此行的 Sequence Number。当执行 `UPDATE` 或 `MERGE INTO` 操作更新数据时，该数值会随着版本号自动递增。
...
1. 必须确保 Iceberg 表的格式版本（`format-version`）配置为 `3` 时，才会生成上述行级别参数。对于 V1 和 V2 格式表查询行血缘列可能报错。
2. 隐藏列由系统进行维护，不允许用户在使用 `INSERT` 等写入语句时显式指定写入 `_row_id` 或 `_last_updated_sequence_number` 列。自 4.1.4 版本起，对 `format-version >= 3` 的表，在 `INSERT` 的列清单中指定行血缘列会直接报错：`Cannot specify row lineage column '<name>' in INSERT statement`。

### Iceberg V3 默认值

> 自 4.1.4 版本开始支持。

Iceberg V3 为列定义了两类默认值，Doris 均已支持：

- `initial-default`：新增列时为**已有历史数据**指定的默认值。读取旧数据文件（写入时该列还不存在）时，Doris 会返回该默认值而不是 NULL。Parquet、ORC 均支持，覆盖顶层列、`STRUCT` 内的嵌套子列，以及整体缺失的复杂类型父列，并且在谓词计算中同样生效。
- `write-default`：写入时为**未指定的列**填充的默认值。`INSERT INTO t (部分列) VALUES (...)` 时，未列出的列会写入当前的 `write-default`（旧版本写入 NULL）。
...
该语句会同时设置 Iceberg 的 `write-default` 与 `initial-default`。**要求表的 `format-version` 为 `3`**，V1 / V2 表不支持带非空默认值的列。

**`DEFAULT()` 表达式：**

自 4.1.4 版本起，新增 `DEFAULT(<column>)` 表达式，用于在 DML 中显式引用某列的默认值，可用于 `INSERT`、`UPDATE ... SET` 以及 `MERGE INTO` 的各个分支：
...
- 对 Iceberg 目标表，`DEFAULT()` 会基于本次写入固定（Pin）的 Iceberg Schema 解析；引用不在写入目标中的列会报错 `DEFAULT(<col>) must reference a column in the pinned Iceberg write target`。
- 对非 Iceberg 的列，取该列的默认值；若列没有默认值且为 NOT NULL 或自增列，报错 `Column '<name>' has no default value and does not allow NULL or column is auto-increment`；若列可空且没有默认值，取 NULL。
- 其他报错：`DEFAULT requires a column reference, but got: <expr>`、`Cannot find column information for DEFAULT(<name>)`、`DEFAULT cannot be used on generated column '<name>'`。

:::info 说明

Iceberg 的 `write-default` 属于写入侧的连接器概念，**不会**通过 `DESCRIBE`、`SHOW CREATE TABLE` 或 `information_schema.columns.COLUMN_DEFAULT` 暴露。
...
Doris 支持查询 Iceberg 系统表，用于查询表的相关元信息。支持使用系统表查看快照历史、清单文件、数据文件、分区等信息。
...
>
> `position_deletes` 系统表从 4.1.4 版本开始支持。

:::caution 版本行为变更（4.1.4）

自 4.1.4 版本起，Iceberg 系统表改为通过原生执行路径（`FileQueryScanNode` -> `IcebergScanNode`）查询，同时**移除了 `iceberg_meta()` 表函数**。请统一使用 `<table>$<system_table_name>` 的系统表语法，详见 [ICEBERG_META](../../sql-manual/sql-functions/table-valued-functions/iceberg-meta.md)。

系统表本身的访问语法和返回结果不变。此外，系统表也支持 `SHOW CREATE TABLE`。
...
自 4.1.0 版本，支持写入数据到静态分区，或者静态分区和动态分区混合使用：
...
DELETE 操作用于删除 Iceberg 表中满足指定条件的数据行。主要基于以下底层机制实现：对于 V2 格式表，系统将写入 Position Delete 文件；对于 V3 格式表，系统将写入 Puffin 格式的 Deletion Vectors 文件。
...
UPDATE 操作用于修改 Iceberg 表中满足指定条件的数据行。内部实现为先写入删除标记信息（V2 使用 Position Delete 文件，V3 使用 Puffin 格式的 Deletion Vectors 文件）标记旧数据行失效，然后追加写入更新后的新数据文件（如果开启的是 V3 格式选项，新插入行的提取追踪值 `_last_updated_sequence_number` 将会自动推进）。
...
1. 以上三种操作（DELETE、UPDATE、MERGE INTO）均要求 Iceberg 表的格式版本（`format-version`）为 2 或更高版本。如果对 V1 格式的表执行这些操作，会报错：`must have format version 2 or higher for position deletes`。当执行于 V3 格式表时，系统不仅会使用 Puffin 格式存储 Deletion Vectors 代替原有的 Position Delete 数据，如果涉及内容更新，还会自动调整写入记录的血缘生命周期属性（`_last_updated_sequence_number`）。
2. 这些操作支持 Parquet 和 ORC 两种文件格式。
3. 并发写入场景下，Iceberg 提供了乐观并发控制机制。当发生写入冲突时，操作可能会抛出事务冲突异常。
4. UPDATE 和 MERGE INTO 中的 UPDATE 操作支持使用表达式和函数，例如 `age = age * 2 + 1` 或 `name = concat(name, '-modified')`。
5. 这些操作同时适用于分区表和非分区表。
6. **自 4.1.4 版本起**（需要所有 BE 都已升级），`MERGE INTO` 会校验源表与目标表的匹配关系：当同一行目标数据被多行源数据匹配时，语句会报错 `multiple source rows matched the same target row`，而不是产生不确定的结果。如果确实存在一对多匹配，请先在源侧去重。
...
* BE
  ...
  | `table_sink_partition_write_min_data_processed_rebalance_threshold`           | 分区表开始触发重平衡的最少数据量阈值。如果 `当前累积的数据量` - `自从上次触发重平衡或者最开始累积的数据量` >= `table_sink_partition_write_min_data_processed_rebalance_threshold`，就开始触发重平衡机制。如果发现最终生成的文件大小差异过大，可以调小该阈值来增加均衡度。当然过小的阈值会导致重平衡的成本增加，可能会影响性能                                                                                   | 25MB |
  | `table_sink_partition_write_min_partition_data_processed_rebalance_threshold` | 分区表开始进行重平衡时的最少的分区数据量阈值。如果 `当前分区的数据量` >= `阈值` * `当前分区已经分配的 task 数目`，就开始对该分区进行重平衡。如果发现最终生成的文件大小差异过大，可以调小该阈值来增加均衡度。当然过小的阈值会导致重平衡的成本增加，可能会影响性能                                                                                                                                        |      |
...
**Database 属性支持矩阵（自 4.1.4 版本起扩展）：**

| Catalog 类型（`iceberg.catalog.type`） | 普通 Database 属性 | `location` 属性 |
| --- | --- | --- |
| `hms` | 支持 | 支持 |
| `glue` | 支持（4.1.4 起） | 支持（4.1.4 起） |
| `jdbc` | 支持（4.1.4 起） | 不支持 |
| `hadoop` / `rest` / `dlf` / `s3tables` | 不支持 | 不支持 |

不支持时的报错信息：

- 普通属性：`Not supported: create database with properties for iceberg catalog type: <type>`
- `location` 属性：`Not supported: database property 'location' for iceberg catalog type: <type> because it does not determine the default table location`

> REST Catalog 不支持 Database 属性，是因为 REST 协议允许服务端忽略 Namespace 属性。

之后可以通过 `SHOW CREATE DATABASE` 命令查看 Database 的 Location 信息：
...
:::caution
对于 Iceberg Database，必须先删除该 Database 下的所有表后，才能删除 Database，否则会报错。
:::
...
* **创建**
  ...
  自 4.1.0 版本开始，Doris 支持创建 Iceberg 表时，指定排序列。并且在写入数据时，会根据指定的排序列进行数据排序，以获得更好的数据查询性能。
...
* **分区**
    ...
    Iceberg 中的分区类型对应 Doris 中的 List 分区。因此，在 Doris 中创建 Iceberg 分区表，需使用 List 分区的建表语句，但无需显式地枚举各个分区。在写入数据时，Doris 会根据数据的值，自动创建对应的 Iceberg 分区。
...
* **压缩格式**
    ...
    * Parquet：snappy、zstd（默认）、plain（plain 即不采用压缩）。

    * ORC：snappy、zlib（默认）、zstd、plain（plain 即不采用压缩）。
...
* **修改列名称**
    ...
    通过 `RENAME COLUMN` 子句修改列名称。自 4.1.4 版本起支持修改嵌套类型中的列名称，且 `TO` 关键字变为可选。
...
* **添加一列**
    ...
    通过 `ADD COLUMN` 添加新列。自 4.1.4 版本起，支持通过点号路径（如 `s.new_field`）为嵌套类型（STRUCT）添加新列，新增的嵌套字段必须是 nullable。
...
* **删除列**
    ...
    通过 `DROP COLUMN` 删除列。自 4.1.4 版本起，支持通过点号路径删除嵌套类型中的列。
...
自 4.1.4 版本起，`ADD COLUMN`、`DROP COLUMN`、`MODIFY COLUMN`、`RENAME COLUMN` 均支持使用点号路径（如 `a.b.c`）直接操作嵌套字段。该能力**仅对 Iceberg 表生效**，对其他类型的表使用会报错：`Nested column path is only supported for Iceberg tables: ...`。

常见限制与报错：

| 报错信息 | 说明 |
| --- | --- |
| `New nested field '<path>' must be nullable` | 新增的嵌套字段必须可为 NULL |
| `Rollup is not supported for Iceberg column operations` | 不支持 Rollup 子句 |
| `PROPERTIES are not supported for Iceberg column operations` | 不支持 PROPERTIES 子句 |
| `KEY is not supported for Iceberg ADD/MODIFY COLUMN` | 不支持 KEY 关键字 |
| `Generated columns are not supported for Iceberg ADD/MODIFY COLUMN` | 不支持生成列 |
| `Modifying default values is not supported for Iceberg columns` | 不支持修改已有列的默认值 |
| `ON UPDATE is not supported for Iceberg ADD COLUMN` | 不支持 ON UPDATE 子句 |
| `Modify column type from complex to primitive is not supported` | 不支持把复杂类型改成基础类型 |
| `Cannot change MAP key type from ...` / `Cannot <op> MAP key nested column` | 不支持修改 MAP 的 Key 类型或对 MAP Key 做嵌套列操作 |
| `Iceberg does not support comments on collection element or value fields` | 不支持给集合元素 / MAP Value 字段加注释 |
| `Multiple Iceberg ALTER clauses are not supported when a statement ...` | 涉及嵌套列时，一条语句只能包含一个 ALTER 子句 |

### 修改表属性

> 自 4.1.4 版本开始支持。

可以通过 `ALTER TABLE ... SET (...)` 修改 Iceberg 表的原生属性，并且**支持在一条语句中设置多个属性**：
...
- 所有属性作为一个 Iceberg `UpdateProperties` 事务原子提交，失败时不会只生效一部分，也不会触发 FE 元数据刷新；
- Doris 自身的 `auto_analyze_policy` 属性不能与连接器原生属性放在同一条语句中设置，否则报错 `auto_analyze_policy cannot be set with external table properties`。单独设置时取值仍为 `enable` / `disable` / `base_on_catalog`；
- 如果 Catalog 未实现元数据写入能力，会报错 `Update table properties operation is not supported for catalog: <name>`。
...
`expire_snapshots` 操作用于删除 Iceberg 表的旧快照，以释放存储空间并提高元数据性能。该操作遵循 Apache Iceberg Spark 过程规范。
...
| 参数名称 | 类型 | 必填 | 描述 |
| -------- | ---- | ---- | ---- |
| `older_than` | String | 否 | 快照过期的时间阈值，早于该时间的快照将被删除。支持 ISO 日期时间格式（如 `2024-01-01T00:00:00`）或毫秒时间戳格式 |
...
1. 该操作不支持 WHERE 条件。
2. 如果同时指定 `older_than` 和 `retain_last`，则两个条件都会生效：只有早于 `older_than` 且不在最近 `retain_last` 个快照中的快照才会被删除。
...
`publish_changes` 操作是“写入 - 审核 - 发布” (Write-Audit-Publish) 模式中的“发布”步骤。该过程会查找带有特定 `wap.id` 属性的快照，并将其添加到当前表状态中。这样，用户就可以在验证后原子性地使“暂存”数据可见。
...
`rewrite_manifests` 操作允许用户通过重写清单文件（Manifest File）来优化 Iceberg 表元数据，从而提高查询性能并减少元数据开销。
...
- 查询 2：总体统计摘要
    ...
    **注意事项：**
    ...
    - 根据实际的重写参数调整参数值（`@min_file_size_bytes`、`@max_file_size_bytes`）。
