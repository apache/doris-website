...
* `{McOptionalProperties}`
    ...
    | `test_connection` | `false` | 创建 Catalog 时是否校验连通性。设置为 `true` 时，会校验 AK/SK、Endpoint、`mc.project` 是否可访问；开启 `mc.enable_namespace_schema` 时还会校验 Schema 列表是否可访问，校验失败则 `CREATE CATALOG` 直接报错。 | 4.1.4（含）之后 |
