...
|---|---|---|---|---|---|
| FE | `fe.conf` | `external_meta_cache_max_weight` | 固定大小，或 JVM 最大堆的百分比 | 未设置（`0`），不启用 FE 总配额 | 当前 FE 上所有受管理的外部元数据缓存 |
| Catalog | Catalog 属性 | `meta.cache.max-weight` | 固定大小，必须为正数 | 未设置，不启用 Catalog 配额 | 当前 Catalog 下所有受管理的缓存模块 |
| 缓存模块 | Catalog 属性 | `meta.cache.<engine>.<entry>.max-weight` | 固定大小，`0` 表示关闭该模块 | 未设置，仅受父级上限约束 | 指定引擎的一个缓存模块 |
...
| `WEIGHT_REJECT_COUNT` | 因预算不足或估算不完整而被拒绝的准入次数。 | 当前缓存实例的累计值 |
...
当 FE Heap 占用偏高并怀疑与外部元数据缓存有关时，可以按以下步骤排查。
...
将汇总后的预留量与配置的 `external_meta_cache_max_weight` 和 FE Heap 总量对比。该值是估算的缓存保留内存，不是 FE 总堆。加载过程中的临时内存、按条目数管理的模块以及连接器 SDK 自带的缓存都不在其中。如果 FE Heap 持续偏高而预留量很低，请直接看第 6 步。
...
`estimated_weight` 接近 `max_weight` 的模块是该 Catalog 的主要占用者。`weight_reject_count` 持续上升说明配额紧张：对象已加载、淘汰了冷数据但仍无法保留，后续访问会再次加载。`last_weight_reject_reason` 可以区分最近一次拒绝是预算不足还是估算不完整。估算不完整不是配额问题，对象会直接返回给请求而不进入缓存。
...
| 防止某个受支持模块独占配额 | `meta.cache.<engine>.<entry>.max-weight` | `ALTER CATALOG` 后下一次访问生效 |
| 宁要更新的元数据而不是缓存 | 降低 `meta.cache.<engine>.<entry>.ttl-second`，或将 `meta.cache.<engine>.<entry>.enable` 设为 `false` | `ALTER CATALOG` 后下一次访问生效 |

### 5. 修改后的效果

- 修改 Catalog 属性会清空该 Catalog 已初始化的缓存组，并在下一次访问时惰性重建。系统表中的累计列从零重新开始。
- 已经在执行的查询继续使用已加载的对象。
...
- 仍按条目数管理的模块，例如 Hive 的 `schema`、`partition` 和 `file`，可调整它们的 `capacity` 和 `ttl-second`。
- 仍被运行中查询引用的元数据对象。
- 连接器 SDK 自带的缓存，例如由 `io.manifest.cache-enabled` 控制的 Iceberg Manifest 缓存，或由 `paimon.cache-enabled` 控制的 Paimon `CachingCatalog`。
...
| `weight_reject_count` 持续上升，原因是预算不足 | 第 3 步 | 对象已加载但未保留，后续会重复加载 | 提高配额，或降低其他竞争模块的上限 |
| `weight_reject_count` 持续上升，原因是估算不完整 | 第 3 步 | 对象无法可靠估算，始终不会被缓存 | 调整配额无效；查询仍可正常执行，只是不使用缓存 |
