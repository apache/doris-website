---
{
    ...
    "language": "zh-CN",
    "description": "Doris Group Commit 通过合并高频小批量写入事务，降低 FE 解析与版本数压力，提升 INSERT 与 Stream Load 高并发性能。",
    "keywords": [
        "Group Commit",
        "高并发导入",
        "高频小批量写入",
...
| 场景 | 推荐模式 | 说明 |
| --- | --- | --- |
| 高并发写入，需立即可见 | `sync_mode` | 多个导入合并为一个事务提交，提交后返回。 |
| 写入延迟敏感、高频写入 | `async_mode` | 数据先写 WAL 立即返回，后台异步提交。 |
| 不希望开启 Group Commit | `off_mode` | 关闭 Group Commit，使用普通导入流程。 |
...
| 导入方式 | 优先级（高 → 低） |
| --- | --- |
| Stream Load | `group_commit` HTTP Header → 表属性 `group_commit_mode` |
| INSERT INTO VALUES | Session 变量 `group_commit` → 表属性 `group_commit_mode` |
...
| 取值示例 | 优点 | 缺点 |
| --- | --- | --- |
| 较短间隔（如 2 秒） | 数据可见性延迟更低，适合对实时性要求较高的场景 | 提交次数增多，版本数增长更快，后台 Compaction 压力更大 |
| 较长间隔（如 30 秒） | 提交批次更大，版本数增长更慢，系统开销更小 | 数据可见性延迟更高 |
...
| 取值示例 | 优点 | 缺点 |
| --- | --- | --- |
| 较小阈值（如 32 MB） | 内存占用更少，适合资源受限的环境 | 提交批次较小，吞吐量可能受限 |
| 较大阈值（如 256 MB） | 批量提交效率更高，系统吞吐量更大 | 占用更多内存 |
...
| 导入方式 | 退化为非 Group Commit 的条件 |
| --- | --- |
| `INSERT INTO VALUES` | 使用事务写入（`Begin; INSERT INTO VALUES; COMMIT`）<br/>指定 Label（`INSERT INTO dt WITH LABEL {label} VALUES`）<br/>VALUES 中包含表达式（`INSERT INTO dt VALUES (1 + 100)`）<br/>列更新写入<br/>表不支持轻量级模式更改 |
| `Stream Load` | 使用两阶段提交<br/>指定 Label（`-H "label:my_label"`）<br/>列更新写入<br/>表不支持轻量级模式更改 |
...
我们分别测试了使用 `Stream Load` 和 `JDBC` 在高并发小数据量场景下 `group commit`（使用 `async_mode`）的写入性能。
...
| `非 group_commit` | 1 MB | 10 | 1883 | 131,305 | 17.3 |
| `非 group_commit` | 10 MB | 10 | 294 | 840,983 | 105.4 |
| `非 group_commit` | 10 MB | 30 | 118 | 2,095,331 | 276.1 |
...
- INSERT INTO VALUES：返回结果中的 `label` 以 `group_commit_` 开头，且多次导入可能复用相同的 `label` 与 `txnId`。
- Stream Load：返回 JSON 中 `GroupCommit` 字段为 `true`，且 `Label` 以 `group_commit_` 开头。
...
- 检查是否处于显式事务、是否指定了 Label、VALUES 中是否包含表达式、是否为列更新写入，或表是否不支持轻量级 Schema Change。详见 [Group Commit 退化场景](#group-commit-退化场景)。
