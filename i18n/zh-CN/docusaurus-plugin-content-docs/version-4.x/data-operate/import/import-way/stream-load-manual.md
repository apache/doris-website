---
{
    "title": "Stream Load",
    "language": "zh-CN",
    "description": "Stream Load 通过 HTTP 协议同步导入本地文件或数据流到 Doris，支持 CSV/JSON/Parquet/ORC 格式，保证导入原子性。",
    "keywords": [
        "Stream Load",
        "Doris 数据导入",
        "HTTP 导入",
...
相比于直接使用 `curl` 的单并发导入，更推荐使用专用导入工具 Doris Streamloader。该工具是一款用于将数据导入 Doris 数据库的专用客户端工具，可以提供**多并发导入**的功能，降低大数据量导入的耗时。点击 [Doris Streamloader 文档](../../../connection-integration/data-integration/doris-streamloader) 了解使用方法与实践详情。
...
:::info 版本说明（4.0.8 / 4.1.4）

Doris 4.0 系列自 4.0.8 版本起、4.1 系列自 4.1.4 版本起，修复了存算分离模式下 Stream Load 的 Compute Group 路由与执行计划不一致的问题：
...
- 该路径同时会校验导入账号对目标 Compute Group 的访问权限、Compute Group 是否存在及其状态。因此，如果导入账号对实际承载请求的 Compute Group 没有权限，升级后该导入会失败并报权限错误，需要为账号补充相应授权。

:::

:::info 版本说明（4.1.4）

自 4.1.4 版本起，BE 上的 Stream Load 相关接口（`PUT /api/{db}/{table}/_stream_load`、`PUT /api/_http_stream`、`PUT /api/{db}/_stream_load_2pc`）不再做全局 `LOAD` 权限预检查，鉴权统一在 FE 侧按实际的库表进行。

因此，只拥有库 / 表级 `LOAD_PRIV`（没有全局 `LOAD` 权限）的账号现在也可以正常执行 Stream Load 和 2PC 提交 / 回滚。
...
建议单次导入文件大小不超过 10GB。如果文件过大，建议先切分文件再导入；也可以调整 BE 配置参数 `streaming_load_max_mb` 提升单次导入上限，或使用 [Doris Streamloader](../../../connection-integration/data-integration/doris-streamloader) 实现多并发导入以提升性能。

### 如何查询历史 Stream Load 作业？

默认情况下系统不记录 Stream Load 历史作业。需要在 `be.conf` 中设置 `enable_stream_load_record=true`，重启后即可使用 `show stream load from <db>` 查看历史记录。
...
1. 通过返回值中的 `ErrorURL` 字段查看具体错误行：`curl "<ErrorURL>"`；
