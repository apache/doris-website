---
{
    ...
    "language": "zh-CN",
    "description": "Spark Doris Connector 是 Apache Doris 与 Apache Spark 的连接器，支持通过 RDD、DataFrame 和 Spark SQL 方式读写 Doris 数据。支持批量和流式写入、数据过滤下推、Arrow Flight SQL 高速传输等特性，兼容 Spark 2.x 和 3.x 版本。"
}
---
...
在源码目录下执行 `sh build.sh`，根据提示输入你需要的 Scala 与 Spark 版本进行编译。

编译成功后，会在 `dist` 目录生成目标 jar 包，如：`spark-doris-connector-spark-3.5-25.2.0.jar`。将此文件复制到 `Spark` 的 `ClassPath` 中即可使用 `Spark-Doris-Connector`。
