...
- **事後フィルタリング**: カラムマッピングと変換の後に最終結果をフィルタリングします。
...
- **ステップ2：Column Mappingと割り当て** - 列名によって中間変数をターゲットテーブルフィールドにマップ
...
#### 指定されたjsonpathsを使ったJSON形式データの読み込み

![](/images/load-data-convert-json1-en.png)

#### 指定されたjsonpathsを使わないJSON形式データの読み込み

![](/images/load-data-convert-json2-en.png)

### 指定されたjsonpathsを使ったJSONデータの読み込み
...
4,\N,chongqing,1.4
```
対象テーブルには4つのカラムがあります：k1、k2、k3、k4。カラムを次のようにマッピングしたいと思います：
...
以下のようなソースデータがあるとします（列名は説明のためのもので、実際のヘッダーは存在しません）：
...
4,\N,chongqing,1.4
```
対象テーブルには3つの列があります：k1、k2、k3。ソースファイルから必要なのは1列目、2列目、4列目のみで、以下のマッピング関係があります：
...
列変換により、ユーザーはソースファイル内の列値を変換することができ、ほとんどの組み込み関数の使用をサポートしています。列変換は通常、列マッピングと併せて定義されます。つまり、まず列をマップしてから変換を行います。
...
4,\N,chongqing,1.4
```
対象テーブルには4つの列があります：k1、k2、k3、k4。以下のように列の値を変換したいと思います：
...
プレフィルタリングには以下のアプリケーションシナリオがあります：
...
例えば、ソースデータに複数のテーブルのデータが含まれている場合（または複数のテーブルのデータが同一のKafkaメッセージキューに書き込まれている場合）。各データ行には、そのデータがどのテーブルに属するかを示すカラムがあります。ユーザーはプレフィルタリング条件を使用して、対応するテーブルデータをフィルタリングしてロードできます。

プレフィルタリングには以下の制限があります：
- カラムフィルタリング制限

プレフィルタリングはカラムリスト内の独立した単純なカラムのみフィルタリングでき、式を含むカラムはフィルタリングできません。例：カラムマッピングが(a, tmp, b = tmp + 1)の場合、カラムbはフィルター条件として使用できません。

- データ処理制限

プレフィルタリングはデータ変換の前に発生し、生データの値を比較に使用し、生データは文字列タイプとして扱われます。例：`\N`のようなデータの場合、比較前にNULLに変換されるのではなく、`\N`文字列として直接比較されます。

### 例1：数値条件を使用したプレフィルタリング

この例では、シンプルな数値比較条件を使用してソースデータをフィルタリングする方法を説明します。フィルター条件k1 > 1を設定することで、データ変換前に不要なレコードをフィルタリングできます。
...
DISTRIBUTED BY HASH(k1) BUCKETS 1;
```
型がINTの列k2において、`abc`は無効な不正データです。このデータをフィルタリングするために、フィルタリング用の中間列を導入できます。
...
Post-filteringは、列マッピングと変換後の最終結果をフィルタリングするプロセスです。


### 列マッピングと変換を行わないフィルタリング
...
4,\N,chongqing,1.4
```
対象テーブルには4つの列があります：k1、k2、k3、k4。4番目の列が1.2より大きいデータのみを読み込みたいと思います。
...
4,\N,chongqing,1.4
```
ターゲットテーブルには4つの列があります：k1、k2、k3、k4。次のように列の値を変換します：
...
column4 -> k4
```
変換されたk3の値が3であるデータを除外したいと考えています。
...
4,\N,chongqing,1.4
```
対象テーブルには4つの列があります：k1、k2、k3、k4です。k1がNULLでk4が1.2未満のデータを除外したいと思います。
