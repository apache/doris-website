...
Auto Partition機能は、データインポートプロセス中に対応するパーティションが存在するかどうかの自動検出をサポートします。パーティションが存在しない場合、パーティションが自動的に作成され、正常にインポートされます。
...
- auto List Partitionでは、パーティション名の長さは**50文字を超えてはいけません**。この長さは、対応するデータ行のパーティションカラムの内容を連結およびエスケープすることから導かれるため、実際に許可される長さはより短くなる可能性があります。
...
- auto List Partitionでは、パーティションカラム内の既存の値でパーティションに対応しないすべての値について、新しい独立したパーティションが作成されます。
...
2つの構文間に競合はなく、対応するclauses/attributesを同時に設定するだけです。現在の期間のパーティションがAuto PartitionとDynamic Partitionのどちらによって作成されたかは不確実であることに注意してください。作成方法が異なると、パーティションの命名形式も異なります。
...
パーティションライフサイクルに制限を設定する必要があるシナリオでは、**Dynamic Partitionの作成を無効にして、パーティションの作成をAuto Partitionに完全に委ね**、Dynamic Partitionの動的パーティション回収機能を通じてパーティションライフサイクルの管理を完了することができます：
...
Auto Partitionが有効な場合、`auto_partition_name`関数を使用してパーティション名をパーティションにマッピングできます。`partitions`テーブル関数はパーティション名から詳細なパーティション情報を生成します。データを挿入した後の現在のパーティションを確認するために、`DAILY_TRADE_VALUE`テーブルを例に見てみましょう：
...
- 通常のパーティション化されたテーブルと同様に、aoto List Partitionは複数列でのパーティション化をサポートし、構文に違いはありません。
- データ挿入またはインポート処理中にパーティションが作成され、インポート処理全体が完了しない場合（失敗またはキャンセル）、作成されたパーティションは自動的に削除されません。
- Auto Partitionを使用するテーブルは、パーティション作成方法のみが異なり、手動から自動に切り替わります。テーブルと作成されたパーティションの元の使用方法は、Auto Partitionではないテーブルやパーティションと同じです。
- 誤って多数のパーティションが作成されることを防ぐため、Apache DorisはFE設定の`max_auto_partition_num setting`を通じて、Auto Partitionテーブルが収容できる最大パーティション数を制御します。必要に応じてこの値を調整できます。
- Auto Partitionが有効になっているテーブルにデータをインポートする際、コーディネータは通常のテーブルとは異なるポーリング間隔でデータを送信します。詳細については、[BE Configuration](../../admin-manual/config/be-config)の`olap_table_sink_send_interval_auto_partition_factor`を参照してください。この設定は`enable_memtable_on_sink_node`が有効になった後は影響しません。
- [insert-overwrite](../../sql-manual/sql-statements/data-modification/DML/INSERT-OVERWRITE)を使用してAuto Partitionテーブルにデータをロードする場合、動作はINSERT OVERWRITEのドキュメントに詳細が記載されています。
