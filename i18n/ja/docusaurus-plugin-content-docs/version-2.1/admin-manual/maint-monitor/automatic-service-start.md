---
{
  "title": "自動サービス起動",
  "language": "ja",
  "description": "この文書では、サービスの後にサービスが適時に立ち上がらないことを確実にするため、Dorisクラスターの自動pull-upを設定する方法について説明します"
}
---
# 自動サービス開始

このドキュメントでは、Dorisクラスターの自動プルアップの設定方法について説明し、本番環境での特殊な状況によるサービス障害後にサービスが適時にプルアップされないことがないようにします。

FEとBEの自動プルアップサービスは、Dorisクラスターが完全にセットアップされた後に設定する必要があります。
...
systemdを使用してdorisサービスを制御するには、sudo権限が必要です。sudo権限割り当ての最小粒度を確保するため、doris-feとdoris-beサービスのsystemd制御権限を指定された非rootユーザーに割り当てることができます。visudoでdoris-feとdoris-beのsystemctl管理権限を設定してください。
...
1. config ファイルの fe.conf と be.conf の両方で "JAVA_HOME" 変数を設定する必要があります。設定しないと "systemctl start" コマンドを使用して doris を起動することができません
...
Supervisor設定はsupervisor設定を自動的に取得します。yumコマンドを使用してsupervisorを直接インストールするか、pipを使用して手動でインストールできます。pipによる手動インストール手順は複雑なため、yumデプロイメントモードのみを示します。手動デプロイメントについては、インストールデプロイメントの[こちら](http://supervisord.org/installing.html)を参照してください。
...
3. BEプロセス管理を設定する
    ...
    ```
BE用のsupervisorプロセス管理設定ファイルを作成する
...
Brokerのスーパーバイザープロセス管理プロファイルを作成する
    ...
    ```
6. 最初にDorisサービスが停止しているかどうかを確認し、次にsupervisorを使用してDorisを自動的に起動させ、その後プロセスが正常に開始されるかどうかを確認する
...
- yumを使用してインストールしたsupervisorを開始すると、エラーが発生します: pkg_resources.DistributionNotFound: The 'supervisor==3.4.0' distribution was not found
...
```
- supervisorがDorisプロセスを自動的に起動するように設定されている場合、DorisでBEノードが異常な要因により故障すると、be.outに出力されるべきエラースタック情報がsupervisorによって傍受されます。詳細な解析のためには、supervisorのログで確認する必要があります。
