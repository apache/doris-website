---
{
  "title": "SHOW ANALYZE",
  "language": "ja",
  "description": "このステートメントは、統計収集ジョブのステータスを表示するために使用されます。"
}
---
## 説明
...
> 自動ジョブの情報を表示します。指定されていない場合は、デフォルトで手動ジョブの情報が表示されます。

**2. `<table_name>`**

> テーブル名。指定後、このテーブルに対応するジョブ情報を表示できます。指定されていない場合は、デフォルトですべてのテーブルのジョブ情報が返されます。
...
> ジョブ状態のフィルター条件。指定されていない場合は、デフォルトですべての状態のジョブの情報が表示されます。
...
|:--------------| :------------- |:------------------------------------------------|
| SELECT_PRIV   | Table    | SHOWを実行する際、照会するテーブルのSELECT_PRIV権限が必要です。 |
