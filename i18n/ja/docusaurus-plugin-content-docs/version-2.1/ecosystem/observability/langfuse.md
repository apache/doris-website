...
Langfuseは、大規模言語モデルアプリケーション向けの包括的な可観測性ソリューションを提供するオープンソースのLLMエンジニアリングプラットフォームです。以下のコア機能を提供します：

- **トレーシング**: LLMアプリケーションの呼び出しチェーンと実行フローの完全な記録
- **評価**: 多次元のモデル性能評価と品質分析
- **プロンプト管理**: プロンプトテンプレートの一元管理とバージョン管理
- **メトリクス監視**: アプリケーションのパフォーマンス、コスト、品質メトリクスのリアルタイム監視

本文書では、Apache Dorisを分析バックエンドとして使用してLangfuseソリューションをデプロイする方法について詳しく説明し、Dorisの強力なOLAP分析機能を最大限に活用して大規模なLLMアプリケーションデータを処理します。

## システムアーキテクチャ

Doris上のLangfuseソリューションは、以下のコアコンポーネントからなるマイクロサービスアーキテクチャを使用します：

| コンポーネント | ポート | 説明 |
|-----------------|-----------|----------------------------------------------------------------------------------|
| Langfuse Web    | 3000      | ユーザーインタラクションとデータ取り込み用のWebインターフェースとAPIサービス |
| Langfuse Worker | 3030      | データ処理と分析タスクのための非同期タスク処理 |
| PostgreSQL      | 5432      | ユーザー設定とメタデータのトランザクショナルデータストレージ |
| Redis           | 6379      | システムレスポンス性能向上のためのキャッシュレイヤーとメッセージキュー |
| MinIO           | 9090      | 生イベントとマルチモーダル添付ファイル用のオブジェクトストレージサービス |
| Doris Fe        | 9030 8030 | Dorisアーキテクチャの一部であるDorisFrontend。ユーザーリクエストの受信、クエリ解析と計画、メタデータ管理、ノード管理を担当 |
| Doris Be        | 8040 8050 | Dorisアーキテクチャの一部であるDorisBackend。データストレージとクエリプラン実行を担当。データはシャードに分割され、BEノードに複数のレプリカとして保存される |

::: note

Apache Dorisをデプロイする際、ハードウェア環境とビジネス要件に基づいて、統合されたコンピュート・ストレージアーキテクチャまたは分離されたコンピュート・ストレージアーキテクチャのいずれかを選択できます。
Langfuseデプロイメントでは、本番環境でのDocker Dorisの使用は推奨されません。Dockerに含まれるFEおよびBEコンポーネントは、ユーザーがDoris上のLangfuse機能を迅速に体験するためのものです。
...
| Apache Doris | 2.1.10+ | 分析データベース、別途デプロイが必要 |
...
| ネットワーク | 1Gbps | 10Gbps | データ転送性能を確保 |

### 前提条件

1. **Dorisクラスタの準備**
    - Dorisクラスタが正常に動作し、安定した性能を持つことを確認
    - FE HTTPポート（デフォルト8030）とクエリポート（デフォルト9030）にネットワーク経由でアクセス可能であることを確認
    - Langfuseは起動後にDoris内に必要なデータベースとテーブル構造を自動作成

2. **ネットワーク接続性**
    - デプロイメント環境からDocker Hubにアクセスしてイメージをプル可能
    - LangfuseサービスからDorisクラスタの関連ポートにアクセス可能
    - クライアントからLangfuse Webサービスポートにアクセス可能

:::tip デプロイメント推奨事項
Langfuseサービスコンポーネント（Web、Worker、Redis、PostgreSQL）のデプロイにはDockerの使用を推奨しますが、Dorisはより良いパフォーマンスと安定性のため別途デプロイすることを推奨します。詳細なDorisデプロイガイドについては公式ドキュメントを参照してください。
:::
...
| `NEXTAUTH_SECRET` | `your-debug-secret-key-here-must-be-long-enough` | セッション暗号化用のNextAuth認証キー |
| `SALT` | `your-super-secret-salt-with-at-least-32-characters-for-encryption` | データ暗号化ソルト（最低32文字） |
...
| パラメータ | 設定例 | 説明 |
|---------|--------|------|
| `LANGFUSE_ENABLE_BACKGROUND_MIGRATIONS` | `false` | バックグラウンド移行を無効にする（Doris使用時は無効にする必要があります） |
| `LANGFUSE_AUTO_DORIS_MIGRATION_DISABLED` | `false` | Doris自動移行を有効にする |
...
デプロイメントが完了したら、以下の手順でサービスにアクセスして初期化を行います：
