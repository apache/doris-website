...
Leading Hintは、ユーザーがDorisオプティマイザーにクエリプランでのテーブル結合順序を決定するよう指示することを可能にする強力なクエリ最適化技術です。Leading Hintの正しい使用により、複雑なクエリのパフォーマンスを大幅に向上させることができます。この記事では、DorisでLeading Hintを使用して結合順序を制御する方法について詳しく説明します。

## 通常のLeading Hint

### 構文

Leading Hintでは、オプティマイザーが従うべきテーブル結合順序を指定することができます。Dorisでは、Leading Hintの基本構文は以下の通りです：
...
+------------------------------------------------------------------------------+
```
Leading Hintが有効でない場合、プランの生成には通常のプロセスが使用されます。EXPLAINでは、使用されたHintが有効かどうかが表示され、主に3つのタイプに分類されます：

| Status        | Description |
|--------------|--------------------------------------------------------------------------------------------------------------------------------|
| `Used`       | Leading Hintが正常に有効です。 |
| `Unused`     | ここでのサポートされていないケースには、Leading Hintで指定された結合順序が元のSQLと等価でない場合や、このバージョンでその機能がサポートされていない場合が含まれます（詳細は制限事項を参照）。 |
| `SyntaxError` | 対応するテーブルが見つからないなど、Leading Hintの構文エラーを示します。 |

1. Leading Hintのデフォルト構文は、左深木を構築します：
...
2. 元の計画：
    ...
    +-------------------------------------------+
    ```
3. t1とt2のjoin順序を交換する必要がある場合、前に`leading(t2 t1)`を追加するだけです。`explain`を実行する際、このhintが使用されているかどうかが表示されます。以下はLeading planです：`Used`はHintが正常に有効であることを示します。
    ...
    +------------------------------------------------------------------------------+
    ```
4. Leading Hintに構文エラーがある場合、`explain`を実行すると、対応する情報が`SyntaxError`に表示されますが、プランは通常通り生成され、単にLeadingが使用されないだけです。例：
...
1. Left-Deep Tree
   ...
   上記で述べたように、Dorisのクエリステートメントでかっこがまったく使用されていない場合、Leadingはデフォルトでleft-deep treeを生成します。
...
5. Non-inner Join
   ...
   Non-inner Join（Outer JoinやSemi/Anti Joinなど）に遭遇した場合、Leading Hintは元のSQLセマンティクスに従って各joinのjoin方法を自動的に導出します。Leading Hintが元のSQLセマンティクスと異なるか生成できない場合、UnUsedに配置されますが、これは計画の正常な生成には影響しません。
   以下は交換できない例です：
    ...
    +--------------------------------------------------------------------------------+
    ```
読者が自分で検証できるよう、Leading Hintを使ったOuter、Semi、およびAnti Joinの例をいくつか示します。
...
6. View
   ...
   エイリアス（Alias）を含む場合、エイリアスは完全で独立したサブツリーとして指定でき、結合順序はテキストの順序に従って生成されます
...
Ordered hintはleading hintの特殊なケースと見なすことができ、テキストの順序と同じようにjoinの順序を制御するために使用されます。
...
Ordered HintとLeading Hintが同時に使用される場合、Ordered HintがLeading Hintよりも優先されます。これは、Leading Hintが指定されていても、Ordered Hintも存在する場合、クエリプランはOrdered Hintのルールに従って実行され、Leading Hintは無視されることを意味します。以下は、両方が同時に使用される場合の状況を示す例です：
...
Leading Hintは結合順序を手動で制御するための強力な機能であり、本番環境でのビジネスチューニングに広く使用されています。Leading Hintを適切に使用することで、現場での結合順序に関するチューニング要件を満たし、システム制御の柔軟性を向上させることができます。
