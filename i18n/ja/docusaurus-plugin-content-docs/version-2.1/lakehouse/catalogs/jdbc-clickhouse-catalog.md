...
* ClickHouseデータベース用のJDBCドライバー。[Maven Repository](https://mvnrepository.com/artifact/com.clickhouse/clickhouse-jdbc)からClickHouse JDBCドライバーの最新版または指定版をダウンロードできます。ClickHouse JDBC Driverのバージョン0.4.6の使用を推奨します。

* Dorisの各FEおよびBEノードとClickHouseサーバー間のネットワーク接続。デフォルトポートは8123です。
...
データソースにグローバルに信頼された証明書がインストールされたTLSを設定している場合、jdbc\_urlプロパティで設定されたJDBC接続文字列にパラメータを追加することで、クラスターとデータソース間のTLSを有効にできます。
...
ClickHouseをマッピングする際、DorisのDatabaseはClickHouseのDatabaseに対応します。そして、DorisのDatabase配下のTableは、ClickHouseのそのDatabase配下のTablesに対応します。マッピング関係は以下の通りです：
...
| float64                   | double                  |                                  |
| int8                      | tinyint                 |                                  |
| int16/uint8               | smallint                | DorisはUNSIGNEDデータタイプを持たないため、1段階スケールアップされます |
| int32/uInt16              | int                     | 上記と同様                    |
| int64/uint32              | bigint                  | 上記と同様                    |
| int128/uint64             | largeint                | 上記と同様                    |
| int256/uint128/uint256    | string                  | Dorisはこの桁数のデータタイプを持たないため、STRINGで処理されます |
| decimal(P, S)             | decimal(P, S) or string | Dorisがサポートする最大精度を超える場合、stringを使用して処理します |
...
1. Clickhouseデータの読み取り時に`NoClassDefFoundError: net/jpountz/lz4/LZ4Factory`エラーメッセージが発生する

   まず[lz4-1.3.0.jar](https://repo1.maven.org/maven2/net/jpountz/lz4/lz4/1.3.0/lz4-1.3.0.jar)パッケージをダウンロードし、各FEおよびBEディレクトリ配下の`custom_lib/`ディレクトリに配置してください（存在しない場合は手動で作成してください）。
