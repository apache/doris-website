...
- バージョン1.2から、Apache Dorisは正式にMulti-カタログ機能を導入し、様々なデータソースの自動メタデータマッピングとデータアクセスを可能にし、外部データ読み取りとクエリ実行の多数のパフォーマンス最適化を提供しています。現在では、高速でユーザーフレンドリーなレイクハウスアーキテクチャの構築を完全にサポートしています。
- バージョン2.1では、Apache DorisはData レイクハウスアーキテクチャをさらに強化し、主要なデータレイクフォーマット（Hudi、Iceberg、Paimonなど）の読み書き機能を向上させ、複数のSQL方言との互換性を導入し、既存システムからApache Dorisへのシームレスな移行を実現しました。データサイエンスと大規模データ読み取りシナリオに対して、DorisはArrow Flight高速読み取りインタフェースを統合し、データ転送効率を100倍改善しました。
...
- Hive Metastore、Hadoop、REST、Glue、Google Dataproc Metastore、DLFなどの複数のIceberg カタログタイプをサポート。
...
将来的に、Apache IcebergはApache Dorisのネイティブテーブルエンジンの一つとして機能し、レイクフォーマットデータに対してより包括的な分析と管理機能を提供します。Apache Dorisは、アップデート/Delete/Merge、書き戻し時のソート、増分データ読み取り、メタデータ管理などのApache Icebergのより高度な機能を段階的にサポートし、統合された高性能なリアルタイムデータレイクプラットフォームを共同で構築します。
...
Doris コマンドラインにログイン後、Doris クラスター内に Iceberg という名前の Iceberg カタログ が既に作成されています（`SHOW CATALOGS`/`SHOW CREATE CATALOG iceberg` で確認可能）。以下がこの カタログ の作成文です：
