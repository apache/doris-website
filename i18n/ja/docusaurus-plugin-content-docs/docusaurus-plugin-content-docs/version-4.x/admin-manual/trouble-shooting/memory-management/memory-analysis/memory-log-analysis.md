---
{
  "title": "メモリログ解析",
  "language": "ja",
  "description": "be/log/be.INFOのプロセスメモリログは主に2つのカテゴリに分けられます。1つはプロセスメモリステータスログで、"
}
---
`be/log/be.INFO`内のプロセスメモリログは主に2つのカテゴリに分かれています。1つはプロセスメモリサイズとシステムの残り利用可能メモリサイズを含むプロセスメモリステータスログです。もう1つはMemory Trackerによってカウントされるメモリサイズを含む、より詳細なプロセスメモリ統計ログです。

## プロセスメモリステータスログ解析

Doris BEのプロセスメモリステータスは、プロセスメモリが256 MB増加または減少するたびに`log/be.INFO`ログに出力されます。また、プロセスメモリが不足した場合、プロセスメモリステータスは他のログとともに出力されます。
...
```
1. `os physical memory 375.81 GB` はシステム物理メモリ 375.81 GB を指します。

2. `process memory used 4.09 GB (= 3.49 GB [vm/rss] - 410.44 MB [tc/jemalloc_cache] + 1 GB [reserved] + 0B [waiting_refresh])`
- 現在、BEプロセスが4.09 GBのメモリを使用していると判断されており、BEプロセスが実際に使用している物理メモリ`vm/rss`は3.49 GBです。
- そのうち410.44 MBが`tc/jemalloc_cache`です。このキャッシュ部分は後続の実行プロセスで最初に再利用されるため、ここではBEプロセスメモリとして計上されていません。
- `reserved`は実行プロセス中に予約されたメモリです。通常、HashTableなど大量のメモリを消費する操作を構築する前に、HashTableの構築プロセスがメモリ不足により終了されることがないように、HashTableのメモリを事前に予約します。この予約済みメモリ部分は、実際にはまだ割り当てられていない場合でも、BEプロセスメモリに計算されます。
- `waiting_refresh`は、2回のメモリステータス更新の間隔で要求された大容量メモリです。Dorisのメモリステータス更新のデフォルト間隔は100msです。2回のメモリステータス更新の間隔で大量のメモリ要求があった場合に、メモリが制限を超えた後にメモリGCが適時に検出・トリガーされないことを避けるため、間隔内で要求された大容量メモリをBEプロセスメモリに計算します。`waiting_refresh`は各メモリステータス更新後に0にクリアされます。

3. `sys available memory 134.41 GB (= 135.41 GB [proc/available] - 1 GB [reserved] - 0B [waiting_refresh])`
- BEプロセスに現在残っている利用可能メモリは134.41 GBであり、システムでBEプロセスが利用可能な実際のメモリ`proc/available`は135.41 GBです。
- 1GBのメモリがすでに予約されているため、BEプロセスの残り利用可能メモリを計算する際は`reserved`が減算されます。`reserved`と`waiting_refresh`の詳細については`limit 3.01 GB, soft limit 2.71 GB`および`low water mark 3.20 GB, warning water mark 6.40 GB`について、MemLimitとWaterMarkの詳細情報については、[Memory limit and watermark calculation method]を参照してください。

## プロセスメモリ統計ログ分析

プロセスの利用可能メモリが不足している場合、BEの大部分のメモリ要求はそれを認識し、Memory GCのトリガーやクエリのCancelなどの事前定義されたコールバック方法を試行し、プロセスメモリ統計ログを出力します。デフォルトの出力間隔は1秒です。ログは`Process Memory Summary`と`Memory Tracker Summary`の2つの部分に分かれています。`be/log/be.INFO`で見つけることができ、現在のプロセスメモリ使用量が期待値と一致しているかを確認できます。
...
```
`Process Memory Summary`はプロセスメモリステータスで、上記の[Process Memory Status Log Analysis]を参照してください。

`Memory Tracker Summary`はプロセスMemory Trackerのサマリーで、`Type=overview`と`Type=global`のすべてのMemory Trackerを含み、ユーザーが現在のメモリステータスを分析するのに役立ちます。メモリの各部分の意味を分析するには[Overview](./../overview.md)を参照してください。
