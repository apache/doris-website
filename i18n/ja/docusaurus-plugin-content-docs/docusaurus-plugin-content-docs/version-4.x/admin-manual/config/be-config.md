---
{
  ...
  "description": "この文書では主にBEの関連設定項目について紹介します。"
}
---
# BE設定

このドキュメントは主にBEの関連する設定項目について説明します。

BE設定ファイル`be.conf`は通常、BE配置パスの`conf/`ディレクトリに格納されます。バージョン0.14では、別の設定ファイル`be_custom.conf`が導入されます。この設定ファイルは、運用中にユーザーが動的に設定し、永続化された設定項目を記録するために使用されます。

BEプロセス開始後、まず`be.conf`の設定項目を読み込み、次に`be_custom.conf`の設定項目を読み込みます。`be_custom.conf`の設定項目は`be.conf`の同じ設定項目を上書きします。

`be_custom.conf`ファイルの場所は、`be.conf`内で`custom_config_dir`設定項目を通して設定できます。

## 設定項目の表示

BEの設定項目を表示するには2つの方法があります：

1. BEウェブページ

    ユーザーはBEのウェブページにアクセスして現在の設定項目を表示できます：

    `http://be_host:be_webserver_port/varz`

2. コマンドによる表示

   MySQLクライアントで以下のコマンドを使用してBEの設定項目を表示できます。具体的な構文については[SHOW-CONFIG](../../sql-manual/sql-statements/cluster-management/instance-management/SHOW-FRONTEND-CONFIG)を参照してください：

    `SHOW BACKEND CONFIG;`

    結果の各列の意味は以下の通りです：

    1. BackendId: backendのid。
    2. Host: backendのIP。
    3. Key: 設定項目の名前。
    4. Value: 現在の設定項目の値。
    5. Type: 設定項目の値の型（integerやstringなど）。
    6. IsMutable: 動的に設定可能かどうか。trueの場合、その設定項目は実行時に動的に設定できます。falseの場合、その設定項目はbe.confでのみ設定でき、BE再起動後に有効になることを意味します。

## 設定項目の設定

BE設定項目を設定するには2つの方法があります：

1. 静的設定

   `conf/be.conf`ファイルに設定項目を追加し設定します。`be.conf`の設定項目はBE起動時に読み込まれます。`be.conf`にない設定項目はデフォルト値が使用されます。
...
2. 動的設定
   ...
   BE起動後、以下のコマンドで設定項目を動的に設定できます。
    ...
    ```
バージョン0.13以前では、この方法で変更された設定項目は、BEプロセス再起動後に無効になります。バージョン0.14以降では、変更された設定を以下のコマンドで永続化できます。変更された設定項目は`be_custom.conf`ファイルに格納されます。
...
```
W0610 11:26:33.804431 56452 storage_engine.cpp:552] base compaction の実行コスト 0.00319222
...
0610 11:23:03.727535 (+     0us) storage_engine.cpp:554] base compaction の実行を開始
0610 11:23:03.728961 (+  1426us) storage_engine.cpp:560] 最適なタブレット 546859 を発見
0610 11:23:03.728963 (+     2us) base_compaction.cpp:40] base compaction ロックを取得
0610 11:23:03.729029 (+    66us) base_compaction.cpp:44] rowset が選択されました
0610 11:24:51.784439 (+108055410us) compaction.cpp:46] 並行性ロックを取得し、compaction を開始
0610 11:24:51.784818 (+   379us) compaction.cpp:74] 準備完了
0610 11:26:33.359265 (+101574447us) compaction.cpp:87] rowset のマージ完了
0610 11:26:33.484481 (+125216us) compaction.cpp:102] 出力 rowset 構築完了
0610 11:26:33.484482 (+     1us) compaction.cpp:106] 正確性チェック完了
0610 11:26:33.513197 (+ 28715us) compaction.cpp:110] rowset の変更完了
0610 11:26:33.513300 (+   103us) base_compaction.cpp:49] compaction 完了
0610 11:26:33.513441 (+   141us) base_compaction.cpp:56] 未使用の rowset は GC キューに移動されました
...
```
ヘッダーからタブレットの読み込みに失敗しました、失敗したタブレットサイズ: xxx、path=xxx
