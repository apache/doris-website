...
inverted indexが成熟し安定したため、古い`BITMAP INDEX`を置き換えることができます。そのため、新しく作成される`BITMAP INDEX`は自動的に`INVERTED INDEX`に切り替わりますが、既存の`BITMAP INDEX`は変更されません。この切り替えプロセス全体はユーザーに対して透過的であり、書き込みやクエリに変更はありません。さらに、ユーザーはFE設定`enable_create_bitmap_index_as_inverted_index`をfalseに設定することで、この自動切り替えを無効にできます。[#35528](https://github.com/apache/doris/pull/35528)
...
- drop stats tableの実行時にaudit logを書き込み

- 非効率的なクエリプランを避けるため、テーブルが部分的に分析されている場合はmin/max列統計を無視

- `set1 - set2`のようなsetに対するminus操作をサポート
