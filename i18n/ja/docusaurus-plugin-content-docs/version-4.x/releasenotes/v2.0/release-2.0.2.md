---
{
  "title": "リリース 2.0.2",
  "language": "ja",
  "description": "コミュニティユーザーと開発者の皆様のおかげで、Doris 2.0.2では489件の改善とバグ修正が行われました。"
}
---
# Release 2.0.2

コミュニティのユーザーと開発者のおかげで、Doris 2.0.2では489の改善とバグ修正が行われました。

## 動作変更

- [Remove json -> operator convert to json_extract #24679](https://github.com/apache/doris/pull/24679)

  json '->' 演算子は lambda 関数の構文と競合するため削除されました。これは json_extract 関数のシンタックスシュガーであり、前者で置き換えることができます。
- [Start the script to set metadata_failure_recovery #24308](https://github.com/apache/doris/pull/24308)

  metadata_failure_recovery を fe.conf から start_fe.sh の引数に移動し、予期しない使用を防ぎます。
...
- [Support custom lib dir to save custom libs #23887](https://github.com/apache/doris/pull/23887)
  ...
  custom_lib ディレクトリを追加し、ユーザーがカスタムライブラリファイルを配置できるようにし、custom_lib が置き換えられることはありません。
- [Optimize priority_ network matching logic #23784](https://github.com/apache/doris/pull/23784) 

  priority_network ロジックを最適化し、この設定が間違っているか設定されていない場合のエラーを回避します。
- [Row policy support role #23022](https://github.com/apache/doris/pull/23022) 

  行ポリシーでロールベース認証をサポートします。

### 新しいオプティマイザ Nereids 統計収集の改善
...
### JDBC、HDFS、Hive、MySQL、MaxCompute、Multi-Catalog のサポート強化
...
改善とバグ修正の完全なリストは [github](https://github.com/apache/doris/issues?q=label%3Adev%2F2.0.2-merged+is%3Aclosed) でご覧ください。
