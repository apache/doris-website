...
* `reset_persist`
    元のpersist設定をクリアするかどうかは、persistパラメータがtrueの場合のみ有効です。元のバージョンとの互換性のため、reset_persistのデフォルトはtrueです。
	persistがtrueに設定され、reset_persistが設定されていないかreset_persistがtrueの場合、この変更された設定が`fe_custom.conf`に書き込まれる前に、`fe_custom.conf`ファイルの設定がクリアされます。
	persistがtrueに設定され、reset_persistがfalseの場合、この変更された設定項目は`fe_custom.conf`に段階的に追加されます。
...
```
`set`フィールドは正常に設定された構成を示します。`err`フィールドは設定に失敗した構成を示します。`persist`フィールドは永続化情報を示します。
