...
---
ユーザーはMySQLプロトコルを使用してFEのクエリポート（`query_port`、デフォルト9030）を通じてDorisに接続します。複数のFEノードをデプロイする場合、ユーザーは複数のFE上にロードバランシング層をデプロイすることで、Dorisクエリの高可用性を実現できます。

このドキュメントでは、Dorisに適した様々なロードバランシングソリューションを紹介し、Proxy Protocolを使用してクライアントIPパススルーを実装する方法を説明します。
...
[Nginx](https://nginx.org/)のTCPリバースプロキシを使用してDorisの負荷分散を実装します。

#### Nginxのインストール

Nginxを正しくインストールするには、[Nginx](https://nginx.org/en/docs/install.html)の公式サイトを参照してください。ここでは、Nginx 1.18.0バージョンを使用したUbuntuシステムでのNginxのコンパイルとインストール手順を例として説明します。
...
パラメータの説明：
    ...
    - `-r`: リモートログを有効にします。
    - `-m 0`: タイムスタンプをマークします。分単位で、0の場合、この機能は無効であることを示します。
...
[ProxySQL](https://proxysql.com/)はC言語で書かれたオープンソースのMySQLデータベースプロキシソフトウェアです。接続管理、読み書き分離、ロードバランシング、フェイルオーバー、その他の機能を実装できます。高性能、設定可能性、動的管理などの利点があり、Webサービス、ビッグデータプラットフォーム、クラウドデータベース、その他のシナリオで一般的に使用されています。
...
`/var/lib/proxysql`ディレクトリに「proxysql.db」ファイルが存在する場合、ProxySQLサービスは最初の起動時にのみ`proxysql.cnf`を読み込んで解析し、その後の起動では読み込まなくなることに特に注意が必要です。
...
```
#### Doris FEノードのモニタリング設定

Doris FEノードを追加した後、これらのバックエンドノードをモニタリングする必要があります。

まず、Dorisでモニタリング用のユーザーを作成します：
...
```
次に、mysql-proxy プロキシレイヤーノードに戻ってモニタリングを設定します
...
ほとんどの場合、プロキシサービスを通してバックエンドのDorisサービスに接続する際、クライアントIP情報は失われ、DorisサーバーはプロキシサーバーのIPアドレス情報のみを取得できます。

バージョン2.1.1以降、Dorisは[Proxy Protocol](https://www.haproxy.org/download/1.8/doc/proxy-protocol.txt)プロトコルをサポートしています。このプロトコルを使用することで、クライアントIPパススルーを実装でき、ロードバランシングを経た後でも、Dorisはクライアントの実際のIPを取得してホワイトリストやその他の権限制御を実装できます。
...
3. Doris 3.1バージョン以前では、有効にした後、Proxy Protocolプロトコルを使用して接続する必要があり、そうでなければ接続に失敗します。バージョン3.1以降では、Proxy Protocolを有効にした後でも、標準のMySQL接続プロトコルを使用して接続できます。
