...
Dorisは現在、様々なメタデータサービスを通じてPaimonテーブルのメタデータにアクセスし、Paimonデータをクエリすることをサポートしています。

現在、Paimonテーブルに対する読み取り操作のみがサポートされています。Paimonテーブルへの書き込み操作は将来サポートされる予定です。
...
| シナリオ | 説明 |
| ------------ | ------------------------------------------------------------ |
| クエリアクセラレーション | Dorisの分散コンピューティングエンジンを使用して、Paimonデータに直接アクセスし、クエリを高速化します。 |
...
* `{MetaStoreProperties}`
  ...
  MetaStorePropertiesセクションは、Metastoreメタデータサービスの接続と認証情報を入力するために使用されます。詳細については、[サポートされているメタデータサービス]のセクションを参照してください。

* `{StorageProperties}`

  StoragePropertiesセクションは、ストレージシステムに関連する接続と認証情報を入力するために使用されます。詳細については、[サポートされているストレージシステム]のセクションを参照してください。
...
これらのパラメータは自動的に変換されて渡されます。以下にいくつかの例を示します：
  ...
  | 入力パラメータ | Paimon Java SDK用変換後パラメータ |
  | --- | --- |
...
* `{CommonProperties}`
  ...
  CommonPropertiesセクションは共通プロパティを入力するために使用されます。[Common Properties]の[Catalog Overview](../catalog-overview.md)セクションを参照してください。
  
### サポートされるPaimonバージョン

現在依存しているPaimonのバージョンは1.0.0です。

### サポートされるPaimonフォーマット

* Paimon Deletion Vectorの読み込みをサポート
...
| date                               | date          |                                                                         |
| timestamp\_without\_time\_zone     | datetime(N)   | 精度に応じてマップされます。精度が6より大きい場合、最大6までマップされます（精度損失の可能性があります）。 |
| timestamp\_with\_local\_time\_zone | datetime(N)/timestamptz(N)  | 精度に応じてマップされます。精度が6より大きい場合、最大6までマップされます（精度損失の可能性があります）。`enable.mapping.timestamp_tz`プロパティ（バージョン4.0.3以降でサポート）がマッピング動作を制御します。デフォルトでは`false`に設定されており、この場合タイプは`datetime`にマップされます。`true`に設定すると、`timestamptz`タイプにマップされます。|
| array                              | array         |                                                                         |
| map                                | map           |                                                                         |
| row                                | struct        |                                                                         |
| other                              | UNSUPPORTED   |                                                                         |
...
:::info 注
Dorisは現在タイムゾーン付きの`Timestamp`タイプをサポートしていません。すべての`timestamp_without_time_zone`と`timestamp_with_local_time_zone`は統一的に`datetime(N)`タイプにマップされます。ただし、読み込み時にDorisは実際のソースタイプに基づいてタイムゾーンを正しく処理します。例えば、`SET time_zone=<tz>`でタイムゾーンを指定した後、`timestamp_with_local_time_zone`カラムの戻り値に影響します。

`DESCRIBE table_name`ステートメントのExtraカラムでソースタイプにタイムゾーン情報が含まれているかどうかを確認できます。`WITH_TIMEZONE`と表示されている場合、ソースタイプがタイムゾーン対応タイプであることを示します。（この機能は3.0.8以降でサポート）

バージョン4.0.3以降でサポートされ、`timestamp_with_local_time_zone`はDorisの`timestamptz`タイプにマップできます。
:::
...
FlinkのようにPaimonの[Batch Incremental](https://paimon.apache.org/docs/master/flink/sql-query/#batch-incremental)クエリをサポートします。

指定されたsnapshotまたはtimestamp間隔内の増分データのクエリをサポートします。間隔は左閉右開です。
...
| --- | --- | --- |
| `startSnapshotId` | 開始スナップショットID、0より大きい値である必要があります。`endSnapshotId`と合わせて指定する必要があります。 | `'startSnapshotId'='3'` |
| `endSnapshotId` | 終了スナップショットID、`startSnapshotId`より大きい値である必要があります。`startSnapshotId`と合わせて指定する必要があります。 | `'endSnapshotId'='10'` |
...
| `endTimestamp` | 終了スナップショットタイムスタンプ、`startTimestamp`より大きい値である必要があります。オプションで、指定しない場合は`startTimestamp`から最新のスナップショットまで読み取ります。単位はミリ秒です。 | `'endTimestamp'='1750944949000'` |
...
+-------------+-------------------------+
```
`FOR TIME AS OF`および`FOR VERSION AS OF`文を使用して、スナップショットIDまたはスナップショットが作成された時刻に基づいて履歴バージョンデータを読み取ることができます。例：
...
+----------+-------------+-----------+-------------------------+--------------+-------------+---------------+
```
Spark/Trinoのようなシステムとの互換性を持つため、様々な構文形式をサポートします：
...
テーブルの現在および過去のスキーマ情報を表示します。`ALTER TABLE`、`CREATE TABLE AS`、または`CREATE DATABASE AS`文を使用してテーブルスキーマを変更する場合、各変更によりschemasテーブルにレコードが生成されます：
...
現在のスナップショットによって参照されているすべてのデータファイルに関する情報を表示します。ファイル形式、レコード数、ファイルサイズなどが含まれます：
...
テーブルのコンシューマー情報を表示します。データ消費の追跡に使用されます：
...
テーブルの集計フィールド情報を表示します。集計テーブルのフィールド設定に使用されます：
...
バージョン3.1以前では`s3a`プロトコルをサポートしていません。どうしても使用する必要がある場合は、回避策として`s3a`関連のパラメータを一時的に追加することができます。

**バージョン3.1以降**

バージョン3.1以降では、`s3a`パラメータを明示的に追加する必要がなくなりました。Paimonストレージパラメータをオーバーライドする必要がある場合は、`paimon.fs.`プレフィックス付きのパラメータを追加できます。PaimonはHDFS APIを内部的に使用するため、すべての`fs.s3a.*`パラメータがサポートされています。
