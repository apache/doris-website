...
この文書では主にTPCDS Connectorのデプロイと使用方法を紹介し、テストデータセットを構築する方法について説明します。
...
...
```
デプロイ後は、Connectorが正しくロードされることを確実にするため、FEおよびBEノードを再起動することを推奨します。
...
TPCDS Catalogには異なるScale Factorの事前構築されたTPCDSデータセットがあり、`SHOW DATABASES`と`SHOW TABLES`コマンドを使用して確認できます。
...
:::tip
これらの事前構築されたデータセットのデータは実際には格納されておらず、クエリ時にリアルタイムで生成されます。そのため、これらの事前構築されたデータセットは直接のBenchmarkテストには適していません。`INSERT INTO SELECT`を通じてデータセットを他のターゲットテーブル（Doris内部テーブル、Hive、Iceberg、その他Dorisが書き込みをサポートするデータソース）に書き込み、その後ターゲットテーブルでパフォーマンステストを実行するのに適しています。
:::
...
```
:::tip
3つの16C BEノードを持つDorisクラスターにおいて、TPCDS 1000 Hiveデータセットの作成には約3〜4時間かかります。
:::
