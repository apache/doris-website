...
### DorisをKyuubiデータソースとして設定

- `$KYUUBI_HOME/conf/kyuubi-defaults.conf`でKyuubi設定を更新
...
|----------------------------------------|---------------------------------------------------------------|
| kyuubi.engine.type                     | Engineタイプ、`jdbc`を指定                                        |
...
- Apache Kyuubiの他の設定については、[Apache Kyuubi Configuration Docs](https://kyuubi.readthedocs.io/en/master/configuration/settings.html)を参照してください。

### MySQL JDBCドライバーの追加

MySQL JDBCドライバー`mysql-connector-j-8.X.X.jar`を`$KYUUBI_HOME/externals/engines/jdbc`にコピーしてください。

### Kyuubi Serverの開始

`$KYUUBI_HOME/bin/kyuubi start`を実行してください。
開始後、デフォルトではポート10009がKyuubi ServerによってThriftプロトコルでリッスンされます。

## 例

以下の例は、beeline CLIを使用してThriftプロトコルでKyuubi経由でDorisをクエリする基本的な例を示しています。
...
クエリステートメント `select * from demo.expamle_tbl;` を実行し、クエリ結果が返されます。
