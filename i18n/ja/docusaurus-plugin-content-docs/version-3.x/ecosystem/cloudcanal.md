---
{
  "title": "BladePipe",
  "language": "ja",
  "description": "BladePipeは、30以上のデータベース、メッセージキュー、検索エンジン、キャッシュ間でデータを移動する、リアルタイムなエンドツーエンドのデータレプリケーションツールです。"
}
---
BladePipeは**リアルタイムでエンドツーエンドのデータレプリケーション**ツールです。**30以上**のデータベース、メッセージキュー、検索エンジン、キャッシュ、リアルタイムデータウェアハウス、データレイクなどの間でデータを移動し、**3秒未満の超低レイテンシ**を実現します。効率性、安定性、スケーラビリティ、多様なデータベースエンジンとの互換性、ワンストップ管理、強化されたセキュリティ、そして複雑なデータ変換機能を備えています。

## 機能
BladePipeは視覚的な管理インターフェースを提供し、**スキーママイグレーション、データマイグレーション、同期、検証および修正**などを実現するDataJobsを簡単に作成できます。さらに、パラメータ設定により、より精密でカスタマイズされた設定をサポートしています。現在、BladePipeは以下のソースDataSourcesからDorisへのデータ移動をサポートしています：

| ソースDataSource             | スキーママイグレーション | データマイグレーション | データ同期 | 検証・修正                 |
|------------------------------|------------------|----------------|-----------|---------------------------|
...
## インストール
[Install Worker (Docker)](https://doc.bladepipe.com/productOP/docker/install_worker_docker)または[Install Worker (Binary)](https://doc.bladepipe.com/productOP/binary/install_worker_binary)の手順に従って、BladePipe Workerをダウンロードしインストールしてください。

## 例
MySQLインスタンスを例に、以下の部分ではMySQLからDorisにデータを移動する方法について説明します。
...
2. ソースとターゲットのDataSourcesを選択し、**Test Connection**をクリックしてソースとターゲットのDataSourcesへの接続がどちらも成功していることを確認します。
  ![Create a DataJob-1](/images/bp-doris-3.png)

1. DataJob Typeとして**Incremental**を、**Full Data**オプションと共に選択します。
