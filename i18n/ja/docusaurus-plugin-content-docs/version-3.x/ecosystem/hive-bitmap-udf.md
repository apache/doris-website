---
{
  ...
  "language": "ja",
  "description": "Hive Bitmap UDFは、hiveテーブルでbitmapを生成し、bitmap操作を行うためのUDFを提供します。HiveのbitmapはDoris bitmapと全く同じです。"
}
---
# Hive UDF
...
3. hiveで柔軟なbitmap操作を提供する。例：積集合、和集合、差集合操作。計算されたbitmapも直接dorisにインポートできる
...
### Hive Bitmap UDF使用方法:
   ...
   Hive Bitmap UDFはHive/Sparkで使用されます。まず、hive-udf-jar-with-dependencies.jarを取得するためにfeをコンパイルする必要があります。
   コンパイル準備：ldbソースコードをコンパイル済みの場合、直接feをコンパイルできます。ldbソースコードをコンパイル済みの場合、直接コンパイルできます。ldbソースコードをコンパイルしていない場合、手動でthriftをインストールする必要があります。
...
Hiveテーブルを作成する際にフォーマットをTEXTとして指定すると、Binary型についてはHiveはbash64エンコードされた文字列として保存されます。そのため、DorisのHive Catalogを使用してbitmap_from_base64関数によりバイナリデータを直接Bitmapとして保存することができます。
...
```
2. [Dorisでカタログを作成する](../lakehouse/catalogs/hive-catalog)
