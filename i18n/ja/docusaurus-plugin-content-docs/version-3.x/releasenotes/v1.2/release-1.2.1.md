---
{
  "title": "リリース 1.2.1",
  "language": "ja",
  "description": "DecimalV3は、より高い精度とより良いパフォーマンスをサポートし、過去のバージョンと比較して以下の利点があります。"
}
---
# 改善
...
DecimalV3は、より高い精度と優れたパフォーマンスをサポートし、過去のバージョンに対して以下の利点があります。

- より大きな表現可能範囲、値の範囲が大幅に拡張され、有効数値範囲は[1,38]です。

- より高いパフォーマンス、異なる精度に応じて占有するストレージ領域を適応的に調整します。

- より完全な精度導出サポート、異なる式に対して、結果の精度に異なる精度導出ルールが適用されます。

[DecimalV3](https://doris.apache.org/docs/dev/sql-manual/sql-reference/Data-Types/DECIMALV3/)

### Iceberg V2をサポート

Iceberg V2をサポート（Position Deleteのみサポート、Equality Deleteは今後のバージョンでサポート予定）。

Iceberg V2形式のテーブルは、Multi-Catalog機能を通じてアクセスできます。

### OR条件からINへの変換をサポート

OR条件をIN条件に変換することをサポートし、一部のシナリオで実行効率を改善できます。[#15437](https://github.com/apache/doris/pull/15437) [#12872](https://github.com/apache/doris/pull/12872)
...
### Stream loadでquoted csvデータをサポート
...
- 一部のケースで、バージョン1.1から1.2にアップグレード後、ユーザー権限情報が失われる問題。[#15144](https://github.com/apache/doris/pull/15144)

- datev2/datetimev2タイプをパーティショニングに使用した際に、パーティション値が間違いになる問題を修正。[#15094](https://github.com/apache/doris/pull/15094)

- リリースされた多数の機能のバグ修正。完全なリストについては：[PR List](https://github.com/apache/doris/pulls?q=is%3Apr+label%3Adev%2F1.2.1-merged+is%3Aclosed)

# アップグレード通知

### 既知の問題

- BEのランタイムJDKとしてJDK11を使用しないでください。BE Crashが発生します。
- このバージョンでは、csv形式の読み取りパフォーマンスが低下し、csv形式のインポートと読み取り効率に影響します。次の3桁バージョンでできるだけ早く修正する予定です。
...
- ベクトル化実行エンジンはデフォルトで強制的に有効になります。[#15213](https://github.com/apache/doris/pull/15213)
