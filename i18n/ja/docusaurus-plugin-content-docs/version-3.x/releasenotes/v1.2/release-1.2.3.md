...
- JDBC CatalogによるDorisクラスターへの接続をサポート。

現在、Jdbc Catalogは5.xバージョンのJDBC jarパッケージを使用した別のDorisデータベースへの接続のみをサポートしています。8.xバージョンのJDBC jarパッケージを使用する場合、カラムのデータ型が一致しない可能性があります。
