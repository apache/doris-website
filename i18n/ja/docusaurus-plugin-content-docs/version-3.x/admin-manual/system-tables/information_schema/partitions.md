---
{
  "title": "パーティション",
  "language": "ja",
  "description": "データベース内のすべてのテーブルのPartition状況を表示します。3.0.2以前（3.0.2を含む）では、テーブルは常に空でした。"
}
---
## 概要

データベース内のすべてのテーブルのPartitionステータスを表示します。3.0.2（含む）以前では、テーブルは常に空でした。
...
| LAST_CONSISTENCY_CHECK_TIME   | text          | 最終整合性チェック時間                   |
| BUCKET_NUM                    | int           | パーティションバケット数                 |
| COMMITTED_VERSION             | bigint        | 最終コミットバージョン                   |
