...
2. コマンドによる表示
    ...
    * Key: 設定項目の名前。
    * Value: 現在の設定項目の値。
    * Type: 設定項目値のタイプ（integer や string など）。
...
ノード（FEまたはBE）は、同じcluster idを持つ場合、同じDorisクラスターに属すると見なされます。Cluster idは通常、master FEが最初に開始されるときに生成されるランダムな整数です。独自に指定することもできます。
...
HTTP Server V2はSpringBootによって実装されています。フロントエンドとバックエンドを分離したアーキテクチャを使用します。HTTPv2が有効な場合のみ、ユーザーは新しいフロントエンドUIインターフェースを使用できます。
