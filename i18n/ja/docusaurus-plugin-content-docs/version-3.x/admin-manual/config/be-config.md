---
{
  ...
  "description": "この文書は主にBEの関連設定項目について説明します。"
}
---
<!-- 設定項目をアルファベット順にソートしてください -->

# BE設定

このドキュメントでは、主にBEの関連する設定項目について説明します。

BE設定ファイル`be.conf`は、通常BEデプロイメントパスの`conf/`ディレクトリに保存されます。バージョン0.14では、別の設定ファイル`be_custom.conf`が導入されます。この設定ファイルは、運用中にユーザーによって動的に設定され、永続化された設定項目を記録するために使用されます。

BEプロセスが開始されると、最初に`be.conf`の設定項目を読み込み、その後`be_custom.conf`の設定項目を読み込みます。`be_custom.conf`の設定項目は、`be.conf`の同じ設定項目を上書きします。

`be_custom.conf`ファイルの場所は、`custom_config_dir`設定項目を通じて`be.conf`で設定できます。

## 設定項目の表示

BEの設定項目を表示する方法は2つあります：
...
2. コマンドによる表示
   ...
   MySQLクライアントで以下のコマンドを使用してBEの設定項目を表示できます。具体的な構文は[SHOW-CONFIG](../../sql-manual/sql-statements/cluster-management/instance-management/SHOW-FRONTEND-CONFIG)を参照してください：
    ...
    6. IsMutable: 動的設定が可能かどうか。trueの場合、設定項目は実行時に動的に設定できます。falseの場合、設定項目はbe.confでのみ設定でき、BEの再起動後に有効になることを意味します。
...
1. 静的設定
   ...
   `conf/be.conf`ファイルで設定項目を追加・設定します。`be.conf`の設定項目はBE起動時に読み込まれます。`be.conf`にない設定項目はデフォルト値が使用されます。
...
0610 11:23:03.729029 (+    66us) base_compaction.cpp:44] rowsets を選択
0610 11:24:51.784439 (+108055410us) compaction.cpp:46] 並行実行ロックを取得し、compaction を開始
0610 11:24:51.784818 (+   379us) compaction.cpp:74] 準備完了
0610 11:26:33.359265 (+101574447us) compaction.cpp:87] rowsets のマージ完了
0610 11:26:33.484481 (+125216us) compaction.cpp:102] 出力 rowset を構築
0610 11:26:33.484482 (+     1us) compaction.cpp:106] 正確性チェック完了
0610 11:26:33.513197 (+ 28715us) compaction.cpp:110] rowsets の変更完了
0610 11:26:33.513300 (+   103us) base_compaction.cpp:49] compaction 完了
0610 11:26:33.513441 (+   141us) base_compaction.cpp:56] 未使用の rowsets を GC キューに移動
...
```
headerからのタブレット読み込みに失敗しました。失敗したタブレットサイズ: xxx、path=xxx
