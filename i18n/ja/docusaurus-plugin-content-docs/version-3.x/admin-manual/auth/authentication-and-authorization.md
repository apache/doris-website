...
ユーザーがApache Dorisにログインするプロセスは、**Authentication**と**Authorization**の2つの部分に分かれています。

- Authentication：ユーザーが提供した認証情報（ユーザー名、クライアントIP、パスワードなど）に基づいて本人確認を行います。確認されると、個々のユーザーがシステム定義のUser Identityにマッピングされます。
