...
2. ソートフィールドc1、c2がTableキーのプレフィックスと正確に一致する場合、さらなる最適化が適用されます。データ読み込み時には、データファイルのヘッダーまたはテール部分のみが読み込まれ、必要なn行分のデータ読み込み量のみに削減されます。
...
1. DUPTableとMOWTableにのみ適用され、MORTableとAGGTableには適用されません。
...
2. `enable_two_phase_read_最適化`：このセッション変数はこの最適化を有効にするかどうかを決定します。デフォルトはtrueで、falseに設定すると最適化が無効になります。

3. `topn_filter_ratio`は、LIMIT nとTable内の総データとの比率で、デフォルト値は0.5です。これは、LIMITの数がTable内のデータの半分を超える場合、フィルターが生成されないことを意味します。
