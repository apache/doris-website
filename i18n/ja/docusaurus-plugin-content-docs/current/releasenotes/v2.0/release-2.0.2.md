---
{
  "title": "リリース 2.0.2",
  "language": "ja",
  "description": "コミュニティユーザーと開発者のおかげで、Doris 2.0.2では489の改善とバグ修正が行われました。"
}
---
# Release 2.0.2

コミュニティユーザーと開発者のおかげで、Doris 2.0.2では489の改善とバグ修正が行われました。

## 動作変更

- [Remove json -> operator convert to json_extract #24679](https://github.com/apache/doris/pull/24679)

  lambda関数の構文と衝突するため、json '->' 演算子を削除しました。これはjson_extract関数のシンタックスシュガーであり、前者で置き換えることができます。
- [Start the script to set metadata_failure_recovery #24308](https://github.com/apache/doris/pull/24308)

  予期しない使用を防ぐため、metadata_failure_recoveryをfe.confからstart_fe.shの引数に移動しました。
...
- [Support custom lib dir to save custom libs #23887](https://github.com/apache/doris/pull/23887)
  ...
  ユーザーがカスタムlibファイルを配置できるcustom_libディレクトリを追加し、custom_libは置き換えられません。
- [Optimize priority_ network matching logic #23784](https://github.com/apache/doris/pull/23784) 

  この設定が間違っているか設定されていない場合のエラーを回避するため、priority_networkロジックを最適化しました。
- [Row policy support role #23022](https://github.com/apache/doris/pull/23022) 

  行ポリシーのロールベース認証をサポートします。

### 新しいオプティマイザーNereids統計収集の改善
...
### JDBC、HDFS、Hive、MySQL、MaxCompute、Multi-Catalogのサポート向上
