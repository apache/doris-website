...
| カタログ              | Doris カタログ、外部テーブルとデータレイクのクエリ時に使用、Advancedで設定 | internal                      |
| Database             | データベース名                                                           | tpch                          |
| 認証       | データベース認証方法を選択：Username / Username and Password | Username and Password         |
...
3. 左側のメニューから、**To a サーバー**セクションの下にある**More**をクリックします。利用可能なコネクタのリストで、**Doris JDBC by VeloDB**を検索します：
...
6. オプションの高度な構成：
   ...
   - Advancedでは、カタログを使用してデータレイクデータソースにアクセスできます；デフォルト値はinternalです、
