...
| column           | 説明                                   |
|------------------|--------------------------------------|
| CurrentConnected | 接続が現在のものであるかどうか                              |
| Id               | このスレッドの一意識別子                            |
...
| Time             | 前のコマンドが現在の状態に提出された時刻（秒単位）                 |
| State            | スレッドの状態                                |
| QueryId          | 現在のクエリステートメントのID                           |
| Info             | 一般的に、スレッドが実行したステートメントが記録され、デフォルトでは最初の100文字のみが表示されます         |

一般的なCommandタイプは以下の通りです：

| column | 説明 |
| -- | -- |
| Query | スレッドがステートメントを実行中 |
| Sleep | クライアントが実行ステートメントを送信するのを待機中 |
| Quit | スレッドが終了中 |
| Kill | killステートメントを実行中 |
...
|:-------------|:----------|:---------------------------|
| ADMIN_PRIV        | DATABASE  | 他のユーザーの接続情報を表示したい場合、ADMIN権限が必要です |
