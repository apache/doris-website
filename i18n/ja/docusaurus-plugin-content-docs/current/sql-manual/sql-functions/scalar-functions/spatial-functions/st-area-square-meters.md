...
地球表面上の閉じた領域の面積を平方メートルで計算します。入力パラメータは地球表面上の領域を表す幾何オブジェクト（ポリゴン、円、多面体など）です。

閉じていない幾何オブジェクト（点や線分など）の場合、面積は0になります。無効な幾何オブジェクト（自己交差するポリゴンなど）の場合は、NULLを返します。
...
- 入力幾何オブジェクトが無効（自己交差するポリゴン、未閉鎖ポリゴンなど）の場合、NULLを返します。
