---
{
  "title": "BOOL_OR",
  "language": "ja",
  "description": "式の全ての非NULL値に対して論理OR集約を実行します。"
}
---
## 説明
...
| パラメータ | 説明 |
| -- | -- |
| `<expr>` | 論理ORで集約される式。BOOLEAN型および0/非0ルール（0はFALSE、非0はTRUE）でbooleanに変換可能な数値型をサポートします。 |

## 戻り値

戻り値はBOOLEANです。すべての非NULL値が存在する場合はTRUEを返し、そうでない場合はFALSEを返します。
...
+----------------+----------------+----------------+----------------+
```
BOOL_ORは数値型も受け入れます。ゼロ以外の値は`TRUE`として扱われます。
...
```
### Window function:
以下の例では、条件 (id > 2) に基づいて行をパーティション化し、それらを2つのグループに分割して、ウィンドウ集計結果を表示します：
