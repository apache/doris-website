...
---
## 説明
`explode_json_array_double`テーブル関数はJSON配列を受け取ります。その実装ロジックは、JSON配列を配列型に変換してから処理のために`explode`関数を呼び出すことです。この動作は次と同等です：`explode(cast(<json_array> as Array<DOUBLE>))`。
...
- `<json>`がNULLまたは空配列（要素数が0）の場合、0行が返されます。
