...
---
## 説明
`explode_json_array_int_outer`テーブル関数はJSON配列を受け取ります。その実装ロジックは、JSON配列を配列型に変換してから、`explode`関数を呼び出して処理することです。動作は次と同等です：`explode_outer(cast(<json_array> as Array<BIGINT>))`。
...
- `<json>`がNULLまたは空配列（要素数が0）の場合、NULLを含む1行を返します。
