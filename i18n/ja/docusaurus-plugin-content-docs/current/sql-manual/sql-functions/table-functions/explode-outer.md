...
---
## 説明
`explode`関数は配列を受け取り、配列の各要素を個別の行にマッピングします。ネストされたデータ構造を標準的なフラットテーブル形式に平坦化するために、[`LATERAL VIEW`](../../../query-data/lateral-view.md)と組み合わせて使用する必要があります。`explode_outer`と[`explode`](./explode.md)の主な違いは、null値の処理方法です。
...
## 戻り値
- `<array>`内のすべての要素から構成される単一列、複数行の結果を返します。
...
2. 複数のパラメータ
    ...
    +------+------+------+------+------+
    ```
> 展開後に最も多くの行を持つ配列は`[null, null, 1, 2, 3, 4, 5]`（c3）で、7行あります。したがって、最終結果は7行となり、他の3つの配列（c0、c1、c2）は不足する行に対してNULLでパディングされます。
3. 空の配列
