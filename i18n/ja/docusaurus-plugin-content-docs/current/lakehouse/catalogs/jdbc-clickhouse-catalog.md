---
{
  ...
  "language": "ja",
  "description": "Doris JDBC CatalogはClickHouseデータベースに標準JDBC interfaceを介して接続することをサポートします。"
}
---
Doris JDBC Catalogは標準のJDBCインターフェースを介してClickHouseデータベースへの接続をサポートします。この文書では、ClickHouseデータベース接続の設定方法について説明します。
...
* DorisのFEおよびBE各ノードとClickHouseサーバー間のネットワーク接続。デフォルトポートは8123です。
...
```
`jdbc_url`は、ClickHouse JDBCドライバーに渡される接続情報とパラメーターを定義します。サポートされているURLパラメーターは、[ClickHouse JDBC Driver Configuration](https://clickhouse.com/docs/en/integrations/java#configuration)で確認できます。

### 接続セキュリティ

データソースにグローバルに信頼された証明書がインストールされたTLSを設定している場合は、jdbc\_urlプロパティに設定されているJDBC接続文字列にパラメーターを追加することで、クラスターとデータソース間のTLSを有効にすることができます。

例えば、jdbc\_url設定プロパティにssl=trueパラメーターを追加してTLSを有効にします：
...
ClickHouseをマッピングする際、DorisのDatabaseはClickHouseのDatabaseに対応します。そして、DorisのDatabase内のTableは、ClickHouseの該当Database内のTableに対応します。マッピング関係は以下の通りです：
...
| float64                   | double                  |                                  |
| int8                      | tinyint                 |                                  |
| int16/uint8               | smallint                | DorisにはUNSIGNEDデータ型がないため、1桁拡張されます |
...
1. Clickhouseデータの読み取り時に`NoClassDefFoundError: net/jpountz/lz4/LZ4Factory`エラーメッセージが発生する場合
   ...
   まず[lz4-1.3.0.jar](https://repo1.maven.org/maven2/net/jpountz/lz4/lz4/1.3.0/lz4-1.3.0.jar)パッケージをダウンロードし、各FEおよびBEディレクトリの`custom_lib/`ディレクトリに配置してください（存在しない場合は手動で作成してください）。
