...
- バージョン1.2以降、Apache Dorisは正式にMulti-カタログ機能を導入し、様々なデータソースに対する自動メタデータマッピングとデータアクセスを可能にし、外部データの読み取りとクエリ実行に対する多数のパフォーマンス最適化を行いました。現在、高速でユーザーフレンドリーなレイクハウスアーキテクチャの構築を完全にサポートしています。
- バージョン2.1では、Apache DorisはData レイクハウスアーキテクチャをさらに強化し、主流のデータレイク形式（Hudi、Iceberg、Paimonなど）の読み取りと書き込み機能を向上させ、複数のSQL方言との互換性を導入し、既存のシステムからApache Dorisへのシームレスな移行を実現しました。データサイエンスと大規模データ読み取りシナリオのため、DorisはArrow Flight高速読み取りインターフェースを統合し、データ転送効率の100倍改善を実現しました。
...
- Hive Metastore、Hadoop、REST、Glue、Google Dataproc Metastore、DLFなど、複数のIceberg カタログタイプをサポートします。
...
ユーザーはApache Doris + Apache Icebergベースの効率的なData レイクハウスソリューションを迅速に構築し、様々なリアルタイムデータ分析と処理ニーズに柔軟に対応できます。
...
将来、Apache IcebergはApache Dorisのネイティブテーブルエンジンの一つとして機能し、レイク形式データにより包括的な分析と管理機能を提供します。Apache Dorisは、アップデート/Delete/Merge、書き戻し時のソート、増分データ読み取り、メタデータ管理など、Apache Icebergのより高度な機能を段階的にサポートし、統合された高性能リアルタイムデータレイクプラットフォームを共同で構築します。
...
Doris コマンドラインにログイン後、Doris クラスターには Iceberg という名前の Iceberg カタログ が既に作成されています（`SHOW CATALOGS`/`SHOW CREATE CATALOG iceberg` で確認可能）。以下がこの カタログ の作成文です：
