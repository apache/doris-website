---
{
  "title": "Huawei OBS",
  "language": "ja",
  "description": "このドキュメントでは、Huawei Cloud OBSにアクセスするために必要なパラメータについて説明します。これらは以下のシナリオに適用されます："
}
---
...
| obs.use_path_style           | s3.use_path_style        | パススタイルアクセスを使用するかどうか。MinIO/CephなどのAWS以外のS3サービスとの互換性のためにtrueに設定することを推奨 | FALSE         | No       |
...
* バージョン3.1以前では、`s3.`プレフィックスを使用した旧来の名前をご使用ください。
* `obs.region`を設定することでアクセスの精度とパフォーマンスが向上するため、設定することを推奨します。
* コネクションプールパラメータは同時実行要件に応じて調整し、コネクションのブロッキングを回避してください。
