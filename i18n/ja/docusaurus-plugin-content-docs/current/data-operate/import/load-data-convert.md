...
- **カラム変換**: 関数と式を使用してソースデータをリアルタイムで変換します。

- **事前フィルタリング**: カラムマッピングと変換の前に不要な生データを除外します。

- **事後フィルタリング**: カラムマッピングと変換後の最終結果をフィルタリングします。
...
> **注意**: Stream Loadは事前フィルタリングをサポートしていません。
...
カラムマッピングは、ソースデータカラムとターゲットテーブルカラム間の対応関係を定義するために使用されます。以下のシナリオに対応できます：
- ソースデータカラムとターゲットテーブルカラムの順序が一致しない
- ソースデータカラムとターゲットテーブルカラムの数が一致しない

### 実装原理

カラムマッピングの実装は2つのステップに分けられます：

- **ステップ1：データソース解析** - データフォーマットに基づいて生データを中間変数に解析
- **ステップ2：カラムマッピングと割り当て** - カラム名によって中間変数をターゲットテーブルフィールドにマッピング

以下は3つの異なるデータフォーマットの処理フローです：

#### CSVフォーマットデータの読み込み

![](/images/load-data-convert-csv-en.png)

#### 指定されたjsonpathsを使用したJSONフォーマットデータの読み込み

![](/images/load-data-convert-json1-en.png)

#### 指定されたjsonpathsを使用しないJSONフォーマットデータの読み込み

![](/images/load-data-convert-json2-en.png)

### 指定されたjsonpathsを使用したJSONデータの読み込み
以下のソースデータを想定します（カラムヘッダーは説明目的のみで、実際のヘッダーは存在しません）：
...
4,\N,chongqing,1.4
```
対象テーブルには4つの列があります：k1、k2、k3、k4。以下のように列をマッピングしたいと思います：
...
4,\N,chongqing,1.4
```
ターゲットテーブルには3つの列があります: k1、k2、k3。ソースファイルからは1番目、2番目、4番目の列のみが必要で、以下のマッピング関係があります:
...
column4 -> k3
```
ソースファイル内の特定の列をスキップするには、列マッピング時にターゲットテーブルに存在しない任意の列名を使用できます。これらの列名はカスタマイズ可能で、制限はありません。これらの列のデータは、ロード時に自動的に無視されます。
...
FROM KAFKA (...);
```
> 注意: この例の`tmp_skip`は、対象テーブルの列定義に含まれていない限り、任意の名前に置き換えることができます。
...
4,\N,chongqing,1.4
```
ターゲットテーブルには5つのカラムがあります：k1、k2、k3、k4、k5。ソースファイルからは1番目、2番目、3番目、4番目のカラムのみが必要で、以下のマッピング関係になります：
...
```
注意:
- k5にデフォルト値がある場合、デフォルト値が入力されます
- k5がnullable列でデフォルト値がない場合、NULLが入力されます
...
Column transformationは、ソースファイルの列値を変換することを可能にし、ほとんどの組み込み関数の使用をサポートします。Column transformationは通常、column mappingと一緒に定義されます。つまり、まず列をマップし、次にそれらを変換します。
...
4,\N,chongqing,1.4
```
ターゲットテーブルには4つの列があります：k1、k2、k3、k4。これらの列の値を以下のように変換したいと思います：
...
プレフィルタリングは、カラムマッピングと変換の前に不要な生データをフィルタリングするプロセスです。この機能はBroker LoadとRoutine Loadでのみサポートされています。

プレフィルタリングには以下の適用シナリオがあります：
...
例えば、ソースデータに複数のテーブルのデータが含まれている場合（または複数のテーブルのデータが同じKafkaメッセージキューに書き込まれている場合）。各データ行には、そのデータがどのテーブルに属するかを示すカラムがあります。ユーザーはプレフィルタリング条件を使用して、ロード対象の対応するテーブルデータをフィルタリングできます。

プレフィルタリングには以下の制限があります：
- カラムフィルタリングの制限

プレフィルタリングは、カラムリスト内の独立した単純なカラムのみをフィルタリングでき、式を含むカラムはフィルタリングできません。例：カラムマッピングが(a, tmp, b = tmp + 1)の場合、カラムbはフィルタ条件として使用できません。

- データ処理の制限

プレフィルタリングはデータ変換の前に実行され、生データの値を比較に使用し、生データは文字列型として扱われます。例：`\N`のようなデータの場合、NULLに変換される前に`\N`文字列として直接比較されます。

### 例1：数値条件を使用したプレフィルタリング

この例では、単純な数値比較条件を使用してソースデータをフィルタリングする方法を示します。フィルタ条件k1 > 1を設定することで、データ変換前に不要なレコードをフィルタリングできます。

以下のソースデータがあるとします（カラム名は説明目的のみで、実際のヘッダーはありません）：
...
DISTRIBUTED BY HASH(k1) BUCKETS 1;
```
列k2はINT型で、`abc`は無効なダーティデータです。このデータをフィルタリングするために、フィルタリング用の中間列を導入できます。
...
Post-filteringは、列マッピングと変換の後に最終結果をフィルタリングするプロセスです。


### 列マッピングと変換なしでのフィルタリング

次のソースデータがあるとします（列名は説明目的のみであり、実際のヘッダーはありません）：
...
4,\N,chongqing,1.4
```
対象テーブルには4つの列があります：k1、k2、k3、k4。4番目の列が1.2より大きいデータのみを読み込みたいと考えています。
...
4,\N,chongqing,1.4
```
対象テーブルには4つの列があります：k1、k2、k3、k4。k1がNULLでk4が1.2未満のデータを除外したいと思います。
