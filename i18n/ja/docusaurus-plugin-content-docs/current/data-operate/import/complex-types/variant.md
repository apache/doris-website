...
---
VARIANT型は半構造化JSONデータを格納でき、テーブル構造で特定の列を事前定義することなく、異なるデータ型（整数、文字列、ブール値など）を含む複雑なデータ構造の格納を可能にします。この型は、いつでも変更される可能性のある複雑なネストした構造の処理に特に適しています。書き込みプロセス中に、VARIANT型は列の構造と型を自動的に推測し、書き込まれたスキーマを動的にマージし、JSONキーとそれに対応する値を列および動的サブ列として格納します。詳細なドキュメントについては、[VARIANT](../../../sql-manual/basic-element/sql-data-types/semi-structured/VARIANT)を参照してください。
...
`VARIANT`型によって生成される動的サブ列が大量にある（例：2000列以上）幅広テーブルの場合、**Storage Format V3**を有効にすることを強く推奨します。

### VariantでのV3の利点
- **メタデータの分離**: V3は列メタデータ（`ColumnMetaPB`）をSegment Footerから別の領域に移動します。これにより、数千の動的列がある場合にFooterが大きくなりすぎることを防ぎ、ファイルのオープン速度を大幅に向上させ、メモリオーバーヘッドを削減します。
- **オンデマンドロード**: メタデータは必要な時にのみロードされ、オブジェクトストレージのアクセス遅延が大きいクラウドネイティブストレージで特に有益です。
- **コンパクトストレージ**: `BINARY_PLAIN_ENCODING_V2`を使用して大きな末尾オフセットテーブルを排除し、`VARIANT`内の文字列およびJSONB型でよりコンパクトなストレージを実現します。
- **数値スキャン性能の向上**: 整数型はデフォルトで`PLAIN_ENCODING`を使用し、LZ4/ZSTDと組み合わせることで高い読み取りスループットを提供します。

V3を有効にするには、テーブルプロパティでストレージ形式を指定します：
...
読み込まれたデータを確認するには、以下のSQLクエリを使用してください：
...
```
DESCは特定のパーティションを指定し、そのパーティションのスキーマを表示するために使用できます。構文は以下の通りです：
