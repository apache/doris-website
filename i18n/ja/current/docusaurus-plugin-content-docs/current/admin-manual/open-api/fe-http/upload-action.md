---
{
  "title": "アップロードアクション",
  "language": "ja",
  "description": "Upload Actionは現在主にFEのフロントエンドページに対応しており、ユーザーが小さなテストファイルを読み込むために使用されます。"
}
---
# Upload Action
...
FEノードにファイルをアップロードするために使用され、後でファイルを読み込むことができます。現在は最大100MBまでのファイルのアップロードのみをサポートしています。
...
* `file_id`
    ...
    ロードファイルIDを指定。ファイルをアップロードするAPIによって返される。

* `file_uuid`

    ファイルuuidを指定。ファイルをアップロードするAPIによって返される。
    
### Header

headerのオプションはStream Loadリクエストのheaderと同じです。
