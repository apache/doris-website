---
{
  ...
  "description": "この文書は主にBEの関連する設定項目について紹介します。"
}
---
# BE設定

このドキュメントでは主にBEの関連設定項目について紹介します。

BE設定ファイル`be.conf`は通常、BEデプロイメントパスの`conf/`ディレクトリに保存されます。バージョン0.14では、もう一つの設定ファイル`be_custom.conf`が導入されます。この設定ファイルは、運用中にユーザーが動的に設定し、永続化された設定項目を記録するために使用されます。
...
2. コマンドによる表示
   ...
   以下のコマンドでMySQLクライアントからBEの設定項目を表示できます。具体的な構文は[SHOW-CONFIG](../../sql-manual/sql-statements/cluster-management/instance-management/SHOW-FRONTEND-CONFIG)を参照してください：
    ...
    6. IsMutable: 動的設定が可能かどうか。trueの場合、その設定項目は実行時に動的に設定できます。falseの場合、その設定項目はbe.confでのみ設定可能で、BEの再起動後に有効になることを意味します。
...
1. 静的設定
   ...
   `conf/be.conf`ファイルで設定項目を追加・設定します。`be.conf`の設定項目はBE起動時に読み込まれます。`be.conf`にない設定項目はデフォルト値を使用します。

2. 動的設定

   BE開始後、以下のコマンドで設定項目を動的に設定できます。
...
0610 11:23:03.728961 (+  1426us) storage_engine.cpp:560] 最適なタブレット546859を発見
0610 11:23:03.728963 (+     2us) base_compaction.cpp:40] base compactionロックを取得
0610 11:23:03.729029 (+    66us) base_compaction.cpp:44] rowsetsが選択されました
0610 11:24:51.784439 (+108055410us) compaction.cpp:46] 並行性ロックを取得し、compactionを開始
0610 11:24:51.784818 (+   379us) compaction.cpp:74] 準備完了
0610 11:26:33.359265 (+101574447us) compaction.cpp:87] rowsetsのマージ完了
0610 11:26:33.484481 (+125216us) compaction.cpp:102] 出力rowsetを構築
0610 11:26:33.484482 (+     1us) compaction.cpp:106] 正確性チェック完了
0610 11:26:33.513197 (+ 28715us) compaction.cpp:110] rowsetsの変更完了
0610 11:26:33.513300 (+   103us) base_compaction.cpp:49] compaction完了
0610 11:26:33.513441 (+   141us) base_compaction.cpp:56] 未使用のrowsetsはGCキューに移動されました
