...
Doris BEのプロセスメモリステータスは、プロセスメモリが256 MB増減するたびに`log/be.INFO`ログに出力されます。また、プロセスメモリが不足している場合、プロセスメモリステータスは他のログと併せて出力されます。
...
```
1. `os physical memory 375.81 GB`は、システムの物理メモリ375.81 GBを指します。

2. `process memory used 4.09 GB (= 3.49 GB [vm/rss] - 410.44 MB [tc/jemalloc_cache] + 1 GB [reserved] + 0B [waiting_refresh])`
- 現在、BEプロセスが4.09 GBのメモリを使用していると考えており、BEプロセスが実際に使用している物理メモリ`vm/rss`は3.49 GBです。
- そのうち410.44 MBは`tc/jemalloc_cache`です。このキャッシュ部分は後続の実行プロセスで最初に再利用されるため、ここではBEプロセスメモリとしてカウントされません。
- `reserved`は実行プロセス中に予約されたメモリです。通常、HashTableや大量のメモリを消費するその他の操作を構築する前に、メモリ不足によりHashTable構築プロセスが終了しないよう、HashTableのメモリを事前に予約します。この予約メモリ部分は、実際に割り当てられていなくても、BEプロセスメモリで計算されます。
- `waiting_refresh`は、2回のメモリステータス更新間隔で要求された大容量メモリです。Dorisメモリステータス更新のデフォルト間隔は100msです。2回のメモリステータス更新間隔での大量のメモリリクエストが、メモリが制限を超えた後にメモリGCが検出されてタイムリーにトリガーされないことを避けるため、間隔内で要求された大容量メモリはBEプロセスメモリで計算されます。`waiting_refresh`は各メモリステータス更新後に0にクリアされます。
...
- メモリの1GBが予約されているため、BEプロセスの残り利用可能メモリを計算する際は`reserved`が差し引かれます。`reserved`と`waiting_refresh`について、`limit 3.01 GB, soft limit 2.71 GB`と`low water mark 3.20 GB, warning water mark 6.40 GB`については、MemLimitとWaterMarkの詳細情報は[Memory limit and watermark calculation method]を参照してください。

## プロセスメモリ統計ログ解析

プロセスの利用可能メモリが不足している場合、BEのほとんどのメモリリクエストはそれを認識し、Memory GCのトリガーやCancel queryなどの事前定義されたコールバック方法を試行し、プロセスメモリ統計ログを出力します。デフォルトの出力間隔は1sです。ログは`Process Memory Summary`と`Memory Tracker Summary`の2つの部分に分かれています。現在のプロセスメモリ使用量が期待に沿っているかを確認するため、`be/log/be.INFO`で見つけることができます。
...
`Memory Tracker Summary`はプロセスMemory Trackerの概要で、`Type=overview`と`Type=global`のすべてのMemory Trackerを含み、ユーザーが現在のメモリステータスを分析するのに役立ちます。メモリの各部分の意味を分析するには、[Overview](./../overview.md)を参照してください。
