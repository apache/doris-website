---
{
  ...
  "language": "ja",
  "description": "バックエンドでtalletの情報を表示します。（doris 3.0.7で追加）"
}
---
<!--
...
BackendsのTablet情報を表示します。（doris 3.0.7で追加）
...
| TABLET_PATH        | varchar(256) | tabletのパス                     |
| TABLET_LOCAL_SIZE  | bigint       | ローカルでのtabletのサイズ       |
| TABLET_REMOTE_SIZE | bigint       | リモートでのtabletのサイズ       |
| VERSION_COUNT      | bigint       | versionの数                      |
| SEGMENT_COUNT      | bigint       | segmentのサイズ                  |
| NUM_COLUMNS        | bigint       | カラムの数                       |
| ROW_SIZE           | bigint       | 行のサイズ                       |
| COMPACTION_SCORE   | int          | Compactionスコア                 |
| COMPRESS_KIND      | varchar(256) | 圧縮の種類                       |
| IS_USED            | bool         | tabletのdatadirが開かれているか  |
