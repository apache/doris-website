---
{
  ...
  "language": "ja",
  "description": "DataX DoriswriterプラグインはMySQL、Oracle、SQL Serverなどの様々なデータソースからのデータ同期をサポートします。"
}
---
# DataX Doriswriter

[DataX](https://github.com/alibaba/DataX) Doriswriter プラグインは、MySQL、Oracle、SQL Server などの様々なデータソースから Stream Load メソッドを使用して Doris にデータを同期することをサポートします。

:::info Note
このプラグインは DataX サービスと一緒に使用する必要があります。
DataX は複数のデータソースをサポートしています。詳細については、こちらを参照してください。
:::

## 使用方法

### DataX インストールパッケージを直接ダウンロード

DataX は、DataX が既に含まれている公式インストールパッケージを提供しており、直接ダウンロードして使用できます。詳細については、[こちら](https://github.com/alibaba/DataX?tab=readme-ov-file#download-datax%E4%B8%8B%E8%BD%BD%E5%9C%B0%E5%9D%80)を参照してください。

### DorisWriter プラグインを手動でコンパイル

DorisWriter プラグインの[ソースコード](https://github.com/apache/doris/tree/master/extension/DataX)をダウンロードしてください。

1. `init-env.sh` を実行
2. 必要に応じて `DataX/doriswriter` の doriswriter のコードを変更
3. doriswriter をビルド
    ...
    > DataX プロジェクト全体をコンパイルする必要がある場合は、[こちら](https://github.com/alibaba/DataX/blob/master/userGuid.md#quick-start)を参照してください
...
次の解決策を試すことができます：
        ...
        2. 解凍後、結果の`alibaba/datax/`ディレクトリを使用するmavenに対応する`.m2/repository/com/alibaba/`にコピーし、再度コンパイルを試してください。

### Datax DorisWriterパラメータ説明：
...
* **loadUrl**
  ...
  - 説明：Stream Loadの接続先として使用されます。フォーマットは"ip:port"です。ここでIPはFEノードのIP、portはFEノードのhttp_portです。複数指定することができ、英語のカンマ`,`で区切ることで、doriswriterはポーリング方式でアクセスします。
  - 必須：はい
  - デフォルト：なし
* **username**
...
* **connection. table**
  - 説明：書き込みが必要なDorisテーブルの名前。
    ...
    - デフォルト：なし
* **flushInterval**
  - 説明：データがバッチで書き込まれる時間間隔。この時間間隔を小さく設定しすぎると、Dorisの書き込みブロッキング問題が発生し、エラーコード-235になります。この時間間隔を小さく設定し、`maxBatchRows`と`batchSize`パラメータを大きく設定すると、設定したデータサイズに達しなくても同様にインポートされる可能性があります。
  - 必須：いいえ
  - デフォルト：30000 (ms)
* **column**
...
* **postSql**
  ...
  - 説明：宛先テーブルにデータを書き込んだ後、ここの標準文が実行されます。
  - 必須：いいえ
  - デフォルト：なし


* **maxBatchRows**
  - 説明：各バッチでインポートするデータの最大行数。**batchSize**と組み合わせて、バッチごとのインポートレコード行数を制御します。各バッチのデータが2つの閾値のいずれかに達すると、このバッチのデータのインポートが開始されます。
  - 必須：いいえ
  - デフォルト：500000

* **batchSize**
  - 説明：各バッチでインポートするデータの最大量。**maxBatchRows**と組み合わせて、バッチごとのインポート数を制御します。各バッチのデータが2つの閾値のいずれかに達すると、このバッチのデータのインポートが開始されます。
  - 必須：いいえ
  - デフォルト：94371840

* **maxRetries**

  - 説明：各バッチのデータインポート失敗後のリトライ回数。
  - 必須：いいえ
  - デフォルト：3
...
* **labelPrefix**
  ...
  - 説明：各バッチのインポートタスクのラベルプレフィックス。最終的なラベルは`labelPrefix + UUID`でグローバルに一意なラベルを形成し、データが重複してインポートされないことを保証します
  - 必須：いいえ
  - デフォルト：`datax_doris_writer_`

* **loadProps**

  - 説明：StreamLoadのリクエストパラメータ。詳細については、StreamLoad紹介ページを参照してください。[Stream load - Apache Doris](https://doris.apache.org/docs/data-operate/import/stream-load-manual)

    これには、インポートするデータフォーマット：format等が含まれます。インポートするデータフォーマットはデフォルトでcsvで、JSONをサポートしています。詳細については、以下の型変換セクションを参照するか、上記のStream loadの公式情報を参照してください。
...
>2. `line_delimiter`はデフォルトで改行文字ですが、データ内の値と競合する可能性があります。特殊文字や不可視文字を使用してインポートエラーを回避できます
>3. strip_outer_array : インポートされるデータのバッチ内の複数行のデータを表します。Dorisは解析時に配列を展開し、その中の各Objectを順番に1行のデータとして解析します。
>4. Stream loadのその他のパラメータについては、[Stream load - Apache Doris](../data-operate/import/import-way/stream-load-manual)を参照してください
...
>```
>
>**CSV形式では、データ内の特殊文字との競合を避けるため、行と列の区切り文字に特に注意する必要があります。ここでは隠し文字を推奨します。デフォルトの列区切り文字は: \t、行区切り文字: \n**
