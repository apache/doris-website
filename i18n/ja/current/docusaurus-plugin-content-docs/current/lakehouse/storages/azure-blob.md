...
本ドキュメントでは、Microsoft Azure Blobストレージにアクセスするために必要なパラメータについて説明します。これらのパラメータは以下のシナリオに適用されます：
...
| azure.endpoint                 |               | Azure Blobストレージのアクセスエンドポイント。通常はhttps://<account_name>.blob.core.windows.net形式です |               | Yes      |
...
- `azure.account_key`を取得する
  ...
  4. **key1**または**key2**の「キーの表示」をクリックし、**キー値**をコピーする
