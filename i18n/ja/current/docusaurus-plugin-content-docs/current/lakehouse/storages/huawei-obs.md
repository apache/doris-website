...
---
この文書では、Huawei Cloud OBSへのアクセスに必要なパラメータについて説明します。以下のシナリオに適用されます：
...
| obs.connection.timeout       | s3.connection.timeout    | OBSサービスへの接続確立のための接続タイムアウト（ミリ秒） | 1000          | No       |

> バージョン3.1より前では、従来の名前を使用
...
* Huawei Cloud OBSとの一貫性と明確性を確保するため、設定パラメータには`obs.`プレフィックスの使用を推奨します。
* バージョン3.1より前では、`s3.`プレフィックスを使用した旧名称を使用してください。
* `obs.region`を設定することでアクセス精度とパフォーマンスが向上するため、設定することを推奨します。
* コネクションプールパラメータは、コネクションのブロッキングを回避するため、同時実行要件に応じて調整できます。
