---
{
  "title": "Hive カタログ",
  "language": "ja",
  "description": "Apache DorisのHive CatalogはHive Metastoreおよび互換サービスに接続し、HDFSとオブジェクトストレージ上のHive、Iceberg、Hudiテーブルをクエリおよび書き込みを行い、豊富な設定、型マッピング、クエリおよび書き込み操作をサポートします。"
}
---
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

Hive Metastoreまたは Hive Metastore と互換性のあるメタデータサービスに接続することで、Doris は Hive データベースとテーブル情報を自動的に取得し、データクエリを実行できます。

Hive に加えて、多くの他のシステムが Hive Metastore を使用してメタデータを保存しています。したがって、Hive Catalog を通じて、Hive テーブルだけでなく、Iceberg や Hudi など、メタデータストレージに Hive Metastore を使用する他のテーブル形式にもアクセスできます。

## 適用シナリオ

| シナリオ | 説明 |
|----------|------|
| クエリ高速化 | Doris の分散コンピューティングエンジンを使用して Hive データに直接アクセスし、クエリを高速化します。 |
| データ統合 | Hive データを読み取って Doris 内部テーブルに書き込むか、Doris コンピューティングエンジンを使用して ZeroETL 操作を実行します。 |
| データライトバック | Doris でサポートされている任意のソースからデータを処理し、Hive テーブルにライトバックします。 |
...
* `{MetaStoreProperties}`
  ...
  MetaStorePropertiesセクションは、Metastoreメタデータサービスの接続情報と認証情報を入力するためのものです。詳細については「サポートされているメタデータサービス」セクションを参照してください。

* `{StorageProperties}`

  StoragePropertiesセクションは、ストレージシステムに関連する接続情報と認証情報を入力するためのものです。詳細については「サポートされているストレージシステム」セクションを参照してください。

* `{HiveProperties}`

  HivePropertiesセクションは、Hive Catalogに関連するプロパティを入力するためのものです。

  * `get_schema_from_table`: デフォルト値はfalseです。デフォルトでは、DorisはHive Metastoreからテーブルスキーマ情報を取得します。しかし、場合によっては`Storage schema reading not supported`などの互換性の問題が発生することがあります。この場合、このパラメータをtrueに設定すると、テーブルスキーマはTableオブジェクトから直接取得されます。ただし、この方法では列のデフォルト値情報が無視されることに注意してください。このプロパティはバージョン2.1.10および3.0.6以降でサポートされています。

  * `hive.recursive_directories`: パーティションディレクトリをリストする際にサブディレクトリを再帰的に検索するかどうか。このパラメータはバージョン3.0.2以降でサポートされています。バージョン4.0以前では、このパラメータはデフォルトで`false`でした。それ以降のバージョンでは、デフォルトで`true`になります。一部のHiveテーブルのパーティションパスは、テーブルスキーマのパーティション情報と一致しない場合があります。サブディレクトリ内のデータファイルを取得するには、このパラメータをtrueに設定する必要があります。

  * `hive.ignore_absent_partitions`: 存在しないパーティションを無視するかどうか。デフォルトは`true`です。`false`に設定すると、存在しないパーティションに遭遇した際にクエリがエラーを報告します。このパラメータはバージョン3.0.2以降でサポートされています。

  * `hive.staging_dir`: Hive書き込み操作の一時ステージングルートディレクトリを設定するために使用されます。書き込みプロセス中、データは最初にこのディレクトリに書き込まれ、完了後に最終的なHiveテーブルディレクトリに移動されます。このパスは、Hiveテーブルルートディレクトリに対する相対パスまたは絶対パスのいずれかです。デフォルト値は`/tmp/.doris_staging`です。場合によっては、Hiveテーブルに書き込む際に、`/tmp`ディレクトリとテーブルディレクトリが異なる名前空間に属し、書き込みが失敗することがあります。このような場合、このパラメータを相対パスを使用するように変更することで問題を解決できます。このパラメータはバージョン4.0.3以降でサポートされています。

* `{CommonProperties}`

  CommonPropertiesセクションは、共通属性を入力するためのものです。[Catalogの概要](../catalog-overview.md)の「共通プロパティ」セクションを参照してください。

### サポートされているHiveバージョン

Hive 1.x、2.x、3.x、および4.xをサポートしています。

Hiveトランザクションテーブルはバージョン3.x以降でサポートされています。詳細については、「Hiveトランザクションテーブル」セクションを参照してください。
...
> 注意：Dorisバージョンによってサポートされるサービスタイプとパラメータが若干異なります。[例]セクションを参照してください。

### Hive Catalog機能サポートマトリックス
...
> DorisでHiveテーブルを作成してデータを書き込むには、Catalog属性に`fs.defaultFS`プロパティを明示的に追加する必要があります。Catalogがクエリのためだけに作成される場合、このパラメータは省略できます。
>
> Dorisバージョンによってサポートされるサービスタイプとパラメータが若干異なります。[例]セクションを参照してください。
...
| float         | float         |                                                |
| double        | double        |                                                |
| decimal(P, S) | decimal(P, S) | 精度が指定されていない場合、デフォルトでdecimal(9, 0) |
| char(N)       | char(N)       |                                                |
| varchar(N)    | varchar(N)    |                                                |
| string        | string        |                                                |
| binary        | string/varbinary  | Catalogの`enable.mapping.varbinary`プロパティによって制御されます（4.0.2以降でサポート）。デフォルトは`false`で、`string`にマッピングされます。`true`の場合、`varbinary`タイプにマッピングされます。|
| timestamp with local time zone| datetime/timestamptz | `enable.mapping.timestamp_tz`プロパティ（バージョン4.0.3以降でサポート）がマッピング動作を制御します。デフォルトでは`false`に設定されており、この場合タイプは`datetime`にマッピングされます。`true`に設定すると、`timestamptz`タイプにマッピングされます。     |
| array         | array         |                                                |
| map           | map           |                                                |
| struct        | struct        |                                                |
| other         | unsupported   |                                                |
...
<details>
    ...
    <Tabs>
        <TabItem value='HDFS' label='HDFS' default>
            ...
            ```
Kerberos認証が有効なHMSおよびHDFSサービスへのアクセス
...
<details>
    ...
    <Tabs>
        <TabItem value='S3' label='S3' default>
            ...
            ```
Glueサービスの認証情報とS3の認証情報が異なる場合、以下の方法でS3の認証情報を別途指定できます。
