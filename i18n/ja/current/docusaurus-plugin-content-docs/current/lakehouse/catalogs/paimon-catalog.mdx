---
{
  "title": "パイモンカタログ",
  "language": "ja",
  "description": "Apache DorisのPaimon Catalogは複数のPaimonメタデータサービスに接続し、HDFSとクラウドオブジェクトストレージ全体でPaimonテーブルをクエリし、詳細な設定、プロパティ、クエリ操作を提供し、書き込みサポートが計画されています。"
}
---
```javascript
...
Dorisは現在、さまざまなメタデータサービスを通じてPaimonテーブルメタデータへのアクセスと、Paimonデータのクエリをサポートしています。

現在、Paimonテーブルに対する読み取り操作のみがサポートされています。Paimonテーブルへの書き込み操作は、将来サポートされる予定です。
...
* `<paimon_warehouse>`
  ...
  Paimonのwarehouseパスです。このパラメータは`<paimon_catalog_type>`が`filesystem`の場合に指定する必要があります。

  `warehouse`パスは`Database`パスの上位レベルを指している必要があります。例えば、テーブルパスが`s3://bucket/path/to/db1/table1`の場合、`warehouse`は`s3://bucket/path/to/`である必要があります。

* `{MetaStoreProperties}`

  MetaStorePropertiesセクションは、Metastoreメタデータサービスの接続および認証情報を記入するために使用されます。詳細については[サポートされているメタデータサービス]のセクションを参照してください。

* `{StorageProperties}`

  StoragePropertiesセクションは、ストレージシステムに関連する接続および認証情報を記入するために使用されます。詳細については[サポートされているストレージシステム]のセクションを参照してください。
...
* `{PaimonProperties}`
  ...
  paimon.fs.xxx
  ```
これらのパラメータは自動的に変換されて渡されます。いくつかの例を示します：
...
* `{CommonProperties}`
  ...
  CommonPropertiesセクションは共通プロパティを入力するために使用されます。[共通プロパティ]については、[Catalog Overview](../catalog-overview.md)セクションを参照してください。
...
| varchar                            | string        |                                                                         |
| char                               | string        |                                                                         |
| binary, varbinary                             | string/varbinary | Catalogの`enable.mapping.varbinary`プロパティによって制御されます（4.0.2以降でサポート）。デフォルトは`false`で、`string`にマッピングされます。`true`の場合、`varbinary`型にマッピングされます。|
| date                               | date          |                                                                         |
| timestamp\_without\_time\_zone     | datetime(N)   | 精度に従ってマッピングされます。精度が6を超える場合、最大6にマッピングされます（精度の損失が発生する可能性があります）。 |
| timestamp\_with\_local\_time\_zone | datetime(N)/timestamptz(N)  | 精度に従ってマッピングされます。精度が6を超える場合、最大6にマッピングされます（精度の損失が発生する可能性があります）。`enable.mapping.timestamp_tz`プロパティ（バージョン4.0.3以降でサポート）がマッピング動作を制御します。デフォルトでは`false`に設定されており、この場合型は`datetime`にマッピングされます。`true`に設定すると、`timestamptz`型にマッピングされます。|
| array                              | array         |                                                                         |
| map                                | map           |                                                                         |
| row                                | struct        |                                                                         |
| other                              | UNSUPPORTED   |                                                                         |
...
:::info 注意
Dorisは現在、タイムゾーン付きの`Timestamp`型をサポートしていません。すべての`timestamp_without_time_zone`と`timestamp_with_local_time_zone`は一律に`datetime(N)`型にマッピングされます。ただし、読み取り時にDorisは実際のソース型に基づいてタイムゾーンを正しく処理します。例えば、`SET time_zone=<tz>`でタイムゾーンを指定した後、`timestamp_with_local_time_zone`カラムの戻り結果に影響します。

`DESCRIBE table_name`文のExtraカラムで、ソース型にタイムゾーン情報が含まれているかどうかを確認できます。`WITH_TIMEZONE`と表示されている場合、ソース型がタイムゾーン対応型であることを示します。（この機能は3.0.8以降でサポートされています）

バージョン4.0.3以降でサポートされ、`timestamp_with_local_time_zone`はDoris `timestamptz`型にマッピングできます。
:::
...
Flinkと同様に、Paimonの[Batch Incremental](https://paimon.apache.org/docs/master/flink/sql-query/#batch-incremental)クエリをサポートしています。

指定されたスナップショットまたはタイムスタンプの間隔内での増分データのクエリをサポートしています。間隔は左閉右開です。
...
| `endTimestamp` | 終了スナップショットタイムスタンプ、`startTimestamp`より大きい値である必要があります。オプションで、指定されていない場合は`startTimestamp`から最新のスナップショットまでを読み取ります。単位はミリ秒です。 | `'endTimestamp'='1750944949000'` |
...
`$branches` および `$tags` システムテーブルを使用して、Paimon テーブルのブランチとタグを表示できます：
...
+----------+-------------+-----------+-------------------------+--------------+-------------+---------------+
```
Spark/Trino などのシステムとの互換性を保つため、さまざまな構文形式をサポートします：
...
Dorisは、テーブル関連のメタデータを取得するためのPaimon system tablesのクエリをサポートしています。System tablesは、スナップショット履歴、manifestファイル、データファイル、パーティション、その他の情報を表示するために使用できます。

Paimonテーブルのメタデータにアクセスするには、テーブル名の後に`$`記号を追加し、その後にsystem table名を続けます：

```sql
SELECT * FROM my_table$system_table_name;
```
:::info Note
DorisはPaimonグローバルシステムテーブルの読み取りをサポートしていません。これらはFlinkでのみサポートされています。
:::

### schemas

テーブルの現在および履歴のスキーマ情報を表示します。`ALTER TABLE`、`CREATE TABLE AS`、または`CREATE DATABASE AS`文を使用してテーブルスキーマを変更する場合、各変更によりschemasテーブルにレコードが生成されます：
...
テーブルのすべての有効なスナップショット情報を表示します。スナップショット作成時間、コミットユーザー、操作タイプなどが含まれます：
...
テーブルのコンシューマー情報を表示し、データ消費の追跡に使用されます：
...
#### テーブルの最新スナップショット情報を表示してその現在の状態を把握する
...
```
:::info Note
Paimonシステムテーブルの多くのフィールドはDorisのキーワードであるため、バッククォートで囲む必要があります。
:::
...
3.1より前のバージョンでは、ストレージ側で's3a'または's3'プロトコルを使用してHMSをメタデータストレージとして使用する際に、以下のエラーが発生します：
...
バージョン 3.1 より前では、`s3a` プロトコルをサポートしていません。どうしても使用する必要がある場合は、回避策として `s3a` 関連のパラメータを一時的に追加できます。

**バージョン 3.1 以降**

バージョン 3.1 以降では、`s3a` パラメータを明示的に追加する必要がなくなりました。Paimon ストレージパラメータをオーバーライドする必要がある場合は、`paimon.fs.` プレフィックス付きのパラメータを追加できます。Paimon は内部的に HDFS API を使用するため、すべての `fs.s3a.*` パラメータがサポートされています。
