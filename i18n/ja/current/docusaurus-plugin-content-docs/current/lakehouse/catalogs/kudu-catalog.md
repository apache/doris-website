...
Kudu CatalogはTrino Kudu Connectorを使用して[Trino Connector](https://doris.apache.org/community/how-to-contribute/trino-connector-developer-guide/)互換フレームワークを通じてKuduテーブルにアクセスします。
...
**方法1: プリコンパイルパッケージを使用（推奨）**

[こちら](https://github.com/apache/doris-thirdparty/releases/tag/trino-435-20240724)からプリコンパイル済みプラグインパッケージをダウンロードして展開してください。
...
2. すべてのFEおよびBEノードを再起動して、Connectorが適切にロードされるようにします。
...
TrinoPropertiesは、`trino.`プレフィックスが付いたTrino Kudu Connector固有のプロパティを設定するために使用されます。一般的なパラメータには以下が含まれます：

| パラメータ名                                          | 必須 | デフォルト | 説明                                   |
| ----------------------------------------------------- | ---- | ---------- | -------------------------------------- |
| `trino.kudu.client.master-addresses`                  | Yes  | -          | Kudu Masterノードアドレスのリスト      |
| `trino.kudu.authentication.type`                      | No   | NONE       | 認証タイプ: NONE または KERBEROS       |
...
CommonPropertiesは、メタデータ更新ポリシーやアクセス制御などの一般的なCatalogプロパティを設定するために使用されます。詳細情報については、[Catalog概要](../catalog-overview.md)の「共通プロパティ」セクションを参照してください。

## データ型マッピング

Kudu Catalogを使用する際、データ型は以下のルールに従ってマッピングされます：

| Kudu Type        | Trino Type    | Doris Type    | 備考                                                                                 |
| ---------------- | ------------- | ------------- | ------------------------------------------------------------------------------------ |
...
| binary           | varbinary     | string        | Trinoと一貫した表示結果を得るには`HEX(col)`関数を使用してクエリする                  |
...
:::tip
`binary`型の場合、16進形式で表示する必要がある場合は、`HEX()`関数を使用して列名を囲んでください。例：`SELECT HEX(binary_col) FROM table`。
:::
