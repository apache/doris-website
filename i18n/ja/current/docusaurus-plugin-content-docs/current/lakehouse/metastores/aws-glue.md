---
{
  "title": "AWS Glue",
  "language": "ja",
  "description": "この文書では、CREATE CATALOGを通じてIcebergテーブルまたはHiveテーブルにアクセスするためにAWS Glue Catalogを使用する際のパラメータ設定について説明します。"
}
---
...
AWS Glue Catalogは現在、3種類のCatalogをサポートしています：
...
このドキュメントは、ユーザーの設定を容易にするために、各タイプの詳細なパラメータ説明を提供します。
...
Example:
    ...
    ```
AWSの認証・認可設定については、ドキュメント[aws-authentication-and-authorization](../../admin-manual/auth/integrations/aws-authentication-and-authorization.md)を参照してください
...
| `glue.catalog_id`        | Glue Catalog ID（未対応）                                     | いいえ    | 空           |
...
| パラメータ名          | 説明                                                      | 必須 | デフォルト値 |
|------------------------|------------------------------------------------------------------|----------|---------------|
| `type`                 | `iceberg`に固定                                               | はい      | なし          |
| `iceberg.catalog.type` | `glue`に固定                                                  | はい      | なし          |
| `warehouse`            | Icebergデータウェアハウスのパス、例：`s3://my-bucket/iceberg-warehouse/` | はい      | s3://doris    |
| `glue.region`          | AWS Glueリージョン、例：`us-east-1`                             | はい      | なし          |
| `glue.endpoint`        | AWS Glueエンドポイント、例：`https://glue.us-east-1.amazonaws.com` | はい      | なし          |
| `glue.access_key`      | AWS Access Key ID                                               | いいえ       | 空         |
| `glue.secret_key`      | AWS Secret Access Key                                           | いいえ       | 空         |
...
| `glue.external_id`     | GlueにアクセスするためのIAM External ID（まだサポートされていません）         | いいえ       | 空         |
...
|----------------------------------|-------------------------------------------------------------------|----------|---------------|
| `type`                           | `iceberg`に固定                                                    | はい      | なし          |
| `iceberg.catalog.type`           | `rest`に固定                                                       | はい      | なし          |
...
| `iceberg.rest.sigv4-enabled`     | V4署名形式を有効化、`true`に固定                                      | はい      | なし          |
| `iceberg.rest.signing-name`      | 署名タイプ、`glue`に固定                                            | はい      | 空         |
...
2. 最小権限の原則
    ...
    - 権限をさらに制限するために、`*`を特定のデータベース/テーブルのARNに置き換えることができます。

3. S3権限

    - 上記のポリシーはGlue Catalogのみに関連します。
