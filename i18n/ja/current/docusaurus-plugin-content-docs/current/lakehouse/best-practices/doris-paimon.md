...
---
新しいオープンデータ管理アーキテクチャとして、Data Lakehouseはデータウェアハウスの高性能・リアルタイム機能とデータレイクの低コスト・柔軟性を統合し、ユーザーが様々なデータ処理・分析ニーズをより便利に満たすことを支援します。企業のビッグデータシステムでの適用が増加しています。

最近のバージョンでは、Apache Dorisはデータレイクとの統合を深化させ、成熟したData Lakehouseソリューションを進化させています。

- バージョン0.15以降、Apache DorisはHiveおよびIceberg外部テーブルを導入し、データレイク向けのApache Icebergとの組み合わせ機能を探求しています。
- バージョン1.2以降、Apache DorisはMulti-Catalog機能を正式に導入し、様々なデータソースに対する自動メタデータマッピングとデータアクセスを実現し、外部データの読み込みとクエリ実行に多くの性能最適化を施しました。現在、高速でユーザーフレンドリーなLakehouseアーキテクチャを構築する能力を完全に備えています。
- バージョン2.1では、Apache DorisのData Lakehouseアーキテクチャが大幅に強化され、主要なデータレイク形式（Hudi、Iceberg、Paimonなど）の読み書き機能を強化し、複数のSQL方言との互換性を導入し、既存システムからApache Dorisへのシームレスな移行を実現しました。データサイエンスと大規模データ読み込みシナリオに対して、DorisはArrow Flight高速読み込みインターフェイスを統合し、データ転送効率を100倍向上させました。

![Building lakehouse using Doris and Paimon](/images/lakehouse-architecture-for-doris-and-paimon.png)

## Apache Doris & Paimon

Apache Paimonは、データレイク形式とLSM構造の利点を革新的に組み合わせたデータレイク形式で、効率的なリアルタイムストリーミング更新機能をデータレイクアーキテクチャに成功裏に導入しています。これにより、Paimonは効率的にデータを管理し、リアルタイム分析を実行でき、リアルタイムData Lakehouseアーキテクチャの構築に強力なサポートを提供します。

Paimonの機能を最大限に活用し、Paimonデータのクエリ効率を向上させるため、Apache DorisはPaimonの最新機能のいくつかをネイティブサポートしています：

- Hive MetastoreやFileSystemなど、様々なタイプのPaimon Catalogをサポート。
...
- レイクへのリアルタイムデータ取り込み：PaimonのLSM-Treeモデルを活用し、レイクへのデータ取り込みを分レベルの即時性まで短縮できます。さらに、Paimonは集約、重複排除、部分列更新を含む様々なデータ更新機能をサポートし、データフローをより柔軟で効率的にします。
- 高性能データ処理・分析：PaimonのAppend Only Table、Read Optimized、Deletion VectorなどのテクノロジーはDorisの強力なクエリエンジンとシームレスに統合でき、レイクデータの高速クエリと分析応答を可能にします。

将来的に、Apache DorisはTime Travelや増分データ読み込みを含む、Apache Paimonのより高度な機能を段階的にサポートし、統合された高性能でリアルタイムなlakehouseプラットフォームを共同で構築します。

この記事では、Docker環境でApache Doris + Apache Paimonテスト・デモ環境を迅速にセットアップし、様々な機能の使用方法をデモンストレートする方法を説明します。
...
この記事では、以下のコンポーネントとバージョンでDocker Composeを使用したデプロイメントを行います：
...
以下に示すように、Dorisクラスターで`paimon`という名前のCatalogが作成されています（SHOW CATALOGSで確認可能）。以下はこのCatalogを作成するためのステートメントです：
...
```
Flink SQLの実行が完了すると、Dorisで最新のデータを直接確認できます：
...
ベースラインデータについては、Apache Paimonバージョン0.6でPrimary Key Table Read Optimized機能を導入した後、クエリエンジンが基盤となるParquet/ORCファイルに直接アクセスできるようになり、ベースラインデータの読み取り効率が大幅に向上しました。マージされていないインクリメンタルデータ（INSERT、UPDATE、またはDELETEによって生成されたデータ増分）については、Merge-on-Readを通じて読み取ることができます。さらに、Paimonはバージョン0.8でDeletion Vector機能を導入し、インクリメンタルデータを読み取るクエリエンジンの効率をさらに向上させました。
Apache Dorisは、ネイティブReaderを通じてDeletion Vectorの読み取りをサポートし、Merge on Readを実行します。DorisのEXPLAINステートメントを使用して、クエリにおけるベースラインデータとインクリメンタルデータのクエリ方法を実証します。
