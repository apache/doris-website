...
マテリアライズドビューはCatalogを通じて取得された結果との整合性のみを保証します。Catalogにはメタデータとデータキャッシュが含まれているため、マテリアライズドビューとHiveデータの整合性を保つには、`REFRESH CATALOG`などの方法を使用してCatalogをリフレッシュし、CatalogデータをHiveと同期する必要がある場合があります。
