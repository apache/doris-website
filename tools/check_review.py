...
def check_review_pass(pr_num, maintainers_file, token):
  ...
  response = requests.get(f"https://api.github.com/repos/apache/doris-website/pulls/{pr_num}/reviews?per_page=100", headers=headers)
  ...
  response = requests.get(f"https://api.github.com/repos/apache/doris-website/pulls/{pr_num}/files", headers=headers)
