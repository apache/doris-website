...
3. if already exists, then `alter table this_table REPLACE WITH TABLE this_table_temp PROPERTIES('swap' = 'False')`，This operation can exchange the table name and delete the `this_table_temp` temporary table,[this](../../../docs/sql-manual/sql-statements/table-and-view/table/ALTER-TABLE-REPLACE) guarantees the atomicity of this operation through the transaction mechanism of the Doris.
