...
Since 3.0.3, Doris supports both Auto and Dynamic Partition. Since 3.0.4, the functions of both work together on the table:
...
:::info Note
In some early versions prior to 3.0.3, this feature was not disabled but not recommended. 3.0.3 the `end` property takes effect original and doesn't have the `create_method` property. 3.0.4 and later the `end` and `create_method` behaviour changes take effect.
:::
