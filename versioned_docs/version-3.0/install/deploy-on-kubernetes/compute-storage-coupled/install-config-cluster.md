...
Doris uses ConfigMap to decouple configuration files from services, in Kubernetes. By default, services use the default configurations in the image as startup parameter configurations. To customize the startup parameters, create a specific ConfigMap following the instructions in the [FE Configuration Document](../../../../../docs/admin-manual/config/fe-config) and the [BE Configuration Document](../../../../../docs/admin-manual/config/be-config). Then deploy the customized ConfigMap to the namespace where the [DorisCluster resource](install-doris-cluster.md#step-2-custom-the-template-and-deploy-cluster) is to be deployed.
...
For more details on creating users, setting passwords, and granting permissions, refer to the [CREATE-USER](../../../../../docs/sql-manual/sql-statements/account-management/CREATE-USER) section.
