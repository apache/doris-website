...
3. Can compute nodes and mixed nodes configure a file cache directory
    ...
    [File cache](../lakehouse/data-cache) caches data files from remote storage systems (HDFS or object storage) that have been recently accessed, speeding up subsequent queries for the same data.
