...
[Apache Doris & Hudi Quick Start](../../lakehouse/lakehouse-best-practices/doris-hudi)
...
To support Incremental Read, you need to enable the [new optimizer](../../query-acceleration/optimization-technology-principle/query-optimizer.md), which is enabled by default. By viewing the execution plan through `desc`, we can find that Doris converts `@incr` into `predicates` and pushes it down to `VHUDI_SCAN_NODE`:
