...
The motivation, design and implementation, performance test results, and more concepts about Arrow Flight and ADBC ​​for Doris to support Arrow Flight SQL can be found at: https://github.com/apache/doris/issues/25514. This document mainly introduces the use of Doris Arrow Flight SQL and some common problems.
...
Compared with the traditional `jdbc:mysql` connection method, the performance test of the Arrow Flight SQL connection method of Jdbc and Java can be found at https://github.com/apache/doris/issues/25514. Here are some usage suggestions based on the test conclusions.
...
2. Currently, `jdbc:arrow-flight-sql` and Java ADBC/JDBCDriver do not support prepared statement passing parameters. For example, `select * from xxx where id=?` will report an error `parameter ordinal 1 out of range`. This is a bug in Arrow Flight SQL (https://github.com/apache/arrow/issues/40118)
