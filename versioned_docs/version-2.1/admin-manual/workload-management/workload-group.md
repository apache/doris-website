...
4. CPU soft limit is relative weight allocation: it does not restrict the absolute CPU time consumed by a task. Instead, it allocates CPU time slices in proportion to the assigned weights when CPU resources are contended (overall utilization ≥ 100%). If the CPU is idle, tasks can consume all available CPU resources without any restrictions. Therefore, it is extremely difficult to evaluate its actual effectiveness, and it is recommended to use CPU hard limit in production environments.
   :::
