...
Extracting a specific group from a match. The regular expression ([[:lower:]]+)C([[:lower:]]+) looks for sequences of one or more lowercase letters separated by 'C'. The group with index 1 corresponds to the first sequence of lowercase letters, so 'b' is returned.
...
An invalid group index.Explanation: Since the pattern has only 2 groups, an index of 5 is out of range, so NULL is returned.
