...
Each engine's cache entry uses a unified configuration key format: `meta.cache.<engine>.<entry>.{enable,ttl-second,capacity,max-weight}`. `max-weight` is currently supported only by selected estimator-backed entries; see [External Metadata Cache Memory Management](../external-meta-cache-memory-management.md).
...
| `max-weight` | `1GB` | Optional estimated retained-memory limit. Currently supported by Hive `partition_values`; `0` disables that entry. |
