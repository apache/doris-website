...
To improve the performance of accessing external data sources, Apache Doris caches Iceberg metadata. Metadata includes table structure (Schema), table objects, snapshots, view objects, and manifest details.
...
Each engine's cache entry uses a unified configuration key format: `meta.cache.<engine>.<entry>.{enable,ttl-second,capacity,max-weight}`. Memory limits apply only to estimator-backed entries; see [External Metadata Cache Memory Management](../external-meta-cache-memory-management).
...
| `capacity` | `10000` | Maximum number of cache entries by count. When `max-weight` is active, it is not a simultaneous count limit, but `0` still disables the cache. |
| `max-weight` | `1GB` | Supported since Doris 4.1.4. Optional estimated retained-memory limit. Supported by Iceberg `table`, `snapshot`, and `manifest`; `0` disables that entry. |

**Effective Logic:** The module cache takes effect when `enable=true`, `ttl-second != 0`, `capacity > 0`, and `max-weight` is either absent or not `0`.
...
| `snapshot` | `meta.cache.iceberg.snapshot.` | Caches snapshot metadata and retained table generations. Impact: Reduces repeated snapshot and partition-projection loading. This entry supports `max-weight`. |
...
* **Performance optimization**:
    * Enabling manifest cache can significantly speed up query planning for large tables:
      ...
      ```
    * Starting from Doris 4.1.4, a successful Catalog property change resets the Catalog execution context and clears initialized entries for every metadata-cache engine routed by that Catalog. Doris rebuilds them with the new context and configuration on the next access; in-flight queries are not affected.
...
Since version 4.1.4, the file format is determined per data file (split), so **a single Iceberg table containing both Parquet and ORC data files can be queried normally**. When the table-level `write.format.default` property is not set, the format defaults to `parquet` instead of being inferred by probing the data files.

Since version 4.1.4, the Iceberg identity partition metadata is delivered with every split, and the path partition keys only include the identity columns shared by all partition specs. As a result, partition column values are also returned correctly for tables that have gone through **Partition Evolution**.
...
Doris preserves the original case of column names in an Iceberg table and matches columns case-insensitively when reading. Column names containing uppercase letters or mixed case (for example `UserId` or `ORDER_ID`) can be queried normally.

:::info Version note

Versions before 4.0.8 located columns by exact match when reading, so Iceberg and Paimon tables with mixed-case column names could read the wrong column or fail the query. This was fixed in 4.0.8.
...
Since version 4.1.4, when `iceberg.rest.signing-name` is set to a managed signing service such as `glue`, `s3tables` or `osstables`, `'iceberg.rest.sigv4-enabled' = 'true'` **must** be set as well. Otherwise, creating the catalog fails with:

```text
Rest Catalog requires sigv4-enabled set to true when signing-name is <name>
```

If an existing catalog only sets `signing-name` without enabling `sigv4-enabled`, the validation fails after the upgrade and the property must be added.
...
`iceberg.rest.signing-name` accepts a new value `osstables` for connecting to Alibaba Cloud OSS Tables. In this case, the REST request signature reuses the OSS credentials (`oss.access_key`, `oss.secret_key`, `oss.session_token`) instead of the `s3.*` credentials.

```sql
CREATE CATALOG iceberg_osstables PROPERTIES (
    'type' = 'iceberg',
    'iceberg.catalog.type' = 'rest',
    'iceberg.rest.uri' = '<oss_tables_rest_endpoint>',
    'warehouse' = '<your-catalog-name>',
    'iceberg.rest.sigv4-enabled' = 'true',
    ...
    'oss.access_key' = '<ak>',
    'oss.secret_key' = '<sk>',
    'oss.endpoint' = 'oss-<region>.aliyuncs.com',
    'oss.region' = '<region>'
);
```

When using `osstables`, both `iceberg.rest.signing-region` and `iceberg.rest.sigv4-enabled = 'true'` are required. Otherwise, the following error is reported:
...
<details>
    <summary>4.2+ Version</summary>
    <Tabs>
        <TabItem value='OSS Tables REST Catalog' label='OSS Tables REST Catalog' default>
...
You can query the snapshots of an Iceberg table using the `$snapshots` system table:

:::caution Behavior Change (4.1.4)

Since version 4.1.4, the `iceberg_meta()` table function has been removed. Use the Iceberg system tables (for example, `iceberg_tbl$snapshots`) to query metadata instead. See the [System Tables](#system-tables) section below.

:::

```sql
SELECT * FROM iceberg_ctl.iceberg_db.iceberg_tbl$snapshots\G
...
- `_row_id`: A numerical value that uniquely identifies a row of data (generated based on the file and position where the data is written).
- `_last_updated_sequence_number`: The Sequence Number of the last modification to this row. When an `UPDATE` or `MERGE INTO` operation updates the data, this numerical value automatically increments along with the version number.

**How to query hidden columns:**

By default, hidden columns are not visible when using `DESC` or `SELECT *`. You can explicitly query them by specifying the column names directly:

```sql
SELECT id, _row_id, _last_updated_sequence_number FROM iceberg_tbl;
```

You can also make hidden columns visible in regular displays (like `DESC` and `SELECT *`) by enabling the session variable `show_hidden_columns`:
...
1. The Iceberg table's format version (`format-version`) must be configured as `3` for these row-level parameters to be generated. Querying row lineage columns on V1 and V2 format tables may result in errors.
2. Hidden columns are maintained by the system. Users are not allowed to explicitly specify or write to the `_row_id` or `_last_updated_sequence_number` columns when using writing statements like `INSERT`. Since version 4.1.4, naming a row lineage column in the column list of an `INSERT` statement on a table with `format-version >= 3` fails directly with: `Cannot specify row lineage column '<name>' in INSERT statement`.

### Iceberg V3 Default Values

> Supported since version 4.1.4.

Iceberg V3 defines two kinds of column default values, both of which are supported by Doris:

- `initial-default`: the default value assigned to **existing historical data** when a column is added. When reading an old data file that was written before the column existed, Doris returns this default value instead of NULL. It is supported for both Parquet and ORC, and covers top-level columns, nested subfields inside a `STRUCT`, and complex parent columns that are missing as a whole. It also takes effect during predicate evaluation.
- `write-default`: the default value filled in for **columns that are not specified** at write time. With `INSERT INTO t (subset of columns) VALUES (...)`, the columns that are not listed are written with the current `write-default` (earlier versions wrote NULL).

**Specifying a default value through DDL:**

```sql
ALTER TABLE iceberg_tbl ADD COLUMN c1 INT DEFAULT 10;
```

This statement sets both the Iceberg `write-default` and `initial-default`. It **requires the table's `format-version` to be `3`**; V1 and V2 tables do not support columns with a non-null default value.

**The `DEFAULT()` expression:**

Since version 4.1.4, the `DEFAULT(<column>)` expression is available for referencing a column's default value explicitly in DML. It can be used in `INSERT`, `UPDATE ... SET`, and in each branch of `MERGE INTO`:

```sql
INSERT INTO iceberg_tbl VALUES (1, DEFAULT(c1));

UPDATE iceberg_tbl SET c1 = DEFAULT(c1) WHERE id = 1;

MERGE INTO iceberg_tbl t USING src s ON t.id = s.id
WHEN NOT MATCHED THEN INSERT VALUES (s.id, DEFAULT(c1));
```

Semantics:

- For an Iceberg target table, `DEFAULT()` is resolved against the Iceberg schema pinned for this write. Referencing a column that is not part of the write target reports `DEFAULT(<col>) must reference a column in the pinned Iceberg write target`.
- For a non-Iceberg column, the column's default value is used. If the column has no default value and is NOT NULL or auto-increment, the error is `Column '<name>' has no default value and does not allow NULL or column is auto-increment`. If the column is nullable and has no default value, NULL is used.
- Other errors: `DEFAULT requires a column reference, but got: <expr>`, `Cannot find column information for DEFAULT(<name>)`, and `DEFAULT cannot be used on generated column '<name>'`.

:::info Note

The Iceberg `write-default` is a write-side connector concept. It is **not** exposed through `DESCRIBE`, `SHOW CREATE TABLE`, or `information_schema.columns.COLUMN_DEFAULT`.
...
>
> The `position_deletes` system table is supported starting from version 4.1.4.

:::caution Behavior Change (4.1.4)

Since version 4.1.4, Iceberg system tables are queried through the native execution path (`FileQueryScanNode` -> `IcebergScanNode`), and the `iceberg_meta()` table function has been **removed**. Use the `<table>$<system_table_name>` system table syntax instead. See [ICEBERG_META](../../sql-manual/sql-functions/table-valued-functions/iceberg-meta.md).

The access syntax and the returned results of the system tables themselves are unchanged. In addition, system tables now support `SHOW CREATE TABLE`.
...
The `DELETE` operation is used to delete rows of data in an Iceberg table that meet specified conditions. This operation is primarily based on the following underlying mechanisms: for V2 format tables, the system writes Position Delete files; for V3 format tables, the system writes Puffin-format Deletion Vectors files.

**Prerequisites:**

- The Iceberg table's format version (`format-version`) must be 2 or higher. V1 format tables do not support the `DELETE` operation.
...
DELETE FROM iceberg_tbl WHERE id IN (SELECT id FROM other_table WHERE status = 'inactive');
```
...
The `UPDATE` operation is used to modify rows of data in an Iceberg table that meet specified conditions. Internally, the implementation involves first writing deletion tracking information (using Position Delete files for V2, and Puffin-format Deletion Vectors for V3) to mark the old data rows as invalid, and then appending the updated new data files (if the V3 format option is enabled, the newly inserted row's tracking value `_last_updated_sequence_number` will automatically advance).

**Prerequisites:**

- The Iceberg table's format version (`format-version`) must be 2 or higher. V1 format tables do not support the `UPDATE` operation.

**Syntax:**

```sql
UPDATE iceberg_tbl SET <column1> = <value1> [, <column2> = <value2>, ...] WHERE <condition>;
```
...
The `MERGE INTO` operation is used to match source data with a target Iceberg table and perform insert, update, or delete operations based on the matching results. This is suitable for scenarios such as data synchronization and incremental updates.

**Prerequisites:**

- The Iceberg table's format version (`format-version`) must be 2 or higher. V1 format tables do not support the `MERGE INTO` operation.
...
- Full match operation (includes update, delete, and insert):
    ...
    * The row with `id = 4` is inserted because it does not exist in the target table.
...
1. The above three operations (`DELETE`, `UPDATE`, `MERGE INTO`) all require the Iceberg table's format version (`format-version`) to be 2 or higher. Attempting these operations on a V1 format table will result in the error: `must have format version 2 or higher for position deletes`. When executed on a V3 format table, the system not only uses Puffin-format Deletion Vectors to replace the original Position Delete data, but also automatically adjusts the lineage lifecycle attributes (`_last_updated_sequence_number`) of the written records if content updates are involved.
2. These operations support both Parquet and ORC file formats.
3. In concurrent writing scenarios, Iceberg provides an optimistic concurrency control mechanism. When write conflicts occur, the operation may throw a transaction conflict exception.
4. The `UPDATE` operation, both independently and within `MERGE INTO`, supports using expressions and functions, such as `age = age * 2 + 1` or `name = concat(name, '-modified')`.
5. These operations apply to both partitioned and non-partitioned tables.
6. **Since version 4.1.4** (all BEs must be upgraded), `MERGE INTO` validates the match relationship between the source and the target table: when the same target row is matched by multiple source rows, the statement fails with `multiple source rows matched the same target row` instead of producing a non-deterministic result. If a one-to-many match is expected, deduplicate on the source side first.
...
You can also create a database using a fully qualified name or specify database properties, such as:
...
**Database property support matrix (extended since version 4.1.4):**

| Catalog type (`iceberg.catalog.type`) | General database properties | `location` property |
| --- | --- | --- |
| `hms` | Supported | Supported |
| `glue` | Supported (since 4.1.4) | Supported (since 4.1.4) |
| `jdbc` | Supported (since 4.1.4) | Not supported |
| `hadoop` / `rest` / `dlf` / `s3tables` | Not supported | Not supported |

Error messages reported when a property is not supported:

- General properties: `Not supported: create database with properties for iceberg catalog type: <type>`
- `location` property: `Not supported: database property 'location' for iceberg catalog type: <type> because it does not determine the default table location`

> REST catalogs do not support database properties because the REST protocol allows the server to ignore namespace properties.
...
* **Rename Column**
    ...
    Use the `RENAME COLUMN` clause to rename columns. Since version 4.1.4, renaming columns within nested types is supported, and the `TO` keyword is optional.

    ```sql
    ALTER TABLE iceberg_table RENAME COLUMN old_col_name TO new_col_name;

    -- Supported since 4.1.4: TO can be omitted
    ALTER TABLE iceberg_table RENAME COLUMN old_col_name new_col_name;

    -- Supported since 4.1.4: rename a nested field
    ALTER TABLE iceberg_table RENAME COLUMN s.old_field TO new_field;
    ```

* **Add a Column**

    Use `ADD COLUMN` to add a new column. Since version 4.1.4, adding a new column to a nested type (STRUCT) is supported through a dotted path (for example, `s.new_field`); a newly added nested field must be nullable.
...
* **Drop Column**
    ...
    Use `DROP COLUMN` to drop columns. Since version 4.1.4, dropping a column within a nested type is supported through a dotted path.

    ```sql
    ALTER TABLE iceberg_table DROP COLUMN col_name;

    -- Supported since 4.1.4: drop a nested field
    ALTER TABLE iceberg_table DROP COLUMN s.field_name;
    ```
...
#### Nested Column Schema Change (4.1.4+)

Since version 4.1.4, `ADD COLUMN`, `DROP COLUMN`, `MODIFY COLUMN` and `RENAME COLUMN` all accept a dotted path (for example, `a.b.c`) to operate on a nested field directly. This capability applies **only to Iceberg tables**; using it on any other table type reports: `Nested column path is only supported for Iceberg tables: ...`.

Common limitations and errors:

| Error message | Description |
| --- | --- |
| `New nested field '<path>' must be nullable` | A newly added nested field must be nullable |
| `Rollup is not supported for Iceberg column operations` | The Rollup clause is not supported |
| `PROPERTIES are not supported for Iceberg column operations` | The PROPERTIES clause is not supported |
| `KEY is not supported for Iceberg ADD/MODIFY COLUMN` | The KEY keyword is not supported |
| `Generated columns are not supported for Iceberg ADD/MODIFY COLUMN` | Generated columns are not supported |
| `Modifying default values is not supported for Iceberg columns` | Modifying the default value of an existing column is not supported |
| `ON UPDATE is not supported for Iceberg ADD COLUMN` | The ON UPDATE clause is not supported |
| `Modify column type from complex to primitive is not supported` | Changing a complex type to a primitive type is not supported |
| `Cannot change MAP key type from ...` / `Cannot <op> MAP key nested column` | Changing the MAP key type, or performing a nested column operation on a MAP key, is not supported |
| `Iceberg does not support comments on collection element or value fields` | Comments on collection element or MAP value fields are not supported |
| `Multiple Iceberg ALTER clauses are not supported when a statement ...` | When nested columns are involved, a statement may contain only one ALTER clause |

### Modifying Table Properties

> Supported since version 4.1.4.

You can use `ALTER TABLE ... SET (...)` to modify the native properties of an Iceberg table, and **multiple properties can be set in a single statement**:

```sql
ALTER TABLE iceberg_ctl.iceberg_db.iceberg_tbl SET (
...
- All properties are committed atomically as one Iceberg `UpdateProperties` transaction. On failure, none of them takes effect, and no FE metadata refresh is triggered.
- Doris's own `auto_analyze_policy` property cannot be set together with connector-native properties in the same statement; otherwise the error `auto_analyze_policy cannot be set with external table properties` is reported. When set on its own, its values are still `enable` / `disable` / `base_on_catalog`.
- If the catalog does not implement metadata writes, the error `Update table properties operation is not supported for catalog: <name>` is reported.
