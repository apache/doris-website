...
Each engine's cache entry uses a unified configuration key format: `meta.cache.<engine>.<entry>.{enable,ttl-second,capacity,max-weight}`. Memory limits apply only to estimator-backed entries; see [External Metadata Cache Memory Management](../external-meta-cache-memory-management.md).
...
| `max-weight` | `1GB` | Optional estimated retained-memory limit. Supported by Iceberg `table`, `snapshot`, and `manifest`; `0` disables that entry. |
...
2. Hidden columns are maintained by the system. Users are not allowed to explicitly specify or write to the `_row_id` or `_last_updated_sequence_number` columns when using writing statements like `INSERT`.
