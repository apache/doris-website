...
Each engine's cache entry uses a unified configuration key format: `meta.cache.<engine>.<entry>.{enable,ttl-second,capacity,max-weight}`. Memory limits apply to the estimator-backed `table` and `snapshot` entries; see [External Metadata Cache Memory Management](../external-meta-cache-memory-management.md).
...
| `max-weight` | `1GB` | Optional estimated retained-memory limit. Supported by Paimon `table` and `snapshot`; `0` disables that entry. |
...
The currently dependent Paimon version for Doris 4.1.4 is 1.4.2.
