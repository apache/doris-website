...
|---|---|---|---|
| AWS S3 | Recommended: `fs.s3.support=true` | `false`; can be omitted | Omit `s3.endpoint` so the client generates it. |
...
<Tabs groupId="lance-filesystem-storage">
    <TabItem value="s3" label="AWS S3" default>
        ...
        This example omits `s3.endpoint` so the client generates the AWS S3 address from the bucket and region. `use_path_style=false` is the default and is included explicitly for comparison with other access styles.
    </TabItem>
