...
| Full-text search | Uses existing Lance FTS inverted indexes through `full_text_search()` and performs a Doris global Top-K merge |
...
`full_text_search()` is a relational TVF that performs BM25 retrieval through an existing FTS inverted index on a Lance table. Doris merges candidates from the physical index segments into a global Top-K and adds `_score` to the source columns. A higher score means greater relevance.

Before using the TVF, create an FTS inverted index on the target string column with the Lance SDK or another Lance writer. Doris does not create this index and does not fall back to a row-by-row full-text scan when no FTS index is available.
...
Use `EXPLAIN` to verify execution: `lanceFtsCoverageMode` shows the coverage mode, `lanceSearchIndexSegments` shows the number of physical FTS index segments used, and `lanceSearchUnindexedFragments` shows how many Fragments in the current snapshot are not covered by the index.
