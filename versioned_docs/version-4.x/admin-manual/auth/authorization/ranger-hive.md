...
- After Doris authenticates a user, it sends the current Doris username in the Ranger request. In 4.x versions that retain the Cluster Namespace in the session name, Doris removes that prefix first. The Ranger username must match the resulting name.
...
- Doris sends the names of the roles currently held by the Doris user as request context. In 4.x, Doris removes the Cluster Namespace prefix from those role names first. Ranger policy `Roles` can match the resulting names. This is only a Ranger policy condition; it does not cause Doris internal object grants to participate in the Ranger-Hive decision.
