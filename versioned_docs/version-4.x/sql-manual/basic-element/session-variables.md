...
Whether equivalent external table scan nodes within one statement reuse the splits they generate.
When enabled, a self-join or a CTE referenced several times over the same external table no longer
re-plans the splits, which reduces FE planning time. Applies to Hive, Hudi, Iceberg and Paimon.
...
Target size of the coarse-grained file splits produced by FE when the BE can refine them further. A
non-positive value disables BE-side refinement and falls back to the legacy FE split policy.
...
Target size of the fine-grained splits the BE produces per Parquet row group or ORC stripe. Because a
row group or stripe is indivisible, an individual split can exceed this target.
...
| Version | Default changed in 4.1.4 |

Maximum scan concurrency of a ScanNode. 4.1.4 raised the default from `4` to `8` to improve
throughput when scan threads are the bottleneck. Set it back to `4` to keep the old behavior.
