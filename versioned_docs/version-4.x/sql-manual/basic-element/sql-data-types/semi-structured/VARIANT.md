...
- JSON key length ≤ 255.
- Use [`PARSE_TO_VARIANT`](../../../sql-functions/scalar-functions/variant-functions/parse-to-variant.md) and [`TRY_PARSE_TO_VARIANT`](../../../sql-functions/scalar-functions/variant-functions/try-parse-to-variant.md) (supported since 4.1.4) to explicitly convert a JSON string to VARIANT. The difference is that the former reports an error when parsing fails, while the latter returns NULL.
