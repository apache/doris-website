...
4. For data exceeding 500 million records, **partitioning and bucketing** strategies must be implemented:
   ...
   6. Fact tables: We recommend using DATE or DATETIME as the partitioning column. In most cases, one partition per day is sufficient. The number of buckets can be calculated as follows:data size per partition ÷ (1GB~10GB).For example, if the data size of a partition is 100GB, a bucket count of around 10 to 100 is appropriate. For the bucketing strategy, use frequently queried columns with relatively even data distribution as the bucketing columns.
