...
Apache Doris 3.x can assign a Ranger-Hive Access Controller to an individual Hive Catalog. Doris then reuses the Hive policies already managed by Apache Ranger for database, table, and column authorization in that Catalog. Ranger row-level filters, data masking, and auditing are also supported.
...
- Confirm the Doris minor version before configuring Ranger Admin Kerberos:

    | Doris version | Factory class | Ranger Admin Kerberos |
    | --- | --- | --- |
    | 3.0 | Full class name only | The embedded Ranger 2.4 plugin does not support the UGI keytab login described in this guide |
    | 3.1 | Full class name or `ranger-hive` | Supported by the embedded Ranger 2.7 plugin |
...
Deploy `ranger-hive-security.xml` and `ranger-hive-audit.xml` together. Keep the audit file and explicitly disable auditing even if it is not needed, so the embedded Ranger plugin does not enter its legacy audit-configuration fallback. `ranger-policymgr-ssl.xml` is only needed when Ranger Admin uses HTTPS with a custom truststore or client certificate.
...
Ranger-Hive does not manage Doris Catalog-level permissions. With the default Catalog privilege checks enabled, the default Access Controller must also let the user discover and use the newly created `hive_ctl`. When the default controller is Doris built-in authorization, run:
...
| --- | --- | --- |
| Catalog `access_controller.class` | `org.apache.doris.catalog.authorizer.ranger.hive.RangerHiveAccessControllerFactory` | Full class name of the Ranger-Hive Access Controller factory. Use the full class name for compatibility across Doris 3.x; do not enter a Ranger Service Name |
...
The following table shows the base mapping in `RangerHiveAccessController` for 3.x. Doris removes the Cluster Namespace prefix from user, role, and database names before sending the request to Ranger. A mapping does not imply that every SQL statement performs that specific check. Statement support also depends on the authorization path implemented in the corresponding Doris version.
...
- After Doris authenticates a user, it removes the Cluster Namespace prefix from the current Doris username and sends the resulting name in the Ranger request. The Ranger username must match that resulting name.
...
- Doris removes the Cluster Namespace prefix from the roles currently held by the Doris user and sends the resulting role names as request context. Ranger policy `Roles` can match these names. This is only a Ranger policy condition; it does not cause Doris internal object grants to participate in the Ranger-Hive decision.
...
| FE/BE to HDFS | NameNode/DataNode service principals | `doris/hive-client.example.com@EXAMPLE.COM` | Read data; configured with Hive Catalog `hadoop.security.authentication` and `hadoop.kerberos.*` properties |

:::caution Version requirement
Doris **3.1 or later** is required for the Ranger Admin Kerberos procedure in this section. Doris 3.0 includes Ranger 2.4, which does not contain the Ranger plugin UGI keytab login used here; the `ranger.plugin.hive.ugi.*` properties below do not enable this capability on Doris 3.0. Doris 3.1 upgrades the embedded Ranger plugin to 2.7.
:::
...
CREATE CATALOG hive_ctl PROPERTIES (
    ...
    'hadoop.kerberos.principal' = 'doris/hive-client.example.com@EXAMPLE.COM',
    'hadoop.kerberos.keytab' = '/etc/security/keytabs/doris-hive.keytab',
...
| Symptom | First check |
| --- | --- |
| Catalog creation cannot find the Access Controller | Verify that `access_controller.class` is the full factory class used by Doris 3.x |
...
Use the full factory class name, which works throughout Doris 3.x:

```sql
'access_controller.class' = 'org.apache.doris.catalog.authorizer.ranger.hive.RangerHiveAccessControllerFactory'
```

Doris 3.1 also accepts the short identifier `ranger-hive`, but Doris 3.0 does not. Use the full class name above when the exact 3.x minor version is unknown or when upgrading from 3.0 to 3.1. Do not use `hive`, `hive_prod`, or the old class name `org.apache.doris.catalog.authorizer.RangerHiveAccessControllerFactory`. Confirm that every FE runs the same Doris version and search the FE log for `ClassNotFoundException` and the configured factory class.
...
- Catalog `access_controller.class` uses the full Ranger-Hive factory class for 3.x compatibility, and the configuration-prefix value is `hive`.
