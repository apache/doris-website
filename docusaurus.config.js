...
function buildRedirectIndex() {
    ...
    // The DOG Stack deployment page was split into Docker Compose and
    // Kubernetes pages; its old URL lands on the Docker Compose page.
    for (const version of ['dev', '4.x', '3.x']) {
        for (const locale of ['', '/zh-CN']) {
            add(
                `${locale}/docs/${version}/observability/dogstack/docker`,
                `${locale}/docs/${version}/observability/dogstack/deployment`,
            );
        }
    }
