...
- New versions may occasionally have bugs, so it's recommended to check if the [community](https://doris.apache.org/slack?utm_source=website&utm_medium=content&utm_content=blog_migrate-lakehouse-from-bigquery-to-doris) has reported any issues before deciding to upgrade.
