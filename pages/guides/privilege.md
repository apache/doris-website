...
5. 忘记密码
    ...
    如果忘记了密码无法登陆Doris，可以在 Doris FE 节点所在机器，使用如下命令无密码登陆 Doris：
...
7. ADMIN_PRIV 和 GRANT_PRIV 权限只能 `GRANT ON *.*` 或 `REVOKE FROM *.*`。因为对于指定的库和表，这两个权限没有意义。同时，拥有 GRANT_PRIV 其实等同于拥有 ADMIN_PRIV，因为 GRANT_PRIV 有授予任意权限的权限。请谨慎使用。
