...
1. [为 Doris 做贡献](#为-doris-做贡献)

1. [初次接触](#初次接触)

1. [Doris 的代码和文档](#doris-的代码和文档)

1. [改进文档](#改进文档)

1. [如果发现了一个 Bug 或问题](#如果发现了一个-bug-或问题)

1. [修改代码和提交PR（Pull Request）](#修改代码和提交prpull-request)
...
正如您在 [GitHub] (https://github.com/apache/incubator-doris) 上看到的，Apache Doris (incubating) 的代码库主要包括三部分：frontend (FE), backend (BE) 和 broker (为了支持 HDFS 文件读取)，文档主要是 Doris 网站和 GitHub 上的 wiki，还有运行 Doris 的时候的在线帮助手册。这些组件的详细情况参见下表：
...
我们的文档大多数是使用 markdown 格式编写的，您可以访问 Doris 的[网站](https://github.com/apache/incubator-doris-website)项目，直接在 github 上编辑后提交 PR 即可。

## 如果发现了一个 Bug 或问题

如果发现了一个 Bug 或问题，您可以直接通过 GitHub 的 [Issues](https://github.com/apache/incubator-doris/issues/new/choose) 提一个新的 Issue，我们会有人定期处理。

您也可以通过阅读分析代码自己修复（当然在这之前最好能和我们交流下，或许已经有人在修复同样的问题了），然后提交一个 [Pull Request](pull-request.html)。

## 修改代码和提交PR（Pull Request）

您可以下载代码，编译安装，部署运行试一试（可以参考[怎样编译安装运行文档](../guides/install.html)），看看是否与您预想的一样工作。如果有问题，您可以直接联系我们，提 Issue 或者通过阅读和分析源代码自己修复。

无论是修复 Bug 还是增加 Feature，我们都非常欢迎。如果您希望给 Doris 提交代码，您需要从 GitHub 上 fork 代码库至您的项目空间下，为您提交的代码创建一个新的分支，添加源项目为upstream，并提交PR，更详细的步骤可以参考[这篇文档](pull-request.html)。
