...
テーブルのデータは、最終的にバケットカラム値のHashとバケット数をモデル化することにより、バケットに分散されます。テーブルのバケット数が8であると仮定すると、8つのバケット`[0, 1, 2, 3, 4, 5, 6, 7]`があります。このようなシーケンスを`Buckets Sequence`と呼びます。各バケットには1つ以上のTabletがあります。テーブルが単一パーティションテーブルの場合、バケット内にはTabletが1つだけあります。マルチパーティションテーブルの場合は、複数のTabletがあります。
