---
{
  "title": "診断ツール",
  "language": "ja",
  "description": "効率的で効果的なパフォーマンス診断ツールは、データベースシステムチューニングにとって重要である。"
}
---
## 概要

効率的で効果的なパフォーマンス診断ツールは、データベースシステムのチューニングにおいて重要です。これらのツールは、問題のあるビジネスSQLクエリを迅速に特定できるかどうか、そしてその後、パフォーマンスのボトルネックを迅速に特定して解決できるかどうかを決定し、データベースシステムがService Level Agreements (SLAs)を満たすことを保証します。

現在、Dorisはデフォルトで実行時間が5秒を超えるSQLクエリをスロー SQLとして扱います。この閾値は`config.qe_slow_log_ms`で設定できます。Dorisは現在、パフォーマンスに問題のあるスロー SQLクエリを迅速に特定するために、以下の3つの診断チャネルを提供しています：

## Doris Manager Logs

Doris Managerのログモジュールは、スロー SQLフィルタリング機能を提供します。ユーザーは特定のFEノードで`fe.audit.log`を選択することで、スロー SQLを表示できます。検索ボックスに`slow_query`と入力するだけで、現在のシステムの履歴スロー SQL情報がページに表示されます。以下の図に示すとおりです：

![Doris Manager Monitoring and Logging](/images/doris-manage-trace-log-2.png)

## Audit Log

現在、Doris FEは`slow_query`、`query`、`load`、`stream_load`を含む4種類のAudit Logsを提供しています。Managerサービスがインストール・デプロイされているクラスターのログページでログにアクセスする以外に、Audit LogsはFEが配置されているノードの`fe/log/fe.audit.log`ファイルにアクセスすることで直接取得することもできます。

`fe.audit.log`内で`slow_query`タグを直接検索することで、実行の遅いSQLクエリを迅速にフィルタリングできます。以下に示すとおりです：
...
```
`fe.audit.log`を通じて取得されたスロークエリにより、ユーザーは実行時間、スキャンした行数、返された行数、およびSQL文自体などの詳細情報に簡単にアクセスでき、パフォーマンス問題のさらなる再現と特定のための基盤を築きます。

さらに、Audit Logには`SqlDigest`フィールド（例：上記の例の`SqlDigest=...`）が含まれています。このフィールドは、SQL文の構造から生成されたハッシュ値です（特定のパラメータ値は除去されます）。`slow_query`内の`SqlDigest`を集約・分析することで、スロークエリの「パターン」を特定できます。これは、特定のSQL文がパラメータによりわずかに異なっていても、構造が同じである限り、それらの`SqlDigest`は同一になることを意味します。

`SqlDigest`を使用することで、ユーザーはどのSQLパターンが最も頻繁に発生するか、または最も多くの時間を消費するかを判断でき、これらの「高頻度」または「高レイテンシ」パターンの最適化を優先することができます。このアプローチは、個々のSQL文を一つずつ分析する非効率性を回避し、スロークエリ最適化の効率を大幅に向上させます。

`SqlDigest`自体は単なるハッシュ値であり、直接読み取ることはできないことに注意することが重要です。最適化すべきスロークエリパターンが特定されたら、そのパターンに対応する具体的なSQL文の内容を取得するために、Audit Logの`Stmt`フィールドを参照する必要があります。さらに、`QueryId`フィールドを使用して、クエリの詳細なProfile情報を取得し（Profileの取得と分析は後続のセクションで詳述）、詳細なパフォーマンス分析と最適化を行うことができます。

### スロークエリ分析の例

上記の`fe.audit.log`内の4つのスロークエリログを例にとると、以下を観察できます：
...
これは、後者の3つのスロークエリが同じSQLパターン（構造的に同一）に属することを示しており、それらの具体的な実行時間（`Time(ms)`）と詳細はわずかに異なります。

実際の最適化シナリオでは、特定の`SqlDigest`（`fec5a7136f9375aa968a4de971b994da`など）がスロークエリログに繰り返し現れる、または総実行時間の高い割合を占めることがわかった場合、このパターンの最適化を優先すべきです。
...
2.  **詳細分析**: ログから`QueryId`（例：`6e1fae453cb04d9a-b1e5f94d9cea1885`）を使用してDorisで対応するQuery Profileを取得します。Profileは、問題が過度なデータスキャン、長時間のJoin操作、またはその他の理由によるものかを分析するのに役立ち、対象を絞った最適化戦略（インデックスの追加、SQL記述の最適化、またはテーブル構造の調整など）を可能にします。

このパターンの問題が解決されると、このパターンに属するすべてのスロークエリが改善されます。

## audit_logシステムテーブル

Dorisバージョン2.1以降、`__internal_schema`データベースの下でユーザーがSQLクエリの実行状況を確認できる`audit_log`システムテーブルが提供されています。使用する前に、グローバル設定`set global enable_audit_plugin=true`;を有効にする必要があります（このスイッチはデフォルトで無効です）。
...
+-------------------+--------------+------+-------+---------+-------+
```
`audit_log` 内部テーブルを通じて、ユーザーは詳細なSQL実行情報を照会し、スロークエリフィルタリングなどの詳細な統計分析を実行できます。

## 概要

Doris Managerログ、監査ログ、および `audit_log` システムテーブルは、スローSQLクエリの自動または手動フィルタリング、およびSQL実行情報のきめ細かい統計分析などの機能を提供します。これらのツールは、体系的なパフォーマンス診断とチューニングに対して強力なサポートを提供します。
