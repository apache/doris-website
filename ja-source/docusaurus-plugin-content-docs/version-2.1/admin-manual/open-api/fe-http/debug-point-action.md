---
{
  "title": "デバッグポイント",
  "language": "ja",
  "description": "デバッグポイントは、FEまたはBEコードに挿入されるコードの一部であり、プログラムがこのコードに到達すると、"
}
---
# Debug Point

Debug pointは、FEまたはBEコードに挿入されるコードの断片で、プログラムがこのコードを実行する際に、

プログラムの変数や動作を変更することができます。

主に、通常の手段では例外をトリガーすることが不可能な場合のunit testやregression testに使用されます。

各debug pointには名前があり、名前は任意に設定できます。debug pointを有効・無効にするスイッチがあり、
...
debug pointをグローバルに有効化するには、`enable_debug_points`をtrueに設定する必要があります。

`enable_debug_points`はFEのfe.confとBEのbe.confに配置されています。


## 指定されたDebug Pointのアクティベート

debug pointがグローバルに有効化された後、debug point名を含むhttpリクエストをFEまたはBEノードに送信する必要があります。<br/>
その後のみ、プログラムが指定されたdebug pointに到達した際に、関連するコードが実行されます。
...
* `timeout`
    タイムアウト（秒）。タイムアウト時、デバッグポイントは非アクティブになります。デフォルトは-1で、タイムアウトしません。オプション。

* `execute`
    アクティブ化後、デバッグポイントが実行できる最大回数。デフォルトは-1で、無制限回数。オプション。
...
debug point `foo`を有効化した後、最大5回まで実行されます。
...
```
## カスタムパラメータの渡し方
debug pointを有効化する際、上記で説明した「timeout」や「execute」に加えて、カスタムパラメータを渡すことも可能です。<br/>
パラメータは「key=value」形式のキーと値のペアで、URLパス内でdebug point名の後に'?'文字で結合されます。<br/>
以下の例を参照してください。
...
```
### 例
fe.conf で http_port=8030 の設定を持つ FE ノードを想定すると、<br/>
以下の http リクエストは FE ノードで `foo` という名前のデバッグポイントをアクティブにし、パラメータ `percent` と `duration` を渡します：
>注記: ユーザー名とパスワードが必要な場合があります。
...
```
### FEとBEコードでパラメータを使用する
以下のリクエストは、FE内のデバッグポイント`OlapTableSink.write_random_choose_sink`を有効化し、パラメータ`needCatchUp`と`sinkNum`を渡します：
...
```
FE のコードは、デバッグポイント `OlapTableSink.write_random_choose_sink` をチェックし、パラメータ値を取得します：
...
```
以下のリクエストは、BE内のデバッグポイント`TxnManager.prepare_txn.random_failed`をアクティブ化し、パラメータ`percent`を渡します：
...
```
## リグレッションテストにおけるデバッグポイント

>コミュニティのCIシステムでは、FEとBEの`enable_debug_points`設定はデフォルトでtrueになっています。

リグレッションテストフレームワークでは、特定のデバッグポイントを有効化および無効化するメソッドも提供されており、<br/>
これらは以下のように宣言されています：
...
`disableDebugPointForAllFEs()` または `disableDebugPointForAllBEs()` はその後に呼び出す必要があります。

### 並行処理の問題

有効化されたdebug pointsはFEまたはBEにグローバルに影響するため、プルリクエスト内の他の並行テストが予期せず失敗する可能性があります。<br/>
これを回避するために、debug pointsを使用するregression testsはディレクトリregression-test/suites/fault_injection_p0に配置する必要があり、<br/>
そのグループ名は "nonConcurrent" である必要があるという規約があります。これらのregression testsはプルリクエストワークフローによって順次実行されるためです。
