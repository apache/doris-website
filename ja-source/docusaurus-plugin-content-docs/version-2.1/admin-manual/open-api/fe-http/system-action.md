...
```
データ部分の `column_names` はヘッダー情報であり、`href_columns` はテーブル内のどの列がハイパーリンク列であるかを示します。`rows` 配列の各要素は一行を表します。その中で、`__hrefPaths` はテーブルデータではなく、ハイパーリンク列のリンクURLであり、`href_columns` 内の列と一対一で対応します。
