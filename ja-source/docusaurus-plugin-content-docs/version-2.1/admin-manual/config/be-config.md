---
{
  ...
  "description": "この文書では、主にBEの関連設定項目について紹介します。"
}
---
...
このドキュメントは主にBEの関連する設定項目について紹介します。

BE設定ファイル`be.conf`は通常、BEデプロイメントパスの`conf/`ディレクトリに保存されています。バージョン0.14では、別の設定ファイル`be_custom.conf`が導入されます。この設定ファイルは、ユーザーが運用中に動的に設定し、永続化した設定項目を記録するために使用されます。

BEプロセスが開始されると、まず`be.conf`の設定項目を読み込み、次に`be_custom.conf`の設定項目を読み込みます。`be_custom.conf`の設定項目は`be.conf`の同じ設定項目を上書きします。

`be_custom.conf`ファイルの場所は、`custom_config_dir`設定項目を通じて`be.conf`で設定できます。

## 設定項目の表示

ユーザーはBEのWebページにアクセスして、現在の設定項目を表示できます：
...
1. 静的設定
   ...
   `conf/be.conf`ファイルに設定項目を追加・設定します。`be.conf`の設定項目はBE開始時に読み込まれます。`be.conf`にない設定項目はデフォルト値を使用します。
...
2. 動的設定
   ...
   BE開始後、以下のコマンドで設定項目を動的に設定できます。
    ...
    ```
バージョン0.13以前では、この方法で変更された設定項目はBEプロセスの再起動後に無効になります。バージョン0.14以降では、以下のコマンドにより変更された設定を永続化できます。変更された設定項目は`be_custom.conf`ファイルに保存されます。
...
0610 11:23:03.729029 (+    66us) base_compaction.cpp:44] rowsetを選択
0610 11:24:51.784439 (+108055410us) compaction.cpp:46] 同時実行ロックを取得してcompactionを開始
...
0610 11:26:33.513197 (+ 28715us) compaction.cpp:110] rowsetの変更完了
0610 11:26:33.513300 (+   103us) base_compaction.cpp:49] compaction完了
0610 11:26:33.513441 (+   141us) base_compaction.cpp:56] 未使用のrowsetをGCキューに移動
