...
2. コマンドによる表示
    ...
    * Key: 設定項目の名前。
    * Value: 現在の設定項目の値。
    * タイプ: 設定項目の値タイプ（integerやstringなど）。
...
同じcluster idを持つnode（FEまたはBE）は、同じDorisクラスタに属するとみなされます。クラスター idは通常、マスターFEが最初に起動するときに生成されるランダムな整数です。独自に指定することもできます。
...
HTTP サーバー V2はSpringBootによって実装されます。フロントエンドとバックエンドを分離したアーキテクチャを使用します。HTTPv2が有効な場合のみ、ユーザーは新しいフロントエンドUIインターフェースを使用できます。
