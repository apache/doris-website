...
クエリレイテンシに敏感な場合、または高並行性で小さなクエリのシナリオがある場合は、*disable_storage_page_cache=false*を設定してpage cacheを再度有効にできる。
...
- 列タイプdatetimev2/decimalv3の様々な正確性および精度の問題を修正。

- Unique Key Merge-on-Readテーブルでのデータ正確性の問題を修正。

- Light Schema Change機能の様々な既知の問題を修正。

- bitmapタイプRuntime Filterの様々なデータ正確性の問題を修正。
...
- 浮動小数点タイプ読み込み時の精度損失の問題を修正。

- 2PC stream load使用時のメモリリークの問題を修正

# その他

BEの総rowsetとsegment数を表示するメトリクスを追加
