...
3. 個々のtabletのデータ量（**Tablet Count = パーティション Count \* バケット Count \* Replication Factor**）は理論的には上限・下限はありませんが、小規模テーブル（数百メガバイトから1ギガバイトの範囲）を除き、1GBから10GBの範囲内に収めることを確実にする必要があります：
