---
{
  "title": "ビットマップ",
  "language": "ja",
  "description": "BITMAP型はDuplicateテーブル、Uniqueテーブル、Aggregateテーブルで使用でき、Value型としてのみ使用可能で、Key型としては使用できません。"
}
---
BITMAP型は、Duplicate テーブル、Unique テーブル、および Aggregate テーブルで使用でき、Key 型としてではなく Value 型としてのみ使用できます。Aggregate テーブルで BITMAP 型を使用する場合、テーブルは集約型 BITMAP_UNION で作成する必要があります。ユーザーは長さとデフォルト値を指定する必要はありません。長さは、データ集約の度合いに基づいてシステムによって制御されます。詳細なドキュメントについては、[Bitmap](../../../sql-manual/basic-element/sql-data-types/aggregate/BITMAP) を参照してください。
...
以下では、stream loadを使用して複数要素を含むbitmapカラムをインポートする2つの方法を示します。ユーザーは、ソースファイルの形式に基づいて適切な方法を選択できます。

### bitmap_from_string

`bitmap_from_string`を使用する場合、ソースファイルのarrカラムに角括弧は使用できません。使用した場合、データ品質エラーとみなされます。
...
`bitmap_from_array`を使用する場合、ソースファイルのarr列に角括弧を含めることができます。ただし、stream loadでは、使用前にstring型を最初にarray型にキャストする必要があります。
