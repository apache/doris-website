---
{
  ...
  "language": "ja",
  "description": "この文書では、DorisでUPDATE コマンドを使用してデータを変更する方法について説明します。"
}
---
...
- 小規模データ更新：少量のデータを修正する必要があるシナリオに最適です。例えば、特定のレコードの誤ったフィールドを修正したり、特定のフィールドのステータスを更新したりする場合（例：注文ステータス更新）。

- 特定フィールのETLバッチ処理：特定のフィールドの大規模更新に適しており、ETL処理シナリオでよく見られます。注意：大規模データ更新は頻繁でないことが前提です。

## 動作原理

クエリエンジンは独自のフィルタリングロジックを使用して、更新が必要な行を特定します。次に、UniqueモデルのValueカラムロジックを使用して古いデータを新しいデータに置き換えます。更新対象の行は変更されてテーブルに再挿入され、行レベルの更新が実現されます。

### 同期

Dorisの`UPDATE`構文は同期的です。これは、`UPDATE`文が正常に実行されると、更新操作が完了し、データが即座に表示されることを意味します。

### パフォーマンス

`UPDATE`文のパフォーマンスは、更新対象の行数とクエリ条件の効率性に密接に関連しています。

- 更新対象の行数：更新が必要な行が多いほど、`UPDATE`文は遅くなります。小規模更新の場合、Dorisは`INSERT INTO`文と同様の頻度をサポートします。大規模更新の場合、実行時間が長いため、頻繁でない呼び出しにのみ適しています。

- クエリ条件の効率性：`UPDATE`の実装は、まずクエリ条件を満たす行を読み取ります。そのため、クエリ条件が効率的であれば、`UPDATE`の速度は高速になります。理想的には、条件カラムがインデックスまたはパーティションバケットプルーニングにヒットすることが望ましく、これによりDorisはテーブル全体をスキャンする必要がなく、更新が必要な行を素早く特定でき、更新効率が向上します。条件カラムにvalueカラムを含めないことを強く推奨します。
...
+----------------+---------+------------------+---------------------+--------------------+--------------------+--------------------+----------------------+--------------------------+---------------------+------------+
```
以下のリスクコントロールルールに従って、すべての日次取引記録のリスクレベルを更新してください：
1. 紛争履歴のある取引のリスクレベルはhighです。
2. 高リスク地域での取引のリスクレベルはhighです。
3. 異常な金額の取引（日次平均の5倍を超える）のリスクレベルはhighです。
4. 過去7日間の頻繁な取引：
  a. 取引回数が50回を超える場合、リスクレベルはhighです。
  b. 取引回数が20回から50回の場合、リスクレベルはmediumです。
5. 非営業時間（午前2時から午前4時）の取引のリスクレベルはmediumです。
