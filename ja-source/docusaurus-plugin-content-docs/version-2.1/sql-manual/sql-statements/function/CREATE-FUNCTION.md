---
{
  ...
  "language": "ja",
  "description": "この文はカスタム関数を作成するために使用されます。"
}
---
## 説明
...
> `function_name`に`db1.my_func`のようなデータベース名が含まれる場合、カスタム関数は対応するデータベースに作成されます。そうでない場合、関数は現在のセッションのデータベースに作成されます。新しい関数の名前とパラメータは、現在の名前空間の既存の関数と同一であってはいけません。同一の場合、作成は失敗します。
...
> 指定された場合、作成される関数はグローバルに有効です。

**2. `AGGREGATE`**

> 指定された場合、作成される関数は集約関数です。

**3. `TABLES`**

> 指定された場合、作成される関数はテーブル関数です。

**4. `ALIAS`**

> 指定された場合、作成される関数はエイリアス関数です。

> 関数型を表す上記のパラメータのいずれも選択されていない場合、作成される関数はスカラー関数であることを示します。
...
> - `type`: UDF呼び出し型を示します。デフォルトはNativeです。Java UDFを使用する場合はJAVA_UDFを使用します。
> - `always_nullable`: UDF結果にNULL値が含まれる可能性があるかどうかを示します。これはデフォルト値がtrueのオプションパラメータです。
