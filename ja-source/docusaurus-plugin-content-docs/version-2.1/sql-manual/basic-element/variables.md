---
{
  "title": "変数",
  "language": "ja",
  "description": "Dorisでは、変数はsystem変数とuser変数に分けられます。どちらも大文字と小文字を区別しません。"
}
---
## 説明

Dorisでは、変数はシステム変数とユーザー変数に分けられます。どちらも大文字小文字を区別しません。
...
- スコープ：
    ...
    - セッション変数（Session）：現在のセッションにのみ影響し、`SET`で設定します。
...
- アクセス方法：
    ...
    - `SHOW VARIABLES LIKE 'pattern'`を使用してパターンマッチングで特定の変数を表示します。
...
ユーザー定義変数は、セッション内でデータを一時的に保存するメカニズムです。主な特徴は以下の通りです：
...
- SET
    ...
    現在のセッションでのみ有効にするには`SET`を使用します。例：
