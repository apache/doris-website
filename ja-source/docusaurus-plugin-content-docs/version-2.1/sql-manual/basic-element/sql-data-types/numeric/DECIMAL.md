...
### 説明
    DECIMAL(P[,S])
    高精度固定小数点数。Pは有効桁数の合計数（精度）を表し、Sは小数点の右側にある小数部分の桁数を表します。
    有効桁数Pの範囲は[1, MAX_P]で、enable_decimal256=falseの場合はMAX_P=38、enable_decimal256=trueの場合はMAX_P=76です。
    ...
    decimal型を出力する際、末尾の桁が0であっても、小数点の後には常にS桁が表示されます。たとえば、decimal(18, 6)型の数値123.456は123.456000として出力されます。

### 精度推定

DECIMALには非常に複雑な型推論ルールセットがあります。異なる式に対して、精度推論に異なるルールが適用されます。

#### 算術演算

e1(p1, s1)とe2(p2, s2)が2つのDECIMAL数である場合、演算結果の精度推定ルールは以下の通りです：
...
|e1 / e2|p1 + s2 + `div_precision_increment`|s1 + `div_precision_increment`|MAX_P|<ol><li>precision - s1がmax_precision - `decimal_overflow_scale`より小さい場合: (max_precision - (precision - s1)) + `div_precision_increment`</li><li>precision - s1がmax_precision - `decimal_overflow_scale`より大きく、かつs1が`decimal_overflow_scale`より小さい場合: s1 + `div_precision_increment`</li><li>precision - s1がmax_precision - `decimal_overflow_scale`より大きく、かつs1が`decimal_overflow_scale`以上の場合: `decimal_overflow_scale` + `div_precision_increment`</li></ol>|pは結果に応じてキャスト、sは結果+e2.scaleに応じてキャスト||
|e1 % e2|max(p1 - s1,p2 - s2) + max(s1, s2)|max(s1, s2)|MAX_P|min(MAX_P, p) - max(p1 - s1,p2 - s2)|結果に応じてキャスト|結果に応じてキャスト|

表内の`Result scale if overflow`の計算ルールにおいて、`precision`は`Result precision`列の`precision`を指し、`scale`は`Result scale`列の`scale`を指します。

`div_precision_increment`はFEの設定パラメータです。詳細は[div_precision_increment](../../../../admin-manual/config/fe-config#div_precision_increment)を参照してください。
...
DECIMAL(p1, s1) / DECIMAL(p2, s2)は最初にDECIMAL(p1 + s2 + div_precision_increment, s1 + s2) / DECIMAL(p2, s2)に変換されてから計算が実行されます。したがって、DECIMAL(p1 + s2 + div_precision_increment, s1 + div_precision_increment)がDECIMALの範囲を満たしていても、DECIMAL(p1 + s2 + div_precision_increment, s1 + s2)への変換により範囲を超えた場合、DorisはデフォルトでArithmetic overflowエラーを報告します。
...
```
乗算結果の精度の計算ルールに従って、結果の型はdecimal(38, 18)となり、オーバーフローは発生しません：
...
```
乗算結果の精度の計算ルールに従って、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）の下では、通常の計算結果タイプは`decimal(41, 11)`です。精度がオーバーフローするため、オーバーフロールールに従って再計算する必要があります：`MAX_P` - `decimal_overflow_scale` = 38 - 6 = 32、`precision` - `scale` = 41 - 11 = 30 < 32、ルール1が適用され、最終結果のscale = min(11, 38 - 30) = 8、最終結果タイプはdecimal(38, 8)です：
...
+----------------------+-----------------------+--------------------------------------+
```
`decimal_overflow_scale`の値が増加した場合、例えば`set decimal_overflow_scale=9;`とすると、オーバーフロールールに従って計算が実行されます：`MAX_P` - `decimal_overflow_scale` = 38 - 9 = 29、`precision` - `scale` = 41 - 11 = 30 > 29、かつscale > `decimal_overflow_scale`であるため、オーバーフロールール3が適用され、最終的な計算結果の型は`decimal(38,9)`となります：
...
```
デフォルトでは（`enable_decimal256`=false）、乗算の実際の結果はオーバーフローします。この場合、decimal256を有効にすることができます：`set enable_decimal256=true`で正確な結果を計算し、結果の型はdecimal(76, 38)になります：
...
```
除算結果の精度の計算ルールに従って、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）では、通常の計算結果の型は`decimal(19 + 9 + 4, 9 + 4)`、すなわち`decimal(32, 13)`となり、精度はオーバーフローしません。結果の最終的な型は`decimal(32, 13)`です：
...
+----------------------+---------------------+-----------------+
```
結果により多くの小数点以下の桁数を保持したい場合は、`div_precision_increment`を増やすことができます。例えば、`admin set frontend config('div_precision_increment'='8');`とします。そうすると、上記の計算規則に従って、計算結果の型は`decimal(36, 17)`になります：
...
```
除算結果の精度の計算ルールによると、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）では、通常の計算結果タイプは`decimal(27 + 8 + 4, 8 + 4)`、つまり`decimal(39, 12)`です。精度がオーバーフローし、オーバーフロールールに従って再計算する必要があります：`MAX_P` - `decimal_overflow_scale` = 38 - 6 = 32、`precision` - s1 = 39 - 8 = 31 < 32なので、`Result scale if overflow`ルール1が適用され、結果のスケールは（`MAX_P` - （`precision` - `s1`））+ `div_precision_increment` = （38 - （39 - 8））+ 4 = 11となり、結果タイプは`decimal(38, 11)`です：
...
+-----------------------------+----------------------------+---------------+
```
`decimal_overflow_scale`の値が増加した場合、例えば`set decimal_overflow_scale=8;`とすると、計算はオーバーフロールールに従って実行されます：`MAX_P` - `decimal_overflow_scale` = 38 - 8 = 30、`precision` - s1 = 39 - 8 = 31 > 30、かつs1 == `decimal_overflow_scale`であるため、オーバーフロールール3が適用され、最終的に計算された結果の型は`decimalv3(38,12)`となります：
...
```
除算結果精度の計算ルールに従い、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）の下で、通常の計算結果型は`decimal(38 + 4 + 4, 4 + 4)`、つまり`decimal(46, 8)`です。精度がオーバーフローしたため、オーバーフロールールに従って再計算する必要があります：`MAX_P` - `decimal_overflow_scale` = 38 - 6 = 32、`precision` - s1 = 46 - 4 = 42 > 32、s1 = 4 < `decimal_overflow_scale`であるため、`Result scale if overflow`ルール2が適用され、結果スケールは`s1` + `div_precision_increment` = 4 + 4 = 8となり、結果型は`decimal(38, 8)`になります：
...
+-------------------------+------------------------+-----------------+
```
decimal256が有効な場合（`set enable_decimal256 = true;`）、結果の精度はオーバーフローなしで正常に計算され、結果の型は`decimal(46, 8)`になります：
...
```
除算結果の精度の計算ルールによると、デフォルト設定（`enable_decimal256`=false、`decimal_overflow_scale`=6、`div_precision_increment`=4）の下では、通常の計算結果型は`decimal(38 + 7 + 4, 7 + 4)`、つまり`decimal(49, 11)`です。精度がオーバーフローするため、オーバーフロールールに従って再計算する必要があります：`MAX_P` - `decimal_overflow_scale` = 38 - 6 = 32、`precision` - s1 = 49 - 7 = 42 > 32、s1 = 7 > `decimal_overflow_scale`のため、`Result scale if overflow`ルール3が適用され、結果のスケールは`decimal_overflow_scale` + `div_precision_increment` = 6 + 4 = 10となり、結果型は`decimal(38, 10)`になります：
...
上記で言及した式を除き、他の式は精度推論にデフォルトルールを使用します。つまり、式`expr(DECIMAL(a, b))`の場合、結果の型も DECIMAL(a, b) となります。

#### 結果精度の調整

DECIMALに対する精度要件はユーザーによって異なります。上記のルールはDorisのデフォルトの動作です。**ユーザーが異なる精度要件を持つ場合、以下の方法で精度を調整できます**：

* 期待される結果精度がデフォルト精度より大きい場合、パラメーターの精度を調整することで結果精度を調整できます。例えば、ユーザーが`AVG(col)`を計算してDECIMAL(x, y)を結果として得たい場合、`col`の型がDECIMAL (a, b)であれば、式を`AVG(CAST(col as DECIMAL (x, y))`に書き換えることができます。
* 期待される結果精度がデフォルト精度より小さい場合、出力結果を近似することで望ましい精度を得ることができます。例えば、ユーザーが`AVG(col)`を計算してDECIMAL(x, y)を結果として得たい場合、`col`の型がDECIMAL(a, b)であれば、式を`ROUND(AVG(col), y)`として書き換えることができます。

### DECIMALが必要な理由

DorisのDECIMALは真の高精度固定小数点数です。DECIMALには以下の主要な利点があります：
1. より広い範囲を表現できます。DECIMALにおける精度とスケールの両方の値範囲が大幅に拡張されました。
2. より高いパフォーマンス。DECIMALの旧バージョンはメモリで16バイト、ストレージで12バイトを必要としましたが、DECIMALは以下に示すような適応調整を行いました。
