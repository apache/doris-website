---
{
  "title": "ランク",
  "language": "ja",
  "description": "RANK()は、順序付けされたデータセット内の値のランクを返すwindow関数です。ランキングは1から始まり、順次増加します。"
}
---
## 説明
...
+------------+---------------+--------+-------------+
```
この例では、データは部門によって分割され、各部門内で給与によってランク付けされています。同じ給与が発生した場合（AliceとBob、EveとFrankのように）、それらは同じランクを受け取りますが、これにより後続のランキングにギャップが生まれます。
