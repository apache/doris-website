---
{
  ...
  "language": "ja",
  "description": "STR文字列の正規表現マッチングを行い、パターンにヒットした部分を新しい文字列に置換します。"
}
---
## 説明

STR文字列の正規表現マッチングを行い、patternにヒットした部分を新しい文字列で置き換えます。
...
| `<pattern>` | このパラメータはVarchar型です。文字列のマッチングに使用される正規表現パターンです。パターンには様々な正規表現メタ文字や構造を含めて、複雑なマッチングルールを定義できます。|
| `<repl>` | このパラメータはVarchar型です。<pattern>にマッチする<str>の部分を置換する文字列です。パターン内でキャプチャしたグループを参照したい場合は、\1、\2などの後方参照を使用できます。ここで\1は最初のキャプチャグループを、\2は2番目のキャプチャグループを参照します。|
...
+---------------------------------------------------------------------------------------------+
```
テストケースを挿入してテストします。このテストケースのセットでは、元の文字列、パターン、および置換文字列を格納するテーブルを作成します。次に、様々なテストデータを挿入し、対応するパターンと置換文字列を使用して元の文字列に対してREGEXP_REPLACE操作を実行します。最後に、置換された文字列を取得して表示します。
