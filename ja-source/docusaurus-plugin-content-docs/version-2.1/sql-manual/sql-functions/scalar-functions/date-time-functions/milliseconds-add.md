...
| パラメータ | 説明                                      |
|-----------|--------------------------------------------------|
| `<basetime>`  | 入力日時値、DATETIMEV2型    |
| `<delta>`     | 追加するミリ秒数、INT型；1秒 = 1,000ミリ秒 = 1,000,000マイクロ秒 |

## 戻り値

DATETIMEV2型の値を返します。入力日時に指定されたミリ秒数を追加した後の時刻値を表します。戻り値の精度は入力パラメータbasetimeと同じです。
...
```
**注意:**
- この例では、1ミリ秒を追加した後、時刻は.435123から.436123に増加します。
- 1ミリ秒は1000マイクロ秒と等しくなります。
