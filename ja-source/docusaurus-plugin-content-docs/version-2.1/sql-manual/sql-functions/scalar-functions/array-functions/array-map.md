---
{
  "title": "ARRAY_MAP",
  "language": "ja",
  "description": "他の入力ARRAY パラメータの内部データに対して対応する式を計算するための入力パラメータとしてlambda式を使用します。"
}
---
## 説明

他の入力ARRAY パラメータの内部データに対して対応する式を計算するために、入力パラメータとしてlambda式を使用します。
lambda式に入力されるパラメータの数は1つ以上で、入力配列の列数と一致している必要があります。
lambdaではスカラー関数を実行でき、集計関数はサポートされていません。
...
| パラメータ | 説明 |
| --- |---|
| `lambda` | 1つまたは複数の入力パラメータを持つlambda式で、入力配列の数と一致する必要があります。lambdaは有効なスカラー関数を実行できますが、集約関数はサポートしていません。 |
| `<arr>` | ARRAY配列 |
