---
{
  ...
  "language": "ja",
  "description": "この関数は、INET6ATON()で返されるバイナリ文字列として数値形式で表現されたIPv6アドレスを受け取ります。"
}
---
## 説明
この関数は、INET6_ATON()によって返される、バイナリ文字列として数値形式で表現されたIPv6アドレスを受け取ります。INET6_ATONはIPV6_STRING_TO_NUM_OR_NULLとも呼ばれます。
- IPv4マップドアドレスは`::ffff:ipv4_address`の形式になります。
...
## 戻り値
引数が有効なIPv4マップドIPv6アドレスの場合は1を、そうでなければ0を返します。
