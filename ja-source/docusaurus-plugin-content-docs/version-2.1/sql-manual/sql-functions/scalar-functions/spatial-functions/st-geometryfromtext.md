...
- `POLYGON` - 1つ以上のリングで定義された閉じた領域。最低3つの異なる点と閉じた端点が必要。
- `MULTIPOLYGON` - ポリゴンの集合。マルチポリゴン内のポリゴンは離散点のみを共有可能。

:::info Note
Apache Doris 2.1.10以降、MULTIPOLYGONフォーマットの解析をサポート
:::
...
入力WKT形式が仕様に準拠していない場合、または入力がNULLの場合はNULLを返します。
