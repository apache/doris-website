---
{
  "title": "九尾",
  "language": "ja",
  "description": "Apache Kyuubiは、データウェアハウスとレイクハウスでサーバーレスSQLを提供する分散マルチテナントゲートウェイです。"
}
---
# Kyuubi と Doris の使用
...
[Apache Kyuubi](https://kyuubi.apache.org/) は、データウェアハウスとレイクハウス上でサーバーレスSQLを提供する分散型マルチテナントゲートウェイです。
Apache Kyuubi は、Spark、Flink、Hive、JDBC などのエンジンに対して、Thrift、Trino、MySQL などの様々なプロトコルを提供しています。
Doris は、Apache Kyuubi でサポートされている Doris ダイアレクトを使用して JDBC データソースとして接続できます。
Apache Kyuubi は、HA、サービスディスカバリー、統合認証、エンジンライフサイクル管理など、一連の有用な機能も提供します。

## 使用方法

### Apache Kyuubi のダウンロード

<https://kyuubi.apache.org/zh/releases.html> から Apache Kyuubi をダウンロードします

Apache Kyuubi 1.6.0 以上を取得し、フォルダに展開します。

### Doris を Kyuubi データソースとして設定

- `$KYUUBI_HOME/conf/kyuubi-defaults.conf` で Kyuubi の設定を更新します
...
以下の例は、ThriftプロトコルでbelineCLIを使用してKyuubi経由でDorisをクエリする基本的な例を示しています。
