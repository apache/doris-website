---
{
  "title": "Logstash",
  "language": "ja",
  "description": "Logstashは、ログETLフレームワーク（収集、前処理、ストレージシステムへの送信）であり、ストレージシステムにデータを書き込むためのカスタムoutputプラグインをサポートしています。"
}
---
...
Logstashは、ストレージシステムにデータを書き込むためのカスタム出力プラグインをサポートするログETLフレームワーク（収集、前処理、ストレージシステムへの送信）です。Logstash Doris output pluginは、Dorisにデータを出力するためのプラグインです。

Logstash Doris output pluginは、[Doris Stream Load](../../data-operate/import/import-way/stream-load-manual) HTTPインターフェースを呼び出してDorisにリアルタイムでデータを書き込み、マルチスレッド並行処理、失敗時の再試行、カスタムStream Loadフォーマットとパラメータ、出力書き込み速度などの機能を提供します。
...
```
標準インストールモードは、プラグインが依存するrubyモジュールを自動的にインストールします。ネットワークが利用できない場合、処理が停止して完了できなくなります。このような場合、完全にオフラインでインストールするために依存関係を含むzipインストールパッケージをダウンロードできます。この際、ローカルファイルシステムを指定するために`file://`を使用する必要があることに注意してください。
...
`headers` | Doris Stream Load headersパラメータ、構文形式はruby mapです、例：headers => { "format" => "json", "read_json_by_line" => "true" }
...
`max_retries` | 失敗時のDoris Stream Load リクエストのリトライ回数、デフォルトは-1で無限リトライによりデータの信頼性を確保
`log_request` | トラブルシューティングのためにDoris Stream Load リクエストとレスポンスメタデータをログに出力するかどうか、デフォルトはfalse
`log_speed_interval` | ログでの速度出力の時間間隔、単位は秒、デフォルトは10、0に設定することでこのタイプのログを無効にできます
...
FEログファイルは通常、Dorisインストールディレクトリ下のfe/log/fe.logファイルに配置されます。これらは典型的なJavaプログラムログで、タイムスタンプ、ログレベル、スレッド名、コード位置、ログ内容などのフィールドが含まれます。通常のログだけでなく、スタックトレースを含む例外ログも含まれており、これらは複数行にわたります。ログ収集と保存では、メインログとスタックトレースを単一のログエントリに結合する必要があります。
...
Logstashシステム全体の設定ファイルは通常config/logstash.ymlに配置されています。Dorisへの書き込み時のパフォーマンスを向上させるには、バッチサイズとバッチ遅延を変更する必要があります。1行あたり平均数百バイトのログの場合、バッチサイズは1,000,000行、バッチ遅延は10秒が推奨されます。

``` 
pipeline.batch.size: 1000000
pipeline.batch.delay: 10000
```
特定のログ収集のための設定ファイル（logstash_doris_log.confなど）は、主にETLの各段階に対応する3つの部分で構成されています：
...
GitHub イベントアーカイブには、JSON 形式でフォーマットされた GitHub ユーザーアクションのアーカイブされたデータが含まれています。[ここ](https://data.gharchive.org/)からダウンロードできます。例えば、2024年1月1日午後3時のデータです。

```shell
wget https://data.gharchive.org/2024-01-01-15.json.gz
```
以下はデータのサンプルです。通常、各データは1行に記載されていますが、表示を見やすくするため、ここでは整形して表示しています。
