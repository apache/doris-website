---
{
  "title": "FluentBit",
  "language": "ja",
  "description": "Fluent Bitは、データをストレージシステムに書き込むためのカスタム出力プラグインをサポートする高速ログプロセッサおよびフォワーダです。"
}
---
[Fluent Bit](https://fluentbit.io/)は高速なログプロセッサおよびフォワーダーで、ストレージシステムにデータを書き込むためのカスタム出力プラグインをサポートしています。Fluent Bit Doris出力プラグインはDorisに出力するためのプラグインです。

Fluent Bit Doris出力プラグインは、[Doris Stream Load](../../data-operate/import/import-way/stream-load-manual) HTTP インターフェースを呼び出すことで、リアルタイムでDorisにデータを書き込みます。マルチスレッド並行処理、失敗時のリトライ、カスタムStream Load形式とパラメータ、および出力書き込み速度などの機能を提供します。
...
3. Fluent Bitを起動してDorisにリアルタイムでデータを書き込む。
...
https://github.com/joker-star-l/fluent-bitのdevブランチをクローンし、build/ディレクトリで以下のコマンドを実行します
...
`log_request` | トラブルシューティングのために Doris Stream Load リクエストとレスポンスのメタデータをログに出力するかどうか、デフォルトは true です
...
FE ログファイルは通常、Doris インストールディレクトリ下の fe/log/fe.log ファイルに配置されます。これらは典型的な Java プログラムログで、タイムスタンプ、ログレベル、スレッド名、コードの場所、ログ内容などのフィールドを含みます。通常のログだけでなく、スタックトレースを含む例外ログも含まれており、これらは複数行にわたります。ログの収集と保存では、メインログとスタックトレースを単一のログエントリに結合する必要があります。
...
この例では、GitHub イベントアーカイブからのデータを使用した JSON ログ収集を実演します。

**1. データ**

GitHub イベントアーカイブには、JSON として形式化された GitHub ユーザーアクションのアーカイブデータが含まれています。[ここ](https://data.gharchive.org/)からダウンロードできます。例えば、2024年1月1日午後3時のデータです。
...
以前のTEXTログ収集とは対照的に、この設定では追加の処理変換が不要であるためFILTERを使用しません。
