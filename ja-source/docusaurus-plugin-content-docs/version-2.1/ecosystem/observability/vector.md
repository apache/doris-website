...
Vector は Rust で書かれた高性能な可観測性データパイプラインで、ログ、メトリクス、トレースの収集、変換、ルーティング用に特別に設計されています。Doris エコシステムをより良くサポートするため、Vector 専用の Doris Sink コンポーネントを開発し、様々なデータソースから Doris への効率的なデータ取り込みを可能にして分析を行えるようにしました。
...
Doris Sinkは、異なるシナリオでのデータ書き込み要件を満たすため、豊富な設定オプションをサポートしています：
...
| `inputs`      | array       | -        | アップストリームデータソース名のリスト |
...
| `auth.password` | string | -         | Dorisパスワード、環境変数やシークレット管理システムと併用可能 |

### リクエストと並行性設定

| パラメータ | 型 | デフォルト | 説明 |
|--------|------|------|-----|
| `request.concurrency` | string/integer | `"adaptive"` | 並行性戦略を制御、`"adaptive"`、`"none"`（シリアル）、または正の整数による並行性制限をサポート |
| `request.timeout_secs` | integer | `60` | 単一Stream Loadリクエストのタイムアウト（秒） |
| `request.rate_limit_duration_secs` | integer | `1` | レート制限時間ウィンドウ（秒） |
| `request.rate_limit_num` | integer | `i64::MAX` | 時間ウィンドウあたりの許可リクエスト数、デフォルトは実質無制限 |
| `request.retry_attempts` | integer | `usize::MAX` | Towerミドルウェアの最大再試行回数、デフォルトは無制限再試行 |
| `request.retry_initial_backoff_secs` | integer | `1` | 初回再試行前の待機時間（秒）、後続の再試行はフィボナッチバックオフを使用 |
| `request.retry_max_duration_secs` | integer | `30` | 単一再試行バックオフの最大待機時間（秒） |
...
| `request.adaptive_concurrency.max_concurrency_limit` | integer | `200` | 適応的並行性の上限、過負荷を防止 |
| `request.adaptive_concurrency.decrease_ratio` | float | `0.9` | 減速トリガー時に使用される削減比率 |
| `request.adaptive_concurrency.ewma_alpha` | float | `0.4` | RTTメトリクスの指数移動平均重み |
| `request.adaptive_concurrency.rtt_deviation_scale` | float | `2.5` | RTT偏差拡大係数、通常の変動を無視するために使用 |

### エンコーディングとデータ形式

Doris Sinkは`encoding`ブロックを使用してイベントシリアライゼーション動作を制御し、デフォルトでNDJSON（改行区切りJSON）を使用します：

| パラメータ | 型 | デフォルト | 説明 |
|--------|------|------|-----|
| `encoding.codec` | string | `"json"` | シリアライゼーションエンコーディング、`json`、`text`、`csv`などのオプション |
...
| `encoding.framing.method` | string | 自動推定 | カスタムフレーミング形式が必要な場合に設定、例：`newline_delimited`、`character_delimited` |

#### Stream Loadヘッダー（`headers`）

`headers`は、Doris Stream LoadのHTTPヘッダーとして直接渡されるキー値ペアマッピングです。Stream Loadヘッダーで利用可能なすべてのパラメータを使用できます。
一般的な設定は以下のとおりです（すべての値は文字列である必要があります）：

| パラメータ | 型 | デフォルト | 説明 |
|--------|------|------|-----|
| `headers.format` | string | `"json"` | データ形式、`json`、`csv`、`parquet`などをサポート |
| `headers.read_json_by_line` | string | `"true"` | JSONを行ごとに読み取るかどうか（NDJSON） |
| `headers.strip_outer_array` | string | `"false"` | 最外側の配列を削除するかどうか |
| `headers.column_separator` | string | - | CSV列区切り文字（`format = csv`の場合有効） |
...
| パラメータ | 型      | デフォルト     | 説明                                         |
|--------|---------|---------|--------------------------------------------|
| `max_retries` | integer | `-1`    | Sinkレベルでの最大再試行回数、`-1`は無制限                  |
...
| `tls.verify_certificate` | boolean | `true`  | アップストリーム証明書検証の有効化/無効化                                |
| `tls.verify_hostname` | boolean | `true`  | ホスト名検証の有効化/無効化                                 |
| `tls.ca_file` / `tls.crt_file` / `tls.key_file` / `tls.key_pass` / `tls.alpn_protocols` / `tls.server_name` | 各種      | -       | カスタムCA、相互認証、またはSNI用の標準Vector TLSクライアント設定オプション    |
| `acknowledgements.enabled` | boolean | `false` | acknowledgementをサポートするSourceで使用するエンドツーエンドacknowledgementを有効化 |

## 使用例

### TEXTログ収集例

この例では、Doris FEログを例としてTEXTログ収集を示します。

**1. データ**

FEログファイルは通常、Dorisインストールディレクトリ下のfe/log/fe.logに配置されます。これは、タイムスタンプ、ログレベル、スレッド名、コード位置、ログメッセージなどのフィールドを含む典型的なJavaアプリケーションログです。通常のログに加えて、複数行にまたがるスタックトレースを持つ例外ログがあります。ログ収集と保存では、メインログとスタックトレースを単一のログエントリに結合する必要があります。
...
この例では、GitHub イベントアーカイブデータを使用した JSON ログ収集を実演します。

**1. データ**

GitHub イベントアーカイブには、GitHub ユーザー操作イベントのアーカイブデータが JSON 形式で含まれています。https://www.gharchive.org/ からダウンロードできます。例えば、2024年1月1日の15:00のデータをダウンロードします。

```
wget https://data.gharchive.org/2024-01-01-15.json.gz

```
以下はサンプルのデータエントリです。実際のデータは1行につき1エントリとなっており、ここでは表示目的で書式設定が追加されています。
