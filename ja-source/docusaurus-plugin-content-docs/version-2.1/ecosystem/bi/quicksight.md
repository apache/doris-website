---
{
  "title": "QuickSight",
  "language": "ja",
  "description": "QuickSightは、直接クエリまたはインポートモードで公式MySQLデータソース経由でApache Dorisに接続できます"
}
---
QuickSightは、直接クエリまたはインポートモードで公式のMySQLデータソース経由でApache Dorisに接続できます
...
- 宣言されたMySQL互換性バージョンを調整するために、Dorisに接続するMySQLクライアントで以下のSQLを実行してください：
...
Doris のマルチテーブル JOIN シナリオでの優れたパフォーマンスにより、このシナリオに基づいてダッシュボードを設計することにしました。異なるステータスでの異なる国の注文統計を知る必要があるとします。この要件に応じてダッシュボードを構築します。
...
5. ダッシュボード作成のワークベンチに入り、現在の Dataset ドロップダウンメニューをクリックして、Add New Dataset を選択します。
...
7. 完了後、nation の操作インターフェースをクリックして、データセット編集インターフェースに入ります。データセットで列の結合を実行します。
...
9. 3つのテーブルを追加した後、結合を実行します。結合関係は次のとおりです：
...
12. VALUE をクリックして o_orderkey を選択し、GROUP/COLOR をクリックして o_orderstatus を選択し、要件のダッシュボードを取得します。

![](/images/ecomsystem/quicksight/Sl8nbfrszok2bexesfwcsNqbngc.png)

13. 右上の Publish をクリックして、ダッシュボードの公開を完了します。
