...
[DataX](https://github.com/alibaba/DataX) Doriswriterプラグインは、MySQL、Oracle、SQL サーバーなどの様々なデータソースから、Stream Load方式を使用してDorisにデータを同期することをサポートします。

:::info 注意
このプラグインはDataXサービスと組み合わせて使用する必要があります。
DataXは複数のデータソースをサポートしています。詳細については、こちらを参照してください。
:::
...
DataXは、すでにDataXが含まれている公式のインストールパッケージを提供しており、直接ダウンロードして使用できます。詳細については、[こちら](https://github.com/alibaba/DataX?tab=readme-ov-file#download-datax%E4%B8%8B%E8%BD%BD%E5%9C%B0%E5%9D%80)を参照してください。
...
以下の解決方法をお試しください：
        ...
        2. 展開後、結果として得られる `alibaba/datax/` ディレクトリを、使用している maven に対応する `.m2/repository/com/alibaba/` にコピーし、再度コンパイルを試してください。

### Datax DorisWriter パラメータ紹介：

* **jdbcUrl**

  - 説明: Doris の JDBC 接続文字列で、ユーザーは preSql または postSQL を実行します。
  - 必須: はい
  - デフォルト: なし
* **loadUrl**

  - 説明: Stream Load の接続対象として使用します。形式は「ip:port」です。ここで IP は FE ノードの IP、port は FE ノードの http_port です。複数入力可能で、英語のカンマ `,` で区切ります。doriswriter はポーリング方式でアクセスします。
  - 必須: はい
  - デフォルト: なし
* **username**
...
* **connection.selectedDatabase**
  - 説明: 書き込みが必要な Doris データベースの名前。
  - 必須: はい
  - デフォルト: なし
* **connection. table**
  - 説明: 書き込みが必要な Doris テーブルの名前。
    - 必須: はい
    - デフォルト: なし
* **flushInterval**
  - 説明: データがバッチで書き込まれる時間間隔。この時間間隔を小さく設定しすぎると、Doris の書き込みブロッキング問題（エラーコード -235）が発生します。この時間間隔を小さく設定し、`maxBatchRows` と `batchSize` パラメータを大きく設定した場合、設定したデータサイズに達しなくてもインポートが実行される可能性があります。
  - 必須: いいえ
  - デフォルト: 30000 (ms)
* **column**
  - 説明: 宛先テーブルがデータを書き込む必要があるフィールド。これらのフィールドは生成される Json データのフィールド名として使用されます。フィールドはカンマで区切られます。例："column": ["id","name","age"]。
  - 必須: はい
  - デフォルト: なし
* **preSql**
...
* **maxBatchRows**
  - 説明: インポートされる各バッチデータの最大行数。**batchSize** と合わせて、バッチごとのインポートレコード行数を制御します。各バッチデータが2つの閾値のいずれかに達すると、このバッチのデータのインポートが開始されます。
  - 必須: いいえ
  - デフォルト: 500000

* **batchSize**
  - 説明: 各バッチでインポートされるデータの最大量。**maxBatchRows** と連携してバッチごとのインポート数を制御します。各バッチデータが2つの閾値のいずれかに達すると、このバッチのデータのインポートが開始されます。
  - 必須: いいえ
  - デフォルト: 94371840
...
* **labelPrefix**
  ...
  - 説明: 各バッチのインポートタスクのラベルプレフィックス。最終的なラベルは `labelPrefix + UUID` で形成され、グローバルに一意なラベルとなり、データが重複してインポートされないことを保証します
  - 必須: いいえ
  - デフォルト: `datax_doris_writer_`

* **loadProps**

  - 説明: StreamLoad のリクエストパラメータ。詳細については、StreamLoad 紹介ページを参照してください。[Stream load - Apache Doris](https://doris.apache.org/docs/data-operate/import/stream-load-manual)

    これには、インポートされるデータフォーマット：format などが含まれます。インポートデータフォーマットのデフォルトは csv で、JSON をサポートしています。詳細については、以下の型変換セクションを参照するか、上記の Stream load の公式情報を参照してください。
...
#### 1. Stream がデータを読み取り、Doris にインポート

doriswriter プラグインの使用方法については、[こちら](https://github.com/apache/doris/blob/master/extension/DataX/doriswriter/doc/doriswriter.md)を参照してください。

#### 2.Mysql がデータを読み取り、Doris にインポート
...
>3. strip_outer_array : インポートデータのバッチ内の複数行データを表します。Dorisは解析時に配列を展開し、その後各Objectを順番に1行のデータとして解析します。
...
>```
>
>**CSV形式では、データ内の特殊文字との競合を避けるために行と列の区切り文字に特別な注意を払う必要があります。ここでは非表示文字を推奨します。デフォルトの列区切り文字は: \t、行区切り文字: \n**
