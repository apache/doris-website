---
{
  "title": "カラムから行へ（側面図）",
  "language": "ja",
  "description": "EXPLODEなどのgenerator関数と組み合わせて使用し、1つまたは複数の行を含む仮想テーブルを生成します。"
}
---
# Column to Row (Lateral View)

`EXPLODE`などのジェネレータ関数と組み合わせて使用し、1つ以上の行を含む仮想テーブルを生成します。`LATERAL VIEW`は各生入力行に対して行を適用します。
