...
Hive以外にも、多くのシステムがメタデータの保存にHive Metastoreを使用しています。そのため、Hive カタログを通じて、Hiveテーブルだけでなく、IcebergやHudiなどのHive Metastoreをメタデータストレージに使用する他のテーブル形式にもアクセスできます。

## 適用シナリオ

| シナリオ     | 説明                                                  |
|--------------|--------------------------------------------------------------|
| クエリ加速 | DorisのDistributedコンピューティングエンジンを使用してHiveデータに直接アクセスし、クエリを高速化します。 |
| Data 統合   | Hiveデータを読み取りDoris内部テーブルに書き込むか、Dorisコンピューティングエンジンを使用してZeroETL操作を実行します。 |
| データ書き戻し   | Dorisがサポートする任意のソースからデータを処理し、Hiveテーブルに書き戻します。 |
