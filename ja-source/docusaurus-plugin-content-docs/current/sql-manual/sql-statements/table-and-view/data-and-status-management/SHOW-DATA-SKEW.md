...
>
> このパラメータが指定されない場合、テーブル内のすべてのパーティションのデータ分布が表示されます。
>
> パーティション分割されていないテーブルの場合、パーティション名はテーブル名と同じです。
...
| Graph            | データ分布の可視化チャート            |
| Percent          | このバケットのデータ量が総データ量に占める割合 |
...
- データ分布は、パーティションとバケットの2つの次元に沿って表示されます。
- Graphカラムは、文字 `>` を使用してデータ分布比率を視覚的に表現します。
- パーセンテージは小数点以下2桁まで正確です。
- パーティション分割されていないテーブルの場合、クエリ結果のパーティション名はテーブル名と同じです。

## 例

- パーティション分割されたテーブルを作成し、そのデータ分布を表示する：
