---
{
  "title": "ファイルを作成",
  "language": "ja",
  "description": "このステートメントは、Dorisクラスタにファイルを作成してアップロードするために使用されます。"
}
---
## 説明

このステートメントはDorisクラスタにファイルを作成およびアップロードするために使用されます。
...
> - **catalog**: 必須。ファイル分類のためのカテゴリ名（ユーザー定義）。特定のコマンドでファイルを特定する際に使用されます（例：スケジュールされたインポートでKafkaがデータソースの場合、'kafka'カタログ下のファイルを検索）。
...
> **数量制限**: Dorisクラスターは最大100ファイルのアップロードをサポートします

## 例

- kafkaとして分類されたファイルca.pemを作成
