...
+ FROM additional_tables: 更新する行の選択や新しい値の設定に使用する1つ以上のテーブルを指定。ここでターゲットテーブルを使用したい場合は、明示的にエイリアスを指定する必要があります。
...
1. 'test'テーブルでk1 =1、k2 =2の条件を満たすv1カラムを1に更新

```sql
UPDATE test SET v1 = 1 WHERE k1=1 and k2=2;
```
2. 'test'テーブルのk1=1列のv1列を1つ増加させる
