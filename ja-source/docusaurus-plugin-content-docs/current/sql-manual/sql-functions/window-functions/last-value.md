---
{
  "title": "LAST_VALUE",
  "language": "ja",
  "description": "LASTVALUE()は、ウィンドウフレーム内の最後の値を返すwindow関数です。"
}
---
## 説明
...
| ------------------- | ----------------------------------------------------------------------------------------------------------- |
| expr                | 最後の値を取得する式、サポート対象: tinyint/smallint/int/bigint/float/double/decimal/string/date/datetime/array/struct/map/bitmap                                                             |
| ignore_null         | オプションのBoolean型。設定された場合、null値は無視され、最後のnull以外の値が返される                              |
