---
{
  ...
  "language": "ja",
  "description": "explodebitmap テーブル関数は bitmap 型データを受け取り、bitmap 内の各ビットを個別の行にマップします。"
}
---
## 説明
`explode_bitmap`テーブル関数はbitmap型データを受け取り、bitmap内の各ビットを個別の行にマッピングします。
bitmap データを処理するために一般的に使用され、bitmap内の各要素を個別のレコードに展開します。[`LATERAL VIEW`](../../../query-data/lateral-view.md)と組み合わせて使用する必要があります。
