...
---
## 説明
`explode_json_array_double_outer`テーブル関数はJSON配列を受け取ります。その実装ロジックは、JSON配列を配列型に変換してから、処理のために`explode_outer`関数を呼び出すことです。この動作は次と同等です：`explode_outer(cast(<json_array> as Array<DOUBLE>))`。
[`LATERAL VIEW`](../../../query-data/lateral-view.md)と一緒に使用する必要があります。
...
## 戻り値
- `<json>`内のすべての要素で構成される、単一列、複数行の結果を返します。列の型は`Nullable<DOUBLE>`です。
- `<json>`がNULLまたは空の配列（要素数が0）の場合、NULLを含む1行が返されます。
- JSON配列内の要素がDOUBLE型でない場合、関数はそれらをDOUBLEに変換しようとします。DOUBLEに変換できない要素はNULLに変換されます。型変換規則については、[JSON型変換](../../basic-element/sql-data-types/conversion/json-conversion.md)を参照してください。
