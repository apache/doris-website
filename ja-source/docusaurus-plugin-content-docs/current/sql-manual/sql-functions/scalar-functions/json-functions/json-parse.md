...
---
## 説明
生のJSON文字列をJSONバイナリ形式に解析します。異なる例外データ処理要件を満たすため、以下のようにJSON_PARSEシリーズの異なる関数が提供されています：
...
## パラメータ
### 必須パラメータ
- `<json_str>` String型、内容は有効なJSON文字列である必要があります。
### オプションパラメータ
...
2. `JSONB_PARSE`/`JSONB_PARSE_ERROR_TO_NULL`/`JSONB_PARSE_ERROR_TO_VALUE`は基本的に同じ動作をしますが、解析が失敗した場合に得られる結果が異なります。
