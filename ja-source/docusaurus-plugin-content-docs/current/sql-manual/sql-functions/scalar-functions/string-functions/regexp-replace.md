---
{
  ...
  "language": "ja",
  "description": "STR文字列の正規表現マッチングを行い、パターンにヒットした部分を新しい文字列で置換する。"
}
---
## 説明

STR文字列の正規表現マッチングを行い、パターンにヒットした部分を新しい文字列で置き換えます。

文字セットマッチングを処理する際は、Utf-8標準の文字クラスを使用する必要があることに注意してください。これにより、関数が異なる言語の様々な文字を正しく識別し、処理できることが保証されます。
...
| パラメータ | 説明 |
| -- | -- |
| `<str>` | このパラメータはVarchar型です。正規表現マッチングが実行される文字列を表します。リテラル文字列または文字列値を含むテーブルのカラムを指定できます。|
| `<pattern>` | このパラメータはVarchar型です。文字列にマッチさせるために使用される正規表現パターンです。パターンには様々な正規表現メタ文字と構文を含めて、複雑なマッチングルールを定義できます。|
| `<repl>` | このパラメータはVarchar型です。<pattern>にマッチする<str>の部分を置き換える文字列です。パターン内でキャプチャされたグループを参照したい場合は、\1、\2などの後方参照を使用できます。\1は最初のキャプチャグループ、\2は2番目のキャプチャグループを参照します。|

## 戻り値

この関数は置換操作後の結果文字列を返します。戻り値はVarchar型です。<str>の一部も<pattern>にマッチしない場合は、元の<str>が返されます。

## 例

基本的な置換の例。この例では、文字列 'a b c' 内のすべてのスペースがハイフンに置き換えられます。
...
+-----------------------------------+
```
キャプチャグループの使用。ここでは、パターン内のグループ(b)によって文字'b'がキャプチャされ、置換文字列内の後方参照\1を使用して<b>に置換されます。
...
+----------------------------------------+
```
中国語文字のマッチング。この例では、文字列内のすべての連続する中国語文字を'123'に置き換えます。
...
+---------------------------------------------------------------------------------------------+
```
挿入とテストケース。このテストケースセットでは、元の文字列、パターン、置換文字列を格納するテーブルを作成します。その後、様々なテストデータを挿入し、対応するパターンと置換文字列を使用して元の文字列にREGEXP_REPLACE操作を実行します。最後に、置換された文字列を取得して表示します。
