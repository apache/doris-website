...
| `<field>` | 区切り文字の出現回数。正の値は左から、負の値は右から数え、0の場合は空文字列を返す。型: INT |

## 戻り値

VARCHAR型を返し、抽出されたサブストリングを表します。

抽出ルール:
- field > 0の場合: 左からfield番目の区切り文字より前のコンテンツを返す
- field < 0の場合: 右から|field|番目の区切り文字より後のコンテンツを返す
- field = 0の場合: 空文字列を返す（contentがNULLの場合はNULLを返す）
- 大文字小文字を区別した完全一致の区切り文字マッチング

特殊ケース:
- いずれかのパラメータがNULLの場合、NULLを返す
- 区切り文字が文字列内に存在しない場合、元の文字列を返す
