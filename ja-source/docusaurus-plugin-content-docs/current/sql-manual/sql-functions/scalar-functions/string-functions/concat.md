---
{
  "title": "CONCAT",
  "language": "ja",
  "description": "CONCAT関数は複数の文字列を順番に連結して1つの文字列にします。"
}
---
## 説明

CONCAT関数は複数の文字列を順番に連結して1つの文字列にします。この関数は可変個の引数をサポートし、文字列処理における最も基本的でよく使用される関数の1つです。データの連結、レポート生成、動的SQL構築などのシナリオで広く使用されています。なお、引数のいずれかがNULLの場合、結果全体がNULLになることに注意してください。
...
| パラメータ       | 説明           |
|----------|--------------|
| `<expr>` | 連結する文字列式。文字列定数、カラム名、またはその他の式を指定可能。型: VARCHAR |
...
- 引数の順序で文字列を連結
- 任意の数の引数をサポート（最低1個）
- UTF-8マルチバイト文字の正しい連結をサポート
- 数値やその他の型は自動的に文字列に変換

特殊ケース:
- いずれかの引数がNULLの場合、NULLを返す（これがCONCAT_WSとの主な違い）
- 引数が提供されない場合、構文エラー
- 空文字列の引数は連結結果に影響しない
- 非文字列型との混在をサポート
