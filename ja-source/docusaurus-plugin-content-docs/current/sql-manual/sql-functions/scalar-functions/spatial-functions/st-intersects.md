---
{
  "title": "ST_INTERSECTS",
  "language": "ja",
  "description": "2つの幾何学図形が交差するかどうか、すなわち少なくとも1つの共通点が存在するか（境界接触や内部重複を含む）を判定する。"
}
---
## 説明

2つの幾何学的図形が交差するかどうか、すなわち少なくとも1つの共通点があるかどうか（境界接触や内部重複を含む）を判定します。
...
|----------|------------------------|
| `<shape1>` | 他の図形との交差を判定するために使用される最初の幾何図形。Point、Line、Polygon、Circleなどの型をサポートします。 |
| `<shape2>` | 	他の図形との交差を判定するために使用される2番目の幾何図形。Point、Line、Polygon、Circleなどの型をサポートします。 |
...
- 2つの図形が境界で接触するのみの場合（共通点が1つのみ）、1を返します（境界接触は交差とみなされます）
...
+-------------------------------------------------------------------------------------------------------------------+
```
線分がポリゴンの完全に外側にあります（交差なし、0を返します）
