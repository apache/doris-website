---
{
  ...
  "language": "ja",
  "description": "地球表面上の閉じた領域の面積を平方キロメートルで計算します。入力パラメータは幾何オブジェクト（Geopolygonなど、"
}
---
## 説明

地球表面上の閉じた領域の面積を平方キロメートルで計算します。入力パラメータは、地球表面上の領域を表す幾何オブジェクト（Geopolygon、GeoCircle、GeoMuitiPolygonなど）です。
...
| パラメータ | 説明     |
| -- |--------|
| `<geo>` | 地球表面上の幾何オブジェクト。GeoPolygon、GeoCircle、GeoMultiCircle型で、閉じた領域を形成する必要があります。|
...
- 入力が非閉じ幾何オブジェクト（点GeoPoint、線分GeoLineStringなど）の場合、0を返します。
- 座標が有効な経度/緯度範囲（経度[-180, 180]、緯度[-90, 90]）を超える場合、NULLを返します。
