---
{
  "title": "ST_ANGLE",
  "language": "ja",
  "description": "3つの点を入力してください。最初の直線はpoint1（最初の端点）とpoint2（2番目の端点）を結ぶ直線です。"
}
---
## 説明

3つの点を入力します。最初の直線はpoint1（第1の端点）とpoint2（第2の端点）を結ぶ直線です。2番目の直線はpoint2（第1の端点）とpoint3（第2の端点）を結ぶ直線です。これら2つの直線はpoint2で交差します。この関数は、最初の直線から2番目の直線への時計回りの角度を返します。各端点座標について、x値（経度）は[-180, 180]の範囲内である必要があり、y値（緯度）は[-90, 90]の範囲内である必要があります。
...
|----------|--------------------------|
| `<point1>` | 最初の線の最初の端点、`GeoPoint`型             |
| `<point2>` | 最初の線の2番目の端点かつ2番目の線の最初の端点、`GeoPoint`型 |
| `<point3>` | 2番目の線の2番目の端点、`GeoPoint`型 |

## 戻り値

2つの線の間の角度をラジアンで返します。倍精度浮動小数点数として、範囲は[0, 2π)です。角度は最初の線から2番目の線への時計回りで測定されます。

ST_ANGLEには以下のエッジケースがあります：

- point2とpoint3が同じ場合、NULLを返します。
- point2とpoint1が同じ場合、NULLを返します。
- point2とpoint3が正確な対蹠点である場合、NULLを返します。
- point2とpoint1が正確な対蹠点である場合、NULLを返します。
...
```
最初の線から2番目の線までの時計回りの角度は4.7ラジアンです。
...
+---------------------------------------------------+
```
いずれかの座標が指定された範囲を超えた場合、NULLを返します。
