---
{
  "title": "MAKEDATE",
  "language": "ja",
  "description": "MAKEDATE関数は、指定された年と年内の日数（dayofyear）に基づいて日付を構築して返すために使用されます。"
}
---
## 説明

MAKEDATE関数は、指定された年と年内の日数（`dayofyear`）に基づいて日付を構築し、返すために使用されます。この関数は、指定された年の最初の日に日数オフセットを追加することで結果を計算します。年の総日数を超える入力に対する自動調整をサポートしています。
...
| パラメータ | 説明                                                                 |
|------------|----------------------------------------------------------------------|
| `year`     | 指定された年。INT型で、有効範囲は0から9999まで。                     |
| `dayofyear`| 年の日数（1-366）。INT型。                                          |

## 戻り値

入力された年と年の日数に基づいて計算された日付を表す`DATE`型の値を返します（フォーマット`YYYY-MM-DD`）。

- `<day_of_year>`が0以下の場合、エラーが返されます。
- `<day_of_year>`が指定された年の総日数を超える場合（平年は365日、うるう年は366日）、関数は自動的に次の年以降に調整されます（例：平年である2021年の366日目は、2022-01-01に調整されます）。
