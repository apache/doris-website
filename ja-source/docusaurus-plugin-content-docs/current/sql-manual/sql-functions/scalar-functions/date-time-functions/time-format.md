...
| パラメータ                 | 説明                                                                                                                                                |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<time_or_datetime_expr>` | 有効な時刻値。TIMEまたはDATETIME型をサポートします。                                                                                                     |
| `<format>`                | 時刻の出力フォーマットを指定する`varchar`型。フォーマット文字列に日付や非準拠のフォーマット指定子が含まれている場合、NULLを返します。 |
...
| %l        | 時間 (1-12)                                   |
| %p        | AMまたはPM                                      |
| %r        | 時刻、12時間形式 (hh:mm:ss の後にAMまたはPM) |
| %S        | 秒 (00-59)                               |
| %s        | 秒 (00-59)                               |
...
- 時刻値に23より大きい時間部分が含まれている場合、`%H`と`%k`フォーマット指定子は23より大きい値を生成します。その他の時間フォーマット指定子は、時間値を12で割った余りを生成します。
...
+------+-------------------+------+------+------+------+------+------+------+------+---------+------+--------------+-----------+------------------+-----------------+-----------------------------+--------------------+------------------------------------+
```
> 注意：パラメータ`838:59:59.999999`および`-838:59:59.999999`については、TIME範囲[-838:59:59, 838:59:59]を超えているため、TIME型に変換できず、戻り値はNULLとなります。
