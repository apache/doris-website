---
{
  "title": "WIDTH_BUCKET",
  "language": "ja",
  "description": "等幅ヒストグラムを構築します。ここで、ヒストグラムの範囲は等しいサイズの区間に分割され、計算後に、"
}
---
## 説明

等幅ヒストグラムを構築します。ヒストグラム範囲は等しいサイズの区間に分割され、計算後、式の値が該当するバケット番号を返します。特殊なケース：

- この関数は整数値またはNULL（任意の入力がNULLの場合、NULLを返します）を返します
...
| パラメータ | 説明 |
| -- | -- |
| `<expr>` | ヒストグラムを作成するための式。この式は数値または数値に暗黙的に変換可能な値として評価される必要があります。値の範囲は `-(2^53 - 1)` から `2^53 - 1`（境界値を含む）である必要があります |
| `<min_value>` | 式の許容範囲の最小値点。このパラメータは数値である必要があり、`<max_value>` と等しくてはいけません。範囲は `-(2^53 - 1)` から `2^53 - 1`（境界値を含む）である必要があります。さらに、最大値点と最小値点の差は `2^53` より小さい必要があります（例：`abs(max_value - min_value) < 2^53)`） |
| `<max_value>` | 式の許容範囲の最大値点。このパラメータは数値である必要があり、`<min_value>` と等しくてはいけません。範囲は `-(2^53 - 1)` から `2^53 - 1`（境界値を含む）である必要があります。さらに、最大値点と最小値点の差は `2^53` より小さい必要があります（例：`abs(max_value - min_value) < 2^53)`） |
