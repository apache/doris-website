---
{
  ...
  "language": "ja",
  "description": "複数の配列の積集合、つまりすべての配列に存在する要素を返します。"
}
---
## array_intersect
...
複数の配列の積集合を返します。つまり、すべての配列に存在する要素を返します。この関数は、すべての入力配列に存在する要素を見つけ、重複を削除した後に新しい配列を形成します。
...
- 空の配列: すべての入力パラメータ配列間で共通する要素がない場合

使用上の注意:
- この関数は、すべての入力配列に存在する要素を見つけ、結果配列内の要素は重複排除されます
- 空の配列と任意の非NULL配列は空の配列になります。重複する要素がない場合、関数は空の配列を返します。
- この関数はNULL配列をサポートしません
- 要素の比較は型の互換性ルールに従います。型が互換性がない場合、変換が試行され、失敗するとnullになります
- 配列要素内のnull値について: null要素は操作において通常の要素として扱われ、nullはnullと同じものとみなされます
...
```
文字列配列と整数配列の積集合：
文字列 '2' は整数 2 に変換できますが、'b' の変換は失敗し null になります：
