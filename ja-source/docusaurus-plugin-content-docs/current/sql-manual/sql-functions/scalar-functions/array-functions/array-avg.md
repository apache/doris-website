---
{
  "title": "ARRAY_AVG",
  "language": "ja",
  "description": "配列内のすべての数値要素の平均を計算します。この関数は配列内のnull値と非数値要素をスキップします、"
}
---
## array_avg
...
戻り値の意味:
- 配列内の全ての有効な数値要素の平均値を返します
- NULL: 配列が空の場合、または全ての要素がNULLもしくは数値に変換できない場合

使用上の注意:
- 配列に他の型（文字列など）が含まれる場合、要素をDOUBLE型に変換することを試行します。変換に失敗した要素はスキップされ、平均値の計算に含まれません
- この関数は全ての要素を互換性のある数値型に変換して平均値を計算しようと試行します。平均値の戻り値の型は入力型に基づいて自動選択されます:
  - 入力がDOUBLEまたはFLOATの場合、DOUBLEを返します
  - 入力が整数型の場合、DOUBLEを返します
  - 入力がDECIMALの場合、元の精度とスケールを維持してDECIMALを返します
- 空の配列はNULLを返し、要素が1つだけの配列はその要素の値を返します
- 配列がNULLの場合、型変換エラーを返します
- 入れ子配列、MAP、STRUCTおよびその他の複合型は平均値計算をサポートしておらず、呼び出すとエラーが発生します
- 配列要素内のnull値について: null要素は平均値の計算に含まれません
