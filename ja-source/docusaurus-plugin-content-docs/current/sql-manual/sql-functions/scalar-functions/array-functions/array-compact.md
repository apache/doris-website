---
{
  "title": "ARRAY_COMPACT",
  "language": "ja",
  "description": "配列から連続する重複要素を削除し、異なる各値の最初の出現のみを保持します。"
}
---
## array_compact
...
- `arr`：ARRAY<T> 型、重複を除去する配列。カラム名または定数値をサポート。
...
戻り値の意味:
- 重複が除去された配列、連続する重複要素の最初の出現のみを保持
- NULL: 入力配列がNULLの場合
...
2. 境界条件の動作:
   ...
   - 配列に要素が1つしかない場合、元の配列を返す

使用上の注意:

- 関数は配列要素の元の順序を維持する
- 連続する重複要素のみを削除し、グローバルな重複除去は行わない
- map、structは重複除去ロジックをサポートしない
- 配列要素のnull値について: null要素は正常に処理され、連続する複数のnull要素は1つに統合される
...
string_array内の連続重複除去: 隣接する'a'または'b'のみが削除され、'c'は保持されます。
...
+-----------------------------+
```
連続していない重複要素は削除されず、元の順序と内容が保持されます。
...
+-------------------------------+
```
null値を含む配列で、連続するnullは1つだけ保持：nullは通常の値として扱われ、連続するnullは1つだけ保持され、連続しないnullはマージされません。
...
ネストした配列型に対する連続重複除去。隣接する完全に同一のサブ配列のみが削除され、連続していないものは削除されません。
