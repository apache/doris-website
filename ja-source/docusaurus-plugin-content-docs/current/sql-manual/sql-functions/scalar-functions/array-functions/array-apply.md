---
{
  "title": "ARRAY_APPLY",
  "language": "ja",
  "description": "指定されたバイナリ演算子を使用して配列要素をフィルタリングし、条件を満たす要素を含む新しい配列を返します。"
}
---
## array_apply
...
指定されたバイナリ演算子を使用して配列要素をフィルタリングし、条件を満たす要素を含む新しい配列を返します。これは、ラムダ式の代わりに事前定義された演算子を使用する簡略化された配列フィルタリング関数です。
...
- `op`：STRING型、フィルタリング条件演算子、定数値である必要があります。サポートされる演算子：`=`、`!=`、`>`、`>=`、`<`、`<=`
- `val`：T型、フィルタリング条件値、定数値である必要があります
...
- サポート型に制限があり、主に数値、日付、Boolean型です
...
+------------------------------------------+
```
int_array内の3と等しくない要素をフィルタリングします:
...
+------------------------------------------+
```
date_array内の指定された日付以上の要素をフィルタリングする：
