---
{
  "title": "ARRAY_EXISTS",
  "language": "ja",
  "description": "配列の要素にlambda式を適用し、各要素が条件を満たすかどうかを示すboolean配列を返します。"
}
---
## array_exists
...
配列の要素にラムダ式を適用し、各要素が条件を満たすかどうかを示すboolean配列を返します。この関数は配列の各要素にラムダ式を適用し、対応するboolean値を返します。
...
戻り値の意味:
- 入力配列と同じ長さのboolean配列を返し、各位置には対応する要素にlambda式を適用した結果が含まれる
...
- 多次元配列と複合型配列の評価をサポート
- 空の配列は空の配列を返す。配列パラメータのみがNULLでlambda式が存在しない場合はNULLを返す。lambda式が存在し配列がNULLの場合はエラーになる
...
+-------------------------------------+
```
NULL配列とラムダ式の組み合わせ。NULLを含むラムダ式がある場合、エラーになります。ラムダ式がない場合、NULLを返します：
...
nullとの比較演算（>、<、=、>=、<=など）は、特別なIS NULLやIS NOT NULLを除いて、nullを返します。これは、未知の値（NULL）が2より大きいかどうかを判定することが不可能であるためです。この動作はMYSQL、POSTGRESSQL等と一致しています。
...
+-------------------------------------------------+
```
マルチ配列評価、最初の配列が2番目の配列より大きいかどうかをチェックする：
...
+----------------------------------------------------------------+
```
Map型の評価で、キー'a'の値が10より大きいかどうかを確認します：
