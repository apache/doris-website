...
与えられたboolean フラグに従って、入力配列を複数のサブ配列に分割します。

- 分割ルール（左から右）: `arr=[a1,a2,...,an]` と `flags=[f1,f2,...,fn]` において、`fi==true` となるすべての位置で、`ai` と `a(i-1)` の間で分割します。
...
- `flags` 内の位置が `NULL` の場合、分割しないものとして扱われます（`false` と同等）。
...
- `lambda= x -> x-1` を `arr0=[1, 2, 3]` に適用すると `flags=[0,1,2]` が生成され、`flags=[false,true,true]` と同等です
  - `ARRAY_SPLIT(x->x-1, [1, 2, 3])` は `ARRAY_SPLIT([1, 2, 3], [false,true,true])` と同等です -> `[[1], [2], [3]]`

- `lambda= (x,y) -> x-y` を `arr0=[1, 2, 3]` と `arr1=[0,1,2]` に適用すると `flags=[true,true,true]` が生成されます
  - `ARRAY_SPLIT((x,y) -> x-y, [1, 2, 3], [0, 1, 2])` は `ARRAY_SPLIT([1, 2, 3], [true,true,true])` と同等です -> `[[1], [2], [3]]`
