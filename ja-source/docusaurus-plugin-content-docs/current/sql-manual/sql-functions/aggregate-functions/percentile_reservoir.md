---
{
  ...
  "language": "ja",
  "description": "この関数は、最大8192のリザーバーサイズとサンプリング用の乱数生成器を使用してreservoir samplingを適用します。"
}
---
## 説明

この関数は、最大8192のリザーバサイズと、サンプリング用の乱数ジェネレータを使用して[reservoir sampling](https://en.wikipedia.org/wiki/Reservoir_sampling)を適用します。これは位置`p`での近似パーセンタイルを計算するために使用されます。
`p`の値は`0`と`1`の間です。
