---
{
  "title": "AI_AGG",
  "language": "ja",
  "description": "ユーザーが提供した指示に従って、large language modelを使用して特定の列を集計します。"
}
---
## 説明
...
| `<instruction>`   | 実行する指示、リテラルのみ受け入れます。                |
...
結果は言語モデルによって生成されるため、出力は固定されません。

## 例

以下の表は顧客サポートチケットをシミュレートします：
...
```
`AI_AGG`を使用して、問題の件名別に顧客の問題を要約できます：
...
以下の表は、eコマースプラットフォームのユーザーレビュー表をシミュレートしたものです。
