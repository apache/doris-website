---
{
  "title": "ARRAY | 半構造化",
  "language": "ja",
  "description": "ARRAY<T>型は、各要素が同じデータ型を持つ要素の順序付きコレクションを表すために使用されます。例えば、",
  "sidebar_label": "ARRAY"
}
---
# ARRAY ドキュメント
...
`ARRAY<T>` 型は、各要素が同じデータ型を持つ、順序付けられた要素のコレクションを表すために使用されます。例えば、整数の配列は `[1, 2, 3]` として表現でき、文字列の配列は `["a", "b", "c"]` として表現できます。

- `ARRAY<T>` は型Tの要素で構成される配列を表し、Tはnull許可です。Tでサポートされる型には以下が含まれます：`BOOLEAN, TINYINT, SMALLINT, INT, BIGINT, LARGEINT, FLOAT, DOUBLE, DECIMAL, DATE, DATETIME, TIMESTAMPTZ, CHAR, VARCHAR, STRING, IPV4, IPV6, STRUCT, MAP, VARIANT, JSONB, ARRAY<T>`。
  - 注意：上記のT型のうち、`JSONB` と `VARIANT` はDorisの計算層でのみサポートされており、**Dorisでのテーブル作成において `ARRAY<JSONB>` と `ARRAY<VARIANT>` の使用はサポートされていません**。

## 型の制約

- `ARRAY<T>` 型でサポートされる最大ネスト深度は9です。
- `ARRAY<T>` 型間の変換は、Tが変換可能かどうかに依存します。`Array<T>` 型は他の型に変換できません。
...
- `AGGREGATE` テーブルモデルにおいて、`ARRAY<T>` 型は `REPLACE` と `REPLACE_IF_NOT_NULL` のみをサポートします。**どのテーブルモデルにおいても、KEYカラムとして使用することはできず、パーティションまたはバケットカラムとしても使用できません**。
- `ARRAY<T>` 型のカラムは **`ORDER BY` と `GROUP BY` 操作をサポート**します。
  - `ORDER BY` と `GROUP BY` をサポートするT型には以下が含まれます：`BOOLEAN, TINYINT, SMALLINT, INT, BIGINT, LARGEINT, FLOAT, DOUBLE, DECIMAL, DATE, DATETIME, TIMESTAMPTZ, CHAR, VARCHAR, STRING, IPV4, IPV6`。
...
- `ARRAY`内の要素型が`VARCHAR`の場合のみ修正が許可されます。
   - `VARCHAR`のパラメータを小さいものから大きいものに変更することのみ許可され、逆方向は許可されません。
...
ARRAYは順序付きの型で、[1, 2, 3]と[3, 2, 1]は2つの異なるARRAYです。2つのARRAYが等しいのは、それらの要素が順番に一対一で等しい場合のみです：
...
+-------------------------------+
```
部分順序比較において、ARRAYは辞書順に従います。2つの配列`A`と`B`が与えられた場合、インデックス`i = 1`から開始して、対応する位置の要素`A[i]`と`B[i]`を比較します：

- `A[i] ≠ B[i]`で等しくない場合、比較結果（<、>）が配列全体の比較結果を直接決定します
- `A[i] = B[i]`の場合、次の位置の比較を続行します
- 共通の長さの範囲内で配列が完全に等しい場合、短い配列の方が小さくなります。
...
- Dorisテーブルの`ARRAY<T>`型の列では、この列の`ARRAY`関数を含む計算を高速化するために転置インデックスの追加をサポートしています。
  - 転置インデックスでサポートされるT型: `BOOLEAN, TINYINT, SMALLINT, INT, BIGINT, LARGEINT, DECIMAL, DATE, DATETIME, TIMESTAMPTZ, CHAR, VARCHAR, STRING, IPV4, IPV6`。
