---
{
  "title": "変数",
  "language": "ja",
  "description": "Dorisでは、変数はsystem variablesとuser variablesに分けられます。両方とも大文字小文字を区別しません。"
}
---
## 説明

Dorisにおいて、変数はシステム変数とユーザー変数に分かれます。どちらも大文字小文字を区別しません。

システム変数はDorisの動作に影響を与えます。システム変数とユーザー変数の両方をユーザークエリで使用できます。

## システム変数

システム変数は、データベースの動作とパフォーマンスを制御するためにDorisによって事前定義された変数セットです。主な特徴は以下のとおりです：

- 変数の種類：

    - 読み取り専用変数：これらの変数はシステムによって設定され、ユーザーが変更することはできません。例：`version`、`current_timestamp`など。

    - 変更可能変数：ユーザーは実行時にこれらの変数の値を変更できます。例：`exec_mem_limit`、`time_zone`など。
...
ユーザー定義変数は、セッション内でデータを一時的に保存するメカニズムです。主な特徴は以下のとおりです：
...
- スコープ：
    ...
    - 異なるセッションでの同名変数は互いに独立しています。

- 代入方法：

    - `SET @var_name = value`構文を使用して代入します。

    - 式の結果を使用した値の代入をサポートしています。

- データ型：

    - 数値（整数、浮動小数点数）を保存できます。
...
- SET
    ...
    `SET`を使用して現在のセッションでのみ有効になるよう設定します。例：
