---
{
  "title": "グローバルメモリ解析",
  "language": "ja",
  "description": "GLobal MemoryはDorisのグローバル共有メモリであり、主にCacheとMetadataを含みます。"
}
---
Global MemoryはDorisのグローバル共有メモリで、主にCacheとMetadataを含みます。
...
```
Memory Trackerタグの一部にはサフィックスがあり、その意味は以下の通りです：

- `[size]`は、Cache Trackerがメモリサイズを記録することを意味します。

- `[number]`は、Cache Trackerがキャッシュされたエントリの数を記録することを意味し、これは通常、現在メモリを正確にカウントできないためです。
...
## Global Memoryが大量に占有している場合

```
MemTrackerLimiter Label=global, Type=overview, Limit=-1.00 B(-1 B), Used=199.37 MB(209053204 B), Peak=199.37 MB(209053204 B)
```
Global Memory Tracker `Label=global, Type=overview` の値は、`Type=global` かつ `Parent Label != DetailsTrackerSet` のすべてのMemory Trackerの合計と等しく、主に異なるタスク間で共有されるCacheとメタデータを含みます。
...
Orphan Memory Trackerの値が大きすぎる場合、Memory Trackerの統計が不足していることを意味します。[Memory Tracker](./../memory-feature/memory-tracker.md)の[Memory Tracker Statistics Missing]の解析を参照してください。
