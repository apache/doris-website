---
{
  "title": "自動サービス起動",
  "language": "ja",
  "description": "このドキュメントでは、サービスの後にサービスが適時にプルアップされないことを確実にするために、Dorisクラスターの自動プルアップを設定する方法について説明します"
}
---
# 自動サービス開始

このドキュメントでは、Dorisクラスターの自動起動を設定する方法について説明します。これにより、本番環境で特別な状況によりサービスが故障した後、サービスが適切に起動されないことを防ぐことができます。

FEとBEの自動起動サービスは、Dorisクラスターが完全にセットアップされた後に設定する必要があります。
...
systemdを使用してdorisサービスを制御するには、sudo権限が必要です。sudo権限付与の最小粒度を確保するために、doris-feとdoris-beサービスのsystemd制御権限を指定した非rootユーザーに割り当てることができます。visudoでdoris-feとdoris-beのsystemctl管理権限を設定してください。
...
1. config ファイル（fe.conf と be.conf の両方）で "JAVA_HOME" 変数を設定する必要があります。設定しないと "systemctl start" コマンドを使用して doris を起動できません
...
- ExecStartとExecStopは実際のfeパスに基づいて設定されています
...
7. 自動起動の設定
    ...
    ```
スタートを設定します。本質は /etc/systemd/system/multi-user.target.wants/ にサービスファイルリンクを追加することです
...
8. サービス開始
    ...
    ```
## SupervisorはDorisサービスを設定します

Supervisorの具体的な使用方法とパラメータ分析については[こちら](http://supervisord.org/)を参照してください。

Supervisor設定はsupervisor設定を自動的に取得します。yumコマンドを使用して直接supervisorをインストールするか、pipを使用して手動でインストールできます。pipによる手動インストールプロセスは複雑なため、yumデプロイメントモードのみを表示します。手動デプロイメントについてはインストールデプロイメントの[こちら](http://supervisord.org/installing.html)を参照してください。
...
```
- supervisorがDorisプロセスを自動的に起動するように設定されている場合、DorisでBEノードが異常要因により故障すると、be.outに出力されるべきエラースタック情報がsupervisorによって傍受されます。さらなる分析のために、supervisorのログでそれを確認する必要があります。
