...
Hive に加えて、多くの他のシステムが Hive Metastore を使用してメタデータを格納しています。そのため、Hive カタログ を通じて、Hive テーブルだけでなく、Iceberg や Hudi など、メタデータストレージに Hive Metastore を使用する他のテーブル形式にもアクセスできます。
...
| データライトバック | Doris がサポートする任意のソースからデータを処理し、Hive テーブルにライトバックします。 |
...
* `{MetaStoreProperties}`
  ...
  MetaStorePropertiesセクションは、Metastoreメタデータサービスの接続と認証情報を入力するためのものです。詳細については「サポートされているメタデータサービス」セクションを参照してください。

* `{StorageProperties}`

  StoragePropertiesセクションは、ストレージシステムに関連する接続と認証情報を入力するためのものです。詳細については「サポートされているストレージシステム」セクションを参照してください。

* `{HiveProperties}`

  HivePropertiesセクションは、Hive Catalogに関連するプロパティを入力するためのものです。

  * `get_schema_from_table`: デフォルト値はfalseです。デフォルトでは、DorisはHive Metastoreからテーブルスキーマ情報を取得します。ただし、場合によっては`Storage schema reading not supported`などのエラーで互換性の問題が発生することがあります。この場合、このパラメータをtrueに設定すると、テーブルスキーマがTableオブジェクトから直接取得されます。ただし、この方法では列のデフォルト値情報が無視されることに注意してください。このプロパティはバージョン2.1.10および3.0.6以降でサポートされています。

  * `hive.recursive_directories`: パーティションディレクトリをリストする際にサブディレクトリを再帰的に検索するかどうか。このパラメータはバージョン3.0.2以降でサポートされています。バージョン4.0より前では、このパラメータはデフォルトで`false`でした。それ以降のバージョンでは、デフォルトで`true`です。一部のHiveテーブルのパーティションパスは、テーブルスキーマのパーティション情報と一致しない場合があります。サブディレクトリ内のデータファイルを取得するには、このパラメータをtrueに設定する必要があります。

  * `hive.ignore_absent_partitions`: 存在しないパーティションを無視するかどうか。デフォルトは`true`です。`false`に設定すると、存在しないパーティションに遭遇したときにクエリがエラーを報告します。このパラメータはバージョン3.0.2以降でサポートされています。

  * `hive.staging_dir`: Hive書き込み操作の一時ステージングルートディレクトリを設定するために使用されます。書き込みプロセス中、データは最初にこのディレクトリに書き込まれ、完了後に最終的なHiveテーブルディレクトリに移動されます。このパスは、Hiveテーブルルートディレクトリからの相対パスまたは絶対パスのいずれかになります。デフォルト値は`/tmp/.doris_staging`です。場合によっては、Hiveテーブルに書き込む際に、`/tmp`ディレクトリとテーブルディレクトリが異なる名前空間に属し、書き込みが失敗することがあります。このような場合、このパラメータを変更して相対パスを使用することで問題を解決できます。このパラメータはバージョン4.0.3以降でサポートされています。
...
> DorisでHiveテーブルを作成してデータを書き込むには、Catalog属性に`fs.defaultFS`プロパティを明示的に追加する必要があります。Catalogがクエリのみに作成される場合、このパラメータは省略できます。
>
...
<details>
    ...
    <Tabs>
        <TabItem value='S3' label='S3' default>
            ...
            ```
Glueサービスの認証情報がS3の認証情報と異なる場合、以下の方法でS3の認証情報を個別に指定できます。
