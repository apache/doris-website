...
---
コミュニティのユーザーと開発者の皆様のおかげで、Doris 2.0.8バージョンでは約65の改善とバグ修正が行われました。
...
`ADMIN SHOW`文は高いバージョンのMySQL 8.x jdbc driverでは実行できません。そのため、これらの文の名前を変更し、`ADMIN`キーワードを削除します。
...
- NereidsでTopN optとInverted Indexが連携するよう対応

- BEメモリ使用量を制御するため、列統計収集時の最大文字列長を1024に制限

- JDBCクライアントが空でない場合のJDBC Catalogのクローズ処理

- すべてのIcebergデータベースを受け入れ、データベースの名前形式チェックを廃止

- キャッシュミスと不安定なクエリプランを回避するため、外部テーブルの行数を非同期で更新
