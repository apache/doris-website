---
{
  "title": "リリース 1.2.1",
  "language": "ja",
  "description": "DecimalV3は、より高い精度とより優れたパフォーマンスをサポートしており、過去のバージョンに対して以下の利点があります。"
}
---
# 改善
...
- より高いパフォーマンス、異なる精度に応じて占有するストレージ容量を適応的に調整します。
...
### Stream loadでクォート付きcsvデータをサポート

ドキュメントでtrim_double_quotesを検索してください：[https://doris.apache.org/docs/dev/sql-manual/sql-reference/Data-Manipulation-Statements/Load/STREAM-LOAD](https://doris.apache.org/docs/dev/sql-manual/sql-reference/Data-Manipulation-Statements/Load/STREAM-LOAD)
...
- datev2/datetimev2タイプをパーティション化に使用する際に、パーティション値が間違っている問題を修正。[#15094](https://github.com/apache/doris/pull/15094)
...
- このバージョンではcsv形式の読み取りパフォーマンスが低下しており、csv形式のインポートと読み取り効率に影響します。次の3桁バージョンでできるだけ早く修正予定です。
...
- FE設定項目`enable_multi_catalog`を削除しました。Multi-カタログ機能はデフォルトで有効です。

- ベクトル化実行エンジンは強制的にデフォルトで有効になります。[#15213](https://github.com/apache/doris/pull/15213)
