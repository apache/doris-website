---
{
  "title": "診断ツール",
  "language": "ja",
  "description": "効率的で効果的なパフォーマンス診断ツールは、データベースシステムのチューニングにとって重要である。"
}
---
## 概要

効率的で効果的なパフォーマンス診断ツールは、データベースシステムのチューニングにおいて重要です。これらのツールは、問題のあるビジネスSQLクエリを迅速に特定し、その後、パフォーマンスのボトルネックを素早く特定して解決できるかを決定し、データベースシステムがService Level Agreements (SLA)を満たすことを保証します。

現在、Dorisはデフォルトで実行時間が5秒を超えるSQLクエリをslow SQLと見なします。この閾値は`config.qe_slow_log_ms`で設定できます。Dorisは現在、パフォーマンスに問題があるslow SQLクエリを迅速に特定するための以下の3つの診断チャネルを提供しています：

## Doris Manager Logs

Doris Managerのlogモジュールは、slow SQLフィルタリング機能を提供します。ユーザーは特定のFEノード上で`fe.audit.log`を選択することでslow SQLを確認できます。検索ボックスに単純に`slow_query`と入力するだけで、現在のシステムの履歴slow SQL情報がページに表示されます。下図に示すとおりです：

![Doris Manager Monitoring and Logging](/images/doris-manage-trace-log-2.png)

## Audit Log

現在、Doris FEは`slow_query`、`query`、`load`、`stream_load`を含む4種類のAudit Logを提供しています。Managerサービスがインストール・デプロイされているクラスターのlogページを通じてログにアクセスする以外に、Audit LogはFEが配置されているノードの`fe/log/fe.audit.log`ファイルにアクセスすることで直接取得することもできます。

`fe.audit.log`内で`slow_query`タグを直接検索することで、実行が遅いSQLクエリを迅速にフィルタリングできます。以下に示すとおりです：
...
```
`fe.audit.log`を通じて取得されるスロークエリにより、ユーザーは実行時間、スキャンした行数、返された行数、SQL文自体などの詳細情報に簡単にアクセスでき、パフォーマンス問題の再現と特定のための基盤を築くことができます。

さらに、Audit Logには`SqlDigest`フィールドが含まれています（上記の例の`SqlDigest=...`など）。このフィールドは、SQL文の構造から生成されるハッシュ値です（特定のパラメータ値は除去されます）。`slow_query`の`SqlDigest`を集約・分析することで、スロークエリの「パターン」を特定できます。これは、特定のSQL文がパラメータによってわずかに異なっていても、構造が同じである限り、それらの`SqlDigest`は同一になることを意味します。

`SqlDigest`を使用することで、ユーザーはどのSQLパターンが最も頻繁に現れるか、または最も多くの時間を消費するかを判断でき、これらの「高頻度」または「高レイテンシ」パターンに優先的に最適化を行うことができます。このアプローチは、個々のSQL文を一つずつ分析する非効率性を回避し、スロークエリ最適化の効率を大幅に向上させます。

`SqlDigest`自体は単なるハッシュ値であり、直接読み取り可能ではないことに注意が重要です。最適化すべきスロークエリパターンが特定されたら、Audit Logの`Stmt`フィールドを参照して、そのパターンに対応する具体的なSQL文の内容を取得する必要があります。さらに、`QueryId`フィールドを使用してクエリの詳細なProfile情報を取得し（Profileの取得と分析については後続セクションで詳述します）、詳細なパフォーマンス分析と最適化を行うことができます。

### スロークエリ分析例

上記の`fe.audit.log`内の4つのスロークエリログを例に取ると、以下を観察できます：
...
これは、後者の3つのスロークエリが同じSQLパターン（構造的に同一）に属していることを示しており、特定の実行時間（`Time(ms)`）と詳細はわずかに異なりますが、構造は同じです。

実際の最適化シナリオでは、特定の`SqlDigest`（`fec5a7136f9375aa968a4de971b994da`など）がスロークエリログに繰り返し現れる、または総実行時間の高い割合を占めることが判明した場合、このパターンを優先的に最適化すべきです。
...
2.  **詳細分析**: ログの `QueryId`（例：`6e1fae453cb04d9a-b1e5f94d9cea1885`）を使用して、Dorisで対応するQuery Profileを取得します。Profileは、問題が過剰なデータスキャン、長時間のJoin操作、またはその他の理由によるものかを分析するのに役立ち、対象を絞った最適化戦略（インデックスの追加、SQL記述の最適化、テーブル構造の調整など）を可能にします。

このパターンの問題が解決されると、このパターンに属するすべての低速クエリが改善されます。

## audit_log システムテーブル

Dorisバージョン2.1以降、`__internal_schema`データベース下で`audit_log`システムテーブルが提供され、ユーザーがSQLクエリの実行状況を確認できます。使用する前に、グローバル設定`set global enable_audit_plugin=true`;を有効にする必要があります（このスイッチはデフォルトで無効になっています）。
