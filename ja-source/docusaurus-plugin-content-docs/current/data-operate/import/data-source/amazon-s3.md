...
CSVファイル s3load_example.csv を作成します。このファイルはS3に保存され、その内容は以下の通りです：
...
10 rows in set (0.04 sec)
```
DorisはS3 LoadとTVFに対して`AWS Assume Role`もサポートしています。詳細については[AWS intergration](../../../admin-manual/auth/integrations/aws-authentication-and-authorization.md#assumed-role-authentication)を参照してください。
