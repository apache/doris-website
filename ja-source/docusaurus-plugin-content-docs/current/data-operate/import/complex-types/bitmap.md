---
{
  "title": "ビットマップ",
  "language": "ja",
  "description": "BITMAP型は、Duplicate表、Unique表、およびAggregate表で使用でき、Key型としてではなく、Value型としてのみ使用できます。"
}
---
BITMAP型は、Duplicateテーブル、Uniqueテーブル、Aggregateテーブルで使用でき、Key型としてではなく、Value型としてのみ使用できます。AggregateテーブルでBITMAP型を使用する場合、テーブルは集約型BITMAP_UNIONで作成する必要があります。ユーザーは長さとデフォルト値を指定する必要はありません。長さは、データ集約の度合いに基づいてシステムによって制御されます。詳細なドキュメントについては、[Bitmap](../../../sql-manual/basic-element/sql-data-types/aggregate/BITMAP)を参照してください。
...
`bitmap_from_string`を使用する場合、ソースファイルのarrカラムでは角括弧は使用できません。そうでない場合、データ品質エラーとみなされます。
...
`bitmap_from_array`を使用する際、ソースファイルのarr列に角括弧を含めることができます。ただし、stream loadでは、使用前にstring型をarray型に最初にキャストする必要があります。
