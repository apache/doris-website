---
{
  "title": "一時パーティション",
  "language": "ja",
  "description": "Dorisはパーティション化されたテーブルに一時パーティションを追加することをサポートしています。"
}
---
Dorisは、パーティション化されたテーブルに一時パーティションを追加することをサポートしています。一時パーティションは通常のパーティションとは異なり、通常のクエリでは取得されず、特別なクエリ文を通じてのみクエリできます。

- 一時パーティションのパーティション列は通常のパーティションと同じであり、変更できません。

- すべての一時パーティションのパーティション範囲は重複できませんが、一時パーティションと通常のパーティションの範囲は重複できます。

- 一時パーティションの名前は、通常のパーティションや他の一時パーティションの名前と重複できません。

**一時パーティションの主な適用シナリオ：**

- **アトミック上書き操作**：ユーザーは、古いデータの削除と新しいデータのインポートの間でデータ損失なしにパーティションのデータを書き換えることができます。この場合、一時パーティションを作成し、新しいデータを一時パーティションにインポートして、replace操作を通じて元のパーティションをアトミックに置換できます。非パーティション化テーブルでのアトミック上書き操作については、[replace table documentation](../../data-operate/delete/atomicity-replace)を参照してください。
...
- **パーティションのマージまたは分割**：ユーザーは、2つのパーティションをマージしたり、大きなパーティションを複数の小さなパーティションに分割したりするなど、パーティション範囲を変更できます。新しい一時パーティションを作成し、`INSERT INTO`コマンドを使用して通常のパーティションのデータを一時パーティションにインポートし、replace操作を通じて元のパーティションをアトミックに置換できます。
...
```
replace操作には、2つの特別なオプションパラメータがあります：

**1. `strict_range`**

デフォルトはtrueです。

Rangeパーティションの場合、このパラメータがtrueの時、置き換えられるすべての通常パーティションの範囲の和集合は、置き換える一時パーティションの範囲の和集合と完全に同じでなければなりません。falseに設定した場合は、置き換え後に新しい通常パーティションの範囲が重複しないことを保証するだけで済みます。

Listパーティションの場合、このパラメータは常にtrueです。置き換えられるすべての通常パーティションの列挙値は、置き換える一時パーティションの列挙値と完全に同じでなければなりません。
...
このパラメータが false で、置き換え対象のパーティション数と置き換えるパーティション数が同じ場合、置き換え後もパーティション名は変更されません。

true の場合、置き換え後のパーティション名は、置き換えるパーティションの名前になります。例は以下の通りです：
...
ALTER TABLE tbl1 REPLACE PARTITION (p1) WITH TEMPORARY PARTITION (tp1);
```
- `use_temp_partition_name`はデフォルトでfalseなので、置換後、パーティション名はp1のままですが、データとプロパティはtp1のものに置き換えられます。
- `use_temp_partition_name`がtrueの場合、置換後、パーティション名はtp1になり、p1は存在しなくなります。
...
パーティションの置き換えが成功すると、置き換えられたパーティションは削除され、復旧できません。
:::
...
- Drop操作を使用してデータベースやテーブルを直接削除した後、データベースやテーブルはRecoverコマンドを使用して復元できますが（制限時間内）、一時パーティションは復元されません。
...
- テーブルが変更操作を実行中の場合、一時パーティションをテーブルに追加することはできません。
