...
SeaTunnelは、大規模データのリアルタイム同期をサポートする、非常に使いやすい超高性能分散データ統合プラットフォームです。毎日数百億のデータを安定かつ効率的に同期できます。

## Connector-V2

SeaTunnel用のconnector-v2は、バージョン2.3.1以降でDoris Sinkをサポートし、exactly-once書き込みとCDCデータ同期をサポートします
...
stream loadインポートで使用されるラベルプレフィックス。2pcシナリオでは、SeaTunnelのEOSセマンティクスを保証するためにグローバルユニークである必要があります。

`sink.enable-2pc [bool]`

2フェーズコミット（2pc）を有効にするかどうか。デフォルトはtrueで、Exactly-Onceセマンティクスを保証します。2フェーズコミットについては、[here](../data-operate/import/import-way/stream-load-manual)を参照してください。

`sink.enable-delete [bool]`

削除を有効にするかどうか。このオプションは、Dorisテーブルでbatch delete機能を有効にする必要があり（0.15+バージョンではデフォルトで有効）、Uniqueモデルのみをサポートします。詳細については、このリンクで確認できます：

[batch delete](../data-operate/delete/batch-delete-manual.md)

`doris.config [map]`

stream loadの`data_desc`のパラメータ。詳細については、このリンクで確認できます：
...
フラッシュ間隔（ミリ秒）、この時間経過後に非同期スレッドがキャッシュ内のデータをDorisに書き込みます。0に設定すると定期書き込みを無効にします。
...
[Stream Loadパラメータ設定の詳細](../data-operate/import/import-way/stream-load-manual)
