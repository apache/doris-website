...
Logstashは、ログETLフレームワーク（収集、前処理、ストレージシステムへの送信）であり、データをストレージシステムに書き込むためのカスタムoutputプラグインをサポートしています。Logstash Doris output pluginは、Dorisにデータを出力するためのプラグインです。
...
プラグインは公式ウェブサイトからダウンロードするか、ソースコードから自分でコンパイルすることができます。
...
```
標準インストールモードでは、プラグインが依存するrubyモジュールが自動的にインストールされます。ネットワークが利用できない場合、処理が停止し完了できません。このような場合は、依存関係を含むzipインストールパッケージをダウンロードして完全オフラインインストールを行うことができます。ローカルファイルシステムを指定するには`file://`を使用する必要があることに注意してください。
...
Logstash Doris output pluginの設定は以下の通りです：
...
`label_prefix` | Doris Stream Load Labelプレフィックス、最終的に生成されるLabelは*{label_prefix}_{db}_{table}_{yyyymmdd_hhmmss}_{uuid}*で、デフォルト値はlogstashです
`headers` | Doris Stream Loadヘッダーパラメーター、構文形式はruby mapで、例：headers => { "format" => "json", "read_json_by_line" => "true" }
...
`log_request` | トラブルシューティングのためにログにDoris Stream Load要求と応答メタデータを出力するかどうか、デフォルトはfalseです
`log_speed_interval` | ログに速度を出力する時間間隔、単位は秒、デフォルトは10、0に設定するとこのタイプのログを無効にできます
...
FEログファイルは通常、Dorisインストールディレクトリ下のfe/log/fe.logファイルに配置されます。これらは典型的なJavaプログラムログで、タイムスタンプ、ログレベル、スレッド名、コードの場所、ログ内容などのフィールドが含まれます。通常のログだけでなく、スタックトレースを含む例外ログも含まれており、これらは複数行にわたります。ログの収集と保存では、メインログとスタックトレースを単一のログエントリに結合する必要があります。
...
Logstashシステム全体の設定ファイルは通常config/logstash.ymlに配置されています。Dorisへの書き込み時のパフォーマンスを向上させるため、バッチサイズとバッチ遅延を変更する必要があります。1行あたり数百バイトの平均サイズのログの場合、バッチサイズ1,000,000行とバッチ遅延10秒が推奨されます。

``` 
pipeline.batch.size: 1000000
pipeline.batch.delay: 10000
```
特定のログ収集のための設定ファイル（logstash_doris_log.confなど）は、主にETLの各段階に対応する3つの部分で構成されます：
...
```
以下はデータのサンプルです。通常、各データは1行に記述されますが、表示しやすくするため、ここではフォーマットされています。
