...
{
  "title": "変数",
  "description": "Dorisでは、変数はシステム変数とユーザー変数に分類されます。どちらも大文字と小文字を区別しません。",
  "language": "ja"
}
---
## 概要

Dorisでは、変数はシステム変数とユーザー変数に分かれています。両方とも大文字小文字を区別しません。
...
- 変数タイプ：
    ...
    - 読み取り専用変数：これらの変数はシステムによって設定され、ユーザーが変更することはできません。`version`、`current_timestamp`などがあります。

    - 変更可能変数：ユーザーは実行時にこれらの変数の値を変更できます。`exec_mem_limit`、`time_zone`などがあります。
...
- アクセス方法：
    ...
    - `SHOW VARIABLES LIKE 'pattern'`を使用してパターンマッチングによって特定の変数を表示します。
...
ユーザー定義変数は、セッション内でデータを一時的に格納するためのメカニズムです。主な特徴は以下の通りです：
