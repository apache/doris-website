...
* 3つのタイプが許可される：
  ...
  3. 長い形式のタイムゾーン名: IANA管理の[Timezone Database](https://www.iana.org/time-zones)に含まれるすべての有効なタイムゾーン名（`Europe/Paris`、`Etc/GMT+2` など）、大文字小文字を区別しない。
...
各フィールドの解析では、先頭の余分なゼロによる長さの超過は許可されなくなりました。例えば、`<day> ::= <digit>{1,2}`に対する`00012`は無効です。
...
| ------------------------------------------- | --------------- | ------------------------------------------ |
| `2023-7-4T9-5-3.1Z`                         | `2023-07-04`    | 先頭と末尾の空白文字；日付`2023-7-4`（`-`区切り文字、1桁の月/日をサポート）；時刻とタイムゾーンは有効 |
| `99.12.31 23.59.59+05:30`                   | `1999-12-31`    | `.`が日付と時刻を区切る；タイムゾーン`+05:30`（分`30`は有効）；'T'なし   |
| `2000/01/01T00/00/00-230`                   | `2000-01-01`    | `/`区切り文字；コロンなしのタイムゾーンと1桁時間`-230`                |
| `85 1 1T0 0 0. CST`                         | `1985-01-01`    | すべてのフィールドが空白で区切られる；2桁年は`1985`にマップ；小数点以下0桁；短いタイムゾーン名は大文字小文字を区別しない     |
...
| `0023-1-1T1:2:3. -00:00`                    | `0023-01-01`    | 4桁年`0023`；1桁/2桁混合時刻フィールド；小数点以下0桁；分の符号なしオフセット       |
