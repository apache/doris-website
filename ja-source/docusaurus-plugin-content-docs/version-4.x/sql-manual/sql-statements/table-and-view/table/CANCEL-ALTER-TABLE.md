...
**3.`<table_name>`**
> データベース（Database）内のTableの識別子（つまり、名前）を指定します。
>
...
> キャンセルする特定のジョブIDです。
>
> ジョブIDが指定された場合、指定されたジョブのみがキャンセルされます。指定されない場合、Table上で実行中の指定されたタイプ（COLUMNまたはROLLUP）のすべての変更がキャンセルされます。
>
