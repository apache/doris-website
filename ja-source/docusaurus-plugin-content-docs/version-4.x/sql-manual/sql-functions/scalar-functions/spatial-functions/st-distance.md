...
経度と緯度の座標を受け取る`ST_DISTANCE_SPHERE`とは異なり、`ST_DISTANCE`はgeometryオブジェクト（点、線、ポリゴン、円など）をパラメータとして受け取り、それらの境界間の最短距離を計算します。2つの形状が交差する場合（接触や包含を含む）、0を返します。
...
- 2つのジオメトリオブジェクトが交差している場合（一方が他方を含んでいる場合を含む）、`0.0`を返します。
