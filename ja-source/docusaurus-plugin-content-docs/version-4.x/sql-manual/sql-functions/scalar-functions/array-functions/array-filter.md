...
条件に基づいて配列要素をフィルタリングし、条件を満たす要素で構成された新しい配列を返します。この関数は2つの呼び出し方法をサポートしています：lambda式を使用した高階関数形式と、boolean配列を直接使用するフィルタリング形式です。
...
- `lambda`：配列要素を評価するために使用されるラムダ式、true/falseまたはboolean値に変換可能な式を返す
...
使用上の注意：
- ラムダ形式：ラムダ式のパラメータ数は配列パラメータ数と一致する必要がある
- Boolean配列形式：`array1`と`filter_array`の長さは理想的には完全に一致する必要がある。boolean配列が長い場合、余分なboolean値は無視される。boolean配列が短い場合、boolean配列内の対応する位置の要素のみが処理される
- 複数配列と複合型配列のフィルタリングをサポート
- 空配列は空配列を返し、NULL配列はNULLを返す
- ラムダは任意のスカラー式を使用でき、集約関数は使用できない
- ラムダ式は他の高階関数を呼び出すことができるが、互換性のある型を返す必要がある
- 配列要素内のnull値について：null要素はラムダ式に渡されて処理され、ラムダはnull値を評価できる
...
double_array内の3以上の要素をフィルタリングするためにlambda式を使用する場合:
...
+------------------------------------------+
```
string_arrayにおいて長さが2より大きい要素をフィルタリングするためのlambda式の使用：
...
+-----------------------------------------------------------+
```
配列の要素を対応する位置のboolean値に基づいて保持するかどうかを決定するboolean配列フィルタリングの例：
...
+--------------------------------------------+
```
boolean配列の長さが元の配列よりも大きい場合、余分なboolean値は無視されます：
...
+-------------------------------------------------------------------+
```
Map型のフィルタリング、キー'a'の値が10より大きい要素をフィルタリング：
...
+---------------------------------------------------------------+
```
構造体型のフィルタリング、ageが18より大きい要素をフィルタリングする場合:
...
**正しい例: lambda内でスカラーを返す高階関数を呼び出す**

現在の例では、内側のarray_countがスカラー値（INT64）を返すため、ネストすることができます。この値はarray_filterで処理可能です。
...
現在の例では、内側のarray_existsがARRAY<BOOLEAN>を返すのに対し、外側のarray_filterではlambdaがスカラー値を返すことを期待しているため、ネストできません
