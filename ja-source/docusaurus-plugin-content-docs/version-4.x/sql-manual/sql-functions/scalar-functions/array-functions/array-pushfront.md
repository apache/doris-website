...
配列の先頭に要素を追加します。この関数は、新たに追加された要素と元の配列のすべての要素を含む新しい配列を返します。
...
使用上の注意：
- この関数は指定された要素を配列の先頭に追加します
- 空の配列でも通常通り要素を追加できます。新しい要素の型は配列要素型と互換性がある必要があります
- 配列要素内のnull値について：null要素は通常通り処理されます
