...
{
  "title": "YEARWEEK",
  "description": "YEARWEEK関数は、指定された日付に対して「年 + 週番号」の組み合わせを返すために使用されます（形式はYYYYWW、例：",
  "language": "ja"
}
---
## 説明

YEARWEEK関数は、指定された日付に対して「年 + 週番号」の組み合わせを返すために使用されます（形式はYYYYWW、例：202301は2023年の第1週を表します）。この関数は、オプションパラメータmodeを通じて週の開始日と「第1週」を決定する基準を柔軟に定義し、デフォルトではmode=0となります。

週番号の範囲は、mode設定に応じて1-53となります。
...
|1    |月曜日            |1-53              |年内に4日以上ある最初の週                             |
|2    |日曜日            |1-53              |年の最初の日曜日を含む週                              |
|3    |月曜日            |1-53              |年内に4日以上ある最初の週                             |
|4    |日曜日            |1-53              |年内に4日以上ある最初の週                             |
|5    |月曜日            |1-53              |年の最初の月曜日を含む週                              |
|6    |日曜日            |1-53              |年内に4日以上ある最初の週                             |
...
- 日付を含む週が翌年に属する場合、翌年の年と週番号1を返します（例：2024年12月30日は202501を返す可能性があります）。
