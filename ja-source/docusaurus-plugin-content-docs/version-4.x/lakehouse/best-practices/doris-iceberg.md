...
- バージョン0.15以降、Apache DorisはHiveとIceberg外部テーブルを導入し、データレイクでのApache Icebergとの組み合わせ機能を探求しました。
- バージョン1.2から、Apache DorisはMulti-Catalog機能を正式に導入し、様々なデータソースに対する自動メタデータマッピングとデータアクセスを可能にし、外部データ読み取りとクエリ実行に対する数多くのパフォーマンス最適化を行いました。現在は高速でユーザーフレンドリーなLakehouseアーキテクチャの構築を完全にサポートしています。
- バージョン2.1において、Apache DorisはData Lakehouseアーキテクチャをさらに強化し、主流のデータレイク形式（Hudi、Iceberg、Paimonなど）の読み書き機能を向上させ、複数のSQL方言との互換性を導入し、既存システムからApache Dorisへのシームレスな移行を実現しました。データサイエンスと大規模データ読み取りシナリオに対して、DorisはArrow Flight高速読み取りインターフェースを統合し、データ転送効率を100倍向上させました。

![Building Lakehouse using Doris and Iceberg](/images/lakehouse-architecture-for-doris-and-iceberg.png)

## Apache Doris & Iceberg

Apache Icebergは、大規模データの分析と管理を可能にするオープンソースの高性能で高信頼性なデータレイクテーブル形式です。Apache Dorisを含む様々な主流クエリエンジンをサポートし、HDFSと様々なオブジェクトクラウドストレージと互換性があり、ACID準拠、スキーマ進化、高度なフィルタリング、隠れたパーティショニング、パーティションレイアウト進化などの機能を備え、高性能クエリ、データ信頼性、一貫性、およびタイムトラベルやバージョンロールバックなどの機能による柔軟性を確保します。

Apache DorisはIcebergのいくつかのコア機能をネイティブサポートしています：

- Hive Metastore、Hadoop、REST、Glue、Google Dataproc Metastore、DLFなどの複数のIceberg Catalogタイプをサポート。
- Iceberg V1/V2テーブル形式およびPosition Delete、Equality Deleteファイルの読み取りをネイティブサポート。
- テーブル関数を通じたIcebergテーブルスナップショット履歴のクエリをサポート。
- Time Travel機能をサポート。
- Icebergテーブルエンジンをネイティブサポート。Apache DorisがIcebergテーブルを直接作成、管理、データ書き込みを行うことを可能にします。包括的なパーティションTransform関数をサポートし、隠れたパーティショニングやパーティションレイアウト進化などの機能を提供します。

ユーザーはApache Doris + Apache Icebergベースの効率的なData Lakehouseソリューションを迅速に構築し、様々なリアルタイムデータ分析・処理ニーズに柔軟に対応できます。

- Dorisの高性能クエリエンジンを使用してIcebergテーブルデータと他のデータソースを関連付けてデータ分析を実行し、**統一された連合データ分析プラットフォーム**を構築。
- Dorisを通じてIcebergテーブルを直接管理・構築し、Doris内でデータクリーニング、処理、Icebergテーブルへの書き込みを完了し、**データレイクの統一データ処理プラットフォーム**を構築。
- Icebergテーブルエンジンを通じて他の上流・下流システムとDorisデータを共有してさらなる処理を行い、**統一されたオープンデータストレージプラットフォーム**を構築。

将来的に、Apache IcebergはApache Dorisのネイティブテーブルエンジンの1つとして機能し、レイク形式データに対してより包括的な分析・管理機能を提供します。Apache DorisもUpdate/Delete/Merge、書き戻し時のソート、増分データ読み取り、メタデータ管理などのApache Icebergのより高度な機能を段階的にサポートし、統一された高性能なリアルタイムデータレイクプラットフォームを共同で構築します。

詳細については、[Iceberg Catalog](../catalogs/iceberg-catalog.mdx)を参照してください。

## ユーザーガイド

このドキュメントでは主に、Docker環境でApache Doris + Apache Icebergのテスト・デモンストレーション環境を迅速にセットアップし、様々な機能の使用方法を実演する方法について説明します。

このドキュメントで言及されているすべてのスクリプトとコードは、このアドレスから入手できます：[https://github.com/apache/doris/tree/master/samples/datalake/iceberg_and_paimon](https://github.com/apache/doris/tree/master/samples/datalake/iceberg_and_paimon)

### 01 環境準備

このドキュメントではデプロイメントにDocker Composeを使用し、以下のコンポーネントとバージョンを使用します：
...
```
Iceberg Catalogでデータベースとテーブルを作成する：
...
```
`FOR VERSION AS OF`文を使用して指定されたスナップショットをクエリします：
...
```
`FOR TIME AS OF`文を使用して、指定されたスナップショットをクエリします：
