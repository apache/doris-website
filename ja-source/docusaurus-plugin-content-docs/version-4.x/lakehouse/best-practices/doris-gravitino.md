...
開始する前に、S3バケットと慎重に設計されたIAMロールシステムを含む完全なインフラストラクチャをAWS上で準備する必要があります。これは安全で信頼性の高いlakehouseアーキテクチャ構築の基盤となります。
...
安全な認証情報管理を実装するために、GravitinoがSTS AssumeRoleメカニズムを通じて使用するIAMロールを作成する必要があります。この設計は、最小権限の原則と職務の分離というセキュリティのベストプラクティスに従います。

**データアクセスロールの作成**

1. Trust Policyファイルの作成
...
2. IAM Roleの作成
   ...
   デモンストレーションを簡単にするため、AWS管理ポリシーを直接使用します。本番環境では、より細かい粒度の権限制御を使用する必要があります。
...
成功レスポンスの例:
    ...
    ```
## 2. Gravitinoのデプロイメントと設定

### 2.1 Gravitinoのダウンロードとインストール

環境を素早くセットアップするために、Gravitinoのプリコンパイル版を使用します：
...
```
## 3. Doris の Gravitino への接続

### 3.1 Vended Credentials の使用

Gravitino は動的に一時的な認証情報を生成し、Doris に配布します:
...
このガイドを通して、GravitinoとDorisに基づくモダンなlakehouseアーキテクチャを正常に構築できるはずです。このアーキテクチャは高いパフォーマンスと高可用性を提供するだけでなく、高度なセキュリティメカニズムを通してデータアクセスのセキュリティとコンプライアンスも保証します。データ規模の拡大とビジネス要件の変化に伴い、このアーキテクチャは様々なエンタープライズレベルのニーズを満たすために柔軟にスケールできます。
