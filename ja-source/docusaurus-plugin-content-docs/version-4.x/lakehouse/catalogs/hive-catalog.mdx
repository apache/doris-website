...
Hive Metastoreまたは Hive Metastore と互換性のあるメタデータサービスに接続することで、Doris は自動的に Hive データベースとテーブル情報を取得してデータクエリを実行できます。

Hive に加えて、他の多くのシステムが Hive Metastore を使用してメタデータを保存しています。そのため、Hive Catalog を通じて、Hive テーブルだけでなく、Iceberg や Hudi などの Hive Metastore をメタデータストレージとして使用する他のテーブル形式にもアクセスできます。

## 適用シナリオ

| シナリオ     | 説明                                                  |
|--------------|--------------------------------------------------------------|
| クエリアクセラレーション | Doris の分散コンピューティングエンジンを使用して Hive データに直接アクセスし、クエリを高速化します。 |
| データ統合   | Hive データを読み込んで Doris 内部テーブルに書き込むか、Doris コンピューティングエンジンを使用して ZeroETL 操作を実行します。 |
| データライトバック   | Doris がサポートする任意のソースからデータを処理し、Hive テーブルに書き戻します。 |
...
* `<fs_defaultfs>`
  ...
  このパラメータは、DorisからこのHive Catalog内のテーブルにデータを書き込む際に必要です。例：

  `'fs.defaultFS' = 'hdfs://namenode:port'`

* `{MetaStoreProperties}`

  MetaStorePropertiesセクションは、Metastoreメタデータサービスの接続および認証情報を入力するためのものです。詳細については「サポートされるメタデータサービス」セクションを参照してください。

* `{StorageProperties}`

  StoragePropertiesセクションは、ストレージシステムに関連する接続および認証情報を入力するためのものです。詳細については「サポートされるストレージシステム」セクションを参照してください。

* `{HiveProperties}`

  HivePropertiesセクションは、Hive Catalogに関連するプロパティを入力するためのものです。

  * `get_schema_from_table`: デフォルト値はfalseです。デフォルトでは、DorisはHive Metastoreからテーブルスキーマ情報を取得します。ただし、場合によっては`Storage schema reading not supported`などのエラーによる互換性の問題が発生することがあります。この場合、このパラメータをtrueに設定すると、テーブルスキーマがTableオブジェクトから直接取得されます。ただし、この方法では列のデフォルト値情報が無視されることにご注意ください。このプロパティはバージョン2.1.10および3.0.6以降でサポートされています。

  * `hive.recursive_directories`: パーティションディレクトリを一覧表示する際にサブディレクトリを再帰的に検索するかどうか。このパラメータはバージョン3.0.2以降でサポートされています。バージョン4.0より前では、このパラメータのデフォルト値は`false`でした。それ以降のバージョンでは、デフォルト値は`true`です。一部のHiveテーブルのパーティションパスは、テーブルスキーマ内のパーティション情報と一致しない場合があります。サブディレクトリ内のデータファイルを取得するには、このパラメータをtrueに設定する必要があります。

  * `hive.ignore_absent_partitions`: 存在しないパーティションを無視するかどうか。デフォルトは`true`です。`false`に設定すると、存在しないパーティションに遭遇した場合にクエリでエラーが報告されます。このパラメータはバージョン3.0.2以降でサポートされています。

  * `hive.staging_dir`: Hive書き込み操作の一時ステージングルートディレクトリの設定に使用されます。書き込み処理中、データは最初にこのディレクトリに書き込まれ、完了後に最終的なHiveテーブルディレクトリに移動されます。このパスは、Hiveテーブルのルートディレクトリに対する相対パスまたは絶対パスのいずれかを指定できます。デフォルト値は`/tmp/.doris_staging`です。場合によっては、Hiveテーブルへの書き込み時に、`/tmp`ディレクトリとテーブルディレクトリが異なる名前空間に属しているために書き込みが失敗することがあります。そのような場合は、このパラメータを変更して相対パスを使用することで問題を解決できます。このパラメータはバージョン4.0.3以降でサポートされています。

* `{CommonProperties}`

  CommonPropertiesセクションは、共通属性を入力するためのものです。[Catalog概要](../catalog-overview.md)の「共通プロパティ」セクションを参照してください。

### サポートされるHiveバージョン

Hive 1.x、2.x、3.x、および4.xをサポートします。

Hiveトランザクションテーブルは、バージョン3.x以降でサポートされています。詳細については、「Hiveトランザクションテーブル」セクションを参照してください。
...
> 注意：Dorisバージョンによってサポートされるサービスタイプとパラメータが若干異なります。[Examples]セクションを参照してください。

### Hive Catalog機能サポート表

| メタデータサービス | テーブルクエリ | ビュークエリ | DDL操作 | データライトバック |
|------------------|-------------|------------|----------------|----------------|
| Hive             | ✅          | ✅         | ✅             | ✅             |
| AWS Glue         | ✅          | ✅         | ❌             | ❌             |
| DLF              | ✅          | ✅         | ✅             | ✅             |
...
> Dorisを通じてHiveテーブルを作成してデータを書き込むには、Catalog属性に`fs.defaultFS`プロパティを明示的に追加する必要があります。Catalogがクエリのみに作成される場合は、このパラメータを省略できます。
>
> Dorisバージョンによってサポートされるサービスタイプとパラメータが若干異なります。[Examples]セクションを参照してください。
...
| varchar(N)    | varchar(N)    |                                                |
| string        | string        |                                                |
| binary        | string/varbinary  | Catalogの`enable.mapping.varbinary`プロパティ（4.0.2以降でサポート）によって制御されます。デフォルトは`false`で、`string`にマッピングされます。`true`の場合、`varbinary`タイプにマッピングされます。|
| timestamp with local time zone| datetime/timestamptz | `enable.mapping.timestamp_tz`プロパティ（バージョン4.0.3以降でサポート）がマッピング動作を制御します。デフォルトでは`false`に設定されており、この場合タイプは`datetime`にマッピングされます。`true`に設定されると、`timestamptz`タイプにマッピングされます。     |
| array         | array         |                                                |
| map           | map           |                                                |
| struct        | struct        |                                                |
| other         | unsupported   |                                                |
...
-- 非パーティション化されたhiveテーブルの作成
  ...
  -- パーティション列はテーブルの列定義リストに含まれている必要があります
