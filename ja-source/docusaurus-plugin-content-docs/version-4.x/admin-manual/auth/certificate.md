---
{
  ...
  "language": "ja",
  "description": "Apache Doris SSL/TLS暗号化接続設定ガイド：MySQL clientセキュア転送をサポートし、一方向認証とmTLS相互認証を提供、証明書生成、設定詳細、およびベストプラクティスを含む。"
}
---
このドキュメントでは、データ送信のセキュリティを保護するために、DorisとMySQLクライアント間の通信でSSL/TLS暗号化を設定する方法について説明します。
...
| SSLモード | 説明 | コマンド例 |
|---------|------|---------|
| PREFERRED (デフォルト) | 最初にSSL接続を試行し、失敗した場合は通常の接続にフォールバック | `mysql -uroot -P9030 -h127.0.0.1` |
...
より高いセキュリティレベルが必要な場合は、mTLS相互認証を有効にできます。これによりクライアントも認証のための証明書を提供する必要があります。

### mTLSの有効化

FE設定ファイル`conf/fe.conf`を変更し、以下の設定を追加してFEを再起動してください:
...
```
Dorisは、`Doris/conf/mysql_ssl_default_certificate/client_certificate/`ディレクトリに配置されたデフォルトのクライアント証明書ファイルを提供します：
...
DorisでSSL機能を有効にするには、CA鍵証明書とサーバーサイド鍵証明書を設定する必要があります。相互認証が有効になっている場合は、クライアントサイド鍵証明書も必要です。
...
```
### Step 3: PKCS#12形式にパッケージ化

CAキーと証明書、Serverキーと証明書をそれぞれ個別にPKCS#12（P12）形式にマージして、Dorisで使用できるようにします：
...
```
:::tip Tip
`conf/fe.conf`設定ファイルを変更して`ssl_trust_store_type`パラメータを追加することで、他の証明書形式を指定することもできます。デフォルトはPKCS12です。
:::
