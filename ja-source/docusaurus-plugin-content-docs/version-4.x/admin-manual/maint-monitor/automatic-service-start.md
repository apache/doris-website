---
{
  "title": "自動サービス起動",
  "language": "ja",
  "description": "この文書では、サービスの後にサービスが適時にpull-upされないことを確保するため、Dorisクラスターの自動pull-upを設定する方法について説明します"
}
---
# 自動サービス開始

このドキュメントでは、Dorisクラスタの自動プルアップを設定する方法について説明します。これにより、本番環境で特別な事情によりサービスが故障した後、サービスが適時にプルアップされないことを防ぎます。

FEとBEの自動プルアップサービスは、Dorisクラスタが完全にセットアップされた後に設定する必要があります。
...
systemdを使用してdorisサービスを制御するには、sudo権限が必要です。sudo権限割り当ての最小粒度を確保するために、doris-feとdoris-beサービスのsystemd制御権限を指定された非rootユーザーに割り当てることができます。visudoでdoris-feとdoris-beのsystemctl管理権限を設定します。
...
1. config ファイル内の "JAVA_HOME" 変数を設定する必要があります。fe.conf と be.conf の両方で設定してください。設定しないと "systemctl start" コマンドを使用して doris を起動できません。
...
4. doris-be.serviceファイルをダウンロードしてください：[doris-be.service](https://github.com/apache/doris/blob/master/tools/systemd/doris-be.service)
...
7. 自動起動の設定
    ...
    ```
スタートを設定し、その本質は/etc/systemd/system/multi - user. Target. Wants/にサービスファイルリンクを追加することです
...
Supervisorの具体的な使用方法とパラメータ解析については[こちら](http://supervisord.org/)を参照してください。

Supervisor設定はsupervisor設定を自動的に取得します。yumコマンドを使用してsupervisorを直接インストールするか、pipを使用して手動でインストールできます。pipによる手動インストールプロセスは複雑なため、yumデプロイメントモードのみを表示します。手動デプロイメントについては、インストールデプロイメントの[こちら](http://supervisord.org/installing.html)を参照してください。

### 設定手順

1. yumによるsupervisorのインストール

    ```
    yum install epel-release
    yum install -y supervisor
    ```
2. サービスを開始してステータスを表示する
...
- yumを使用してインストールしたsupervisorを起動すると、エラーが発生します：pkg_resources.DistributionNotFound: The 'supervisor==3.4.0' distribution was not found
...
```
- supervisorがDorisプロセスを自動的に起動するように設定されている場合、DorisのBEノードが異常な要因により故障すると、be.outに出力されるべきエラースタック情報がsupervisorによって傍受されます。さらなる分析のために、supervisorのログでそれを確認する必要があります。
