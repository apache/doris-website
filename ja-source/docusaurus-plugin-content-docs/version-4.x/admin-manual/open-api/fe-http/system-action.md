...
```
データ部分の`column_names`はヘッダー情報であり、`href_columns`はテーブルのどの列がハイパーリンク列かを示します。`rows`配列の各要素は1つの行を表します。その中で、`__hrefPaths`はテーブルデータではなく、ハイパーリンク列のリンクURLであり、`href_columns`の列と一対一で対応しています。
