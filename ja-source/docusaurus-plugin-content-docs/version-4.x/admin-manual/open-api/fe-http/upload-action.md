---
{
  "title": "アップロードアクション",
  "language": "ja",
  "description": "Upload Actionは現在主にFEのフロントエンドページを提供しており、ユーザーが小さなテストファイルを読み込むために使用されます。"
}
---
# Upload Action
...
* `column_separator`
    ...
    オプション、ファイルの列区切り文字を指定します。デフォルトは `\t`
...
アップロードするファイルの内容、Content-type は `multipart/form-data`
...
headerのオプションは、Stream Loadリクエストのheaderのオプションと同じです。
