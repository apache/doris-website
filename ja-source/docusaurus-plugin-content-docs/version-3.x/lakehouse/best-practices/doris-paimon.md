---
{
  ...
  "description": "新しいオープンデータ管理アーキテクチャとして、Data レイクハウスはdata warehouseの高性能でリアルタイムな機能と",
  "language": "ja"
}
---
新しいオープンデータ管理アーキテクチャとして、Data レイクハウスはデータウェアハウスの高性能とリアルタイム機能を、データレイクの低コストと柔軟性と統合し、ユーザーが様々なデータ処理と分析のニーズをより便利に満たすことを支援します。これは企業のビッグデータシステムにおいてますます適用されています。

最近のバージョンにおいて、Apache Dorisはデータレイクとの統合を深化させ、成熟したData レイクハウスソリューションを発展させました。

- バージョン0.15以降、Apache DorisはHiveとIceberg外部tableを導入し、データレイクにおけるApache Icebergとの結合機能を探索しました。
- バージョン1.2以降、Apache DorisはMulti-カタログ機能を正式に導入し、様々なデータソースに対する自動メタデータマッピングとデータアクセスを実現するとともに、外部データの読み取りとクエリ実行に対する多くのパフォーマンス最適化を行いました。現在、高速でユーザーフレンドリーなレイクハウスアーキテクチャを構築する能力を完全に備えています。
- バージョン2.1において、Apache DorisのData レイクハウスアーキテクチャは大幅に強化され、主要データレイク形式（Hudi、Iceberg、Paimonなど）の読み書き機能を強化し、複数のSQL方言との互換性を導入し、既存システムからApache Dorisへのシームレスな移行を実現しました。データサイエンスと大規模データ読み取りシナリオのために、DorisはArrow Flight高速読み取りインターフェースを統合し、データ転送効率を100倍向上させました。

![Building lakehouse using Doris and Paimon](/images/lakehouse-architecture-for-doris-and-paimon.png)

## Apache Doris & Paimon

Apache Paimonは、データレイク形式とLSM構造の利点を革新的に組み合わせたデータレイク形式であり、効率的なリアルタイムストリーミング更新機能をデータレイクアーキテクチャに導入することに成功しました。これにより、Paimonは効率的にデータを管理し、リアルタイム分析を実行することができ、リアルタイムData レイクハウスアーキテクチャの構築に強力なサポートを提供します。
