...
[LoongCollector (iLogtail)](https://github.com/alibaba/loongcollector) は、Alibaba Cloud発のオープンソース高性能ログ収集・処理フレームワークです。バージョン3.0以前は、Logtail/iLogtailという名前でした。ストレージシステムにデータを書き込むためのカスタム出力プラグインをサポートしており、LoongCollector Doris FlusherはDorisにデータを出力するためのプラグインです。

Doris FlusherはDoris Stream Load HTTPインターフェースを呼び出してリアルタイムでDorisにデータを書き込み、マルチスレッド並行処理、障害リトライ、カスタムStream Load形式とパラメータ、出力書き込み速度監視などの機能を提供します。

Doris Flusherを使用するには、主に3つのステップがあります：
1. LoongCollectorをインストールする
2. Doris出力アドレスとその他のパラメータを設定する
3. LoongCollectorを開始してリアルタイムでDorisにデータを書き込む
...
`Authentication.PlainText.Username` | Dorisユーザー名、このユーザーは対応するDorisデータベースとテーブルに対するインポート権限を持つ必要があります
...
`GroupCommit` | グループコミットモード、選択可能な値は"sync"、"async"、または"off"、デフォルトは"off"
`Concurrency` | データ送信を並行処理するgoroutineの数、デフォルトは1（同期モード）
`QueueCapacity` | 非同期モードでのタスクキューの容量、デフォルトは1024
`Convert.Protocol` | データ変換プロトコル、デフォルトはcustom_single
`Convert.Encoding` | データ変換エンコーディング、デフォルトはjson
`Convert.TagFieldsRename` | tagsから1つ以上のフィールドの名前を変更
`Convert.ProtocolFieldsRename` | プロトコルフィールドの名前を変更、プロトコルフィールドオプションは次のもののみ可能：contents、tags、time
...
FEログファイルは通常、Dorisインストールディレクトリ下のfe/log/fe.logファイルに配置されています。これらは典型的なJavaプログラムログで、タイムスタンプ、ログレベル、スレッド名、コードの場所、ログ内容などのフィールドを含んでいます。通常のログだけでなく、スタックトレースを含む例外ログも含まれており、これらは複数行にわたります。ログ収集と保存では、メインログとスタックトレースを単一のログエントリに結合する必要があります。
...
GitHub イベントアーカイブには、GitHub ユーザーアクションのアーカイブデータが JSON 形式で含まれています。これは https://www.gharchive.org/ からダウンロードできます。例えば、2024年1月1日午後3時のデータです。

```
wget https://data.gharchive.org/2024-01-01-15.json.gz

```
以下がデータのサンプルです。通常、各データは1行に記載されますが、表示を見やすくするため、ここではフォーマットされています。
...
2. JSONデータにはすでに構造化されたフィールドがあるため、複雑なprocessorプラグインは使用されません
