...
全文検索演算子は、列が指定された全文検索条件（キーワード、フレーズなど）を満たすかどうかを判定し、結果としてTRUE、FALSE、またはUNKNOWNを返します。

制限事項：

1. 左オペランドは列名である必要があり、右オペランドは文字列リテラルである必要があります。
2. WHERE句でのみ使用でき、SELECT、GROUP BY、ORDER BYなどの他の句では使用できません。WHERE句内でAND、OR、NOT論理演算を使用して他の条件と組み合わせることができます。
...
| `<column_name>` MATCH_PHRASE_PREFIX `<string_literal>` | MATCH_PHRASEの拡張で、string_literalのトークン化後の最後の単語が完全な単語ではなくプレフィックスのみをマッチングすることを可能にします。Webサーチエンジンのサジェスト機能に類似しています。 | `SELECT * FROM t WHERE column1 MATCH_PHRASE_PREFIX 'word1 wor'` |
| `<column_name>` MATCH_PHRASE_EDGE `<string_literal>` | MATCH_PHRASE_PREFIXの拡張で、string_literalのトークン化後の最初の単語がサフィックスをマッチングし、最後の単語がプレフィックスをマッチングすることを可能にします。 | `SELECT * FROM t WHERE column1 MATCH_PHRASE_EDGE 'rd wor'` |
| `<column_name>` MATCH_REGEXP `<string_literal>` | MATCH_ANYの拡張で、string_literalが正規表現を指定し、column_nameのトークンがその正規表現にマッチします。 | `SELECT * FROM t WHERE column1 MATCH_REGEXP 'word.'` |
