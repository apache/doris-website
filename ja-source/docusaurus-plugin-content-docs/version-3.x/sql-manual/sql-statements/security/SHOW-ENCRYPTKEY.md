---
{
  ...
  "description": "データベース配下のすべてのカスタムキーを表示します。ユーザーがデータベースを指定した場合、対応するデータベースを確認し、",
  "language": "ja"
}
---
## 説明
...
| 権限         | オブジェクト    | 注意事項                                                                         |
|:-------------|:------------|:-----------------------------------------------------------------------------|
| `ADMIN_PRIV` | User / Role | 暗号化キーを表示するには、対象ユーザー/ロールに対して`ADMIN_PRIV`権限を持つ必要があります |

## 使用上の注意

データベースが明示的に指定されている場合、システムは指定されたデータベース内の暗号化キーをクエリします。それ以外の場合、
現在のセッションのデータベース内のキーを自動的にクエリします。
