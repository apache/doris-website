...
本記事では、可観測性データの中核の一つであるTraceのストレージと分析の実践について紹介します。完全な可観測性ソリューションの概要については、[概要](./overview.mdx)を参照してください。リソース評価、クラスターデプロイメント、および最適化については、[Log](./log.md)を参照してください。
