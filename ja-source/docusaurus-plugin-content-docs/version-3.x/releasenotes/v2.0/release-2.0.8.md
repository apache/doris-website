...
---
コミュニティユーザーと開発者の皆様のおかげで、Doris 2.0.8版では約65の改善とバグ修正が行われました。
...
- NereidsでTopN最適化とInverted Indexを連携させる

- 列統計収集時の最大文字列長を1024に制限してBEメモリ使用量を制御

- JDBCクライアントが空でない場合のJDBC Catalogクローズ

- すべてのIcebergデータベースを受け入れ、データベースの名前フォーマットをチェックしない

- キャッシュミスと不安定なクエリプランを避けるため、外部テーブルの行数を非同期でリフレッシュ

- 過剰なhadoopメトリクスを避けるため、hive外部テーブルのisSplitableメソッドを簡素化

改善とバグ修正の完全なリストは[GitHub](https://github.com/apache/doris/compare/2.0.7...2.0.8)をご覧ください。

## 4 謝辞

このリリースにご協力いただいたすべての方に感謝いたします：
