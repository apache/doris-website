...
---
Storage Vaultは、Dorisが分離されたストレージ・コンピュートモデルで使用するリモート共有ストレージです。1つまたは複数のStorage Vaultを設定して、異なるテーブルを異なるStorage Vaultに保存できます。
...
HDFSベースの分離ストレージ・コンピュートDorisクラスターを作成するには、すべてのノード（FE/BEノード、Meta Serviceを含む）が指定されたHDFSにアクセスする権限を持つことを確認してください。これには、事前にKerberos認証設定と接続性チェックの完了が含まれます（これは各対応ノードでHadoop Clientを使用してテストできます）。
...
```
より詳細なパラメータの説明と例については、[CREATE-STORAGE-VAULT](../sql-manual/sql-statements/cluster-management/storage-management/CREATE-STORAGE-VAULT.md)を参照してください。
...
テーブルを作成する際に、`PROPERTIES`で`storage_vault_name`を指定すると、指定した`vault name`に対応するStorage Vaultにデータが格納されます。テーブルの作成が正常に完了すると、`storage_vault`は変更できなくなります。つまり、Storage Vaultの変更はサポートされていません。
...
テーブル作成時にStorage Vaultを使用する優先順位は、テーブル -> データベース -> デフォルトのStorage Vaultです。テーブルのPROPERTYでStorage Vaultが指定されていない場合、データベースでStorage Vaultが指定されているかを確認します。データベースでも指定されていない場合、さらにデフォルトのStorage Vaultがあるかを確認します。

Storage VaultのVAULT_NAME属性が変更された場合、データベースに設定されているStorage Vaultが無効になり、エラーが発生する可能性があります。ユーザーは実際の状況に基づいて、データベースに有効なstorage_vault_nameを設定する必要があります。
...
S3 Storage Vault許可されているプロパティ：
- `VAULT_NAME`
- `s3.access_key`
- `s3.secret_key`
- `use_path_style`

HDFS Storage Vault禁止されているプロパティ：
- `path_prefix`
- `fs.defaultFS`

プロパティの説明は[CREATE-STORAGE-VAULT](../sql-manual/sql-statements/cluster-management/storage-management/CREATE-STORAGE-VAULT.md)で確認できます。
...
指定されたMySQLユーザーに特定のStorage Vaultの使用権限を付与し、ユーザーがテーブルを作成する際やStorage Vaultを表示する際にそのStorage Vaultを指定できるようにします。
...
#### Q1. 特定のstorage vaultがどのテーブルによって参照されているかを確認するにはどうすればよいですか？
